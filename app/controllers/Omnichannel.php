        ...
        if (!empty($this->data['website_setting'])) {
            foreach ($this->data['website_setting'] as $item) {
                if ($item->fields === 'logo_image' && !empty($item->value)) {
                    ...
                    $order_image = end($parts); 
                    $this->data['order_image'] = $order_image;
                }        
            }
        }  
    }
...
//        $this->page_construct('urbanpiper/order', $meta, $this->data);
    ...
    public function orders_inactive() {
        ...
        $meta = array('page_title' => lang('Orders 3P'), 'bc' => $bc);
    ...
    public function orders() {
        ...
        $this->data['status_counts'] = $this->get_order_status_counts();
        $this->page_construct('urbanpiper/Omnichannel', $meta, $this->data);
    }
    public function order_details($saleid) {
                ...
                <table class="table table-bordered">
                   ...
                   <tbody>
                        <tr>
                           ...
                           <td>&nbsp;' . $this->sma->formatMoney($up_response->order->details->order_subtotal) . '</td>
                               ' . $coupon . '
                           <td>&nbsp;' . $this->sma->formatMoney(($up_response->order->details->discount - $up_response->order->details->total_external_discount)) . '</td>
                           <td>&nbsp;' . $this->sma->formatMoney($up_response->order->details->total_external_discount) . '</td>
                           <td>&nbsp;' . $this->sma->formatMoney($up_response->order->details->total_taxes) . '</td>
                           <td>&nbsp;' . $this->sma->formatMoney(($up_response->order->details->order_level_total_charges + $up_response->order->details->order_level_total_charges)) . '</td>
        ...
        foreach ($up_response->order->items as $items) {
            ...
            echo ' <tr>
                           ...
                           <td>&nbsp;' . $this->sma->formatMoney($price) . '</td>    
                           <td>&nbsp;' . $this->sma->formatMoney($items->discount) . '</td>    
                           <td>&nbsp;' . $this->sma->formatMoney($taxes) . '</td>    
                           <td>&nbsp;' . $this->sma->formatMoney($charges) . '</td>    
                       </tr>';
        }//end foreach    
        ...
        foreach ($up_response->order->payment as $payments) {
            ...
            . '<td> ' . $this->sma->formatMoney($payments->amount) . '</td>'
   ...
   /**
    ...
    public function get_item_image_exist($image = '') {
        ...
        if ($image != '') {
            ...
            $fx = file_exists('assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/' . $image);

            if ($fx) {
                $img_url = site_url('assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/' . $image);
            } else {
                $fx2 = file_exists('assets/mdata/'.$this->Customer_assets.'/uploads/' . $image);
                if ($fx2) {
                    $img_url = site_url('assets/mdata/'.$this->Customer_assets.'/uploads/' . $image);
                }
            }
        }
        return $img_url;
    ...
    public function AddUpStoreCategories($category_id, $store_id, $newstatus= NULL) {
        ...
        if ($getcategory[$category_id]->image) {
            $file_headers = @get_headers(site_url('assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/') . $getcategory[$category_id]->image);
            $img_url = (!$file_headers || $file_headers[0]) ? site_url('assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/') . $getcategory[$category_id]->image : site_url('assets/mdata/'.$this->Customer_assets.'/uploads/no_image.png');
        } else {
            $img_url = site_url('assets/mdata/'.$this->Customer_assets.'/uploads/no_image.png');
        }
...
$URL = $this->upApiUrl . 'hub/api/v1/location/';
            ...
            case 'category_add': // Single Category Add
                ...
                $field = array(array(
                        ...
                        'sort_order' => 1,
                        'active' => true,
                        'img_url' => ($getcategory->image) ? site_url('assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/') . $getcategory->image : '',
                        'translations' => array()
                ));
                $collect_category['categories'] = $field;
...
//               $this->load->view($this->theme . 'urbanpiper/store_update_modal', $this->data);
    ...
    public function getproduct_list() {
        ...
        foreach ($getproduct as $items) {
            $imgcat = $items->image;
            if ($items->image) {
                if (!file_exists('assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/' . $imgcat)) {
                    $imgcat = 'no_image.png';
                }
            } else {
                $imgcat = 'no_image.png';
            ...
            $tabledata.='<td class="text-center"> <img src="' . base_url('assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/') . $imgcat . '" style="height:32px;"> </td>';
 ...
 $getWarehousestock = $this->UPM->check_dependancy('sma_warehouses_products', array('product_id' => $productval->id, 'warehouse_id' =>$get_storedetails->warehouse_id), '*');
   ...
   /**
    ...
    public function order_kot($saleid = NULL) {
        ...
        // $saleData =  $this->getSalesById($saleid);
        // if($saleData){
        //     $up_response = @unserialize($saleData['up_response']);
        //     $object = $this->objectToArray($up_response);
        //     $suspend_items = $object['order']['items'];
        //     if ($Settings->pos_type == 'restaurant') {
        //         $did = '';
        //         // $this->getProductdetailsBytitle()
        //         $this->sma->saveSuspendDataForKitchenPrinter($saleData,$suspend_items,$did);
        //     }else{
...
//***************************    Webshop   ***************************************** */
    ...
    public function orders_list_webshop($start = 1) {
        $this->load->model('orders_model');
        $this->load->model('sales_model');
        $include_dispatched = $this->input->get('include_dispatched', true);
        $upOrderStatusList = $this->UPM->orderStatuslistwebshop();
        $upOrders = $this->UPM->getUpOrdersforwebshop('active', $include_dispatched);
        $tablelist = '';
        $i = $start - 1;
        if (is_array($upOrders)) {
            foreach ($upOrders as $key => $order) {
                $inv_items = $this->UPM->getAllEshopOrderItemsbyid($order->id);
                $up_response = unserialize($order->up_response);
                $alert = ($order->eshop_order_alert_status == 1) ? '<br/><i title="New Order Received" style="font-size:20px;" class="fa fa-bell text-danger" aria-hidden="true"></i>' : '';
                $sale = $this->orders_model->getEshopOrderByID($order->id);
                $get_sale_by_order_no = $this->orders_model->get_sale_by_order_no($sale->invoice_no);
             
                if (!$get_sale_by_order_no) {
                    $pay_now = "pay_now_function(
                        '{$order->id}', 
                        " . htmlspecialchars(json_encode($order), ENT_QUOTES, 'UTF-8') . ", 
                        " . htmlspecialchars(json_encode($inv_items), ENT_QUOTES, 'UTF-8') . ", 
                        " . htmlspecialchars(json_encode($payments), ENT_QUOTES, 'UTF-8') . ")";
                }
                $tablelist .= '<tr>
                <td>' . $order->date . '</td>
                <td class="text-center">' . $order->id . '<br/><strong>' . $order->delivery_type . '</strong></td>
                <td>' . $order->customer . '<br/>Mb.:' . $order->customer_phone . '</td>';
    
                // Amount + Payment Status Combined Column
                $tablelist .= '<td style="text-align: center">
                    <div>' . $this->sma->formatMoney($order->grand_total) . '</div>';
    
                    if (strtolower($order->payment_status) == 'paid') {
                        $tablelist .= '<div style="color: green; font-weight: bold; font-size: 16px; margin-top: 4px; text-align: center;">
                            <img src="' . base_url('assets/images/PaidNowIcon.svg') . '" alt="Paid" style="height: 15px; width: 15px; vertical-align: middle; margin-right: 4px;">
                            <span>Paid</span>
                        </div>';
                    } else {
                        $tablelist .= '<div style="margin-top: 4px; text-align: center;">';
                        if (!$get_sale_by_order_no) {
                            $tablelist .= '<button class="btn btn-xs btn-danger" style="min-width: 60px; min-height: 25px; border-radius: 6px !important;" onclick="' . $pay_now . '(' . $order->id . ')">Pay Now</button>';
                        } else {
                            $tablelist .= '<a href="' . site_url('orders/add_3p_order_payments/' . $order->id) . '" 
                                class="btn btn-xs btn-danger pay-now-btn" 
                                style="min-width: 60px; min-height: 25px; border-radius: 6px !important;" 
                ...
                // $tablelist .= '<td><img class="img img-responsive" style = "margin-left :10px !important;" src="' . base_url('assets/images/Minatshi_Logo.png') . '" alt="Webshop" title="Webshop" /></td>';
                $tablelist .= '</td>';
                $tablelist .= '<td><img class="img img-responsive" style = "margin-left :10px !important;" src="' . base_url('assets/images/' . $this->data['order_image']) . '" alt="Webshop" title="Webshop" /></td>';
                if ($order->sale_status == 'pending') {
                    $order->sale_status = 'Received';
                }
                $color = '';
                switch (strtolower($order->sale_status)) {
                    case 'received': $color = '#000000'; break;
                    case 'in progress': $color = '#E88D00'; break;
                    case 'ready': $color = '#00A500'; break;
                    case 'dispatched': $color = '#009DFF'; break;
                }

                $status_data = '<div class="text-left" style="display: flex; align-items: center; height: 20px; line-height: 20px; margin: 0; padding: 0;">
                    <div style="width: 16px; height: 9rem;margin-left: -8px !important; background-color: ' . $color . ';"></div> <!-- vertical color block -->
                    <span id="current_status_' . $order->up_order_id . '" style="color: ' . $color . '; font-size: 14px; font-weight: bold; margin-left: 6px;">' . ucfirst($order->sale_status) . '</span>
                </div>';
                $tablelist .= '<td>' . $status_data . '</td>';
                if ($order->payment_status == 'paid') {
                    $payments = $this->orders_model->getPaymentsForOrder($order->id);
                }
                $onclick = "callOrderReady(
                    '{$order->id}', 
                    " . htmlspecialchars(json_encode($order), ENT_QUOTES, 'UTF-8') . ", 
                    " . htmlspecialchars(json_encode($inv_items), ENT_QUOTES, 'UTF-8') . ", 
                    " . htmlspecialchars(json_encode($payments), ENT_QUOTES, 'UTF-8') . ")";
                ...
                $action_button = '';
                $button_style = 'min-width: 60px;min-height: 35px;border-radius: 7px !important;';
                switch ($order->sale_status) {
                    case 'Received':
                        // $action_button = '<button class="btn btn-xs " style="' . $button_style . '" data-toggle="modal" data-target="#orderdetails" onclick="' . $onclick . '" id="kot_' . $order->id . '">KOT</button>';
                        $action_button = '<button class="btn btn-xs order_status_whatsapp" style="' . $button_style . ' border-color:#E88D00; color:#E88D00; background-color: white;" data-toggle="modal" data-target="#orderdetails" onclick="' . $onclick . '"  data-order-id="' . $order->id . '"id="kot_' . $order->id . '">KOT</button>';
                        break;
                        case 'In Progress':
                            $action_button = '<button class="btn btn-xs order_status_whatsapp" style="' . $button_style . 'border-color: #00A500; color: #00A500; background-color: white;" data-order-id="' . $order->id . '" onclick="updatestatus(\'' . $order->id . '\', \'' . $order->sale_status . '\')">Ready</button>';
                            break;
                        case 'Ready':
                            $action_button = '<button class="btn btn-xs order_status_whatsapp" style="' . $button_style . 'border-color: #009DFF; color: #009DFF; background-color: white;"  data-order-id="' . $order->id . '" onclick="updatestatus(\'' . $order->id . '\', \'' . $order->sale_status . '\')">Dispatch</button>';
                            break;
                ...
                $tablelist .= '<td class="text-center">' . $action_button . '</td>';
                $tablelist .= '<td>
                    <button class="btn btn-xs btn-primary" style="min-width: 60px; min-height: 35px; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_detailsforwebshop(\'' . $order->id . '\');" id="' . $order->id . '">Order Details</button>
                    <a class="btn btn-xs btn-primary" style="background-color: #E9176B; color: #fff; min-width: 60px; min-height: 35px; border-radius: 7px !important; border: none; padding-top: 7px;" target="_new" href="' . base_url('Omnichannel/view_up_webshop/' . $order->id) . '">Receipt</a>';
                if ($order->sale_status == 'In Progress') {
                    $tablelist .= ' <button class="btn btn-xs" style="background-color:#E88D00; border-color:#E88D00 ;color:#fff; min-width: 60px; min-height: 35px; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_kotforwebshop(\'' . $order->id . '\');" id="kot_' . $order->id . '">KOT</button>';
                } else {
                    $tablelist .= ' <button class="btn btn-xs" style="border-color:#777; color:#777;background-color: #f5f5f5; min-width: 60px; min-height: 35px; border-radius: 7px !important;" disabled>KOT</button>';
                }
                $tablelist .= '</td></tr>';
            }
        }
    ...
    public function order_kot_forwebshop($saleid = NULL) {
        $this->data['order'] = $this->UPM->getOrdersforwebshop($saleid);
        $this->data['items'] = $this->UPM->getOrderItemsforwebshop($saleid);
        $this->data['salesid'] = $saleid;
        $Settings = $this->Settings;
        if ($Settings->pos_type == 'restaurant') {
            $invoice =  $this->getSalesAndItemsById($saleid);
            if ($invoice) {
                $data = $invoice['data'];
        ...
        $this->load->view($this->theme . 'urbanpiper/order_kot_webshop', $this->data);
    }   
    public function order_detailsforwebshop($saleid)
    {
        $upOrders = $this->UPM->getOrdersforwebshop($saleid);
        $upOrdersItems = $this->UPM->getOrderItemsforwebshop($saleid);
        $up_foodtype = $this->UPM->get_foodtype();

        if (empty($upOrders[$saleid])) {
            echo 'Invalid order.';
            return;
        }
        foreach ($upOrders as $upOrder) {
            $up_response = unserialize($upOrder->up_response);
            $shipping_address = $this->orders_model->getShipingAdress($upOrder->shipping_address_id);
            $billing_address = $this->orders_model->getShipingAdress($upOrder->billing_address_id);
            $address_to_show = !empty($shipping_address) ? $shipping_address : $billing_address;
            
            $order_status = $upOrder->sale_status;
            $btn_class = $order_status == 'Cancelled' ? 'btn-danger' : ($order_status == 'Placed' ? 'btn-info' : ($order_status == 'Completed' ? 'btn-success' : 'btn-primary'));
            $coupon_title = ($upOrder->coupon) ? '<td>Coupon</td>' : '';
            $coupon = ($upOrder->coupon) ? '<td>' . $upOrder->coupon . '</td>' : '';
            $instructions = ($upOrder->instructions) ? '<div class="text text-info">Instructions : ' . $upOrder->instructions . '</div>' : '';
            echo '
            <div>
                <table class="table table-bordered">
                    <thead>
                    ...
                    <tbody>
                        <tr>
                            <td>' . $upOrder->id . '</td>
                            <td>' . $upOrder->date . '</td>
                            <td>' . $upOrder->up_delivery_datetime . '</td>
                            <td>' . Webshop . '</td>
                            <td>' . $upOrder->order_type . '</td>
                            <td class="text-right">&nbsp;' . $this->sma->formatMoney($upOrder->grand_total) . '</td>
                            ' . $coupon . '
                            <td class="text-right">&nbsp;' . ($upOrder->discount - $upOrder->total_external_discount) . '</td>
                            <td class="text-right">&nbsp;' . $upOrder->total_external_discount . '</td>
                            <td class="text-right">&nbsp;' . $upOrder->total_taxes . '</td>
                            <td class="text-right">&nbsp;' . ($upOrder->order_level_total_charges + $upOrder->order_level_total_charges) . '</td>
                            <td class="text-right"><button class="btn btn-xs ' . $btn_class . '">' . $order_status . '</button></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>';
            foreach ($upOrdersItems as $items) {
                $price = $items->mrp;
                $optionAddName = '';
                $optionRemoveName = '';
                $chargeArr = [];
                $taxes = 0;
                if (!empty($items->options_to_add)) {
                    $optionAddName = $items->product_name;
                    $price += $items->mrp;
                }
                if (!empty($items->options_to_remove)) {
                    $optionRemoveName = $items->product_name;
                }
                if (!empty($items->taxes)) {
                    foreach ($items->taxes as $tax) {
                        $taxes += $tax->value;
                ...
                if (!empty($upOrder->product_tax)) {
                        $taxes = $upOrder->product_tax;
                }
                $charges = !empty($chargeArr) ? implode('<br/>', $chargeArr) : '0';
                echo '
                    <tr>
                        <td>' . $items->product_name . '</td>
                        <td>' . $items->merchant_id . '</td>
                        <td>' . $optionAddName . '</td>
                        <td>' . $optionRemoveName . '</td>
                        <td>' . $up_foodtype[$items->food_type] . '</td>
                        <td>' . $this->sma->formatDecimal($items->quantity) . '</td>
                        <td>' . $items->product_unit_code . '</td>
                        <td class="text-right">&nbsp;' . $this->sma->formatMoney($items->unit_price) . '</td>
                        <td class="text-right">&nbsp;' . $items->discount . '</td>
                        <td class="text-right">&nbsp;' . $this->sma->formatMoney($taxes) . '</td>
            ...
            <div class="row">
                <div class="col-sm-6">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr><th>Name</th><td>' . $upOrder->customer . '</td></tr>
                            <tr><th>Phone</th><td>' . $upOrder->customer_phone . '</td></tr>
                            <tr><th>Email</th><td>' . $upOrder->customer_email . '</td></tr>
                        </tbody>
                        <thead>
                            <tr><th colspan="2">Delivery Address</th></tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td colspan="2">
                                    ' . $address_to_show->line1 . '<br/>' .
                                    (!empty($address_to_show->line2) ? $address_to_show->line2 . '<br/>' : '') . 
                                    $address_to_show->city . ' ' . $address_to_show->postal_code . '
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="col-sm-6">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr>
                                <td>' . $upOrder->payment_method . '</td>
                                <td class="text-right"> ' . $this->sma->formatMoney($upOrder->grand_total) . '</td>
                                <td>' . $upOrder->srvr_trx_id . '</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    public function order_detailsforpos($saleid){
        
        $upOrders = $this->UPM->getOrdersforPos($saleid);
        $upOrdersItems = $this->UPM->getOrderItems($saleid);
        $up_foodtype = $this->UPM->get_foodtype();
    ...
    public function get_orders_urbanpiper() {
        $include_dispatched = $this->input->get('include_dispatched', true);
        $orders = $this->UPM->getUpOrdersfor3p('active', $include_dispatched);
        return $orders; 
    }
    public function get_orders_webshop() {
        $include_dispatched = $this->input->get('include_dispatched', true);
        $orders = $this->UPM->getUpOrdersforwebshop('active', $include_dispatched);
        return $orders;
    }
    public function get_orders_amazon()
{
    $include_dispatched = $this->input->get('include_dispatched', true);
    $orders = $this->UPM->getUpOrdersforAmazon('active', $include_dispatched);

    // ✅ Safety: always return array
    return is_array($orders) ? $orders : [];
}
    public function get_orders_pos(){
        $include_dispatched = $this->input->get('include_dispatched', true);
        $orders = $this->UPM->getUpOrdersforpos('active', $include_dispatched);
        return $orders;
    }

    public function orders_list_combined() {
        $urbanPiperOrders = $this->get_orders_urbanpiper();
        $webshopOrders    = $this->get_orders_webshop();
        $amazonOrders     = $this->get_orders_amazon();
        $posOrders     = $this->get_orders_pos();
        
        // $allOrders = array_merge($urbanPiperOrders, $webshopOrders, $amazonOrders);
        $allOrders = $urbanPiperOrders + $webshopOrders + $amazonOrders + $posOrders;
        usort($allOrders, function($a, $b) {
            // Pick the correct date field depending on order type
            $dateA = isset($a->date) ? $a->date : (isset($a->purchase_date) ? $a->purchase_date : null);
            $dateB = isset($b->date) ? $b->date : (isset($b->purchase_date) ? $b->purchase_date : null);
        
            // Convert both to timestamps, safely handling nulls
        ...
        try {
            foreach ($allOrders as $order) {
                $i++;
                if (isset($order->up_channel)) {
                    $html .= $this->render_urbanpiper_order_row($order, $i);
                } elseif (isset($order->amazon_order_id)) {
                    $html .= $this->render_amazon_order_row($order, $i);
                } elseif (isset($order->product_id)) {
                    $html .= $this->render_pos_order_row($order, $i);
                }else {
                    $html .= $this->render_webshop_order_row($order, $i);
                }
                
            }
        } catch (Exception $e) {
            log_message('error', 'Error rendering orders: ' . $e->getMessage());
            echo '<div class="alert alert-danger">Error rendering orders: ' . $e->getMessage() . '</div>';
            return;
        }
        
        echo $html;
    }
    private function render_urbanpiper_order_row($order, $i) {
        $up_response = unserialize($order->up_response);
        ...
        $sale_status = strtolower($order->sale_status);
        if ($sale_status === 'acknowledged') {
            $label = 'In Progress';
            $color = '#E88D00'; // Blue
        } elseif ($sale_status === 'food ready') {
            $label = 'Food Ready';
            $color = '#28a745'; // Green
        } elseif ($sale_status === 'cancelled') {
            $label = 'Cancelled';
            $color = '#dc3545'; // Red
        } elseif ($sale_status === 'placed') {
            $label = 'Received';
            $color = '#000000'; // Black
            ...
            $color = '#343a40'; // Default dark gray
        }
       $status_data = '<div class="text-left" style="display: flex; align-items: center; height: 20px; line-height: 20px; margin: 0; padding: 0;">
            <div style="width: 16px; height: 9rem; margin-left: -8px !important; background-color: ' . $color . ';"></div>
            <span id="current_status_' . $up_response->order->details->ext_platforms[0]->id . '" style="color: ' . $color . '; font-size: 14px; font-weight: bold; margin-left: 6px;">' . $label . '</span>
        </div>';
        $button_style = 'min-width: 60px;min-height: 35px;border-radius: 7px !important;';
       
        switch ($order->sale_status) {
            case 'Placed':
                $mapped_status = 'Acknowledged';
                // $action_button = '<button class="btn btn-xs" style="' . $button_style . ' border-color:#E88D00; color:#E88D00; background-color: white;" data-toggle="modal" data-target="#orderdetails" onclick="' . $onclick . '" id="kot_' . $order->id . '">KOT</button>';
                $action_button.='<button class="btn btn-xs" style="' . $button_style . ' border-color:#E88D00; color:#E88D00; background-color: white;" id="' . str_replace(' ', '', $order->sale_status) . '_status_' . $order->up_order_id . '" onclick="update_order_status(\'' . $order->up_order_id . '\',\'' . $mapped_status  . '\')" >' . KOT . '</button>';
                break;
            case 'Acknowledged':
                $action_button = '<button class="btn btn-xs" style="' . $button_style . 'border-color:#00A500; color:#00A500; background-color: white;" id="' . str_replace(' ', '', $order->sale_status) . '_status_' . $order->up_order_id . '" onclick="update_order_status(\'' . $order->up_order_id . '\', \'' . $order->sale_status . '\')">Ready</button>';
                break;
            // case 'Food Ready':
                // $action_button = '<button class="btn btn-xs" style="' . $button_style . 'border-color:#009DFF; color:#009DFF; background-color: white;" id="' . str_replace(' ', '', $order->sale_status) . '_status_' . $order->up_order_id . '" onclick="update_order_status(\'' . $order->up_order_id . '\', \'' . $order->sale_status . '\')">Dispatch</button>';
                // break;
                case 'Food Ready':
            $action_button = '
                <div>
                    <button class="btn btn-xs" style="' . $button_style . 'border-color:#009DFF; color:#009DFF; background-color: white;" 
                        id="' . str_replace(' ', '', $order->sale_status) . '_status_' . $order->up_order_id . '" 
                        onclick="update_order_status(\'' . $order->up_order_id . '\', \'' . $order->sale_status . '\')">
                        Dispatch
                    </button>
                    <div style="margin-top: 4px; font-size: 12px;">';

            if ($order->order_rider_id) {
                $action_button .= $order->name;
            } else {
                $action_button .= 'Not Assign';
            }
            $action_button .= '<br>' . substr($up_response->customer->phone, -4);
        ...
        $html = '<tr>
                    ...
                    <td style="max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" 
                        title="' . htmlspecialchars($up_response->order->details->ext_platforms[0]->id) . '">
                        <span style="display: inline-block; max-width: 70%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                            ' . htmlspecialchars($up_response->order->details->ext_platforms[0]->id) . '
                        </span>
                    </td>
                    <td>' . $up_response->customer->name . '<br/>Mb.:' . $up_response->customer->phone . '</td>
                    <td style="text-align: right">&nbsp;' . $this->sma->formatMoney($up_response->order->details->order_total) . '</td>
                    ...
                    <td>' . $status_data . '</td>
                    <td class="text-center" style = "padding-top: 22px;">' . $action_button . '</td>
                    <td>
                    <button class="btn btn-xs btn-primary" style="min-width: 60px; min-height: 35px; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_details(\'' . $order->sale_id . '\');" id="' . $order->sale_id . '" >Order Details</button>
                    <a class="btn btn-xs btn-primary" style="background-color: #E9176B; color: #fff; min-width: 60px; min-height: 35px; border-radius: 7px !important; border: none; padding-top: 7px;" target="_new" href="' . base_url('pos/view_up/' . $order->sale_id) . '">Receipt</a>
                    <button class="btn btn-xs" style="min-width: 60px;min-height: 35px;border-color:#E88D00; color:#E88D00; background-color: white; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_kot(\'' . $order->sale_id . '\');" id="kot_' . $order->sale_id . '">KOT</button>
                    </td>
                </tr>';
        return $html;
    }
    private function render_webshop_order_row($order, $i){
        $this->load->model('orders_model');
        $this->load->model('sales_model');        
        $inv_items = $this->UPM->getAllEshopOrderItemsbyid($order->id);
        $up_response = unserialize($order->up_response);
        ...
        switch (strtolower($order->sale_status)) {
            case 'received': $color = '#000000'; break;
            case 'in progress': $color = '#E88D00'; break;
            case 'food ready': $color = '#00A500'; break;
            case 'dispatched': $color = '#009DFF'; break;
        }

        $status_data = '<div class="text-left" style="display: flex; align-items: center; height: 20px; line-height: 20px; margin: 0; padding: 0;">
            <div style="width: 16px; height: 9rem;margin-left: -8px !important; background-color: ' . $color . ';"></div>
            <span id="current_status_' . $order->id . '" style="color: ' . $color . '; font-size: 14px; font-weight: bold; margin-left: 6px;">' . ucfirst($order->sale_status) . '</span>
        </div>';
        ...
        switch ($order->sale_status) {
            case 'Received':
                ...
                break;
            case 'In Progress':
                $action_button = '<button class="btn btn-xs order_status_whatsapp" style="' . $button_style . 'border-color: #00A500; color: #00A500; background-color: white;" onclick="updatestatus(\'' . $order->id . '\', \'' . $order->sale_status . '\')">Ready</button>';
                break;
            case 'Food Ready':
                $action_button = '<button class="btn btn-xs order_status_whatsapp" style="' . $button_style . 'border-color: #009DFF; color: #009DFF; background-color: white;" onclick="updatestatus(\'' . $order->id . '\', \'' . $order->sale_status . '\')">Dispatch</button>';
                break;
            default:
                $action_button = '-';
                break;
        ...
        // Pay Now or Paid badge
        if (strtolower($order->payment_status) == 'paid') {
            $pay_now_btn = '<div style="color: green; font-weight: bold; font-size: 16px; margin-top: 4px; text-align: center;">
                <img src="' . base_url('assets/images/PaidNowIcon.svg') . '" alt="Paid" style="height: 15px; width: 15px; vertical-align: middle; margin-right: 4px;">
                <span>Paid</span>
            </div>';
        } else {
            if (!$get_sale_by_order_no) {
                $pay_now_btn = '<button class="btn btn-xs btn-danger" style="min-width: 60px; min-height: 25px; border-radius: 6px !important;" onclick="' . $pay_now . '(' . $order->id . ')">Pay Now</button>';
            } else {
                $pay_now_btn = '<a href="' . site_url('orders/add_3p_order_payments/' . $order->id) . '" class="btn btn-xs btn-danger pay-now-btn" style="min-width: 60px; min-height: 25px; border-radius: 6px !important;" data-order-id="' . $order->id . '">Pay Now</a>';
            }
        }
        if ($order->sale_status == 'Received') {
            $receipt_btn = '<a class="btn btn-xs btn-primary" style="background-color: #E9176B; color: #fff; min-width: 60px; min-height: 35px; border-radius: 7px !important; border: none; padding-top: 7px;" target="_new" href="' . base_url('Omnichannel/view_up_webshop/' . $order->id) . '">Receipt</a>';
        } else {
            $receipt_btn = '<a class="btn btn-xs btn-primary" style="background-color: #E9176B; color: #fff; min-width: 60px; min-height: 35px; border-radius: 7px !important; border: none; padding-top: 7px;" target="_new" href="' . base_url('Omnichannel/view_up_webshop_after_sale/' . $order->id) . '">Receipt</a>';
        }
        // <td><img class="img img-responsive" style="" src="' . base_url('assets/images/Minatshi_Logo.png') . '" alt="Webshop" title="Webshop" /></td>
        $html = '<tr>
            <td>' . $order->date . '</td>
            <td>' . $order->id . '</td>
            <td style="padding: 2px;">' . $order->customer . '<br/>Mb.:' . $order->customer_phone . '</td>
            <td style="text-align: center">
                <div>' . $this->sma->formatMoney($order->grand_total) . '</div>
                <div style="margin-top: 4px; text-align: center;">' . $pay_now_btn . '</div>
            </td>
            <td><img class="img img-responsive" style="" src="' . base_url('assets/images/' . $this->data['order_image']) . '" alt="Webshop" title="Webshop" /></td>
            <td>' . $status_data . '</td>
            <td class="text-center">' . $action_button . '</td>
            <td>
                <button class="btn btn-xs btn-primary" style="min-width: 60px; min-height: 35px; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_detailsforwebshop(\'' . $order->id . '\');" id="' . $order->id . '">Order Details</button>' . $receipt_btn;

        if ($order->sale_status == 'In Progress') {
            $html .= ' <button class="btn btn-xs" style="background-color:#E88D00; border-color:#E88D00 ;color:#fff; min-width: 60px; min-height: 35px; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_kotforwebshop(\'' . $order->id . '\');" id="kot_' . $order->id . '">KOT</button>';
        } else {
            $html .= ' <button class="btn btn-xs" style="border-color:#777; color:#777;background-color: #f5f5f5; min-width: 60px; min-height: 35px; border-radius: 7px !important;" disabled>KOT</button>';
        }
    ...
    private function render_amazon_order_row($order, $i)
    {
        // --- Color mapping for order status ---
        $color = '#343a40'; // default dark
        switch (strtolower($order->order_status)) {
            case 'pending': $color = '#000000'; break;
            case 'shipped': $color = '#009DFF'; break;
        ...
        // --- Status bar HTML ---
        $status_data = '
            <div class="text-left" style="display:flex;align-items:center;height:20px;line-height:20px;margin:0;padding:0;">
                <div style="width:16px;height:9rem;margin-left:-8px;background-color:' . $color . ';"></div>
                <span style="color:' . $color . ';font-size:14px;font-weight:bold;margin-left:6px;">' . ucfirst($order->order_status) . '</span>
            </div>';
    
        // --- Button styling ---
        $button_style = 'min-width:60px;min-height:35px;border-radius:7px !important;margin:2px 2px;';
    
        // --- Build table row ---
        $html = '
        <tr>
            <td>' . ($order->purchase_date ?: '-') . '</td>
            <td>' . htmlspecialchars($order->amazon_order_id) . '</td>
            <td style="padding:2px;">' . htmlspecialchars($order->buyerinfo_email) . '<br/><small>' . htmlspecialchars($order->shipping_address_City) . '</small></td>
            <td style="text-align:right;">' . $this->sma->formatMoney($order->total_amount) . '</td>
            <td style="text-align:center;"><img src="' . base_url('assets/logs/Amazon_icon.png') . '" alt="Amazon" title="Amazon" style="height:28px;"/></td>
            <td>' . $status_data . '</td>
            ...
            <td class="text-center" style="padding-top:20px;">
                <button class="btn btn-xs btn-primary" style="' . $button_style . '" 
                    data-toggle="modal" data-target="#orderdetails" 
                    onclick="order_details_amazon(\'' . $order->amazon_order_id . '\');">
                    Order Details
                </button>
                <a class="btn btn-xs btn-primary" 
                    style="' . $button_style . 'background-color:#E9176B;color:#fff;border:none;padding-top:7px;" 
                    target="_blank" 
                    href="' . base_url('pos/view_up/' . $order->amazon_order_id) . '">
                    Receipt
                </a>
                <button class="btn btn-xs" 
                    style="' . $button_style . 'border-color:#E88D00;color:#E88D00;background-color:white;" 
                    data-toggle="modal" data-target="#orderdetails" 
                    onclick="order_kot_amazon(\'' . $order->amazon_order_id . '\');">
                    KOT
                </button>
            </td>
        </tr>';
    ...
    private function render_pos_order_row($order, $i) {
       
        $delivery_person = $this->sales_model->getDelivaryPerson();
        ...
        $status_data = '
            <div class="text-left" style="display:flex;align-items:center;height:20px;line-height:20px;margin:0;padding:0;">
                <div style="width:16px;height:9rem;margin-left:-8px;background-color:' . $color . ';"></div>
                <span style="color:' . $color . ';font-size:14px;font-weight:bold;margin-left:6px;">' . ucfirst($order->sale_status) . '</span>
            </div>';
        // ---------------- Rider Dropdown ----------------
        $rider_dropdown = '
        <select name="delivered_by"
            class="form-control rider-select rider-assign"
            data-sale-id="' . $order->id . '"
            data-reference-no="' . $order->sale_id . '"
            data-customer-id="' . $order->customer_id . '">
            <option value="">' . lang('select') . '</option>';

        if (!empty($delivery_person)) {
            foreach ($delivery_person as $deliveryP) {
                $selected = '';

                if (!empty($order->delivery_person_id) && $order->delivery_person_id == $deliveryP->id) {
                    $selected = 'selected';
                } elseif (
                    empty($order->delivery_person_id) &&
                    !empty($order->rider) &&
                    $order->rider == ($deliveryP->name . '~' . $deliveryP->phone)
                ) {
                    $selected = 'selected';
                }
                ...
                <option value="' . $deliveryP->id . '"
                    data-rider-name="' . htmlspecialchars($deliveryP->name) . '"
                    data-rider-phone="' . htmlspecialchars($deliveryP->phone) . '"
                    ' . $selected . '>
                    ' . $deliveryP->name . '
                </option>';
            }
        ...
        $html = '<tr>
            <td>' . $order->date . '</td>
            <td>' . $order->sale_id . '</td>
            <td style="padding: 2px;">' . $order->customer . '<br/>Mb.:' . $order->phone . '</td>
            <td style="text-align: center">
                <div>' . $this->sma->formatMoney($order->grand_total) . '</div>
                <div style="margin-top: 4px; text-align: center;">' . $pay_now_btn . '</div>
            </td>
            <td><img class="img img-responsive" style="" src="' . base_url('assets/images/POS_Logo.svg') . '" alt="POS" title="POS" /></td>
            <td>' . $status_data . '</td>
            <td class="text-center">' . $rider_dropdown . '</td>
            <td>
                <button class="btn btn-xs btn-primary" style="min-width: 60px; min-height: 35px; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_detailsforpos(\'' . $order->sale_id . '\');" id="' . $order->sale_id . '" >Order Details</button>
                <a class="btn btn-xs btn-primary" style="background-color: #E9176B; color: #fff; min-width: 60px; min-height: 35px; border-radius: 7px !important; border: none; padding-top: 7px;" target="_new" href="' . base_url('Omnichannel/view_up_pos/' . $order->sale_id) . '">Receipt</a>
                <button class="btn btn-xs" style="min-width: 60px;min-height: 35px;border-color:#E88D00; color:#E88D00; background-color: white; border-radius: 7px !important;" data-toggle="modal" data-target="#orderdetails" onclick="order_kot(\'' . $order->sale_id . '\');" id="kot_' . $order->sale_id . '">KOT</button>
            </td>
    ...
    public function order_details_amazon($amazon_order_id)
    {
        $order = $this->UPM->getAmazonOrderById($amazon_order_id);
        $items = $this->UPM->getAmazonOrderItems($amazon_order_id);
    
        if (!$order) {
            echo '<div class="alert alert-danger">Amazon order not found.</div>';
            return;
        }
                ...
                <table class="table table-bordered">
                   ...
                   <tbody>
                        <tr>
                           ...
                           <td>' . $order->order_total_amount . ' ' . $order->order_total_currency_code . '</td>
                        </tr>
                   </tbody>
                </table>
            </div>';
        ...
        echo '<div class="table-responsive">
                ...
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>#</th><th>Title</th><th>ASIN</th><th>Qty</th><th>Price</th><th>Tax</th>
                        </tr>
                    </thead>
                    <tbody>';
        $i = 1;
        foreach ($items as $it) {
            echo '<tr>
                    ...
                    <td>' . $it->item_price_amount . '</td>
                    <td>' . $it->item_tax_amount . '</td>
                </tr>';
        }
        ...
        echo '<div class="row">
                <div class="col-sm-6">
                    <table class="table table-bordered">
                        <thead><tr><th colspan="2">Buyer Info</th></tr></thead>
                        <tbody>
                            <tr><th>Email</th><td>' . $order->buyerinfo_email . '</td></tr>
                            <tr><th>Prime Order</th><td>' . ($order->is_prime == "true" ? "Yes" : "No") . '</td></tr>
                ...
                <div class="col-sm-6">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr>
                                <td>' . $order->shipping_address_City . ', ' . $order->shipping_address_StateOrRegion . ', ' . $order->shipping_address_PostalCode . ', ' . $order->shipping_address_CountryCode . '</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    public function order_kot_amazon($amazon_order_id)
    {
        $order = $this->UPM->getAmazonOrderById($amazon_order_id);
        $items = $this->UPM->getAmazonOrderItems($amazon_order_id);
        ...
        echo '<div id="printableArea">
                <h3 style="text-align:center;">KOT – Amazon Order ' . $order->amazon_order_id . '</h3>
    ...
    public function add_delivery($id = null) {
        ...
        if ($delivery = $this->sales_model->getDeliveryBySaleID($id)) {
            $this->edit_delivery($delivery->id);
             ...
             if ($sale->sale_status != 'Order-Ready') {
            ...
            if ($this->form_validation->run() == true && $this->sales_model->addDelivery($dlDetails)) {
                ...
                $this->load->view($this->theme . 'urbanpiper/add_delivery', $this->data);
    ...
    public function edit_delivery($id = null) {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateDelivery($id, $dlDetails)) {
            ...
            redirect("urban_piper/index");
        } else {
            $delivery = $this->sales_model->getDeliveryByID($id);
            $this->data['sale'] = $this->pos_model->getwebshopInvoiceByID($delivery->sale_id);
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['delivery'] = $delivery;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['delivery_person'] = $this->sales_model->getDelivaryPerson();

            $this->data['inv_items'] = $this->pos_model->getAllWebshopInvoiceItems($delivery->sale_id);
            $this->load->view($this->theme . 'urbanpiper/edit_delivery', $this->data);
        }
    }

    public function createWebshopSales(){
        $this->load->model('orders_model');
        $postData = $this->input->post();
        $items = isset($postData['inv_items']) ? $postData['inv_items'] : [];
        $payments = isset($postData['payments']) ? $postData['payments'] : [];
        $flag = isset($postData['flag']) ? $postData['flag'] : [];
        unset($postData['inv_items']);unset($postData['payments']);unset($postData['customer_phone']);unset($postData['customer_name']);unset($postData['flag']);
        $orderId = $postData['id'];
        $postData['updated_at'] = date('Y-m-d H:i:s');
        $result = $this->UPM->update_webshop_order_and_items($orderId, $postData, $items, $payments, $flag);
        if ($postData['sale_status'] == 'Received') {
            if (!empty($postData['sale_invoice_no']) && $postData['sale_invoice_no'] > 0) {
                echo json_encode([
                    'status' => 'success',
                ]);
                return;
            }
            redirect('Omnichannel/add_sale_from_eshop_order?order_id=' . $postData['id'] . '&syncQuantity=1&sale_action=eshop_order');
        } 
        if ($result) {
            echo json_encode(['status' => 'success',]);
        } else {
            ...
            if ($order_id) {
                ...
                $extrasPara = array('sale_action' => $sale_action, 'syncQuantity' => $syncQuantity, 'order_id' => $order_id);
                    ...
                    if ($order->return_id) {
                        ...
                        echo json_encode([
                            'status' => 'success',
                            // 'message' => $this->lang->line("Delivery eshop order add to sale successfully"),
                        ]);
                        return;
    ...
    public function updatestatus(){
        ...
        $current_status = $postData['sale_status'];
        $next_status = '';
        if ($current_status == 'In Progress') {
            $next_status = 'Food Ready';
        } elseif ($current_status == 'Food Ready') {
        ...
        $this->load->model('orders_model');
        $update = $this->orders_model->updateOrderStatus($order_id, $next_status);
        if ($update) {
            echo json_encode(['status' => 'success', 'new_status' => $next_status]);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to update status.']);
        }
    }
    public function view_up_webshop($order_id = null, $modal = null) {
        ...
        $inv = $this->pos_model->getwebshopInvoiceByID($order_id);
        $upOrder = unserialize($inv->up_response);
        if ($this->data['default_printer']->tax_classification_view):
            $inv->rows_tax = $this->pos_model->getwebshopAllTaxItems($order_id, $inv->return_id);
        endif;
        $isGstSale = $this->pos_model->isGstSaleforwebshop($order_id);
        $inv->GstSale = !empty($isGstSale) ? 1 : 0;
        $print = array();
        $print['print_option'] = $this->site->defaultPrinterOption($_PID);
        $print['rows'] = $this->data['rows'] = $this->pos_model->getAllWebshopInvoiceItems($order_id);
        $biller_id = $inv->biller_id;
        $customer_id = $inv->customer_id;
        $print['biller'] = $this->data['biller'] = $this->pos_model->getCompanyBywebshopID($biller_id);
        $print['customer'] = $this->data['customer'] = $this->pos_model->getCompanyByID($customer_id);
        $print['payments'] = $this->data['payments'] = $this->pos_model->getwebshopInvoicePayments($order_id);
        ...
        $print['sid'] = $this->data['sid'] = $order_id;
        $print['modal'] = $this->data['modal'] = $modal;
        $print['page_title'] = $this->data['page_title'] = $this->lang->line("invoice");
        $print['taxItems'] = $this->data['taxItems'] = $this->pos_model->getwebshopAllTaxItemsGroup($inv->id, $inv->return_id);
        if (!empty($print['rows'])) {
        ...
        $shipping_address = $this->orders_model->getShipingAdress($inv->shipping_address_id);
        $billing_address = $this->orders_model->getShipingAdress($inv->billing_address_id);
        $this->data['address_to_show'] = !empty($shipping_address) ? $shipping_address : $billing_address;
        $this->load->view($this->theme . 'pos/view_up_webshop', $this->data);
        ...
        if ($order_id != $_SESSION['print'] && $_SESSION['print_type'] == null) {
        ...
        $_SESSION['print'] = $order_id;
    }
    public function view_up_webshop_after_sale($order_id = null, $modal = null) {
        $this->data['pos_settingss'] = $this->data['pos_settings'];
        $this->load->model('pos_model');
        if ($this->input->get('id')) {
            $order_id = $this->input->get('id');
        ...
        $billing_address = $this->orders_model->getShipingAdress($inv->billing_address_id);
        $this->data['address_to_show'] = !empty($shipping_address) ? $shipping_address : $billing_address;
        $this->load->view($this->theme . 'pos/view_up_webshop_after_sale', $this->data);
        $print['brcode'] = $this->sma->save_barcode($inv->reference_no, 'code128', 66, false);
        $print['qrcode'] = $this->sma->qrcode('link', urlencode(site_url('sales/view/' . $inv->id)), 2);
        $arr = explode("'", $print['brcode']);
        $print['brcode'] = $arr[1];
    ...
    public function view_up_pos($sale_id = null, $modal = null) {
        $this->data['pos_settingss'] = $this->data['pos_settings'];
        $this->sma->checkPermissions('index');
        if ($this->input->get('id')) {
        ...
        $this->load->view($this->theme . 'urbanpiper/view_up_pos', $this->data);
    ...
    /////////////////// Add Schdule and holiday ///////////////////////////////////////////////////////////
    public function get_products_by_category(){
        $this->load->model('orders_model');

        $category_id = $this->input->get('category_id');
        $products = $this->orders_model->get_products_by_category($category_id);
    ...
    public function add_schedule() {
        ...
        $this->form_validation->set_rules('product_id[]', 'Products', 'required');
        $this->form_validation->set_rules('weekday[]', 'Weekdays', 'required');
        $this->form_validation->set_rules('order_lead_time', 'Order Lead Time', 'required|integer');
        $this->form_validation->set_rules('start_time', 'Start Time', 'required');
        $this->form_validation->set_rules('end_time', 'End Time', 'required');

        if ($this->form_validation->run() == FALSE) {
            // Reload form with error
            $this->data['categories'] = $this->orders_model->get_categories();
            $bc = array(
                array('link' => base_url(), 'page' => lang('home')),
                array('link' => site_url('Omnichannel/view_schdules'), 'page' => lang('Schedules')),
                array('link' => '#', 'page' => lang('Add_Schedule'))
            );
            $meta = array('page_title' => lang('Add Schedule'), 'bc' => $bc);
            $this->page_construct('urbanpiper/add_schdule', $meta, $this->data);
            ...
            $product_ids      = array_unique($this->input->post('product_id'));
            $order_lead_time  = $this->input->post('order_lead_time');
            $start_time       = $this->input->post('start_time');
            $end_time         = $this->input->post('end_time');
            $weekdays         = array_unique($this->input->post('weekday'));
            $weekday_string = implode(',', $weekdays);
            foreach ($product_ids as $product_id) {
                if (!$this->orders_model->schedule_exists_combined($product_id, $category_id)) {
                    $schedule_data = array(
                    ...
                    $this->orders_model->insert_schedule($schedule_data);
                }
            }
            $this->session->set_flashdata('success', 'Schedule added successfully.');
            redirect('Omnichannel/view_schdules');
        }
    }
    public function add_holiday(){
        ...
        $this->form_validation->set_rules('start_time', 'Start Time', 'required');
        $this->form_validation->set_rules('end_time', 'End Time', 'required');
        $this->form_validation->set_rules('special_hours', 'Special Hours', 'required|trim');
        $this->form_validation->set_rules('site_banner_text', 'Site Banner Text', 'required|trim');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $bc = array(
                array('link' => base_url(), 'page' => lang('home')),
                array('link' => site_url('Omnichannel/view_holidays'), 'page' => lang('Holiday')),
                array('link' => '#', 'page' => lang('Add_Holiday'))
            );
            $meta = array('page_title' => lang('Add Holiday'), 'bc' => $bc);
            $this->page_construct('urbanpiper/add_holiday', $meta, $this->data);
        } else {
            $start_time = $this->input->post('start_time');
            $end_time   = $this->input->post('end_time');
            ...
            $data = [
                ...
                'special_hours' => $this->input->post('special_hours'),
                'start_time' => $this->input->post('start_time'),
                'end_time' => $this->input->post('end_time'),
                'site_banner_text' => $this->input->post('site_banner_text')
            ];

            if ($this->orders_model->insert_holiday($data)) {
                $this->session->set_flashdata('success', 'Holiday added successfully.');
                redirect('Omnichannel/view_holidays');
            } else {
                 $bc = array(
                    array('link' => base_url(), 'page' => lang('home')),
                    array('link' => site_url('Omnichannel'), 'page' => lang('Add_Holiday')),
                    array('link' => '#', 'page' => lang('Add_Holiday'))
                );
                $meta = array('page_title' => lang('Add Holiday'), 'bc' => $bc);
                $this->session->set_flashdata('error', 'Failed to add schedule.');
                $this->page_construct('urbanpiper/view_holidays', $meta, $this->data);
            }
        }
    }
    public function get_holidays() {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("id, day, date, start_time, end_time, special_hours,site_banner_text")
            ->from('sma_holiday')
                ->add_column('actions',
                    "<div class='text-center'>
                        <a href='" . site_url('Omnichannel/edit_holiday/$1') . "' class='tip' title='" . lang('Edit_Holiday') . "'>
                            <i class='fa fa-edit'></i>
                        </a>
                        <a href='" . site_url('Omnichannel/delete_holiday/$1') . "' class='tip' title='" . lang('delete') . "' onclick=\"return confirm('" . lang('r_u_sure') . "')\">
                            <i class='fa fa-trash'></i>
                        </a>
                    </div>",
                'id');
            // ->where('deleted', 0);  // Exclude deleted holidays, if you have a 'deleted' flag
    ...
    public function view_holidays() {
        ...
        $bc = array(
                    array('link' => base_url(), 'page' => lang('home')),
                    array('link' => '#', 'page' => lang('Holidays'))
                );
                $meta = array('page_title' => lang('List_Holiday'), 'bc' => $bc);
        // $this->session->set_flashdata('error', 'Failed to add schedule.');
        $this->page_construct('urbanpiper/list_holiday', $meta, $this->data);
    }
    public function view_schdules() {
        $this->load->model('orders_model');
        $bc = array(
                    array('link' => base_url(), 'page' => lang('home')),
                    array('link' => '#', 'page' => lang('List_Schedule'))
                );
                $meta = array('page_title' => lang('List Schedule'), 'bc' => $bc);
        // $this->session->set_flashdata('error', 'Failed to add schedule.');
        $this->page_construct('urbanpiper/list_schdules', $meta, $this->data);
    }
    //mangesh
    public function get_schdules() {
        $this->load->model('orders_model');
        $this->load->library('datatables');
        $this->datatables
            ->select("sma_product_schedule.id AS id, sma_categories.name as category_name, sma_products.name as product_name, sma_product_schedule.order_lead_time, sma_product_schedule.start_time, sma_product_schedule.end_time, sma_product_schedule.weekday")
            ->from('sma_product_schedule')
            ->join('sma_categories', 'sma_categories.id = sma_product_schedule.category_id', 'left')
            ->join('sma_products', 'sma_products.id = sma_product_schedule.product_id', 'left')
            //->order_by('sma_product_schedule.id', 'desc')
            ->add_column('actions',
                    "<div class='text-center'>
                        <a href='" . site_url('Omnichannel/edit_schedule/$1') . "' class='tip' title='" . lang('Edit_Schedule') . "'>
                            <i class='fa fa-edit'></i>
                        </a>
                        <a href='" . site_url('Omnichannel/delete_schedule/$1') . "' class='tip' title='" . lang('delete') . "' onclick=\"return confirm('" . lang('r_u_sure') . "')\">
                            <i class='fa fa-trash'></i>
                        </a>
                    </div>",
                'id');
    ...
    public function delete_holiday($id) {
        $this->load->model('orders_model');
        if ($this->orders_model->delete_holiday($id)) {
            $this->session->set_flashdata('success', 'Holiday deleted successfully.');
        } else {
            $this->session->set_flashdata('error', 'Failed to delete holiday.');
        }
        redirect('Omnichannel/view_holidays');
    }
    public function delete_schedule($id) {
        $this->load->model('orders_model');
        if ($this->orders_model->delete_schedule($id)) {
    ...
    public function edit_holiday($id) {
        $this->data['holiday'] = $this->orders_model->get_holiday_by_id($id);
        $this->form_validation->set_rules('day', 'Day', 'required');
        $this->form_validation->set_rules('date', 'Date', 'required');

        if ($this->form_validation->run() == FALSE) {
            $meta = array('page_title' => 'Edit Holiday');
            $this->page_construct('urbanpiper/edit_holiday', $meta, $this->data);
        } else {
            $data = [
                'day' => $this->input->post('day'),
                'date' => $this->input->post('date'),
            ...
            $this->orders_model->update_holiday($id, $data);
    ...
    public function edit_schedule($id) {
        ...
        $this->data['categories'] = $this->orders_model->get_all_categories();
        $this->data['schedule'] = $this->orders_model->get_schedule_by_id($id);
        $this->data['products'] = [$this->orders_model->get_schedule_products($id)];
        $weekdays = $this->orders_model->get_schedule_weekdays($id);
        $this->data['weekdays'] = explode(',', $weekdays);
        $this->form_validation->set_rules('category_id', 'Category');
        $this->form_validation->set_rules('product_id[]', 'Products', 'required');
        $this->form_validation->set_rules('start_time', 'Start Time', 'required');
        $this->form_validation->set_rules('end_time', 'End Time', 'required');
        $this->form_validation->set_rules('order_lead_time', 'Order Lead Time', 'required');
        if ($this->form_validation->run() == FALSE) {
            $meta = array('page_title' => 'Edit Schedule');
            $this->page_construct('urbanpiper/edit_schedule', $meta, $this->data);
        } else {
            $schedule_data = array(
                'category_id'      => $this->input->post('category_id'),
                'product_id'       => $this->input->post('product_id')[0], // assuming single selection; adjust if needed
                'order_lead_time'  => $this->input->post('order_lead_time'),
                'start_time'       => $this->input->post('start_time'),
                'end_time'         => $this->input->post('end_time'),
                'weekday'          => implode(',', $this->input->post('weekdays')) // assuming it's a multi-select
            );
    
            // Update the schedule
            $this->orders_model->update_schedule($id, $schedule_data);
    
            // Optionally update relations if needed (product/weekday split tables)
            // $this->orders_model->update_schedule_relations($id, $this->input->post('product_id'), $this->input->post('weekday'));
    ...
    //     $this->load->model('orders_model');
    //     $holidays = $this->orders_model->get_all_holidays();

    //     header("Content-Type: application/vnd.ms-excel");
    //     header("Content-Disposition: attachment; filename=holidays.xls");

    //     echo "ID\tDay\tDate\tStart Time\tEnd Time\tSpecial Hours\n";
    //     foreach ($holidays as $row) {
    //         echo "{$row->id}\t{$row->day}\t{$row->date}\t{$row->start_time}\t{$row->end_time}\t{$row->special_hours}\n";
    //     }
    // }
    ...
    //                 'special_hours' => $row[4]
    //             );
    //             $this->orders_model->insert_holiday($data);
    //         }

    //         fclose($file);
    //         $this->session->set_flashdata('success', 'Holidays imported successfully.');
    //     } else {
    ...
    //     redirect('Omnichannel/view_holidays');
    // }
    // public function export_schedule_excel() {
    //     $this->load->model('orders_model');
    //     $schedules = $this->orders_model->get_all_schedules();

    //     header("Content-Type: application/vnd.ms-excel");
    //     header("Content-Disposition: attachment; filename=schedules.xls");

    //     echo "ID\tCategory\tProduct\tOrder Lead Time\tStart Time\tEnd Time\n";

    //     foreach ($schedules as $row) {
    //         echo "{$row->id}\t{$row->category_name}\t{$row->product_name}\t{$row->order_lead_time}\t{$row->start_time}\t{$row->end_time}\n";
    //     }
    // }
    ...
    //             $schedule_data = array(
    //                 'category_id' => $row[0],
    //                 'order_lead_time' => $row[2],
    //                 'start_time' => $row[3],
    //                 'end_time' => $row[4],
    //             );

    //             $schedule_id = $this->orders_model->insert_schedule($schedule_data);

    //             if ($schedule_id) {
    //                 $this->orders_model->insert_schedule_product($schedule_id, $row[1]); // product_id
    //             }
    //         }

    //         fclose($file);
    //         $this->session->set_flashdata('success', 'Schedules imported successfully.');
    //     } else {
    //         $this->session->set_flashdata('error', 'No file uploaded.');
    //     }
    ...
    public function schedule_actions(){
        ...
        if ($this->form_validation->run() == true) {
            ...
            //             $this->sma->storeDeletedData('product_schedule', 'id', $id);
            //             $this->orders_model->deleteSchedule($id);
            //             }
            //         }

            //         if ($error) {
            //             $this->session->set_flashdata('warning', lang('some_schedules_not_deleted'));
            //         } else {
            ...
            //     }
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $style = [
                        ...
                        'font' => ['name' => 'Arial', 'color' => ['rgb' => 'FF0000']],
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A1', 'Product Schedules');
                    $this->excel->getActiveSheet()->setTitle(lang('product_schedule'));

                    $this->excel->getActiveSheet()->SetCellValue('A2', 'Category');
                    $this->excel->getActiveSheet()->SetCellValue('B2', 'Order Lead Time');
                    $this->excel->getActiveSheet()->SetCellValue('C2', 'Start Time');
                    $this->excel->getActiveSheet()->SetCellValue('D2', 'End Time');
                    $this->excel->getActiveSheet()->SetCellValue('E2', 'Weekdays');
                    $this->excel->getActiveSheet()->SetCellValue('F2', 'Products');

                    $row = 3;
                    foreach ($_POST['val'] as $id) {
                        $schedule = $this->orders_model->get_schedule_by_id($id);
                        $category = $this->site->getCategoryByID($schedule->category_id);
                        $weekdays = $this->orders_model->get_schedule_weekdays($id);
                        $products = $this->orders_model->get_schedule_products($id);


                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $category ? $category->name : '');
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $schedule->order_lead_time);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $schedule->start_time);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $schedule->end_time);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $weekdays);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $products->product_name);
                        $row++;
                    }
                    ...
                    $filename = 'schedules_' . date('Y_m_d_H_i_s');
                    ...
                    if ($this->input->post('form_action') == 'export_pdf') {
                        ...
                        if (!PHPExcel_Settings::setPdfRenderer($rendererName, $rendererLibraryPath)) {
                            die('Please set the PDF renderer correctly.');
                        }
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        $this->sma->storeDeletedData('holidays', 'id', $id);
                        if (!$this->orders_model->deleteHoliday($id)) {
                            $error = true;
                        }
                    }

                    if ($error) {
                        $this->session->set_flashdata('warning', lang('some_holidays_not_deleted'));
                    } else {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->getStyle("A1:F1")->applyFromArray($style);
                    $this->excel->getActiveSheet()->mergeCells('A1:F1');
                    $this->excel->getActiveSheet()->SetCellValue('A1', 'Holidays');
                    $this->excel->getActiveSheet()->setTitle(lang('holidays'));

                    $this->excel->getActiveSheet()->SetCellValue('A2', 'Day');
                    $this->excel->getActiveSheet()->SetCellValue('B2', 'Date');
                    $this->excel->getActiveSheet()->SetCellValue('C2', 'Start Time');
                    $this->excel->getActiveSheet()->SetCellValue('D2', 'End Time');
                    $this->excel->getActiveSheet()->SetCellValue('E2', 'Special Hours');
                    $this->excel->getActiveSheet()->SetCellValue('F2', 'Banner Text');

                    $row = 3;
                    foreach ($_POST['val'] as $id) {
                        $holiday = $this->orders_model->get_holiday_by_id($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $holiday->day);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $holiday->date);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $holiday->start_time);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $holiday->end_time);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $holiday->special_hours);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $holiday->site_banner_text);
                        $row++;
                    }
    ...
    public function import_schedule(){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $headers = ['category_id', 'product_id', 'order_lead_time', 'start_time', 'end_time', 'weekday'];
                $csvData = [];
                ...
                for ($i = 2; $i <= count($sheetData); $i++) {
                    ...
                    if (count($rowData) >= count($headers)) {
                        $csvData[] = array_combine($headers, array_slice($rowData, 0, count($headers)));
                    }
                }
                ...
                foreach ($csvData as $row) {
                    ...
                    $product_field   = trim($row['product_id']);
                    $order_lead_time = (int) $row['order_lead_time'];
                    $start_time = is_numeric($row['start_time']) ? gmdate("H:i", round($row['start_time'] * 86400)) : trim($row['start_time']);
                    $end_time   = is_numeric($row['end_time']) ? gmdate("H:i", round($row['end_time'] * 86400)) : trim($row['end_time']);

                    $weekdays        = explode(',', trim($row['weekday']));
                    $weekday_string  = implode(',', array_unique($weekdays));

                    // if (strtotime($end_time) >= strtotime($start_time)) {
                    //     $errors[] = "End Time must be greater than Start Time at line $lineNo";
                    //     $lineNo++;
                    //     continue;
                    // }

                    if (strtoupper($product_field) === 'ALL*') {
                        $products = $this->products_model->getCategoryProducts($category_id);
                        if (!$products) {
                            $errors[] = "No products found for category ID {$category_id} at line $lineNo";
                            $lineNo++;
                            continue;
                        }
                        foreach ($products as $product) {
                            if (!$this->orders_model->schedule_exists_combined($product->id, $category_id)) {
                                $schedule_data = [
                                    'category_id'      => $category_id,
                        ...
                        $product_ids = explode(',', $product_field);
                        foreach ($product_ids as $pid) {
                            $pid = trim($pid);
                            $product = $this->products_model->getProductByID($pid);
                            if (!$product) {
                                $errors[] = "Invalid product ID ($pid) at line $lineNo";
                                continue;
                            }
                            if (!$this->orders_model->schedule_exists_combined($pid, $category_id)) {
                                $schedule_data = [
                                    'category_id'      => $category_id,
                                    'product_id'       => $pid,
                                    'order_lead_time'  => $order_lead_time,
                ...
                if ($insertCount > 0) {
                    $this->session->set_flashdata('message', "$insertCount schedule(s) imported successfully.");
                } elseif (empty($errors)) {
        ...
        $this->data['modal_js'] = $this->site->modal_js();
        $this->load->view($this->theme . 'urbanpiper/import_schedule', $this->data);
    }
    public function exportcsvschedule(){
        ...
        $sheet->SetCellValue('D1', 'Order Lead Time');
        $sheet->SetCellValue('E1', 'Start Time');
        $sheet->SetCellValue('F1', 'End Time');
        $sheet->SetCellValue('G1', 'Weekday');

        // Content rows
        $row = 2;
        foreach ($getSchedule as $schedule) {
            // $sheet->setCellValueExplicit('A' . $row, $schedule->product_code, PHPExcel_Cell_DataType::TYPE_STRING);
            $sheet->SetCellValue('A' . $row, $schedule->product_name);
            $sheet->SetCellValue('B' . $row, $schedule->category_name);
            $sheet->SetCellValue('C' . $row, $schedule->category_id);
            $sheet->SetCellValue('D' . $row, $schedule->order_lead_time);
            $sheet->SetCellValue('E' . $row, $schedule->start_time);
            $sheet->SetCellValue('F' . $row, $schedule->end_time);
            $sheet->SetCellValue('G' . $row, $schedule->weekday);
            $row++;
        }

        $filename = 'schedules_export_' . date('Y_m_d_H_i_s') . '.xls';
...
public function import_holiday(){
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        if (isset($_FILES['userfile'])) {
            ...
            for ($i = 2; $i <= count($sheetData); $i++) {
                ...
                $day              = trim($row['A']);
                $date             = trim($row['B']);
                $special_hours    = isset($row['C']) ? trim($row['C']) : null;
                $start_time       = trim($row['D']);
                $end_time         = trim($row['E']);
                $site_banner_text = isset($row['F']) ? trim($row['F']) : null;

                if (!$day || !$date || !$start_time || !$end_time) {
                    $errors[] = "Missing required fields at line $lineNo";
                    $lineNo++;
                    continue;
                }
                if (is_numeric($date)) {
                ...
                $start_time = is_numeric($start_time) ? gmdate("H:i", round($start_time * 86400)) : $start_time;
                $end_time   = is_numeric($end_time) ? gmdate("H:i", round($end_time * 86400)) : $end_time;

                if (strtotime($end_time) <= strtotime($start_time)) {
                    $errors[] = "End Time must be after Start Time at line $lineNo";
                    $lineNo++;
                    continue;
                }
            ...
            if ($insertCount > 0) {
                $this->session->set_flashdata('message', "$insertCount holiday(s) imported successfully.");
            } elseif (empty($errors)) {
                $this->session->set_flashdata('error', 'No valid data found.');
            }
    ...
    $this->data['modal_js'] = $this->site->modal_js();
    $this->load->view($this->theme . 'urbanpiper/import_holiday', $this->data);
}
...
////////////////////////// Kitchen Printer/////////////////////////////////////
    private function objectToArray($data) {
        if (is_array($data)) {
            ...
            return array_map([$this, 'objectToArray'], get_object_vars($data));
        } else {
            return $data;
        }
    }
    ...
    public function getSalesAndItemsById($id) {
       
       // Step 1: Get the sale_id from the orders table using the order id
        $order_query = $this->db->get_where('orders', array('id' => $id), 1);
        if ($order_query->num_rows() > 0) {
            $order = $order_query->row_array(); // Get order data as array
            $sale_id = $order['sale_id'];

            // Step 2: Get sale data using sale_id
            $sale_query = $this->db->get_where('sales', array('id' => $sale_id), 1);
            if ($sale_query->num_rows() > 0) {
                $sale = $sale_query->row_array(); // Get sale data

                // Step 3: Get sale items
                $this->db->where('sale_id', $sale_id);
                $items_query = $this->db->get('sale_items');
                $items = $items_query->result_array();

                // Step 4: Add item count to sale data
                $sale['count'] = count($items);
...
////////////////////////// Kitchen Printer END/////////////////////////////////////
////////////////////////////3P Orders status count//////////////////////////////////
public function get_order_status_counts(){
    $statuses = ['Received', 'In Progress', 'Food Ready', 'Dispatched'];
    $stat = ['Placed', 'Acknowledged', 'Food Ready', 'Dispatched'];
    $counts = array_fill_keys($statuses, 0);
    $today = date('Y-m-d');
    // --- Get counts from sma_orders ---
    $this->db->select('sale_status, COUNT(*) as total')
             ->from('sma_orders')
             ->where_in('sale_status', $statuses)
             ->where('payment_status !=', 'Failed')
             ->like('date', $today, 'after') // filter all for today
             ->group_by('sale_status');
    $query1 = $this->db->get()->result();
    foreach ($query1 as $row) {
        $counts[$row->sale_status] += (int)$row->total;
    }
    // --- Get counts from sma_sales ---
    $this->db->select('sale_status, COUNT(*) as total')
             ->from('sma_sales')
             ->where_in('sale_status', $stat)
             ->like('date', $today, 'after') // filter all for today
             ->group_by('sale_status');
