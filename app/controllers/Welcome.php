...
class Welcome extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['apinotify'] = $this->db_model->getLatestApinotify();
        $this->data['accept_apinotify'] = $this->db_model->getAcceptedApinotify();
    ...
    function image_upload()
    {
        ...
        if (isset($_FILES['file'])) {
            ...
            $array = array(
                'filelink' => base_url() . 'assets/mdata/'.$this->Customer_assets.'/uploads/images/' . $photo
            );
    ...
    {
        if (file_exists('./files/'.$this->Customer_assets.'/'.$file)) {
            $this->load->helper('download');
            force_download('./files/'.$this->Customer_assets.'/'.$file, NULL);
            exit();
        }
