...
class system_settings extends MY_Controller {
    ...
    public function __construct() {
        ...
        if ($this->Owner || $this->Admin) {
            // full access
        } elseif ($show_settings_menu) {
            // User menu: sma_settings_screens per group decides what is shown; no per-method map here.
        } elseif ($user_printer && in_array($this->v, $this->user_printer_setting_access, true)) {
            // printer GP only for printer actions
        } else {
            $this->session->set_flashdata('warning', lang('access_denied'));
            redirect('welcome');
        }
    ...
    private function ensureAutoCustomerNumberSettingColumn() {
        if (!$this->db->field_exists('auto_customer_number', 'settings')) {
            $this->load->dbforge();
            $this->dbforge->add_column('settings', array(
                'auto_customer_number' => array('type' => 'TINYINT', 'constraint' => 1, 'default' => 0, 'null' => FALSE),
            ));
        }
    }
    ...
    public function index() {
        ...
        $this->form_validation->set_rules('packing_size_column', 'Packing Size Column', 'trim|required');
        ...
        $this->form_validation->set_rules('discount_on_mrp', 'Discount on MRP', 'trim|required');
        ...
        if ($this->input->post('protocol') == 'smtp') {
            ...
            $this->form_validation->set_rules('smtp_pass', lang('smtp_pass'));
        ...
        $this->form_validation->set_rules('business_hour', lang('business_hour'), 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data = array(
                ...
                'packing_size_column' => $this->input->post('packing_size_column') ? 1 : 0,
                ...
                'view_tax_classification_on_challan' => $this->input->post('view_tax_classification_on_challan'),
                ...
                'payment_settlement' => $this->input->post('payment_settlement_method'),
                'sms_brand_name' => $this->input->post('sms_brand_name'),
                'display_job_work' => $this->input->post('display_job_work'),
                'sms_promotional_header' => $this->input->post('sms_promotional_header'),
                'sms_sender' => $this->input->post('sms_sender'),
				'production_order_age' => $this->input->post('production_order_age'),
				'set_order_dispatch' => $this->input->post('set_order_dispatch'),
                'product_batch_setting' => $this->input->post('product_batch_setting'), // Added this line
                'product_name_auto_generate' => $this->input->post('product_name_auto_generate'),
                'enable_module_production_unit' => $this->input->post('enable_module_production_unit') ? 1 : 0, 
                'enable_module_procurment_order' => $this->input->post('enable_module_procurment_order') ? 1 : 0,
                'mother_kitchen' => $this->input->post('mother_kitchen') ? 1 : 0,
                'active_omnichannel' => $this->input->post('active_omnichannel') ? 1 : 0,
                'other_category_for_product' => $this->input->post('othercategoryforproduct'),
            ...
            if ($this->db->field_exists('enable_module_attendance', 'settings')) {
                $data['enable_module_attendance'] = $this->input->post('enable_module_attendance') ? 1 : 0;
            }
            if ($this->db->field_exists('auto_customer_number', 'settings')) {
                $data['auto_customer_number'] = $this->input->post('auto_customer_number') ? 1 : 0;
            }
            ...
            if( $this->input->post('edit_code')=='e603796dddf1b823126b32fd6ad212d2') {
                ...
                $data['use_invoice_number_prefix'] = $this->input->post('use_invoice_number_prefix');
                $data['whatsapp_service'] = $this->input->post('whatsapp_service');
        ...
        if ($this->form_validation->run() == TRUE && $this->settings_model->updateSetting($data)) {
            $this->db->cache_delete_all(); // Clear all database cache
            ...
            // Show only non-Vendor active warehouses in System Settings default warehouse dropdown
            /////////////////////////////////  Generate PO/////////////////////////////////////////
            $this->data['warehouses'] = $this->site->getNonVendorActiveWarehouses();

            $this->data['printers'] = $this->site->getAllPrinter();
            $this->data['countrys'] = $this->site->getAllcountrys();
    ...
    public function install_update($file, $m_version, $version) {
        ...
        $this->sma->unzip('./files/'.$this->Customer_assets.'/updates/' . $file . '.zip');
        if ($m_version) {
        ...
        unlink('./files/'.$this->Customer_assets.'/updates/' . $file . '.zip');
    ...
    public function backups() {
        ...
        $this->data['dbs'] = glob('./files/'.$this->Customer_assets.'/backups/*.txt', GLOB_BRACE);
    ...
    public function backup_database() {
        ...
        $save = './files/'.$this->Customer_assets.'/backups/' . $db_name;
    ...
    public function restore_database($dbfile) {
        ...
        $file = './files/'.$this->Customer_assets.'/backups/' . $dbfile . '.txt';
    ...
    public function download_database($dbfile) {
        ...
        $this->zip->read_file('./files/'.$this->Customer_assets.'/backups/' . $dbfile . '.txt');
        $name = $dbfile . '.zip';
    ...
    public function delete_database($dbfile) {
        ...
        unlink('./files/'.$this->Customer_assets.'/backups/' . $dbfile . '.txt');
    ...
    public function delete_backup($zipfile) {
        ...
        unlink('./files/'.$this->Customer_assets.'/backups/' . $zipfile . '.zip');
    ...
    public function create_group() {
        ...
        if ($this->form_validation->run() == TRUE) {
            $data = array('name' => strtolower($this->input->post('group_name')), 'description' => $this->input->post('description'), 'screen_selector' => $this->input->post('screen_selector'));
        ...
        if ($this->form_validation->run() == TRUE && ($new_group_id = $this->settings_model->addGroup($data))) {
            ...
            $this->data['getAllStartUpScreens'] = $this->settings_model->getAllStartUpScreens();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function edit_group($id) {
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            $this->data['screen_selector'] = array('name' => 'screen_selector', 'id' => 'screen_selector', 'type' => 'text', 'class' => 'form-control', 'value' => $this->form_validation->set_value('screen_selector', $group->screen_selector),);
            $this->data['getAllStartUpScreens'] = $this->settings_model->getAllStartUpScreens();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function permissions($id = NULL) {
        $this->settings_model->ensureProductionUnitPermissionColumns();
        ...
        if ($this->form_validation->run() == TRUE) {
            $data = array('products-index' => $this->input->post('products-index'),
                ...
                'products-mrp'   => $this->input->post('products-mrp'),
                ////////////////////////// Mrp , Price , Cost//////////////////////////////////
                ...
                'Allow_Stock_Addition' =>  $this->input->post('products-ad_type_addition') == '1' ? 1 : 0,
                'Allow_Stock_Subtraction' =>  $this->input->post('products-ad_type_subtraction') == '1' ? 1 : 0,
                'products-adjustments_byCSV' =>  $this->input->post('products-adjustments_byCSV') == '1' ? 1 : 0,
                ...
                'pos-select_season' => (int) $this->input->post('pos-select_season'),
                ...
                'product_remove' => $this->input->post('product_remove'),
				...
				'Procurement_Orders' => $this->input->post('Procurement_Orders'), 
                'Order_Dispatch' => $this->input->post('Order_Dispatch'), 
                'production_manager_dashboard' => $this->input->post('production_manager_dashboard'), 
                'production_dashboard' => $this->input->post('production_dashboard') ? 1 : 0,
                'production_unit-add_product' => $this->input->post('production_unit-add_product'),
                'production_unit-ready_to_dispatch' => $this->input->post('production_unit-ready_to_dispatch'),
                // 'production_unit-recipes' => $this->input->post('production_unit-recipes'),
                // 'production_unit-ok_dashboard' => $this->input->post('production_unit-ok_dashboard'),
                // 'production_unit-kds' => $this->input->post('production_unit-kds'),
                'production_unit-receive_delivery' => $this->input->post('production_unit-receive_delivery'),
                'production_unit-inventory' => $this->input->post('production_unit-inventory'),
                'production_unit-ordering_history' => $this->input->post('production_unit-ordering_history'),
                'outlet_kitchen' => $this->input->post('outlet_kitchen'),
                'Recipies' => $this->input->post('Recipies'),
                'OK_Recipies' => $this->input->post('OK_Recipies'),
                'OK_Dashboard' => $this->input->post('OK_Dashboard'),
                'KDS' => $this->input->post('KDS'),
                // New Report
                'reports-adjustments-reports' => $this->input->post('adjustments-reports'), // returns '1' if checked, NULL if unchecked
                'reports-best-seller' => $this->input->post('best-seller'),
                'reports-brand-reports' => $this->input->post('brand-reports'),
                'reports-categories-reports' => $this->input->post('categories-reports'),
                'reports-challan-reports' => $this->input->post('challan-reports'),
                'reports-customer-ledgers' => $this->input->post('customer-ledgers'),
                'reports-deposit-ledgers' => $this->input->post('deposit-ledgers'),
                'reports-deposit-recharge-ledgers' => $this->input->post('deposit-recharge-ledgers'),
                'reports-due-purchase-reports' => $this->input->post('due-purchase-reports'),
                'reports-due-sales-reports' => $this->input->post('due-sales-reports'),
                'reports-overview-chart' => $this->input->post('overview-chart'),
                'reports-payments-summary' => $this->input->post('payments-summary'),
                'reports-product-varient-purchase-report' => $this->input->post('product-varient-purchase-report'),
                'reports-product-varient-stock-report' => $this->input->post('product-varient-stock-report'),
                'reports-product_variant_sale_report' => $this->input->post('reports-product_variant_sale_report'),
                'reports-product-combo-item' => $this->input->post('product-combo-item'),
                'reports-product-costing' => $this->input->post('product-costing'),
                'reports-product-profit-loss-report' => $this->input->post('product-profit-loss-report'),
                'reports-product-wise-sale-report' => $this->input->post('product-wise-sale-report'),
                'reports-profit-loss-report' => $this->input->post('profit-loss-report'),
                'reports-register-report' => $this->input->post('register-report'),
                'reports-sales-extended-report' => $this->input->post('sales-extended-report'),
                'reports-sales-person-report' => $this->input->post('reports-sales-person-report'),
                'reports-staff-report' => $this->input->post('reports-staff-report'),
                'reports-tax-report' => $this->input->post('tax-report'),
                'reports-term-wise-sale-report' => $this->input->post('term-wise-sale-report'),
                'reports-transfer-report' => $this->input->post('transfer-report'),
                'reports-user-log-report' => $this->input->post('user-log-report'),
                'reports-warehouse-stock-report' => $this->input->post('warehouse-stock-report'),
                'reports-cash-transaction-report' => $this->input->post('cash-transaction-report'),
                'attendance-report' => $this->input->post('attendance-report'),
                //RawMaterials
                'raw_materials' => $this->input->post('raw_materials'),
                'sales_transaction_report_UAE' => $this->input->post('sales_transaction_report_UAE'),
						  
				//POS Screen 
               'pos-category_search' => $this->input->post('pos-category_search'),
                'pos-discription' => $this->input->post('pos-discription'),
                'pos-apply_coupon' => $this->input->post('pos-apply_coupon'),

                'pos-return' => $this->input->post('pos-return'),

                'pos-exchange' => $this->input->post('pos-exchange'),

                'pos-customer_scan' => $this->input->post('pos-customer_scan'),

                'pos-show_season' => $this->input->post('pos-show_season'),		  



                // 3p_orders

                'urbanpiper_orders_3p' => $this->input->post('orders_3p'),

                'redirecting_login_orders_3p' => $this->input->post('redirecting_login_orders_3p'),

                'up_orders_3p' => $this->input->post('up_orders_3p'),



                //Todays Special

                'todays_special' => $this->input->post('todays_special'),
                ///////////////////////////////////////////////////////////////attendance_module/////////////////////////////////////////////////////////////
                'attendance-index' => $this->input->post('attendance-index'),
                'attendance-create' => $this->input->post('attendance-create'),
                'attendance-edit' => $this->input->post('attendance-edit'),
                'attendance-delete' => $this->input->post('attendance-delete'),
                ///////////////////////////////////////////////////////////////attendance_module/////////////////////////////////////////////////////////////
                ...
                'challan-index' => $this->input->post('challan-index'),
                'challan-add' => $this->input->post('challan-add'),
                'challan-edit' => $this->input->post('challan-edit'),
                'challan-delete' => $this->input->post('challan-delete'),
                'challan-payments' => $this->input->post('challan-payments'),
                'challan-return' => $this->input->post('challan-return'),
                'challan-pdf' => $this->input->post('challan-pdf'),
                'challan-email' => $this->input->post('challan-email'),
                'challan-add_sale' => $this->input->post('challan-add_sale'),
                'challan-add_payment' => $this->input->post('challan-add_payment'),
                'challan-edit_payment' => $this->input->post('challan-edit_payment'),
                'reports-challan' => $this->input->post('reports-challan'),
                'challancompleted_status' => $this->input->post('challancompleted_status'),
            );
            if (POS) {
    ...
    function add_category() {
        ...
        if ($this->form_validation->run() == TRUE) {
           ...
           if($this->input->post('fix_tax_rate')=='variable tax'){
                $taxslabs = array();
                foreach($this->input->post('price') as $key => $price){
                    if($price){
                        $taxslabs[]=[
                            'price'=>$price,
                            'condition'=>$this->input->post('condition')[$key],
                            'taxratevalue'=>$this->input->post('taxratevalue')[$key],
                            'upto' => $this->input->post('upto')[$key]
                        ];
                    }
                }
                $taxdata = serialize($taxslabs);
            }
            ...
            $data = array(
                ...
                'warehouse_id' =>  ($this->input->post('warehouse') == '') ? "*All" : implode(",",$this->input->post('warehouse')),
                'fix_tax_rate' => $this->input->post('fix_tax_rate'),
                'tax_slabs' => isset($taxdata)? $taxdata : NULL,
    ...
    public function edit_category($id = NULL) {
        ...
        if ($this->form_validation->run() == TRUE && $this->settings_model->updateCategory($id, $data)) {
            ...
            $this->data['tax_rates'] = $this->settings_model->getAllTaxRates();
            $this->data['modal_js'] = $this->site->modal_js();  
            $this->data['warehous'] =  $id = $this->site->warehouseById($id);
            $this->data['warehouses'] = $this->site->getAllWarehouses();
    ...
    public function category_actions() {
        ...
        $this->form_validation->set_rules('form_action', lang("form_action"), 'required');
        $values = explode(",", $_POST['Checkbox'][0]);
        $_POST['Checkbox'] = array_map('intval', $values);
        if ($_POST['Checkbox'][0] < 1) {
           $_POST['Checkbox'] = '';
       }
        if ($this->form_validation->run() == TRUE) {
    ...
    public function add_tax_rate() {
        ...
        $this->form_validation->set_rules('surcharge_rate', lang("surcharge_rate"), 'numeric');
        $this->form_validation->set_rules('surcharge_type', lang("surcharge_type"), 'trim');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data = array(
                ...
                'surcharge_rate' => $surcharge_rate !== '' ? $surcharge_rate : NULL,
                'surcharge_type' => $surcharge_type !== '' ? $surcharge_type : NULL,
                'tax_config' => serialize($tax_config),
            );
    ...
    public function add_warehouse() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data = [
                ...
                'workstations' => ($this->input->post('workstations') && is_array($this->input->post('workstations')) && !empty($this->input->post('workstations'))) ? implode(',', array_filter($this->input->post('workstations'))) : NULL,
           ...
           $radiusPost = $this->warehouse_location_radius_post_value();
            if ($radiusPost !== null) {
                $data['location_radius_m'] = $radiusPost;
            }
        ...
        if ($this->form_validation->run() == TRUE && $this->settings_model->addWarehouse($data)) {
            ...
            // Get only warehouses with location_type = 'Workstation' for the Workstations dropdown
            $this->data['warehouses_list'] = $this->settings_model->getWarehousesByLocationType('Workstation');
            if (!$this->data['warehouses_list']) {
                $this->data['warehouses_list'] = array();
            }
    ...
    public function edit_warehouse($id = NULL) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $stateInfo = explode('~', $this->input->post('state'));
            $other_billers = $this->input->post('other_billers');
            if (is_array($other_billers)) {
                $other_billers = implode(',', $other_billers);
            } else {
                $other_billers = $other_billers;
            ...
            $data = [
                ...
                'address_line1' => $this->input->post('address_line1'),
                'address_line2' => $this->input->post('address_line2'),
                'primary_biller_id' => $this->input->post('primary_billers'),
                'other_biller_id' => $other_billers,
                'bill_prefix' => $this->input->post('bill_prefix'),
                'workstations' => ($this->input->post('workstations') && is_array($this->input->post('workstations')) && !empty($this->input->post('workstations'))) ? implode(',', array_filter($this->input->post('workstations'))) : NULL,
                'customer_id' => $customer_id,
            ...
            if ($_FILES['userfile']['size'] > 0) {
                ...
                $this->load->library('image_lib');
                $config['image_library'] = 'gd2';
                $config['source_image'] = 'assets/mdata/'.$this->Customer_assets.'/uploads/' . $data['map'];
                $config['new_image'] = 'assets/mdata/'.$this->Customer_assets.'/uploads/thumbs/' . $data['map'];
                $config['maintain_ratio'] = TRUE;
                $config['width'] = 76;
                $config['height'] = 76;
        ...
        if ($this->form_validation->run() == TRUE && $this->settings_model->updateWarehouse($id, $data)) { //check to see if we are updateing the customer
            ...
            $this->data['id'] = $id;
            $customer = $this->site->getCompanyByID($this->data['warehouse']->customer_id);
            $this->data['customer_name'] = $customer ? $customer->name : '';
            $this->data['customer_id']   = $customer ? $customer->id : '';
            $this->data['country'] = $this->site->getCountry();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function delete_warehouse($id = NULL) {
        if ($this->settings_model->warehouseUsedInTransactions($id)) {
            if ($this->input->is_ajax_request()) {
                echo lang("Warehouses can't be deleted because it is already used in transactions.");
                die();
            }
    ...
    public function getVariants() {
        ...
        $this->load->library('datatables');
        $this->datatables->select("{$this->db->dbprefix('variants')}.id as id, {$this->db->dbprefix('variants')}.name as name, {$this->db->dbprefix('variants_group')}.name as variant_group")->from("variants")->join('variants_group', 'variants_group.id=variants.group_id', 'left')->add_column("Actions", "<div class=\"text-center\"><a href='" . site_url('system_settings/edit_variant/$1') . "' class='tip' title='" . lang("edit_variant") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_variant") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('system_settings/delete_variant/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        //->unset_column('id');
    ...
    public function add_variant() {
        ...
        if ($this->form_validation->run() == TRUE) {
            $data = array('name' => $this->input->post('name'), 'group_id' => $this->input->post('variant_groupId'));
        ...
        if ($this->form_validation->run() == TRUE && $this->settings_model->addVariant($data)) {
            ...
            // Also insert into sma_manage_variant table
            $manage_variant_data = array(
                'variant_id' => $insert_id,
                'variant_name' => $data['name'],
                'status' => '1'
            );
            $this->db->insert('sma_manage_variant', $manage_variant_data);
            
            $DataLog = array(
                'action_type' => 'Add',
                'product_id' => '',
                'quantity' => '',
                'action_reff_id' => $insert_id,
                'action_affected_data' => json_encode($data),
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['variant_group'] = $this->settings_model->getAllvariantgroup();
    ...
    public function getExpenseCategories() {
        ...
        $this->load->library('datatables');
        $this->datatables->select("expense_categories.id as id, expense_categories.code as code, expense_categories.name as name, p.name as parent", false)
            ->from("expense_categories")
            ->join("expense_categories p", "p.id = expense_categories.parent_id", "left")
    ...
    public function add_expense_category() {
        ...
        if ($this->form_validation->run() == TRUE && $this->settings_model->addExpenseCategory($data)) {
            ...
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['categories'] = $this->settings_model->getParentExpenseCategories();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function edit_expense_category($id = NULL) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ((int) $parent_id === (int) $id) {
                $this->session->set_flashdata('error', 'Parent category cannot be same as category.');
                redirect("system_settings/expense_categories");
            }
            $data = array(
                'code' => $this->input->post('code'),
        ...
        if ($this->form_validation->run() == TRUE && $this->settings_model->updateExpenseCategory($id, $data)) {
            ...
            $categories = $this->settings_model->getParentExpenseCategories();
            if (!empty($categories)) {
                foreach ($categories as $key => $pcat) {
                    if ((int) $pcat->id === (int) $id) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A2', lang('code'));
                    $this->excel->getActiveSheet()->SetCellValue('B2', lang('name'));
                    $this->excel->getActiveSheet()->SetCellValue('C2', lang('parent_category'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        if (!empty($sc->parent_id)) {
                            $parent_category = $this->settings_model->getExpenseCategoryByID($sc->parent_id);
                            $parent_name = $parent_category ? $parent_category->name : '';
                        }
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sc->code);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $sc->name);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $parent_name);
                        $row++;
                    }
...
// As per requrement  New updated Code for validations of import Category  (11-6-2025)
    public function import_categories() {

        $this->load->helper('security');
        $this->form_validation->set_rules('userfile', lang("upload_file"), 'xss_clean');
        // 🔧 FIX 1: Allow upload to proceed even if form validation fails
        if (!empty($_FILES['userfile']['name'])) {
            $upload_ready = true; // allow upload manually
        } else {
            $upload_ready = false;
        }
        if ($this->form_validation->run() == TRUE || $upload_ready == true) { // 🔧 FIX 2: Add OR condition for file upload
            ...
            if (isset($_FILES["userfile"])) {
                ...
                // 🔧 FIX 3: Specify input name in do_upload('userfile')
                ...
                if ($handle) {
                    while (($row = fgetcsv($handle, 5000, ",")) !== FALSE) {
                        // 🔧 FIX 4: Skip blank or BOM lines
                        if (empty(array_filter($row))) continue;
                        $row = array_map(function ($field) {
                            return trim(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $field));
                        }, $row);
                        $arrResult[] = $row;
                    }
                    fclose($handle);
                ...
                $titles = array_shift($arrResult);
                $keys = array('code', 'name', 'image', 'pcode', 'up_category', 'tax_rate', 'flag_visible');
                $final = array();
                ...
                // 🔧 FIX 6: Initialize $data before use
                $data = array();
                foreach ($final as $index => $row_check) {
                    $cat_code = trim($row_check['code']);
                    $cat_name = trim($row_check['name']);
                    $pcode = trim($row_check['pcode']);
                    // 🆕 NEW VALIDATION: name & code required (strict)
                    if (empty($cat_code) && empty($cat_name)) {
                        $this->session->set_flashdata('error', 'Error on line ' . ($index + 2) . ': Category code and name are required.');
                        redirect("system_settings/categories");
                        exit;
                    }

                    if (empty($cat_code)) {
                        $this->session->set_flashdata('error', 'Error on line ' . ($index + 2) . ': Category code is required.');
                        redirect("system_settings/categories");
                        exit;
                    }

                    if (empty($cat_name)) {
                        $this->session->set_flashdata('error', 'Error on line ' . ($index + 2) . ': Category name is required.');
                        redirect("system_settings/categories");
                        exit;
                    }

                    // 1️⃣ Existing validation: code and parent code same
                    if (!empty($pcode) && $cat_code === $pcode) {
                        $this->session->set_flashdata('error', 'Error on line ' . ($index + 2) . ': Category code and parent category code should not be the same (' . $cat_code . ').');
                        redirect("system_settings/categories");
                        exit;
                    }

                    // 2️⃣ NEW VALIDATION: name and parent code same
                    if (!empty($pcode) && strcasecmp($cat_name, $pcode) === 0) {
                        $this->session->set_flashdata('error', 'Error on line ' . ($index + 2) . ': Category name and parent category code should not be the same (' . $cat_name . ').');
                        redirect("system_settings/categories");
                        exit;
                    }
                    // 3️⃣ NEW VALIDATION: parent category must exist in DB
                    if (!empty($pcode)) {
                        $parent_exists = $this->settings_model->getCategoryByCode($pcode);
                        if (!$parent_exists) {
                            $this->session->set_flashdata('error', 'Error on line ' . ($index + 2) . ': Parent category code (' . $pcode . ') not found in system. Please import parent category first.');
                            redirect("system_settings/categories");
                            exit;
                        }
                    }
                ...
                foreach ($final as $csv_ct) {
                    if (!$this->settings_model->getCategoryByCode(trim($csv_ct['code'])) && !in_array(trim($csv_ct['code']), $dup_category)) {
                        ...
                        // 🔧 FIX 7: Always define $tax_rate object
                        $tax_rate = (object)['id' => 1];
                        if (!empty($csv_ct['tax_rate'])) {
                            $tax_rate_found = $this->settings_model->getTaxByRate(str_replace('%', '', $csv_ct['tax_rate']));
                            if ($tax_rate_found) {
                                $tax_rate = $tax_rate_found;
                            }
                        }

                        // 🔧 FIX 8: Correct parent-child logic
                        if (empty($pcode)) {
                            ...
                            $data[] = array(
                                ...
                                'flag_visible' => (isset($csv_ct['flag_visible']) && $csv_ct['flag_visible'] != '') ? trim($csv_ct['flag_visible']) : 1
                            );
                        } else {
                            // Child category
                            if ($pcategory = $this->settings_model->getCategoryByCode(trim($csv_ct['pcode']))) {
        ...
        // 🔧 FIX 9: Ensure data exists before inserting
        if (!empty($data) && $this->settings_model->addCategories($data)) {
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['userfile'] = array('name' => 'userfile', 'id' => 'userfile', 'type' => 'file', 'value' => $this->form_validation->set_value('userfile'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/import_categories', $this->data);
        }
    }
    ...
    public function import_subcategories() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $titles = array_shift($arrResult);
                $keys = array('code', 'name', 'category_code', 'image', 'tax_rate', 'flag_visible');
                $final = array();
                foreach ($arrResult as $key => $value) {
                    $final[] = array_combine($keys, $value);
                }
    ...
    public function import_expense_categories() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $parentRefsInFile = array();
                foreach ($final as $csv_ct) {
                    $currentCode = trim($csv_ct['code']);
                    $currentName = trim($csv_ct['name']);
                    if ($currentCode !== '') {
                        $parentRefsInFile[strtolower($currentCode)] = 0;
                    }
                    if ($currentName !== '') {
                        $parentRefsInFile[strtolower($currentName)] = 0;
                    }
                }

                $pendingRows = array();
                $existingRows = array();
                foreach ($final as $line_no => $csv_ct) {
                    $code = trim($csv_ct['code']);
                    $name = trim($csv_ct['name']);
                    $parent_code = trim($csv_ct['parent_code']);
                    if ($code === '' || $name === '') {
                        continue;
                    }
                    if ($this->settings_model->getExpenseCategoryByCode($code)) {
                        $existingRows[] = array('code' => $code, 'line_no' => $line_no + 2);
                        continue;
                    }
                    if ($parent_code === '') {
                        $data[] = array('code' => $code, 'name' => $name, 'parent_id' => 0);
                        continue;
                    }
                    $parent_category = $this->settings_model->getExpenseCategoryByCode($parent_code);
                    if (!$parent_category) {
                        $parent_category = $this->settings_model->getExpenseCategoryByCode(strtoupper($parent_code));
                    }
                    if (!$parent_category) {
                        $parent_category = $this->settings_model->getExpenseCategoryByCode(strtolower($parent_code));
                    }
                    if (!$parent_category) {
                        $parent_category = $this->settings_model->getExpenseCategoryByName($parent_code);
                    }
                    if ($parent_category) {
                        $data[] = array('code' => $code, 'name' => $name, 'parent_id' => $parent_category->id);
                    } elseif (array_key_exists(strtolower($parent_code), $parentRefsInFile)) {
                        $pendingRows[] = array('code' => $code, 'name' => $name, 'parent_code' => $parent_code, 'line_no' => $line_no + 2);
                    } else {
                        $this->session->set_flashdata('error', lang("check_category_code") . " (" . $parent_code . "). " . lang("category_code_x_exist") . " " . lang("line_no") . " " . ($line_no + 2));
                        redirect("system_settings/expense_categories");
                    }
                }
                ...
                if (!empty($existingRows)) {
                    ...
                    foreach ($existingRows as $existing) {
                        $existingList[] = $existing['code'] . ' (' . lang('line_no') . ' ' . $existing['line_no'] . ')';
                    }
                    $this->session->set_flashdata('error', 'These category code(s) already exist: ' . implode(', ', $existingList));
                    redirect("system_settings/expense_categories");
                }

                if (!empty($pendingRows)) {
                    if (!empty($data)) {
                        $this->settings_model->addExpenseCategories($data);
                        $data = array();
                    }
                    foreach ($pendingRows as $pending) {
                        $parent_category = $this->settings_model->getExpenseCategoryByCode($pending['parent_code']);
                        if (!$parent_category) {
                            $parent_category = $this->settings_model->getExpenseCategoryByCode(strtoupper($pending['parent_code']));
                        }
                        if (!$parent_category) {
                            $parent_category = $this->settings_model->getExpenseCategoryByCode(strtolower($pending['parent_code']));
                        }
                        if (!$parent_category) {
                            $parent_category = $this->settings_model->getExpenseCategoryByName($pending['parent_code']);
                        }
                        if (!$parent_category) {
                            $this->session->set_flashdata('error', lang("check_category_code") . " (" . $pending['parent_code'] . "). " . lang("category_code_x_exist") . " " . lang("line_no") . " " . $pending['line_no']);
                            redirect("system_settings/expense_categories");
                        }
                        $data[] = array('code' => $pending['code'], 'name' => $pending['name'], 'parent_id' => $parent_category->id);
                    }
                }
            }
        ...
        if ($this->form_validation->run() == TRUE && !empty($data) && $this->settings_model->addExpenseCategories($data)) {
            ...
            $this->session->set_flashdata('message', lang("categories_added"));
            redirect('system_settings/expense_categories');
        } elseif ($this->form_validation->run() == TRUE && empty($data) && $this->input->post('import')) {
            $this->session->set_flashdata('message', lang("categories_added"));
            redirect('system_settings/expense_categories');
        } else {
    ...
    public function update_product_group_price($group_id = NULL) {
        ...
        if (!empty($product_id) && isset($price)) {
    ...
    public function import_pending_subcategory($subcategory_arr) {
        ...
        foreach ($subcategory_arr as $csv_ct):
            if (!$this->settings_model->getCategoryByCode(trim($csv_ct['code'])) && !in_array(trim($csv_ct['code']), $dup_data)) {
                ...
                if (!empty($pcode)) {
                    if ($pcategory = $this->settings_model->getCategoryByCode(trim($csv_ct['pcode']))) {
                        $data[] = array('code' => trim($csv_ct['code']), 'name' => trim($csv_ct['name']), 'image' => trim($csv_ct['image']), 'parent_id' => $pcategory->id, 'tax_rate' => $csv_ct['tax_rate'], 'flag_visible' => (isset($csv_ct['flag_visible']) && $csv_ct['flag_visible'] != '') ? trim($csv_ct['flag_visible']) : 1);
    ...
    public function edit_printer_bill($id) {
        ...
        if ($this->form_validation->run() == TRUE) {
            if (!empty($_POST['id']) && $id == $_POST['id']) {
                ...
                $l_field_name = !empty($l_field_name) ? $l_field_name : $l_field_obj->name;
                ...
                $data['show_invoice_lines'] = !empty($this->input->post('show_invoice_lines')) ? $this->input->post('show_invoice_lines') : 0;
                $data['show_transporter_details'] = !empty($this->input->post('show_transporter_details')) ? $this->input->post('show_transporter_details') : 0;
                $show_discount_in_percentage = (int) $this->input->post('show_discount_in_percentage');
                $data['show_discount_in_percentage'] = in_array($show_discount_in_percentage, array(0, 1, 2), true) ? $show_discount_in_percentage : 0;
                $data['invoice_display_time'] = !empty($this->input->post('invoice_display_time')) ? $this->input->post('invoice_display_time') : 0;
                ...
                $data['show_order_type'] = !empty($this->input->post('show_order_type')) ? $this->input->post('show_order_type') : 0;
                /* Display Category */
                $data['display_category'] = !empty($this->input->post('display_category')) ? 1 : 0;
   ...
   public function get_restaurant_tables() {
    ...
    public function custom_fields() {
        ...
        if ($this->input->post('update_settings')) {
            ...
            $existing = $this->settings_model->getCustomFields();
            $data = array();

            foreach ($cf_type as $type) {
                $row = array('type' => $type);
                $start = ($type === 'employee') ? 2 : 1;

                for ($i = 1; $i <= 6; $i++) {
                    if ($i < $start) {
                        $row['cf' . $i] = isset($existing[$type][0]->{'cf' . $i}) ? $existing[$type][0]->{'cf' . $i} : null;
                    } else {
                        $posted = isset($_POST[$type]['cf' . $i]) ? trim($_POST[$type]['cf' . $i]) : '';
                        $row['cf' . $i] = $posted !== '' ? $posted : null;
                    }
                }
    ...
    public function add_discount_coupon() {
        if ($_POST) {
            ...
            $this->form_validation->set_rules('expiry_date', lang('Expiry Date'), 'trim|required');
             $this->form_validation->set_rules('max_coupons', lang('Max Coupons'), 'trim|required');
            if ($this->form_validation->run() == TRUE) {
   ...
   /**
    ...
    function orderingItemByCategories($CategoryId){
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $this->data['categoryName'] = $this->products_model->getCategoryById($CategoryId);
        $this->data['products'] = $this->products_model->getCategoryByProducts($CategoryId);
        $this->data['modal_js'] = $this->site->modal_js();
        $this->load->view($this->theme . 'settings/ordering_item', $this->data);
	}
    // function saveOrderingItem($CategoryId = null){
    //     $variants = $this->input->get('variants');
    //     if (empty($variants)) {
    //         $this->output
    ...
    //             'rank_no'      => $variant['rankNo'],
    //             'is_selected'  =>  $variant['isSelected'] == 'true' ? '1' : '0'
    //         ];
    //     }
    //     if (!empty($data)) {
    //         $result =   $this->settings_model->saveOrdering($data);
    //         if ($result) {
    ...
    public function saveCategories() {
        ...
        foreach ($categories as $category) {
            $data[] = [
                'id' => $category['id'],
                'rank' => $category['rank'],
                'flag_visible' => $category['visible']
            ];
        }
        $this->settings_model->update_category($data);
        $affected_rows = $this->db->affected_rows();
        if ($affected_rows > 0) {
            $this->session->set_flashdata('message', lang("Rank Updated Successfully"));
            echo json_encode(['status' => 'success']);
        } else {
            $this->session->set_flashdata('error', 'No Rank Updated.');
            echo json_encode(['status' => 'error']);
        }
    }

    //Manage Variant
    public function variant_manage(){
        if($_POST){
            foreach($_POST['manage_variant'] as $kye => $variant){
                  $getvariant = explode("~", $variant);
                  $vareiantdata[]= array(
                    'variant_id' => $getvariant[0],
                    'variant_name' => $getvariant[1],
                    'status' => '1',
                  );
            }
            $result = $this->settings_model->storeManageVariant($vareiantdata);
            if($result){
                $this->session->set_flashdata('message', 'Variant has been manage successfuly.');
                return redirect('system_settings/variant_manage');  
            } else {
                $this->session->set_flashdata('error', 'Variant not manage. Please try again.');
                return redirect('system_settings/variant_manage');  
            }
        } else {
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('Manage Variant')));
            $meta = array('page_title' => lang('Manage Variant'), 'bc' => $bc);
            $this->data['variant'] = $this->settings_model->getvariant(SIZE);
            $this->data['managevariant'] = $this->settings_model->getManagevareiant();
            $this->page_construct('settings/manage_variant', $meta, $this->data);
        } 
    }
    ///////////////////////////////////// POS type labels ///////////////////////////////////// 
    /**
     * POS type labels: list labels for current Settings->pos_type (admin & user menu).
     */
    public function pos_type_labels() {
        $pos_type = isset($this->Settings->pos_type) ? trim($this->Settings->pos_type) : '';
        if ($pos_type === '') {
            $this->session->set_flashdata('error', lang('pos_type_not_set'));
            redirect('system_settings');
        }
        $this->load->model('pos_type_labels_model');
        $this->data['pos_type'] = $pos_type;
        $this->data['labels'] = $this->pos_type_labels_model->get_rows_by_pos_type($pos_type);
        $this->data['suggested_label_keys'] = $this->_get_suggested_pos_label_keys();
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $this->data['message'] = $this->session->flashdata('message');
        $bc = array(
            array('link' => base_url(), 'page' => lang('home')),
            array('link' => site_url('system_settings'), 'page' => lang('system_settings')),
            array('link' => '#', 'page' => lang('menu_labels'))
        );
        $meta = array('page_title' => lang('menu_labels'), 'bc' => $bc);
        $this->page_construct('settings/pos_type_labels', $meta, $this->data);
    }

    /**
     * Label keys for dropdown: read from admin_access_menu and user_access_menu files only (no hardcoded list).
     */
    protected function _get_suggested_pos_label_keys() {
        if (!function_exists('get_pos_menu_label_keys_from_views')) {
            $this->load->helper('genfun_helper');
        }
        return get_pos_menu_label_keys_from_views();
    }

    /**
     * Add a new label (modal form or post).
     */
    public function add_pos_type_label() {
        $pos_type = isset($this->Settings->pos_type) ? trim($this->Settings->pos_type) : '';
        if ($pos_type === '') {
            $this->session->set_flashdata('error', lang('pos_type_not_set'));
            redirect('system_settings');
        }
        $this->form_validation->set_rules('label_key', lang('label_key'), 'trim|required');
        $this->form_validation->set_rules('label_value', lang('label_value'), 'trim|required');
        if ($this->form_validation->run() == TRUE) {
            $this->load->model('pos_type_labels_model');
            $label_key = $this->input->post('label_key');
            $label_value = $this->input->post('label_value');
            if ($this->pos_type_labels_model->set_label($pos_type, $label_key, $label_value)) {
                $this->session->set_flashdata('message', lang('label_added'));
                redirect('system_settings/pos_type_labels');
            }
        }
        if ($this->input->post('add_label')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect('system_settings/pos_type_labels');
        }
        $this->data['pos_type'] = $pos_type;
        ...
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $this->data['modal_js'] = $this->site->modal_js();
        $this->load->view($this->theme . 'settings/add_pos_type_label', $this->data);
    }

    /**
     * Edit label by id (modal form or post).
     */
    public function edit_pos_type_label($id = NULL) {
        if (!$id) {
            redirect('system_settings/pos_type_labels');
        }
        $pos_type = isset($this->Settings->pos_type) ? trim($this->Settings->pos_type) : '';
        ...
        $this->load->model('pos_type_labels_model');
        $row = $this->db->get_where('pos_type_labels', array('id' => (int) $id, 'pos_type' => $pos_type), 1)->row();
        if (!$row) {
            $this->session->set_flashdata('error', lang('label_not_found'));
            redirect('system_settings/pos_type_labels');
        }
        $this->form_validation->set_rules('label_key', lang('label_key'), 'trim|required');
        $this->form_validation->set_rules('label_value', lang('label_value'), 'trim|required');
        if ($this->form_validation->run() == TRUE) {
            $label_key = $this->input->post('label_key');
            $label_value = $this->input->post('label_value');
            $this->db->where('id', (int) $id)->where('pos_type', $pos_type)
                ->limit(1)->update('pos_type_labels', array('label_key' => $label_key, 'label_value' => $label_value));
            $this->session->set_flashdata('message', lang('label_updated'));
            redirect('system_settings/pos_type_labels');
        }
        if ($this->input->post('edit_label')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect('system_settings/pos_type_labels');
        }
        $this->data['label_row'] = $row;
        $this->data['pos_type'] = $pos_type;
        $this->data['suggested_label_keys'] = $this->_get_suggested_pos_label_keys();
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $this->data['modal_js'] = $this->site->modal_js();
        $this->load->view($this->theme . 'settings/edit_pos_type_label', $this->data);
    }

    /**
     * Save edit: update single label row by id.
     */
    public function save_pos_type_label() {
        ...
        $this->load->model('pos_type_labels_model');
        $row = $this->db->get_where('pos_type_labels', array('id' => $id, 'pos_type' => $pos_type), 1)->row();
        if (!$row) {
            $this->session->set_flashdata('error', lang('label_not_found'));
            redirect('system_settings/pos_type_labels');
        }
        ...
        if ($this->form_validation->run() == TRUE) {
            $label_key = $this->input->post('label_key');
            $label_value = $this->input->post('label_value');
            $this->db->where('id', $id)->update('pos_type_labels', array('label_key' => $label_key, 'label_value' => $label_value));
            $this->session->set_flashdata('message', lang('label_updated'));
            redirect('system_settings/pos_type_labels');
        }
        $this->session->set_flashdata('error', validation_errors());
     ...
     * Delete label by id.
     */
    public function delete_pos_type_label($id = NULL) {
        if (!$id) {
            redirect('system_settings/pos_type_labels');
        }
        $pos_type = isset($this->Settings->pos_type) ? trim($this->Settings->pos_type) : '';
        ...
        $this->load->model('pos_type_labels_model');
        $row = $this->db->get_where('pos_type_labels', array('id' => (int) $id, 'pos_type' => $pos_type), 1)->row();
        if ($row && $this->pos_type_labels_model->delete_label($id)) {
            $this->session->set_flashdata('message', lang('label_deleted'));
        } else {
            $this->session->set_flashdata('error', lang('label_not_found'));
        }
        redirect('system_settings/pos_type_labels');
    ...
    public function search_customers(){
        ...
        foreach ($customers as $c) {
            $name = !empty($c->name) ? $c->name : 'No Name';
            $data[] = [
                'id'    => $c->id,
                'label' => $name,   
                'value' => $name    
     ...
     /**
     * Posted warehouse attendance geofence radius (meters), or null if DB column is missing.
     *
     * @return int|null
     */
    private function warehouse_location_radius_post_value()
    {
        if (!$this->db->field_exists('location_radius_m', 'warehouses')) {
            return null;
        }
        $raw = $this->input->post('location_radius_m');
        $r = ($raw !== '' && $raw !== null) ? (int) $raw : 100;
        if ($r < 10) {
            $r = 10;
        }
        // Fits INT UNSIGNED; practical “no ceiling” for regional geofences.
        if ($r > 9999999) {
            $r = 9999999;
        }
        return $r;
    }
    public function group_settings_screens($group_id = NULL)
    {
        ...
        $group = $this->settings_model->getGroupByID($group_id);
        if (!$group) {
            $this->output->set_content_type('application/json')->set_output(json_encode(array(
                'status' => 'error',
        ...
        $row = $this->settings_model->getGroupSettingsScreens($group_id);
        $screens = array();
        foreach (settings_screen_name_column_map() as $screen_name => $column) {
            ...
            $screens[] = array(
                ...
                'column' => $column,
                'active' => (int) (($row && isset($row[$column])) ? $row[$column] : 0),
            );
        }
    ...
    public function update_group_settings_screen()
    {
        $csrf_hash = $this->security->get_csrf_hash();
        if (!$this->Owner) {
            $this->output->set_content_type('application/json')->set_output(json_encode(array(
        ...
        $column = trim((string) $this->input->post('column'));
        $active = (int) $this->input->post('active');
        $allowed_columns = array_values(settings_screen_name_column_map());

        if ($group_id <= 0 || !$column || !in_array($column, $allowed_columns, true)) {
            $this->output->set_content_type('application/json')->set_output(json_encode(array(
                'status' => 'error',
        ...
        if (!$this->settings_model->getGroupByID($group_id)) {
            $this->output->set_content_type('application/json')->set_output(json_encode(array(
                'status' => 'error',
                'message' => 'Group not found',
                'csrf_hash' => $csrf_hash,
        ...
        $ok = $this->settings_model->updateGroupSettingsScreenStatus($group_id, $column, ($active === 1 ? 1 : 0));
        $this->output->set_content_type('application/json')->set_output(json_encode(array(
            'status' => $ok ? 'success' : 'error',
            'message' => $ok ? 'Updated' : 'Unable to update',
            'csrf_hash' => $csrf_hash,
        )));
    }
