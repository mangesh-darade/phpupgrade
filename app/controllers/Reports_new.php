...
class Reports_new extends MY_Controller {
    ...
    public function sales_extended_report() {
        // $this->sma->checkPermissions('sales-extended-report');
    ...
    public function xls_export_sales_extended_report() {
        ...
        $currency_symbol = $this->sma->formatMoney(0);
        $currency_symbol = preg_replace('/[0-9.,\s]/', '', $currency_symbol); // Extract just the symbol
        
        $this->excel->getActiveSheet()->SetCellValue('M2', lang('Grand Total') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('N2', lang('Taxable Amount') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('O2', lang('Tax Amount') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('P2', lang('Discount Amount') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('Q2', lang('Shipping Amount') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('R2', lang('Paid') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('S2', lang('Balance') . ' (' . $currency_symbol . ')');
        ...
        $this->excel->getActiveSheet()->SetCellValue('X2', lang('Sale CGST') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('Y2', lang('Sale SGST') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('Z2', lang('Sale IGST') . ' (' . $currency_symbol . ')');
        ...
        $this->excel->getActiveSheet()->SetCellValue('AJ2', lang('unit_price') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('AK2', lang('quantity'));
        $this->excel->getActiveSheet()->SetCellValue('AL2', lang('unit'));

        $this->excel->getActiveSheet()->SetCellValue('AM2', lang('mrp') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('AN2', lang('item_discount') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('AO2', lang('tax_method'));
        $this->excel->getActiveSheet()->SetCellValue('AP2', lang('tax_rate') . '(%)');
        $this->excel->getActiveSheet()->SetCellValue('AQ2', lang('item_tax') . ' (' . $currency_symbol . ')');
        $this->excel->getActiveSheet()->SetCellValue('AR2', lang('Subtotal') . ' (' . $currency_symbol . ')');

        $this->excel->getActiveSheet()->SetCellValue('AS2', lang('CGST Rate (%)'));
        $this->excel->getActiveSheet()->SetCellValue('AT2', lang('CGST') . ' (' . $currency_symbol . ')');

        $this->excel->getActiveSheet()->SetCellValue('AU2', lang('SGST Rate (%)'));
        $this->excel->getActiveSheet()->SetCellValue('AV2', lang('SGST') . ' (' . $currency_symbol . ')');

        $this->excel->getActiveSheet()->SetCellValue('AW2', lang('IGST (%)'));
        $this->excel->getActiveSheet()->SetCellValue('AX2', lang('IGST') . ' (' . $currency_symbol . ')');
        ...
        foreach ($data as $sale_id => $salesdata) {
            ...
            $address = "$sale_data->address $sale_data->city $sale_data->state_code $sale_data->postal_code";
            ...
            $this->excel->getActiveSheet()->SetCellValue('G' . $row, $address);
            ...
            // Monetary fields
            $fields = ['M' => $sale_data->grand_total, 'N' => $taxable_amt, 'O' => $sale_data->total_tax, 'P' => $sale_data->total_discount,
                       'Q' => $sale_data->shipping, 'R' => $sale_data->paid, 'S' => $balance, 'X' => $sale_data->sale_cgst,
                       'Y' => $sale_data->sale_sgst, 'Z' => $sale_data->sale_igst];
        
            foreach ($fields as $col => $value) {
                $this->excel->getActiveSheet()->SetCellValue($col . $row, $this->sma->formatMoney($value));
                $this->excel->getActiveSheet()->getStyle($col . $row)->applyFromArray($rightAlign);
            }
            ...
            // Quantities with formatQuantity
            $this->excel->getActiveSheet()->SetCellValue('AA' . $row, $this->sma->formatQuantity($sale_data->total_weight));
            $this->excel->getActiveSheet()->SetCellValue('AB' . $row, $this->sma->formatQuantity($sale_data->total_items));
            $this->excel->getActiveSheet()->getStyle('AA' . $row)->applyFromArray($centerAlign);
            $this->excel->getActiveSheet()->getStyle('AB' . $row)->applyFromArray($centerAlign);
        
            // Columns that need left alignment
            $alignColsLeft = ['V', 'H', 'I', 'L', 'W', 'AD', 'AE', 'AH', 'AP'];
            // Columns that need center alignment
            $alignColsCenter = ['K', 'AA', 'AB', 'AC', 'AK', 'AO', 'AQ', 'AU', 'AW', 'AY'];
            // Columns that need right alignment
            $alignColsRight = ['AR', 'AS'];
        
            foreach ($alignColsLeft as $col) {
                $this->excel->getActiveSheet()->getStyle($col . $row)->applyFromArray($leftAlign);
            }
            foreach ($alignColsCenter as $col) {
                $this->excel->getActiveSheet()->getStyle($col . $row)->applyFromArray($centerAlign);
            }
            foreach ($alignColsRight as $col) {
                $this->excel->getActiveSheet()->getStyle($col . $row)->applyFromArray($rightAlign);
            }
            ...
            // Sale Items (nested loop for items)
            $item_sr = 0;
            foreach ($salesdata['items'] as $sales_items_data) {
                $row++;
                $sales_items_data = (object) $sales_items_data;
                $item_sr++;
                ...
                // Prices & taxes (monetary fields)
                $item_fields = [
                    'AJ' => $sales_items_data->unit_price, 'AM' => $sales_items_data->mrp, 'AN' => $sales_items_data->item_discount,
                    'AT' => $sales_items_data->cgst, 'AV' => $sales_items_data->sgst, 'AX' => $sales_items_data->igst
                ];
                foreach ($item_fields as $col => $val) {
                    $this->excel->getActiveSheet()->SetCellValue($col . $row, $this->sma->formatMoney($val));
                    $this->excel->getActiveSheet()->getStyle($col . $row)->applyFromArray($rightAlign);
                }
        
                // Quantities
                $this->excel->getActiveSheet()->SetCellValue('AK' . $row, $this->sma->formatQuantity($sales_items_data->unit_quantity));
                $this->excel->getActiveSheet()->SetCellValue('AY' . $row, $this->sma->formatQuantity($sales_items_data->item_weight));
                $this->excel->getActiveSheet()->SetCellValue('AU' . $row, $this->sma->formatQuantity($sales_items_data->gst_rate));
                $this->excel->getActiveSheet()->SetCellValue('AW' . $row, $this->sma->formatQuantity($sales_items_data->gst_rate));
                ...
                $this->excel->getActiveSheet()->SetCellValue('AR' . $row, $this->sma->formatMoney($sales_items_data->subtotal));
                $this->excel->getActiveSheet()->SetCellValue('AS' . $row, $sales_items_data->gst_rate);
        ...
        // Footer values with formatting
        $this->excel->getActiveSheet()->SetCellValue('M' . $row, $this->sma->formatMoney($total));
        $this->excel->getActiveSheet()->SetCellValue('N' . $row, $this->sma->formatMoney($total_taxable_amt));
        $this->excel->getActiveSheet()->SetCellValue('O' . $row, $this->sma->formatMoney($total_tax));
        $this->excel->getActiveSheet()->SetCellValue('P' . $row, $this->sma->formatMoney($total_discount));
        $this->excel->getActiveSheet()->SetCellValue('Q' . $row, $this->sma->formatMoney($total_shipping));
        $this->excel->getActiveSheet()->SetCellValue('R' . $row, $this->sma->formatMoney($paid));
        $this->excel->getActiveSheet()->SetCellValue('S' . $row, $this->sma->formatMoney($total_balance));
        $this->excel->getActiveSheet()->SetCellValue('X' . $row, $this->sma->formatMoney($total_cgst));
        $this->excel->getActiveSheet()->SetCellValue('Y' . $row, $this->sma->formatMoney($total_sgst));
        $this->excel->getActiveSheet()->SetCellValue('Z' . $row, $this->sma->formatMoney($total_igst));

        // Quantities with center alignment
        $this->excel->getActiveSheet()->SetCellValue('AA' . $row, $this->sma->formatQuantity($total_weight));
        $this->excel->getActiveSheet()->SetCellValue('AB' . $row, $this->sma->formatQuantity($total_items));

        // Apply alignment
        $moneyCols = ['M', 'N', 'O', 'P', 'Q', 'R', 'S', 'X', 'Y', 'Z'];
        $qtyCols = ['AA', 'AB'];
        ...
        $this->excel->getActiveSheet()->SetCellValue('M' . $rowend, lang('Grand Total'));
        $this->excel->getActiveSheet()->SetCellValue('N' . $rowend, lang('Taxable Amount'));
        $this->excel->getActiveSheet()->SetCellValue('O' . $rowend, lang('Tax Amount'));
        $this->excel->getActiveSheet()->SetCellValue('P' . $rowend, lang('Discount Amount'));
        $this->excel->getActiveSheet()->SetCellValue('Q' . $rowend, lang('Shipping Amount'));
        $this->excel->getActiveSheet()->SetCellValue('R' . $rowend, lang('Paid'));
        $this->excel->getActiveSheet()->SetCellValue('S' . $rowend, lang('Balance'));
        ...
        $this->excel->getActiveSheet()->SetCellValue('X' . $rowend, lang('Sale CGST'));
        $this->excel->getActiveSheet()->SetCellValue('Y' . $rowend, lang('Sale SGST'));
        $this->excel->getActiveSheet()->SetCellValue('Z' . $rowend, lang('Sale IGST'));
        ...
        $this->excel->getActiveSheet()->SetCellValue('AJ' . $rowend, lang('unit_price'));
        $this->excel->getActiveSheet()->SetCellValue('AK' . $rowend, lang('quantity'));
        $this->excel->getActiveSheet()->SetCellValue('AL' . $rowend, lang('unit'));
        $this->excel->getActiveSheet()->SetCellValue('AM' . $rowend, lang('mrp'));
        $this->excel->getActiveSheet()->SetCellValue('AN' . $rowend, lang('item_discount'));
        ...
        $this->excel->getActiveSheet()->SetCellValue('AQ' . $rowend, lang('item_tax'));
        $this->excel->getActiveSheet()->SetCellValue('AR' . $rowend, lang('Subtotal'));
        $this->excel->getActiveSheet()->SetCellValue('AS' . $rowend, lang('CGST Rate (%)'));
        $this->excel->getActiveSheet()->SetCellValue('AT' . $rowend, lang('CGST'));
        $this->excel->getActiveSheet()->SetCellValue('AU' . $rowend, lang('SGST Rate (%)'));
        $this->excel->getActiveSheet()->SetCellValue('AV' . $rowend, lang('SGST'));
        $this->excel->getActiveSheet()->SetCellValue('AW' . $rowend, lang('IGST (%)'));
        $this->excel->getActiveSheet()->SetCellValue('AX' . $rowend, lang('IGST'));
    ...
    public function getSalesReportCnew($pdf = NULL, $xls = NULL, $img = NULL) {
        ...
        if ($pdf || $xls || $img) {
            ...
            if ($q->num_rows() > 0) {
                ...
                if (is_array($SalesItems)) {
                    foreach ($SalesItems as $key => $SaleItemsRow) {
                        ...
                        $data[$SaleItemsRow->sale_id]['items'][$SaleItemsRow->items_id]['CGST'] = ($datacgst[0]->totalgst) ? $this->sma->formatMoney($datacgst[0]->totalgst) : '0.00';
                        $data[$SaleItemsRow->sale_id]['items'][$SaleItemsRow->items_id]['CGST_rate'] = ($datacgst[0]->gstrrate) ? $datacgst[0]->gstrrate . '%' : '0.00';
                        $data[$SaleItemsRow->sale_id]['items'][$SaleItemsRow->items_id]['SGST'] = ($datasgst[0]->totalgst) ? $this->sma->formatMoney($datasgst[0]->totalgst) : '0.00';
                        $data[$SaleItemsRow->sale_id]['items'][$SaleItemsRow->items_id]['SGST_rate'] = ($datasgst[0]->gstrrate) ? $datasgst[0]->gstrrate . '%' : '0.00';

                        $data[$SaleItemsRow->sale_id]['items'][$SaleItemsRow->items_id]['IGST'] = ($dataigst[0]->totalgst) ? $this->sma->formatMoney($dataigst[0]->totalgst) : '0.00';
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('I2', lang('Grand Total'));
                $this->excel->getActiveSheet()->SetCellValue('J2', lang('Discount'));
                $this->excel->getActiveSheet()->SetCellValue('K2', lang('Taxable Amount'));
                $this->excel->getActiveSheet()->SetCellValue('L2', lang('Tax Amount'));
                $this->excel->getActiveSheet()->SetCellValue('M2', lang('Paid'));
                ...
                $this->excel->getActiveSheet()->SetCellValue('AB2', lang('Subtotal'));
...
//                            $this->excel->getActiveSheet()->SetCellValue('AB' . $row, $sales_items_data->IGST);
                    ...
                    //exit;
                    $this->excel->getActiveSheet()->SetCellValue('AE' . $row, ($VAT->taxamount) ? $this->sma->formatMoney($VAT->taxamount) : '0.00' );
                    $this->excel->getActiveSheet()->SetCellValue('AF' . $row, ($VAT->taxrate) ? $VAT->taxrate . '%' : '0.00' );
                    $this->excel->getActiveSheet()->SetCellValue('AG' . $row, ($CESS->taxamount) ? $this->sma->formatMoney($CESS->taxamount) : '0.00' );
...
//                $this->excel->getActiveSheet()->SetCellValue('AB' . $row, $igst);
                ...
                if (img) {
                    ...
                    $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'PDF');
                    $objWriter->save(str_replace(__FILE__, 'assets/mdata/'.$this->Customer_assets.'/uploads/pdf/sales_gst_report.pdf', __FILE__));
    ...
    public function getPurchasesReportC_new($pdf = NULL, $xls = NULL, $img = NULL) {
        ...
        if ($pdf || $xls || $img) {
           ...
           /* $this->db->select("" . $this->db->dbprefix('purchases') . ".date," . $this->db->dbprefix('purchases') . ".id, reference_no, " . $this->db->dbprefix('warehouses') . ".name as wname, supplier,IF(comp.gstn_no IS NULL or comp.gstn_no = '', '-', comp.gstn_no) as gstn_no,
            ...
            (SELECT IF(cgst > 0,CONCAT('(',format(gst_rate,2), '%)',format(sum(cgst),2)), 0.00) FROM `sma_purchase_items` WHERE `sma_purchase_items`.`purchase_id` = `sma_purchases`.`id` AND `sma_purchase_items`.`cgst` > 0 ) as CGST,
            (SELECT IF(sgst > 0,CONCAT('(',format(gst_rate,2), '%)',format(sum(sgst),2)), 0.00) FROM `sma_purchase_items` WHERE `sma_purchase_items`.`purchase_id` = `sma_purchases`.`id` AND `sma_purchase_items`.`sgst` > 0) as SGST,
            (SELECT IF(igst > 0, CONCAT('(',format(gst_rate,2), '%)',format(sum(igst),2)), 0.00)  FROM `sma_purchase_items` WHERE `sma_purchase_items`.`purchase_id` = `sma_purchases`.`id`) as IGST,
                grand_total, (grand_total - total_tax ) as tax_able_amount, 
                (SELECT (GROUP_CONCAT(DISTINCT CONCAT(' ' , format(tax,2),'%'))) as tax_rate  FROM `sma_purchase_items` WHERE `sma_purchase_items`.`purchase_id` = `sma_purchases`.`id` ) as tax_rate,(SELECT  CONCAT(format(sum(item_tax),2), '')  FROM `sma_purchase_items` WHERE `sma_purchase_items`.`purchase_id` = `sma_purchases`.`id` ) as tax_amt, paid, (grand_total-paid) as balance, {$this->db->dbprefix('purchases')}.status, {$this->db->dbprefix('purchases')}.id as id", FALSE)->from('purchases')->join('companies comp', 'purchases.supplier_id=comp.id', 'left')->join('warehouses', 'warehouses.id=purchases.warehouse_id', 'left');
            ...
            */
            ...
            GROUP_CONCAT(
                CONCAT(
                    ...
                    IF({$this->db->dbprefix('product_variants')}.name <> 'NULL', CONCAT(' (', {$this->db->dbprefix('product_variants')}.name, ')'), ''),
                    ' - ',
                    ROUND({$this->db->dbprefix('purchase_items')}.quantity, 2)
                ) 
                SEPARATOR '\n'
                ) AS item_name,
                {$this->db->dbprefix('products')}.hsn_code
                 AS hsn_code,
                GROUP_CONCAT(
                    DISTINCT NULLIF(NULLIF({$this->db->dbprefix('purchase_items')}.batch_number, ''), 'N/A')
                    ORDER BY {$this->db->dbprefix('purchase_items')}.batch_number
                    SEPARATOR ', '
                ) AS batch_numbers
                FROM {$this->db->dbprefix('purchase_items')}
                LEFT JOIN {$this->db->dbprefix('products')} 
                    ON {$this->db->dbprefix('products')}.id = {$this->db->dbprefix('purchase_items')}.product_id
                LEFT JOIN {$this->db->dbprefix('product_variants')} 
                    ON {$this->db->dbprefix('purchase_items')}.option_id = {$this->db->dbprefix('product_variants')}.id
                GROUP BY {$this->db->dbprefix('purchase_items')}.purchase_id
            ) FSI";
            ...
            FSI.batch_numbers,
            FSI.item_name,
            (grand_total + rounding) AS grand_total,
            (grand_total - total_tax) AS tax_able_amount,
            (
                SELECT GROUP_CONCAT(DISTINCT CONCAT(' ', FORMAT(tax, 2), '%')) 
                FROM {$this->db->dbprefix('purchase_items')} 
                WHERE {$this->db->dbprefix('purchase_items')}.purchase_id = {$this->db->dbprefix('purchases')}.id
            ) AS tax_rate,
            (
                SELECT IFNULL(FORMAT(SUM(IFNULL(item_tax, 0)), 2), 0)
                FROM {$this->db->dbprefix('purchase_items')}
                WHERE {$this->db->dbprefix('purchase_items')}.purchase_id = {$this->db->dbprefix('purchases')}.id
            ) AS tax_amt,
            paid,
            ...
            if (!empty($data)) {
                ...
                $nextCol = 'G';
                if ($this->Settings->product_batch_setting == 1 || $this->Settings->product_batch_setting == 2) {
                    $this->excel->getActiveSheet()->SetCellValue($nextCol . '2', lang('Batch No.'));
                    $nextCol++;
                }

                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('grand_total'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('Taxable_Amount'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('GST_Rate'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('Tax Amount'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('paid'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('balance'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('status'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('CGST'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('CGST Rate (%)'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('SGST'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('SGST Rate (%)'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('IGST'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('IGST Rate (%)'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('VAT'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol++ . '2', lang('CESS'));
                $this->excel->getActiveSheet()->SetCellValue($nextCol . '2', lang('Product Name'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $datacgst = $this->reports_model_new->getPurcahseAsGst($id, 'cgst'); 
                    $datasgst = $this->reports_model_new->getPurcahseAsGst($id, 'sgst'); 
                    $dataigst = $this->reports_model_new->getPurcahseAsGst($id, 'igst'); 
                    ...
                    // --- Excel output ---
                    $col = 'A';
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->hrld($data_row->date));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->reference_no);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->wname);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->supplier);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->gstn_no);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->hsn_code);
                    // Left align HSN Code column
                    ...
                    if ($this->Settings->product_batch_setting == 1 || $this->Settings->product_batch_setting == 2) {
                        $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->batch_numbers);
                    }
                
                    // Continue remaining columns dynamically
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($data_row->grand_total));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($data_row->tax_able_amount));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->tax_rate);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($data_row->tax_amt));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($data_row->paid));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($data_row->grand_total - $data_row->paid));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $data_row->status);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($CGST));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $CGSTRATE);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($SGST));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $SGSTRATE);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($IGST));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $IGSTRATE);
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($Vat->taxamount));
                    $this->excel->getActiveSheet()->SetCellValue($col++ . $row, $this->sma->formatMoney($Cess->taxamount));
                    $this->excel->getActiveSheet()->SetCellValue($col . $row, $data_row->item_name);
                ...
                if ($this->Settings->product_batch_setting == 1 || $this->Settings->product_batch_setting == 2) {
                    $totalCol = 'H';
                    // --- FOOTER TOTALS (BATCH ON MODE) ---
                    $this->excel->getActiveSheet()->SetCellValue('H'.$row, $this->sma->formatMoney($total));         // Grand Total
                    $this->excel->getActiveSheet()->SetCellValue('I'.$row, $this->sma->formatMoney($total_tax));     // Taxable Amount
                    $this->excel->getActiveSheet()->SetCellValue('K'.$row, $this->sma->formatMoney($tax_amt));       // Tax Amount
                    $this->excel->getActiveSheet()->SetCellValue('L'.$row, $this->sma->formatMoney($paid));          // Paid
                    $this->excel->getActiveSheet()->SetCellValue('M'.$row, $this->sma->formatMoney($balance));       // Balance

                } else {
                    $totalCol = 'G';
                    $this->excel->getActiveSheet()->SetCellValue('G'.$row, $this->sma->formatMoney($total));
                    $this->excel->getActiveSheet()->SetCellValue('H'.$row, $this->sma->formatMoney($total_tax));
                    $this->excel->getActiveSheet()->SetCellValue('J'.$row, $this->sma->formatMoney($tax_amt));
                    $this->excel->getActiveSheet()->SetCellValue('K'.$row, $this->sma->formatMoney($paid));
                    $this->excel->getActiveSheet()->SetCellValue('L'.$row, $this->sma->formatMoney($balance));
                }
                if ($this->Settings->product_batch_setting == 1 || $this->Settings->product_batch_setting == 2) {

                    // Totals row columns: H, J, K, L, M
                    $totalsCols = ['H','I','K','L','M'];
                
                } else {
                
                    // Without Batch: G, H, J, K, L
                    $totalsCols = ['G','H','J','K','L'];
                }
                
                // Apply right alignment ONLY to totals row
                foreach ($totalsCols as $colLetter) {
                    $this->excel->getActiveSheet()
                ...
                // Fix the batch-based column shifting
                if ($this->Settings->product_batch_setting == 1 || $this->Settings->product_batch_setting == 2) {
                    // Batch column exists → shift everything by +1
                    $rightAlignCols = ['H','I','J','K','L','M','N','P','R','T','V','W', 'O','Q', 'S', 'U', 'V'];
                } else {
                    // No batch column → normal positions
                    $rightAlignCols = ['G','H','I','J','K','L','M','N','Q','S', 'P', 'R', 'T', 'U'];
                }
                ...
                if ($this->Settings->product_batch_setting == 1 || $this->Settings->product_batch_setting == 2) {
                    ...
                    // Text Columns (Left Align + Force STRING)
                    $textCols = ['A','B','C','D','E','F','G','N','P','R','T','V'];  // product, supplier, gstn, hsn, batch, status, etc.
                
                    // Qty Columns (Center)
                    $qtyCols = []; // NO qty column in this report (only GST_PURCHASE), so leave empty
                
                    // Money Columns (Right)
                    $moneyCols = ['H','I','K','L','M','O','Q','S','U','W','V'];
                } else {

                    // Text Columns
                    $textCols = ['A','B','C','D','E','F','N','P','R','T','V'];
                
                    // Qty Columns
                    $qtyCols = []; // no qty column here
                
                    // Money Columns
                    $moneyCols = ['G','H','J','K','L','O','Q','S','U','W', 'V'];
                }
                ...
                // Right Align Amount / Money Columns
                foreach ($moneyCols as $col) {
                    $sheet->getStyle($col . '3:' . $col . $lastRow)
                        ->getAlignment()
                        ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                ...
                if ($img) {
                    ...
                    $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'PDF');
                    $objWriter->save(str_replace(__FILE__, 'assets/mdata/'.$this->Customer_assets.'/uploads/pdf/purchase_gst_report.pdf', __FILE__));
            ...
            // --- Subquery to collect HSN, Batch, and Item details ---
            $si = "( SELECT 
                {$this->db->dbprefix('purchase_items')}.purchase_id, 
                {$this->db->dbprefix('purchase_items')}.product_id,
                GROUP_CONCAT(
                    CONCAT(
                        ' ', {$this->db->dbprefix('purchase_items')}.product_name,
                        IF({$this->db->dbprefix('product_variants')}.name <> 'NULL', 
                            CONCAT(' (', {$this->db->dbprefix('product_variants')}.name, ')'), ''
                        ),
                        CONCAT('(', ROUND({$this->db->dbprefix('purchase_items')}.subtotal, 2), ')'),
                        ' - ', ROUND({$this->db->dbprefix('purchase_items')}.quantity)
                    ) SEPARATOR ', '
                ) AS item_name,
                 {$this->db->dbprefix('products')}.hsn_code
                 AS hsn_code,
            ...
            // ✅ Conditionally add Batch No. column
            if ($this->Settings->product_batch_setting == 1 || $this->Settings->product_batch_setting == 2) {
                $select_columns .= "FSI.batch_numbers, ";
            }
            ...
            $this->datatables->select($select_columns, FALSE)
                ->add_column('CGST', '', '')
                ->add_column('SGST', '', '')
                ->add_column('IGST', '', '')
                ->add_column('VAT', '', '')
            ...
            // --- Filters ---
            if ($this->session->userdata('view_right') == '0' && $user) {
    ...
    function daily_sales($warehouse_id = NULL, $year = NULL, $month = NULL, $pdf = NULL, $user_id = NULL) {
        ...
        if ($pdf) {
            ...
            $this->load->library('excel');
            $this->excel->setActiveSheetIndex(0);
            $style = array('alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,), 'borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN, 'color' => array('rgb' => 'DDDDDD'))));
            ...
            foreach ($sales_pdf as $data_row) {
                ...
                $this->excel->getActiveSheet()->setCellValueExplicit('D' . $row, $this->sma->formatMoney($data_row->return_amt), PHPExcel_Cell_DataType::TYPE_STRING);
                $this->excel->getActiveSheet()->getStyle('D' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->setCellValueExplicit('E' . $row, $this->sma->formatMoney($data_row->discount), PHPExcel_Cell_DataType::TYPE_STRING);
                $this->excel->getActiveSheet()->getStyle('E' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->setCellValueExplicit('F' . $row, $this->sma->formatMoney($data_row->shipping), PHPExcel_Cell_DataType::TYPE_STRING);
                $this->excel->getActiveSheet()->getStyle('F' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->setCellValueExplicit('G' . $row, $this->sma->formatMoney($data_row->tax1), PHPExcel_Cell_DataType::TYPE_STRING);
                $this->excel->getActiveSheet()->getStyle('G' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->setCellValueExplicit('H' . $row, $this->sma->formatMoney($data_row->tax2), PHPExcel_Cell_DataType::TYPE_STRING);
                $this->excel->getActiveSheet()->getStyle('H' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->setCellValueExplicit('I' . $row, $this->sma->formatMoney($data_row->total), PHPExcel_Cell_DataType::TYPE_STRING);
                $this->excel->getActiveSheet()->getStyle('I' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->setCellValueExplicit('J' . $row, $this->sma->formatMoney($data_row->urban_piper), PHPExcel_Cell_DataType::TYPE_STRING);
            ...
            if ($pdf == 'pdf') {
                ...
                $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'PDF');
                $objWriter->save(str_replace(__FILE__, 'assets/mdata/'.$this->Customer_assets.'/uploads/pdf/daily_sales_report.pdf', __FILE__));
    ...
    public function daily_sales_items_taxes() {
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                    ...
                    if (!empty($saletax_data)) {
                        foreach ($saletax_data as $key => $item) {
                            ...
                            <tr>
                                ...
                                <td class="text-right"><?= $this->sma->formatMoney($amount, 2); ?></td>
                            </tr>
                            <?php
                            $totalTax += $amount;
                        }
                ...
                <tfoot>
                    <tr>
                        <th colspan="4" style="text-align:right;">Total Tax</th>
                        <th style="text-align:right;"><?= $this->sma->formatMoney($totalTax); ?></th>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    function monthly_sales($warehouse_id = NULL, $year = NULL, $pdf = NULL, $user_id = NULL) {
        ...
        if ($pdf) {
            ...
            foreach ($sales_pdf as $data_row) {
                ...
                // Format and align monetary values
                $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->sma->formatMoney($data_row->GrossSale + $data_row->return_amt + $data_row->total_discount));
            ...
            // $this->excel->getActiveSheet()->getStyle("C2:J" . ($row - 1))->applyFromArray($style);
            // Apply right alignment to headings only
            $this->excel->getActiveSheet()->getStyle("A2:J2")
            ...
            if ($pdf == 'pdf') {
                ...
                $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'PDF');
                $objWriter->save(str_replace(__FILE__, 'assets/mdata/'.$this->Customer_assets.'/uploads/pdf/monthly_sales_report.pdf', __FILE__));
...
//        echo $warehouse_id;exit;
            ...
            <table class="table table-bordered">
                    ...
                    if (!empty($saletax_data)) {
                        foreach ($saletax_data as $key => $item) {
                            ...
                            <tr>
                                ...
                                <td class="text-center"><?= (($item->SGST > 0) ? number_format($item->gst_rate, 1) : 0) ?>%</td>
                                <td class="text-center"><?= (($item->IGST > 0) ? number_format($item->gst_rate, 1) : 0) ?>%</td>
                                <td style="text-align:right;"><?= $this->sma->formatMoney($amount); ?></td>
                            </tr>
                            <?php
                            $totalTax += $amount;
                        }
                ...
                <tfoot>
                    <tr>
                        ...
                        <th class="text-right"><?= $this->sma->formatMoney($totalTax); ?></th>
                    </tr>
                </tfoot>
            </table>
        </div>
            ...
            <table class="table table-bordered">
                ...
                <tfoot>
                    <tr>
                        ...
                        <th style="text-align:right;"><?= $this->sma->formatMoney($total); ?></th>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    //     // $this->sma->checkPermissions('daily_purchases');
    //     if ($warehouse_id != NULL) {
    //         $warehouse_id = $warehouse_id;
    //     } else if (!$this->Owner && !$this->Admin && $this->session->userdata('warehouse_id')) {
    //         $warehouse_id = str_replace(",", "_", $this->session->userdata('warehouse_id'));
    ...
    //         $this->load->library('excel');
    //         $this->excel->setActiveSheetIndex(0);
    //         $style = array('alignment' => array('horizontal'), 'borders' => array('allborders' => array('style' => PHPExcel_Style_Border::BORDER_THIN, 'color' => array('rgb' => 'DDDDDD'))));
    ...
    //             // Center align columns A and B
    //             $this->excel->getActiveSheet()
    //                 ->getStyle('A' . $row . ':B' . $row)
    //                 ->getAlignment()
    //                 ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
            
    //             // Right align formatted money columns C to G
    //             $this->excel->getActiveSheet()
    ...
    //             $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'PDF');
    //             $objWriter->save(str_replace(__FILE__, 'assets/mdata/'.$this->Customer_assets.'/uploads/pdf/' . $filename . '.pdf', __FILE__));
    ...
    function daily_purchases($warehouse_id = NULL, $year = NULL, $month = NULL, $pdf = NULL, $user_id = NULL) {
        ...
        if ($pdf) {
            ...
            foreach ($purchase_pdf as $data_row) {
                $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sr);
                $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->date . '/' . $month . '/' . $year);
                $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->discount);
                $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->shipping);
                $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->tax1);
                $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->tax2);
                $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->total);

                            $total_discount += $data_row->discount;
                            $total_shipping += $data_row->shipping;
                            $total_tax1     += $data_row->tax1;
                            $total_tax2     += $data_row->tax2;
                            $total_total    += $data_row->total;
            ...
            $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->sma->formatMoney($total_discount));
            $this->excel->getActiveSheet()->SetCellValue('D' . $row, $this->sma->formatMoney($total_shipping));
            $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->sma->formatMoney($total_tax1));
            $this->excel->getActiveSheet()->SetCellValue('F' . $row, $this->sma->formatMoney($total_tax2));
            $this->excel->getActiveSheet()->SetCellValue('G' . $row, $this->sma->formatMoney($total_total));

            // Make footer bold
            $this->excel->getActiveSheet()->getStyle('A' . $row . ':G' . $row)->applyFromArray(['font' => ['bold' => true],'borders' => ['allborders' => ['style' => PHPExcel_Style_Border::BORDER_THIN] ] ]);
    ...
    function monthly_purchases($warehouse_id = NULL, $year = NULL, $pdf = NULL, $user_id = NULL) {
        ...
        if ($pdf) {
            ...
            $this->excel->getActiveSheet()->getStyle("C2:G" . ($row - 1))->applyFromArray($style);
            ...
            if ($pdf == 'pdf') {
                ...
                $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'PDF');
                $objWriter->save(str_replace(__FILE__, 'assets/mdata/'.$this->Customer_assets.'/uploads/pdf/monthly_purchases_report.pdf', __FILE__));
    ...
    public function daily_purchase_items_taxes() {
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tfoot>
                    <tr>
                        <th  colspan="6" style="text-align: left;">Total Tax</th>
                        <th style="text-align: right;"><?= $this->sma->formatMoney($totalTax); ?></th>
                ...
                <tbody>
                    <?php foreach ($purchase_data as $key => $item) { ?>
                        <tr>
                            ...
                            <td><?= $item->name ?></td>
                            <td><?= $item->product_code ?></td>
                            <td style="text-align: right;"><?= $this->sma->formatMoney($item->net_unit_cost) ?></td>
                            <td style="text-align: center;"><?= $this->sma->formatQuantity($item->qty) ?></td>
                            <td><?= $item->unit ?></td>
                            <td><?= $item->tax_rate ? number_format($item->tax_rate, 2) : 0; ?>%</td>
                            <td style="text-align: right;"><?= $this->sma->formatMoney($item->tax) ?></td>
    ...
    public function purchaseGSTReport($start_date, $end_date) {
        ...
        if (!empty($data)) {
            ...
            $this->excel->getActiveSheet()->SetCellValue('J3', lang('Tax Amount'));
    ...
    public function salesGSTReport($start_date, $end_date) {
        ...
        if (!empty($data)) {
            ...
            $this->excel->getActiveSheet()->SetCellValue('I3', lang('Grand Total'));
            $this->excel->getActiveSheet()->SetCellValue('J3', lang('Taxable Amount'));
            $this->excel->getActiveSheet()->SetCellValue('K3', lang('Tax Amount'));
            $this->excel->getActiveSheet()->SetCellValue('L3', lang('Paid'));
            $this->excel->getActiveSheet()->SetCellValue('M3', lang('Balance'));
            ...
            $this->excel->getActiveSheet()->SetCellValue('AC3', lang('Subtotal'));
            ...
            foreach ($data as $sale_id => $salesdata) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('AD' . $row, ($VAT->taxamount) ? $this->sma->formatMoney($VAT->taxamount) : '0.00' );
                $this->excel->getActiveSheet()->SetCellValue('AE' . $row, ($VAT->taxrate) ? $VAT->taxrate . '%' : '0.00' );
                $this->excel->getActiveSheet()->SetCellValue('AF' . $row, ($CESS->taxamount) ? $this->sma->formatMoney($CESS->taxamount) : '0.00' );
   ...
   private function __sale_gst_report($rowData, $rowFooter, $sale_items) {
        ...
        if (!empty($rowData)) {
            ...
            foreach ($rowData as $row_id => $row) {
                ...
                $invoiceRowStart = '<tr data-rowid="' . $row_id . '">
                            ...
                            <td rowspan="' . $taxRow . '" align="right">' . $this->sma->formatMoney($row['taxable_amount']) . '</td>';
                ...
                if($sale_items == TRUE){
                    ...
                    foreach ($row['gst'] as $tax) {
                        ...
                        $td_taxes = '<td>'.number_format($tax['tax_rate'], 2).'%</td> 
                                        <td>'.$this->sma->formatQuantity($tax['total_items']).'</td> 
                                        <td>' . (($tax['cgst'] > 0) ? number_format($tax['gst_rate'], 2) . '%' : '00') . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['cgst']) . '</td>
                                        <td>' . (($tax['sgst'] > 0) ? number_format($tax['gst_rate'], 2) . '%' : '00') . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['sgst']) . '</td>
                                        <td>' . (($tax['igst'] > 0) ? number_format($tax['gst_rate'], 2) . '%' : '00') . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['igst']) . '</td> 
                                        <td align="right">' . $this->sma->formatMoney($tax['cgst']+$tax['sgst']+$tax['igst']) . '</td>';
                ...
                $invoiceRowEnd = '<td rowspan="' . $taxRow . '" align="right">' . $this->sma->formatMoney($row['total_tax_amount']) . '</td>
                                <td rowspan="' . $taxRow . '" align="right">' . $this->sma->formatMoney($row['taxable_amount'] + $row['total_tax_amount']) . '</td>
        ...
        $table .= ' </tbody>
                        ...
                        <tr class="bg-primary">
                            ...
                            <th style="color:#ffffff;" title="Total CGST Amount">' . $this->sma->formatMoney($rowFooter['cgst']) . '</th>
                            <th style="color:#ffffff;">SGST</th>
                            <th style="color:#ffffff;" title="Total SGST Amount">' . $this->sma->formatMoney($rowFooter['sgst']) . '</th>
                            <th style="color:#ffffff;">IGST</th>                            
                            <th style="color:#ffffff;" title="Total IGST Amount">' . $this->sma->formatMoney($rowFooter['igst']) . '</th>
                            <th style="color:#ffffff;">' . $this->sma->formatMoney(($rowFooter['cgst']+$rowFooter['sgst']+$rowFooter['igst'])) . '</th>
                            <th style="color:#ffffff;" title="Total Tax Amount">' . $this->sma->formatMoney($rowFooter['total_tax_amount']) . '</th>
                            <th style="color:#ffffff;" title="Total Invoice Amount">' . $this->sma->formatMoney($rowFooter['taxable_amount'] + $rowFooter['total_tax_amount'], 2) . '</th>
                            <th></th>                             
                        </tr>
                        </tfoot>
                    </table>';
           ...
           // $this->excel->getActiveSheet()->getStyle("A" . $row . ":Y" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
            ...
            foreach ($rowData as $invoice_no => $inv) {
               ...
               $total_amount += $inv[$cellData['U']];
                ...
                $sr++;
                $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sr);
                $this->excel->getActiveSheet()->getStyle('A' . $row)
               ...
               $this->excel->getActiveSheet()->SetCellValue('AB' . $row, $inv[$cellData['AB']]);
               ...
               $this->excel->getActiveSheet()->SetCellValue('AD' . $row, $inv[$cellData['AD']]);
                ...
                foreach ($inv['gst'] as $tax) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('I' . $tax_row, number_format($tax['tax_rate'], 2) . '%');
                    // echo "<pre>";
                    // print_r($this->sma->formatQuantity($tax['total_items']));
                    // exit;
                    $this->excel->getActiveSheet()->setCellValueExplicit(
                        'J' . $tax_row,
                        $this->sma->formatQuantity($tax['total_items']),
                        PHPExcel_Cell_DataType::TYPE_STRING
                    );
                    $this->excel->getActiveSheet()->getStyle('J' . $tax_row)
                        ->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
                        ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    


                    $cgst_rate = ($tax['cgst'] > 0) ? number_format($tax['gst_rate']) . '%' : '00';
                    $sgst_rate = ($tax['sgst'] > 0) ? number_format($tax['gst_rate']) . '%' : '00';
                    $igst_rate = ($tax['igst'] > 0) ? number_format($tax['gst_rate']) . '%' : '00';

                    $this->excel->getActiveSheet()->SetCellValue('K' . $tax_row, $cgst_rate);
                    $this->excel->getActiveSheet()->getStyle('K' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);


                    // Applying formatMoney() to S, T, U columns
                    $this->excel->getActiveSheet()->SetCellValue('S' . $row, $this->sma->formatMoney($inv[$cellData['S']]));
                    $this->excel->getActiveSheet()->getStyle('S' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('T' . $row, $this->sma->formatMoney($inv[$cellData['T']]));
                    $this->excel->getActiveSheet()->getStyle('T' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('U' . $row, $this->sma->formatMoney($inv[$cellData['U']]));
                    $this->excel->getActiveSheet()->getStyle('U' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('L' . $tax_row, $this->sma->formatMoney($tax['cgst']));
                    $this->excel->getActiveSheet()->getStyle('L' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('M' . $tax_row, ($sgst_rate));
                    $this->excel->getActiveSheet()->getStyle('M' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

                    $this->excel->getActiveSheet()->SetCellValue('N' . $tax_row, $this->sma->formatMoney($tax['sgst']));
                    $this->excel->getActiveSheet()->getStyle('N' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('O' . $tax_row, ($igst_rate));
                    $this->excel->getActiveSheet()->getStyle('O' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

                    $this->excel->getActiveSheet()->SetCellValue('P' . $tax_row, $this->sma->formatMoney($tax['igst']));
                    $this->excel->getActiveSheet()->getStyle('P' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('Q' . $tax_row, $this->sma->formatMoney($tax['taxable_amount']));
                    $this->excel->getActiveSheet()->getStyle('Q' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('R' . $tax_row, $this->sma->formatMoney($tax['tax_total']));
                    $this->excel->getActiveSheet()->getStyle('R' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            ...
            // Set values and apply right alignment
            $this->excel->getActiveSheet()->SetCellValue('L' . $row, $this->sma->formatMoney($total_cgst));
            $this->excel->getActiveSheet()->getStyle('L' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('N' . $row, $this->sma->formatMoney($total_sgst));
            $this->excel->getActiveSheet()->getStyle('N' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('P' . $row, $this->sma->formatMoney($total_igst));
            $this->excel->getActiveSheet()->getStyle('P' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('Q' . $row, $this->sma->formatMoney($total_tax_amount));
            $this->excel->getActiveSheet()->getStyle('Q' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('R' . $row, $this->sma->formatMoney($total_product_tax_amount));
            $this->excel->getActiveSheet()->getStyle('R' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('S' . $row, $this->sma->formatMoney($total_invoice_tax_amount));
            $this->excel->getActiveSheet()->getStyle('S' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('T' . $row, $this->sma->formatMoney($total_discount));
            $this->excel->getActiveSheet()->getStyle('T' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('U' . $row, $this->sma->formatMoney($total_amount));
            $this->excel->getActiveSheet()->getStyle('U' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    ...
    private function __purchase_gst_report($rowData, $rowFooter) {
        ...
        if (!empty($rowData)) {
            ...
            foreach ($rowData as $row_id => $row) {
                ...
                $invoiceRowStart = '<tr data-rowid="' . $row_id . '">
                            ...
                            <td rowspan="' . $taxRow . '">' . $row['state_code'] . '</td>
                            <td rowspan="' . $taxRow . '">' . $row['gstn_no'] . '</td>
                            <td rowspan="' . $taxRow . '" align="right">' . $this->sma->formatMoney($row['taxable_amount'], 4) . '</td>';

                $tx = 0;
                foreach ($row['gst'] as $tax) {

                    $tx++;
                    $td_taxes = '<td align="right">' . (($tax['cgst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '00') . '</td>
                                <td align="right">' . $this->sma->formatMoney($tax['cgst']) . '</td>
                                <td align="right">' . (($tax['sgst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '00') . '</td>
                                <td align="right">' . $this->sma->formatMoney($tax['sgst']) . '</td>
                                <td align="right">' . (($tax['igst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '00') . '</td>
                                <td align="right">' . $this->sma->formatMoney($tax['igst']) . '</td>';
                ...
                $invoiceRowEnd = '<td rowspan="' . $taxRow . '" align="right">' . $this->sma->formatMoney($row['total_tax_amount'], 2) . '</td>
                                    <td rowspan="' . $taxRow . '" align="right">' . $this->sma->formatMoney($row['taxable_amount'] + $row['total_tax_amount'], 2) . '</td>
        ...
        $table .= ' </tbody>
                        ...
                        <tr class="bg-primary">
                            ...
                            <th style="color:#ffffff;" title="Total IGST Amount">' . $this->sma->formatMoney($rowFooter['igst']) . '</th>
                            <th style="color:#ffffff;" title="Total Tax Amount">' . $this->sma->formatMoney($rowFooter['total_tax_amount']) . '</th>
                            <th style="color:#ffffff;" title="Total Invoice Amount">' . $this->sma->formatMoney($rowFooter['taxable_amount'] + $rowFooter['total_tax_amount']) . '</th>
                            <th></th>                    
                        </tr>
                        </tfoot>
                    </table>';
    ...
    private function __purchase_gst_report_download($rowData, $rowFooter, $report_title) {
        ...
        if (!empty($rowData)) {
            ...
            foreach ($rowData as $invoice_no => $inv) {
                ...
                foreach (range('B', 'F') as $cell_1) {
                    $this->excel->getActiveSheet()->SetCellValue($cell_1 . $row, $inv[$cellData[$cell_1]]);
                }
            
                // G - Apply formatMoney
                $this->excel->getActiveSheet()->SetCellValue('G' . $row, $this->sma->formatMoney($inv['grand_total']));
                $this->excel->getActiveSheet()->getStyle('G' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                ...
                // N - Apply formatMoney
                $this->excel->getActiveSheet()->SetCellValue('N' . $row, $this->sma->formatMoney($inv['taxable_amount']));
                $this->excel->getActiveSheet()->getStyle('N' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            
                // O - Apply formatMoney
                $this->excel->getActiveSheet()->SetCellValue('O' . $row, $this->sma->formatMoney($inv['total_tax_amount']));
                $this->excel->getActiveSheet()->getStyle('O' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                ...
                foreach ($inv['gst'] as $tax) {
                    ...
                    // H - GST Rate
                    $this->excel->getActiveSheet()->SetCellValue('H' . $tax_row, ($tax['cgst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '00');
                    $this->excel->getActiveSheet()->getStyle('H' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            
                    // I - CGST Amount
                    $this->excel->getActiveSheet()->SetCellValue('I' . $tax_row, $this->sma->formatMoney($tax['cgst']));
                    $this->excel->getActiveSheet()->getStyle('I' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            
                    // J - GST Rate
                    $this->excel->getActiveSheet()->SetCellValue('J' . $tax_row, ($tax['sgst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '00');
                    $this->excel->getActiveSheet()->getStyle('J' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            
                    // K - SGST Amount
                    $this->excel->getActiveSheet()->SetCellValue('K' . $tax_row, $this->sma->formatMoney($tax['sgst']));
                    $this->excel->getActiveSheet()->getStyle('K' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            
                    // L - GST Rate
                    $this->excel->getActiveSheet()->SetCellValue('L' . $tax_row, ($tax['igst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '00');
                    $this->excel->getActiveSheet()->getStyle('L' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            
                    // M - IGST Amount
                    $this->excel->getActiveSheet()->SetCellValue('M' . $tax_row, $this->sma->formatMoney($tax['igst']));
                    $this->excel->getActiveSheet()->getStyle('M' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                }
            ...
            // Adjust bottom border for the last row
            $this->excel->getActiveSheet()->getStyle("A" . $row . ":P" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
    ...
    public function sales_vat_report_uae() {

        $this->data['users'] = $this->reports_model_new->getStaff();
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['customer'] = $this->reports_model_new->getCustomerCompanies();
        // $this->data['salevatcount'] = $this->getCountSalesVat(); // Optional if you track counts
        $this->data['billers'] = $this->site->getAllCompanies('biller');

        $bc = [
            ['link' => base_url(), 'page' => lang('home')],
            ['link' => site_url('reports'), 'page' => lang('reports')],
            ['link' => '#', 'page' => lang('Sales Transaction Report')]
        ];
        $meta = [
            'page_title' => lang('Sales Transaction Report'),
            'bc' => $bc
        ];

        $this->page_construct('new_reports/sales_vat_report_uae', $meta, $this->data);
    }

    public function getSalesVatReportUAE()
    {
        $product      = $this->input->post('product') ?: NULL;
        $user         = $this->input->post('user') ?: NULL;
        $customer     = $this->input->post('customer') ?: NULL;
        $biller       = $this->input->post('biller') ?: NULL;
        ...
        $trn_no       = $this->input->post('gstn_no') ?: NULL; // UAE: TRN instead of GSTN
        $xls = $this->input->get('xls') ?: NULL;
        ...
        if ($xls) {
            $customer     = $this->input->get('customer') ?: NULL;
            $user         = $this->input->get('user') ?: NULL;
            $biller       = $this->input->get('biller') ?: NULL;
            $warehouse    = $this->input->get('warehouse') ?: NULL;
            $reference_no = $this->input->get('reference_no') ?: NULL;
            $start_date   = $this->input->get('start_date') ?: NULL;
            $end_date     = $this->input->get('end_date') ?: NULL;
            $trn_no       = $this->input->get('gstn_no') ?: NULL;
            if ($start_date) {
                $start_date = $this->sma->fld($start_date);
                $end_date   = $this->sma->fld($end_date);
            }
            ...
            $si = "( 
                SELECT 
                    ...
                    GROUP_CONCAT(
                        CONCAT(
                            ' ', si.product_name, 
                            IF(pv.name <> 'NULL', CONCAT(' (', pv.name, ')'), ''), 
                            CONCAT('(', si.subtotal, ')'), '-', ROUND(si.quantity, 2))
                        SEPARATOR ', '
                    ) AS item_nane,
                    SUM(si.item_tax) AS item_tax_summary,
                    MIN(si.tax_method) AS tax_method,
                    MIN(si.tax) AS tax_value
                FROM sma_sale_items si
                LEFT JOIN sma_product_variants pv ON si.option_id = pv.id
                GROUP BY si.sale_id
            ) FSI";
            ...
            $sql = "
                SELECT 
                    ...
                    IF(comp.gstn_no IS NULL OR comp.gstn_no = '', '-', comp.gstn_no) AS gstn_no,
                    FSI.item_nane AS iname,
                    FSI.item_tax_summary,
                    ROUND(FSI.tax_value) AS tax_value,
                    (grand_total + rounding) AS grand_total,
                    total_discount,
                    (grand_total - total_tax) AS taxable_amount,
                    total_tax,
                    paid,
                    (grand_total + rounding - paid) AS balance,
                    GROUP_CONCAT(' ', sma_payments.paid_by) AS paid_by,
                    payment_status
                FROM sma_sales
                LEFT JOIN sma_companies comp ON sma_sales.customer_id = comp.id
                ...
                LEFT JOIN $si ON FSI.sale_id = sma_sales.id
                WHERE 1=1
            ";
        
            // Filters
            if ($user) {
                $sql .= " AND sma_sales.created_by = " . $this->db->escape($user);
            }
            if ($customer) {
                $sql .= " AND sma_sales.customer_id = " . $this->db->escape($customer);
            }
            if ($biller) {
                $sql .= " AND sma_sales.biller_id = " . $this->db->escape($biller);
            }
            if ($warehouse) {
                $getwarehouse = str_replace("_", ",", $warehouse);
                $sql .= " AND sma_sales.warehouse_id IN ({$getwarehouse})";
            }
            if ($reference_no) {
                $like = $this->db->escape('%' . $reference_no . '%');
                $sql .= " AND sma_sales.reference_no LIKE {$like}";
            }
            if ($start_date && $end_date) {
                $sql .= " AND DATE(sma_sales.date) BETWEEN '{$start_date}' AND '{$end_date}'";
            }
            if ($trn_no) {
                $sql .= " AND comp.gstn_no = " . $this->db->escape($trn_no);
            }
        
            $sql .= " GROUP BY sma_sales.id";
            ...
            // Headers
            $headers = [
                'A' => 'Date', 'B' => 'Invoice No.', 'C' => 'Reference No.',
                'D' => 'Biller', 'E' => 'Customer', 'F' => 'State', 'G' => 'TRN Number',
                'H' => 'Product Qty', 'I' => 'Item Tax Amount', 'J' => 'Tax Value (%)',
                'K' => 'Grand Total', 'L' => 'Discount', 'M' => 'Taxable Amt',
                'N' => 'Total Invoice Tax Amount', 'O' => 'Paid', 'P' => 'Balance',
                'Q' => 'Paid Method', 'R' => 'Payment Status'
            ];
            ...
            foreach ($results as $sale) {
                ...
                $sheet->setCellValue("C{$row}", $sale->reference_no);
                $sheet->setCellValue("D{$row}", $sale->biller);
                $sheet->setCellValue("E{$row}", $sale->customer);
                $sheet->setCellValue("F{$row}", $sale->state);
                $sheet->setCellValue("G{$row}", $sale->gstn_no);
                // Optional (advanced): Format each subtotal inside `iname`
                $items = explode(',', $sale->iname);
                $formatted_items = array_map(function($item) {
                    // Match pattern: product (variant) (subtotal)-qty
                    if (preg_match('/^(.*)\(([\d.]+)\)-([\d.]+)$/', trim($item), $matches)) {
                        $name = trim($matches[1]);
                        $subtotal = $matches[2];
                        $qty = $matches[3];
                        return "{$name}(" . $this->sma->formatMoney($subtotal) . ")-" . $qty;
                    }
                    return $item; // fallback if pattern doesn't match
                }, $items);

                $formatted_iname = implode(', ', $formatted_items);
                $sheet->setCellValue("H{$row}", $formatted_iname);

                $sheet->setCellValue("I{$row}", $this->sma->formatMoney($sale->item_tax_summary));
                $sheet->setCellValue("J{$row}", (int)$sale->tax_value . '%');
                // Center-align J column for this row
                $sheet->getStyle("J{$row}")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $sheet->setCellValue("K{$row}", $this->sma->formatMoney($sale->grand_total));
                $sheet->setCellValue("L{$row}", $this->sma->formatMoney($sale->total_discount));
                $sheet->setCellValue("M{$row}", $this->sma->formatMoney($sale->taxable_amount));
                $sheet->setCellValue("N{$row}", $this->sma->formatMoney($sale->total_tax));
                $sheet->setCellValue("O{$row}", $this->sma->formatMoney($sale->paid));
                $sheet->setCellValue("P{$row}", $this->sma->formatMoney($sale->balance));
                $sheet->setCellValue("Q{$row}", $sale->paid_by);
                $sheet->setCellValue("R{$row}", $sale->payment_status);
                $row++;
            }
            $lastRow = $row - 1;

            // apply borders from header (row 2) to last data row
            ...
            $filename = 'vat_report_uae_' . date('Ymd_His') . '.xls';
            ob_end_clean();
            header('Content-Type: application/vnd.ms-excel');
            ...
            $si = "( SELECT 
                ...
                CONCAT(
                    ...
                    IF(sma_product_variants.name <> 'NULL', CONCAT(' (', sma_product_variants.name, ')'), ''),
                    ' (', FORMAT(sma_sale_items.subtotal, 2), ')',
                    '-', ROUND(sma_sale_items.quantity, 2)
                )
                SEPARATOR ', '
                ) AS item_nane,
                SUM(sma_sale_items.item_tax) AS item_tax_summary,
                MIN(sma_sale_items.tax_method) AS tax_method,
                MIN(sma_sale_items.tax) AS tax_value
                FROM sma_sale_items
                LEFT JOIN sma_product_variants ON sma_sale_items.option_id = sma_product_variants.id
            ...
            $this->datatables->select("
                ...
                FSI.item_tax_summary,
                CASE 
                    WHEN FSI.tax_value IS NULL OR FSI.tax_value = '' THEN 0 
                    ELSE FSI.tax_value 
                END AS tax_value,
                (sma_sales.grand_total + sma_sales.rounding),
                sma_sales.total_discount, 
                (sma_sales.grand_total - sma_sales.total_tax) as tax_able_amount,
                sma_sales.total_tax,
                sma_sales.paid,
                (sma_sales.grand_total + sma_sales.rounding - sma_sales.paid) as balance,
                GROUP_CONCAT(' ', sma_payments.paid_by),
                sma_sales.payment_status,
                sma_sales.id as id
            ", FALSE)
            ->from('sma_sales')
            ->join('sma_companies comp', 'sma_sales.customer_id=comp.id', 'left')
            ->join('sma_payments', 'sma_sales.id=sma_payments.sale_id', 'left')
            ->join($si, 'FSI.sale_id=sma_sales.id', 'left')
            ...
            if ($biller) {
                $this->datatables->where('sma_sales.biller_id', $biller);
            }

            if ($warehouse) {
                $getwarehouse = str_replace("_", ",", $warehouse);
                $this->datatables->where('sma_sales.warehouse_id IN (' . $getwarehouse . ')');
            }
            ...
            if ($start_date && $end_date) {
                $this->datatables->where("DATE(sma_sales.date) BETWEEN '{$start_date}' AND '{$end_date}'");
            }

            if ($trn_no) {
                $this->datatables->where("comp.gstn_no", $trn_no);
            }

            $this->datatables->group_by('sma_sales.id');
            echo $this->datatables->generate();
    ...
    public function gst_reports_new() {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('monthly_gst_report')));
        $meta = array('page_title' => lang('gst_report'), 'bc' => $bc);
        $this->page_construct('new_reports/gst_reports_new', $meta, $this->data);
    }
    ...
    public function request_gst_report_new($type = NULL, $year = NULL, $month = NULL, $xls = NULL, $warehouse = NULL, $biller = NULL, $supplier = NULL, $sale_items = FALSE, $customer = NULL, $start_date = NULL,  $end_date = NULL, $customer_group = NULL) {
        ...
        if ($type == "sale_gst_report") {
            ...
            $data = $this->reports_model_new->getSaleGstData_new($year, $month, $warehouse, $biller, $sale_items, $customer, $start_date, $end_date, $customer_group );
            

            if (!empty($data)) {
                $sale_ids = array_unique(array_column($data, 'sale_id'));
                // Retrieve dynamic tax details using global helper
                $global_tax_details = $this->sma->getSalesItemsTaxDetails($sale_ids);
                ...
                foreach ($data as $row) {
                    ...
                    $rowData[$row->sale_id]['sale_status']      = $row->sale_status;
                    $rowData[$row->sale_id]['date']             = $row->date;
                    $rowData[$row->sale_id]['time']             = date('H:i:s', strtotime($row->date));
                    $rowData[$row->sale_id]['customer_id']      = $row->customer_id;
                    $rowData[$row->sale_id]['customer']         = $row->customer;
                    $rowData[$row->sale_id]['customer_group_name'] = $row->customer_group_name;
                    $rowData[$row->sale_id]['note']         = $row->note;
                    ...
                    $rowData[$row->sale_id]['taxable_amount']   += $row->taxable_amount;
                    $rowData[$row->sale_id]['total_tax_amount'] += ($row->cgst + $row->sgst + $row->igst + $row->vat + $row->surcharge);
                    $rowData[$row->sale_id]['total_extra']      += ($row->shipping + $row->rounding);
                    $rowData[$row->sale_id]['invoice_amount']   += (($row->cgst + $row->sgst + $row->igst + $row->vat + $row->surcharge) + $row->taxable_amount);
                    ...
                    if($sale_items == TRUE){
                        ...
                        $rowData[$row->sale_id]['items'][$i]['sgst']             = $row->sgst;
                        $rowData[$row->sale_id]['items'][$i]['igst']             = $row->igst;
                        $rowData[$row->sale_id]['items'][$i]['vat']              = $row->vat;
                        $rowData[$row->sale_id]['items'][$i]['vat_rate']         = $row->vat_rate;
                        $rowData[$row->sale_id]['items'][$i]['surcharge']        = $row->surcharge;
                        $rowData[$row->sale_id]['items'][$i]['tax_total']        = ($row->cgst + $row->sgst + $row->igst + $row->vat + $row->surcharge);
                        $rowData[$row->sale_id]['items'][$i]['product_id']       = $row->product_id;
                        $rowData[$row->sale_id]['items'][$i]['product_code']     = $row->product_code;
                        $rowData[$row->sale_id]['items'][$i]['product_name']     = $row->product_name;
                        $rowData[$row->sale_id]['items'][$i]['option_name']      = $row->option_name;
                        ...
                        $rowData[$row->sale_id]['gst'][$i]['sgst']           = $row->sgst;
                        $rowData[$row->sale_id]['gst'][$i]['igst']           = $row->igst;
                        $rowData[$row->sale_id]['gst'][$i]['vat']            = $row->vat;
                        $rowData[$row->sale_id]['gst'][$i]['vat_rate']       = $row->vat_rate;
                        $rowData[$row->sale_id]['gst'][$i]['surcharge']      = $row->surcharge;
                        $rowData[$row->sale_id]['gst'][$i]['tax_total']      = ($row->cgst + $row->sgst + $row->igst + $row->vat + $row->surcharge);
                    }
                    
                    
                    // $rowFooter['total_extra']       += ($row->shipping + $row->rounding);
                    $rowFooter['taxable_amount'] += $row->taxable_amount;
                    $rowFooter['total_tax_amount'] += ($row->cgst + $row->sgst + $row->igst + $row->vat + $row->surcharge);
                    $rowFooter['cgst'] += $row->cgst;
                    $rowFooter['sgst'] += $row->sgst;
                    $rowFooter['igst'] += $row->igst;
                    $rowFooter['vat']  = isset($rowFooter['vat']) ? $rowFooter['vat'] + $row->vat : $row->vat;
                    $rowFooter['surcharge'] = isset($rowFooter['surcharge']) ? $rowFooter['surcharge'] + $row->surcharge : $row->surcharge;
                }

                if ($xls !== NULL) {
                    if($sale_items == TRUE){
                        $this->__sale_items_gst_report_download_new($rowData, $rowFooter, $sale_items);
                    } else {
                        $this->__sale_gst_report_download_new($rowData, $rowFooter, $sale_items);
                    }
                } else {
                    $report_title = "sale_gst_report Month: $month  Year : $year";
                    $this->__sale_gst_report_new($rowData, $rowFooter, $sale_items);
                }
            }
        }
        ...
        if ($type == "purchase_gst_report") {
            ...
            $data = $this->reports_model_new->getPurchaseGstData_new($year, $month, $warehouse, $supplier , $start_date, $end_date);
                       
             if (!empty($data)) {
                $purchase_ids = array_unique(array_column($data, 'purchase_id'));
                // Retrieve purchase dynamic tax details using global helper
                $global_purchase_tax_details = $this->sma->getSalesItemsTaxDetails($purchase_ids, NULL, NULL, NULL, FALSE, 'purchase');
                ...
                foreach ($data as $row) {
                    ...
                    $rowData[$row->purchase_id]['total_items'] += $row->total_items;
                    $rowData[$row->purchase_id]['taxable_amount'] += $row->taxable_amount;
                    $rowData[$row->purchase_id]['total_tax_amount'] += ($row->cgst + $row->sgst + $row->igst + $row->vat);
                    $rowData[$row->purchase_id]['total_extra'] += ($row->shipping + $row->surcharge + $row->rounding);
                    $rowData[$row->purchase_id]['bill_amount'] += (($row->cgst + $row->sgst + $row->igst + $row->vat) + $row->taxable_amount);
                    ...
                    $rowData[$row->purchase_id]['gst'][$i]['sgst'] = $row->sgst;
                    $rowData[$row->purchase_id]['gst'][$i]['igst'] = $row->igst;
                    $rowData[$row->purchase_id]['gst'][$i]['vat'] = $row->vat;
                    $rowData[$row->purchase_id]['gst'][$i]['vat_rate'] = $row->vat_rate;

                    // $rowFooter['total_extra']       += ($row->shipping + $row->rounding);
                    $rowFooter['taxable_amount'] += $row->taxable_amount;
                    $rowFooter['total_tax_amount'] += ($row->cgst + $row->sgst + $row->igst + $row->vat);
                    $rowFooter['cgst'] += $row->cgst;
                    $rowFooter['sgst'] += $row->sgst;
                    $rowFooter['igst'] += $row->igst;
                    $rowFooter['vat']  = isset($rowFooter['vat']) ? $rowFooter['vat'] + $row->vat : $row->vat;
                ...
                if ($xls !== NULL) {
                    ...
                    $this->__purchase_gst_report_new($rowData, $rowFooter, $report_title);
                }
            }
    ...
    private function __sale_gst_report_new($rowData, $rowFooter, $sale_items) {
                ...
                if($sale_items) {           
                    ...
                    $table .= '<th rowspan="2">' . lang("Items") . '</th>';
                          ...
                          $table .= ' <th colspan="2">' . lang("CGST") . '</th>
                            <th colspan="2">' . lang("SGST") . '</th>
                            <th colspan="2">' . lang("IGST") . '</th>
                            <th colspan="2">' . lang("VAT") . '</th>
                            <th rowspan="2">' . lang("Surcharge") . '</th>
                            <th rowspan="2">' . lang("Tax Total") . '</th>                            
                            <th rowspan="2">' . lang("Invoice Total GST") . '</th>                            
                            <th rowspan="2">' . lang("Invoice Amount") . '</th>
                            <th rowspan="2">' . lang("Invoice Status") . '</th> 
            ...
            foreach ($rowData as $row_id => $row) {
                ...
                if($sale_items == TRUE){
                    ...
                    foreach ($row['items'] as $item) {
                        ...
                        $itemTds .= '<td>' . (($item['cgst'] > 0) ? number_format($item['gst_rate'], 2) . '%' : '0') . '</td>
                                    <td align="right">' . $item['cgst'] . '</td>
                                    <td>' . (($item['sgst'] > 0) ? number_format($item['gst_rate'], 2) . '%' : '0') . '</td>
                                    <td align="right">' . $item['sgst'] . '</td>
                                    <td>' . (($item['igst'] > 0) ? number_format($item['gst_rate'], 2) . '%' : '0') . '</td>
                                    <td align="right">' . $item['igst'] . '</td> 
                                    <td>' . (($item['vat'] > 0) ? number_format($item['vat_rate'], 2) . '%' : '0') . '</td>
                                    <td align="right">' . $item['vat'] . '</td> 
                                    <td align="right">' . $this->sma->formatMoney($item['surcharge']) . '</td>
                                    <td align="right">'.  $item['tax_total'] . '</td>';
                    ...
                    foreach ($row['gst'] as $tax) {
                        ...
                        $td_taxes = '<td>'.number_format($tax['tax_rate'], 2).'%</td> 
                                        <td>'.$this->sma->formatQuantity($tax['total_items']).'</td> 
                                        <td>' . (($tax['cgst'] > 0) ? number_format($tax['gst_rate'], 2) . '%' : '0') . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['cgst']) . '</td>
                                        <td>' . (($tax['sgst'] > 0) ? number_format($tax['gst_rate'], 2) . '%' : '0') . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['sgst']) . '</td>
                                        <td>' . (($tax['igst'] > 0) ? number_format($tax['gst_rate'], 2) . '%' : '0') . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['igst']) . '</td> 
                                        <td>' . (($tax['vat'] > 0) ? number_format($tax['vat_rate'], 2) . '%' : '0') . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['vat']) . '</td> 
                                        <td align="right">' . $this->sma->formatMoney($tax['surcharge']) . '</td>
                                        <td align="right">' . $this->sma->formatMoney($tax['tax_total']) . '</td>';
        ...
        $table .= ' </tbody>
                        ...
                        <tr class="bg-primary">
                            ...
                            <th style="color:#ffffff;" title="Total VAT Amount">' . $this->sma->formatMoney($rowFooter['vat']) . '</th>
                            <th style="color:#ffffff;" title="Total Surcharge Amount">' . $this->sma->formatMoney($rowFooter['surcharge']) . '</th>
                            <th style="color:#ffffff;">' . $this->sma->formatMoney(($rowFooter['cgst']+$rowFooter['sgst']+$rowFooter['igst']+$rowFooter['vat']+$rowFooter['surcharge'])) . '</th>
                            <th style="color:#ffffff;" title="Total Tax Amount">' . $this->sma->formatMoney($rowFooter['total_tax_amount']) . '</th>
                            <th style="color:#ffffff;" title="Total Invoice Amount">' . $this->sma->formatMoney($rowFooter['taxable_amount'] + $rowFooter['total_tax_amount'], 2) . '</th>
                            <th></th>                             
                        </tr>
    ...
    private function __purchase_gst_report_new($rowData, $rowFooter) {
        ...
        if (!empty($rowData)) {
            ...
            foreach ($rowData as $row_id => $row) {
                ...
                foreach ($row['gst'] as $tax) {
                    ...
                    $tx++;
                    $td_taxes = '<td align="right">' . (($tax['cgst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '0') . '</td>
                                <td align="right">' . $this->sma->formatMoney($tax['cgst']) . '</td>
                                <td align="right">' . (($tax['sgst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '0') . '</td>
                                <td align="right">' . $this->sma->formatMoney($tax['sgst']) . '</td>
                                <td align="right">' . (($tax['igst'] > 0) ? $this->sma->formatQuantity($tax['gst_rate']) . '%' : '0') . '</td>
                                <td align="right">' . $this->sma->formatMoney($tax['igst']) . '</td>
                                <td align="right">' . (($tax['vat'] > 0) ? $this->sma->formatQuantity($tax['vat_rate']) . '%' : '0') . '</td>
                                <td align="right">' . $this->sma->formatMoney($tax['vat']) . '</td>';
    ...
    private function __sale_items_gst_report_download_new($rowData, $rowFooter, $sale_items) {
        ...
        if (!empty($rowData)) {
            ...
            $this->excel->getActiveSheet()->getStyle("A1:AM1")->applyFromArray($style);
            $this->excel->getActiveSheet()->mergeCells('A1:AM1', 'GST Sales Report');
            ...
            $cellColoumnTitle = ['A' => 'Sr No',
                ...
                'X' => 'VAT Amt',
                'Y' => 'Surcharge',
                'Z' => "Product's Tax Amount",
                'AA' => 'Total Taxable Amount',
                'AB' => 'Invoice Tax Amount',
                'AC' => 'Total Amount',
                'AD' => 'Invoice Status',
                'AE' => 'Biller Name',
                'AF' => 'Warehouse',
            ...
            $row = 3;
            $sr = 0;
            $total_cgst = $total_sgst = $total_igst = $total_vat = $total_surcharge = $total_tax_amount = $total_taxable_amount
                    = $toal_invoice_tax_amount = $total_invoice_amount = 0;
            foreach ($rowData as $sale_id => $inv) {
                
                 $total_taxable_amount += $inv[$cellData['AA']];
                 $toal_invoice_tax_amount  += $inv[$cellData['AB']];
                 $total_invoice_amount += $inv[$cellData['AC']];
               
                $sr++;
                $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sr);

                foreach (range('B', 'H') as $cell_1) {
                    $this->excel->getActiveSheet()->SetCellValue($cell_1 . $row, ( ($cell_1 == 'C')? date('Y-m-d',strtotime($inv[$cellData[$cell_1]])) :$inv[$cellData[$cell_1]]));
                }//end foreach

                // Write all invoice-level columns once at the invoice row before the items inner loop
                $this->excel->getActiveSheet()->SetCellValue('AA' . $row, $inv[$cellData['AA']]);
                $this->excel->getActiveSheet()->SetCellValue('AB' . $row, $inv[$cellData['AB']]);
                $this->excel->getActiveSheet()->SetCellValue('AC' . $row, $inv[$cellData['AC']]);
                $this->excel->getActiveSheet()->SetCellValue('AD' . $row, $inv[$cellData['AD']]);
                $this->excel->getActiveSheet()->SetCellValue('AE' . $row, $inv[$cellData['AE']]);
                $this->excel->getActiveSheet()->SetCellValue('AF' . $row, $inv[$cellData['AF']]);
                $this->excel->getActiveSheet()->SetCellValue('AG' . $row, $inv[$cellData['AG']]);
                $this->excel->getActiveSheet()->SetCellValue('AH' . $row, $inv[$cellData['AH']]);
                $this->excel->getActiveSheet()->SetCellValue('AI' . $row, $inv[$cellData['AI']]);
                $this->excel->getActiveSheet()->SetCellValue('AJ' . $row, $inv[$cellData['AJ']]);
                $this->excel->getActiveSheet()->SetCellValue('AK' . $row, $inv[$cellData['AK']]);
                $this->excel->getActiveSheet()->SetCellValue('AL' . $row, $inv[$cellData['AL']]);
                $this->excel->getActiveSheet()->SetCellValue('AM' . $row, $inv[$cellData['AM']]);
                ...
                foreach ($inv['items'] as $item) {
                    ...
                    $sgst_rate = (($item['sgst'] > 0) ? number_format($item['gst_rate'], 2) . '%' : '00');
                    $igst_rate = (($item['igst'] > 0) ? number_format($item['gst_rate'], 2) . '%' : '00');
                    $vat_rate = (($item['vat'] > 0) ? number_format($item['vat_rate'], 2) . '%' : '00');
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('U' . $tax_row, $igst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('V' . $tax_row, $item['igst']);
                    $this->excel->getActiveSheet()->SetCellValue('W' . $tax_row, $vat_rate);
                    $this->excel->getActiveSheet()->SetCellValue('X' . $tax_row, $item['vat']);
                    $this->excel->getActiveSheet()->SetCellValue('Y' . $tax_row, $item['surcharge']);
                    $this->excel->getActiveSheet()->SetCellValue('Z' . $tax_row, $item['tax_total']);

                    $total_cgst += $item['cgst'];
                    $total_sgst += $item['sgst'];
                    $total_igst += $item['igst']; 
                    $total_vat += $item['vat'];
                    $total_surcharge += $item['surcharge'];
                    $total_tax_amount += $item['tax_total'];
             ...
             $this->excel->getActiveSheet()->SetCellValue('T' . $row , $total_sgst);
             $this->excel->getActiveSheet()->SetCellValue('V' . $row , $total_igst);
             $this->excel->getActiveSheet()->SetCellValue('X' . $row , $total_vat);
             $this->excel->getActiveSheet()->SetCellValue('Y' . $row , $total_surcharge);
             $this->excel->getActiveSheet()->SetCellValue('Z' . $row , $total_tax_amount);
            
            $this->excel->getActiveSheet()->SetCellValue('AA' . $row , $total_taxable_amount);
            $this->excel->getActiveSheet()->SetCellValue('AB' . $row , $toal_invoice_tax_amount);
            $this->excel->getActiveSheet()->SetCellValue('AC' . $row , $total_invoice_amount);
            
            $this->excel->getActiveSheet()->getStyle("A" . $row++ . ":AM" . $row++)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);

            foreach (range('B', 'AM') as $cell_4) {
                $this->excel->getActiveSheet()->getColumnDimension($cell_4)->setWidth(20);
            }//end foreach
     ...
     private function __sale_gst_report_download_new($rowData, $rowFooter) {
        ...
        if (!empty($rowData)) {
            ...
            $this->excel->getActiveSheet()->getStyle("A1:AG1")->applyFromArray($style);
            $this->excel->getActiveSheet()->mergeCells('A1:AG1', 'GST Sales Report');
           ...
           // $this->excel->getActiveSheet()->getStyle("A" . $row . ":Y" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
            ...
            $row = 3;
            $sr = 0;
            $total_cgst = $total_sgst = $total_igst = $total_vat = $total_surcharge = $total_taxable_amount = $total_product_tax_amount = $total_invoice_tax_amount = $total_discount = $total_amount = 0;

            foreach ($rowData as $invoice_no => $inv) {
                // $parts = explode(' ', $inv['date']);
                // $time = $parts[1];
               
               $total_taxable_amount += $inv[$cellData['T']];
               $total_invoice_tax_amount += $inv[$cellData['V']];
               $total_discount += $inv[$cellData['W']];
               $total_amount += $inv[$cellData['X']];
                ...
                // Write invoice-level columns once at the invoice row (V=Invoice Tax, W=Discount, X=Total)
                $this->excel->getActiveSheet()->SetCellValue('V' . $row, $this->sma->formatMoney($inv[$cellData['V']]));
                $this->excel->getActiveSheet()->getStyle('V' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->SetCellValue('W' . $row, $this->sma->formatMoney($inv[$cellData['W']]));
                $this->excel->getActiveSheet()->getStyle('W' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                $this->excel->getActiveSheet()->SetCellValue('X' . $row, $this->sma->formatMoney($inv[$cellData['X']]));
                $this->excel->getActiveSheet()->getStyle('X' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                // Write remaining invoice-level columns (Y through AD) once at the invoice row
                $this->excel->getActiveSheet()->SetCellValue('Y' . $row, $inv[$cellData['Y']]);
                $this->excel->getActiveSheet()->SetCellValue('Z' . $row, $inv[$cellData['Z']]);
                $this->excel->getActiveSheet()->SetCellValue('AA' . $row, $inv[$cellData['AA']]);
                $this->excel->getActiveSheet()->SetCellValue('AB' . $row, $inv[$cellData['AB']]);
                $this->excel->getActiveSheet()->SetCellValue('AC' . $row, $inv[$cellData['AC']]);
                $this->excel->getActiveSheet()->SetCellValue('AD' . $row, $inv[$cellData['AD']]);
               ...
               $this->excel->getActiveSheet()->SetCellValue('AG' . $row, $inv[$cellData['AG']]);
                ...
                foreach ($inv['gst'] as $tax) {
                    ...
                    $vat_rate = ($tax['vat'] > 0) ? number_format($tax['vat_rate'], 2) . '%' : '00';
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('Q' . $tax_row, ($vat_rate));
                    $this->excel->getActiveSheet()->getStyle('Q' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

                    $this->excel->getActiveSheet()->SetCellValue('R' . $tax_row, $this->sma->formatMoney($tax['vat']));
                    $this->excel->getActiveSheet()->getStyle('R' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('S' . $tax_row, $this->sma->formatMoney($tax['surcharge']));
                    $this->excel->getActiveSheet()->getStyle('S' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('T' . $tax_row, $this->sma->formatMoney($tax['taxable_amount']));
                    $this->excel->getActiveSheet()->getStyle('T' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

                    $this->excel->getActiveSheet()->SetCellValue('U' . $tax_row, $this->sma->formatMoney($tax['tax_total']));
                    $this->excel->getActiveSheet()->getStyle('U' . $tax_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
            ...
            $this->excel->getActiveSheet()->SetCellValue('R' . $row, $this->sma->formatMoney($total_vat));
            $this->excel->getActiveSheet()->getStyle('R' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('S' . $row, $this->sma->formatMoney($total_surcharge));
            $this->excel->getActiveSheet()->getStyle('S' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('T' . $row, $this->sma->formatMoney($total_taxable_amount));
            $this->excel->getActiveSheet()->getStyle('T' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('U' . $row, $this->sma->formatMoney($total_product_tax_amount));
            $this->excel->getActiveSheet()->getStyle('U' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('V' . $row, $this->sma->formatMoney($total_invoice_tax_amount));
            $this->excel->getActiveSheet()->getStyle('V' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('W' . $row, $this->sma->formatMoney($total_discount));
            $this->excel->getActiveSheet()->getStyle('W' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

            $this->excel->getActiveSheet()->SetCellValue('X' . $row, $this->sma->formatMoney($total_amount));
            $this->excel->getActiveSheet()->getStyle('X' . $row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);

             $this->excel->getActiveSheet()->getStyle("A" . $row . ":AG" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);

            foreach (range('B', 'AG') as $cell_4) {
                $this->excel->getActiveSheet()->getColumnDimension($cell_4)->setWidth(20);
            }//end foreach
