...
class Offline extends MY_Controller
{
    ...
    {        
       ...
       // $edit_link = anchor('sales/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_sale'), 'class="sledit"');
        ...
        if ($warehouse_id) {            
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, date, reference_no,".$this->db->dbprefix('sales') . ".id as invoice_id, biller, customer, (grand_total+IFNULL(`rounding`, 0)) , paid, (grand_total-paid) as balance,sale_status, payment_status")
                ->from('sales')
                ->where('offline_sale', 1);
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, date, reference_no,".$this->db->dbprefix('sales') . ".id as invoice_id, biller, customer, (grand_total+IFNULL(`rounding`, 0)) , paid, (grand_total+IFNULL(`rounding`, 0)-paid) as balance,sale_status, payment_status")
                ->from('sales')
                ->where('offline_sale', 1);
        }
