...
class Pos_elite extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->lang->load('pos_elite', $this->Settings->user_language);
    ...
    public function sales($warehouse_id = null) {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('pos_elite'), 'page' => lang('pos_elite')), array('link' => '#', 'page' => lang('pos_sales')));
    ...
    public function getSales($warehouse_id = null) {
        ...
        $detail_link = anchor('pos_elite/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('view_receipt'));
    ...
    public function index($sid = null) {
        ...
        if ($register = $this->pos_model->registerData($this->session->userdata('user_id'))) {
            ...
            redirect('pos_elite/open_register');
        }
        ...
        if (($customerRefNo ||$result)) {
            ...
            if ($customerRefNo) {
                $customerRefNo = explode("/", $customerRefNo)[1];
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_mrp_discount = isset($_POST['discount_on_mrp'][$r]) ? $_POST['discount_on_mrp'][$r] : '0%';
                $mrp = isset($_POST['mrp'][$r]) ? $_POST['mrp'][$r] : 0; // product mrp
                if (empty($mrp)) {
                    $mrp = $unit_price; // Fallback to unit price if mrp is missing or zero
                }

                $batch_number = (isset($_POST['batch_number'][$r]) && $_POST['batch_number'][$r] != 'undefined' && $_POST['batch_number'][$r] != '') ? $_POST['batch_number'][$r] : NULL;
                $customer_group_discount = isset($_POST['customer_group_discount'][$r]) ? $_POST['customer_group_discount'][$r] : 0; // flag for customer discount applied
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($suspend) {
                ...
                if ($this->pos_model->suspendSale($suspend_data, $suspend_products, $did)) {
                    $this->session->set_userdata('remove_posls', 1);
                        ...
                        if ($Settings->pos_type == 'restaurant') {
                            return redirect("pos_elite/kot");
                        } else {
                            return redirect("pos_elite");
                        }
                    exit;
                }
            } else {
                ...
                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
                    ...
                    //redirect($this->pos_settings->after_sale_page ? "pos_elite" : "pos/view/" . $sale['sale_id']);
                    if ($_POST['submit_type'] == 'notprint') {
                        ...
                        return redirect($this->pos_settings->after_sale_page ? "pos_elite/kot" : "pos_elite/view/" . $sale['sale_id']);
                ...
                if ($suspended_sale) {
                    ...
                    foreach ($inv_items as $item) {
                        ...
                        $pr[$ri] = ['id' => $c, 'item_id' => $row_id, 'label' => $row->name . " (" . $row->code . ")", 'divisionid' => $row->divisionid,
                            'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'batchs' => $batchoption, 'product_batches' => $productbatches, 'note' => $row->note, 'categoryTaxData' => $categoryTax, 'category' => $row->category_id, 'sub_category' => $row->subcategory_id,'fixtax' => $fixtaxrate, 'category_tax' => $varaibletax];
                        $c++;
            ...
            $this->load->view($this->theme . 'pos/add_epelite', $this->data);
    ...
    public function registers() {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('pos_elite'), 'page' => lang('pos_elite')), array('link' => '#', 'page' => lang('open_registers')));
        $meta = array('page_title' => lang('open_registers'), 'bc' => $bc);
        $this->page_construct('pos_elite/registers', $meta, $this->data);
    }
    ...
    public function open_register() {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->openRegister($data)) {
            ...
            $this->page_construct('pos_elite/open_register', $meta, $this->data);
        }
    }
    ...
    public function getProductDataByCode($code = null, $warehouse_id = null) {
        ...
        if ($row) {
            ...
            $pr = array('id' => $c, 'item_id' => $row_id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'sub_category' => $row->subcategory_id, 'divisionid' => $row->divisionid, 'brand' => $row->brand, 'fixtax' => $fixtaxrate, 'category_tax' => $varaibletax, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
    ...
    public function ajaxproducts($category_id = null, $brand_id = null,$seasons_id = null) {
        ...
        $config = array();
        $config["base_url"] = base_url() . "pos_elite/ajaxproducts";
            ...
            foreach ($products as $product) {
                ...
                if ($Settings->theme != 'default') {
                    $prods .= "<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' title=\"" . $product->name . "\" class=\"btn-prni btn-" . $this->pos_settings->product_button_color . " product pos-tip\" data-container=\"body\" style='" . $background_and_text_color . "'>
                                ...
                                <br><span class='bgspan' style='" . $background_and_text_color . "'>" . character_limiter($product->name, 15) . "</span></button>";
                } else {
                    $prods .= "<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' title=\"" . $product->name . "\" class=\"btn-prni btn-" . $this->pos_settings->product_button_color . " product pos-tip\" data-container=\"body\" style='" . $background_and_text_color . "'>
                                <img src=\"" . $imgsrc . "\" alt=\"" . $product->name . "\" style='width:" . $this->Settings->twidth . "px;height:" . $this->Settings->theight . "px;' class='img-rounded' />
                                <span class='' style='" . $background_and_text_color . "'>" . character_limiter($product->name, 20) . "</span></button>";
                }
                $pro++;
    ...
    public function view_up($sale_id = null, $modal = null) {
        ...
        $print['pos_elite'] = $this->data['pos_elite'] = $this->pos_model->getSetting();
        unset($print['pos_elite']->pos_theme);
...
<?php
    ...
    public function view($sale_id = null, $modal = null) {
        ...
        if ($inv->order_no) {
            ...
            $order = $this->pos_model->getEshopOrderByInvoice($inv->order_no);
            $this->data['billing_details'] = $this->pos_model->getBillingDetails($order->shipping_address_id);
            $this->data['shipping_details']->shipping_name  = $this->data['billing_details']->address_name;
            $this->data['shipping_details']->shipping_phone = $this->data['billing_details']->phone;
            $this->data['shipping_details']->shipping_email = $this->data['billing_details']->email_id;
            $this->data['shipping_details']->shipping_addr  = trim(implode(', ', array_filter([
                $this->data['billing_details']->line1,
                $this->data['billing_details']->line2,
                $this->data['billing_details']->city,
                $this->data['billing_details']->state
            ])));
        }
...
//        }//end if.
        ...
        if (isset($Settings->pos_type) && $Settings->pos_type == 'pharma') {
            ...
            // Detect Android device and route to android_view.php
            $user_agent = isset($_SERVER['HTTP_USER_AGENT']) ? strtolower($_SERVER['HTTP_USER_AGENT']) : '';
            $is_android = (strpos($user_agent, 'android') !== false);
            
            if ($is_android) {
                $this->load->view($this->theme . 'pos/android_view', $this->data);
            } else {
    ...
    public function settings() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'razorpay' => $this->input->post('razorpay'),
        ...
        if ($this->form_validation->run() == true && $this->pos_model->updateSetting($data)) {
            ...
            $this->data['pos_elite'] = $this->pos_model->getSetting();

            $this->data['pos_elite']->pos_theme = json_decode($this->data['pos_elite']->pos_theme);
...
//$this->data['customer'] = $this->pos_model->getCompanyByID($this->pos_settings->default_customer);
            ...
            $this->page_construct('pos_elite/settings', $meta, $this->data);
        }
    }
                ...
                if (is_array($paymentDetail)):
                    ...
                    if (isset($paymentDetail["status"]) && in_array($paymentDetail["status"], array('Credit', 'credit', 'Completed'))):
                        ...
                        if ($res):
                            ...
                            redirect($this->pos_settings->after_sale_page ? "pos_elite" : "pos_elite/view/" . $sid);
                        else:
    ...
    public function ccavenue_init() {
        ...
        if ((int) $sale_id > 0):
            ...
            if ($_req->id):
                ...
                redirect($this->pos_settings->after_sale_page ? "pos_elite" : "pos_elite/view/" . $sale_id);
            endif;
            ...
            if ($sale->id == $sale_id):
                ...
                $arr['redirect_url'] = base_url('pos_elite/ccavenue_notify');
                $arr['cancel_url'] = base_url('pos_elite/ccavenue_cancel');
    ...
    public function ccavenue_notify() {
        ...
        try {
            ...
            redirect($this->pos_settings->after_sale_page ? "pos_elite" : "pos_elite/");
        } catch (Exception $e) {
    ...
    public function paytm_init() {
        ...
        if ((int) $sale_id > 0):
            ...
            if ($sale->id == $sale_id):
                ...
                $paramList['CALLBACK_URL'] = base_url('pos_elite/paytm_notify');
    ...
    public function paytm_notify() {
        ...
        if ($_POST['STATUS'] != 'TXN_SUCCESS') {
            ...
            if ((int) $ORDERID > 0):
                redirect("pos_elite/view/" . $ORDERID);
            else:
                redirect("pos_elite/");
            endif;
        }
    ...
    public function payswiff_init() {
        ...
        if ((int) $sale_id > 0):
            ...
            if ($sale->id == $sale_id):
                ...
                $paramList["channel"] = '3';
                $paramList["responseURL"] = base_url('pos_elite/payswiff_notify'); //Email ID of customer
    ...
    public function paynear_init() {
        ...
        if ((int) $sale_id > 0):
            ...
            if ($sale->id == $sale_id):
                ...
                $paramList["amount"] = $this->sma->formatDecimal($sale->grand_total);
                $paramList["channel"] = '3';
                $paramList["responseURL"] = base_url('pos_elite/paynear_notify'); //Email ID of customer
...
<?php
    ...
    public function payumoney_init() {
        ...
        if ((int) $sale_id > 0):
            ...
            if ($sale->id == $sale_id):
                ...
                $posted['udf5'] = '';
                $posted['pg'] = '';
                $posted['furl'] = base_url('pos_elite/payumoney_cancel');
                $posted['surl'] = base_url('pos_elite/payumoney_notify');
    ...
    public function featuerdProducts() {
        ...
        $config = array();
        $config["base_url"] = base_url() . "pos_elite/featuerdProducts";
            ...
            if ($suspend) {
                ...
                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
                    ...
                    if ($_POST['submit_type'] == 'notprint') {
                        ...
                        redirect($this->pos_settings->after_sale_page ? "pos_elite" : "pos_elite/view/" . $sale['sale_id']);
                        exit;
                    }
                }
            }
 ...
 //
    ...
    public function get_dependancy() {
        ...
        $id = $this->input->get('id');
        $this->db->select('id,name,phone,customer_group_id');
        if ($phone !== NULL && $phone !== '') {
            ...
            if (!empty($getKyes)) {
                ...
                $this->db->where($getKyes[0], $get_data);
            } else {
                echo json_encode(null);
                exit;
            }
        }
        $get_customer = $this->db->get('sma_companies')->row();
...
//print_r( $res );exit;
    ...
    public function SendAutoEmail() {
        ...
        if ($sendemailurl == 'close_register') {
            redirect('pos_elite/registers/1');
    ...
    public function searchAwardPointByCustomer($customer_id = null, $bill_amt = null) {
        ...
        $ArrData = array('msg' => $Msg, 'redeem_point' => $FinalRedeemPoint, 'invoice_amount' => $InvoiceAmt, 'phone' => $CustomerPhone );
    ...
    // item return 
     public function returnsale($id = null) {
        ...
        for ($r = 0; $r < $i; $r++) {
            ...
            if (count($productIds) == count($returnQuantities)) {
                foreach ($productIds as $index => $productId) {
                    ...
                    $item_mrp = !empty($item_mrp) ? $item_mrp : $Products->mrp;
                    $item_mrp = $this->sma->formatDecimal($item_mrp);
                    $pr_discount = 0;
                    $unit_discount = 0;
                    $option_ids = explode(',', $_POST['option_ids'][$r]);
                    $unit_price = isset($unit_prices[$index]) ? $unit_prices[$index] : null;
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        ...
                        if ($tax_details->type == 1 && $tax_details->rate != 0) {
                            ...
                            if ($Products && $Products->tax_method == 1 && $manualeditprice == '') {
                                $item_tax = $this->sma->formatDecimal((($item_unit_price_less_discount) * $tax_details->rate) / 100, 6);
                                $tax = $tax_details->rate . "%";
                                $net_unit_price = $item_unit_price_less_discount;
                                $unit_price = $item_unit_price_less_discount + $item_tax;
                            ...
                            if ($Products && $Products->tax_method == 1) {
                                $item_tax = $this->sma->formatDecimal((($item_unit_price_less_discount) * $tax_details->rate) / 100, 6);
                                $tax = $tax_details->rate . "%";
                                $net_unit_price = $item_unit_price_less_discount;
                                $unit_price = $item_unit_price_less_discount + $item_tax;
                    ...
                    $productmrp = isset($mrp[$index]) ? $mrp[$index] : null;
        ...
        $data = array(
            ...
            'order_tax_id' => $order_tax_id,
            'order_tax' => $order_tax,
            'total_discount' => $this->sma->remove_commas($total_discount),
            'total_tax' => $total_tax,
    ...
    public function loadSuspendScanItem($SuspendIds){
        ...
        if ($SuspendIds) {
            ...
            foreach ($inv_items as $item) {
                ...
                $row->discount = $item->discount ? $item->discount : '0';
                $row->unit_price = $this->sma->formatDecimal($item->net_unit_price + $this->sma->formatDecimal($item->item_discount / $item->quantity));
                $row->unit_price = $row->tax_method ? $item->unit_price + $this->sma->formatDecimal($item->item_discount / $item->quantity) + $this->sma->formatDecimal($item->item_tax / $item->quantity) : $item->unit_price + ($item->item_discount / $item->quantity);
                $row->real_unit_price = $item->real_unit_price + $item->item_discount;
                $row->base_quantity = $item->quantity;
                $row->base_unit = isset($row->unit) ? $row->unit : $item->product_unit_id;
                $row->base_unit_price = $row->price ? $row->price : $item->unit_price;
                $row->unit = $item->product_unit_id;
