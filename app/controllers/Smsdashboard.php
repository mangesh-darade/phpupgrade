...
class Smsdashboard extends MY_Controller
{
    ...
    /**
     * Wrapper to send WhatsApp messages from SMS Dashboard using global Sma helper.
     *
     * @param array $mobiles
     * @param string $content
     * @return void
     */
    private function SendWhatsappDashboard($mobiles, $content)
    {
        if (empty($mobiles) || empty($content)) {
            return;
        }
        ...
        foreach ($mobiles as $mobile) {
            $res = $this->sma->send_whatsapp_raw($mobile, $content);
            if (is_array($res) && isset($res['status']) && $res['status'] !== 'success') {
                // Try to extract a meaningful error message
                $msg = 'Unknown error';
                if (isset($res['message']) && $res['message']) {
                    $msg = $res['message'];
                } elseif (isset($res['response']['message']) && $res['response']['message']) {
                    $msg = $res['response']['message'];
                } elseif (isset($res['http_code'])) {
                    $msg = 'HTTP ' . $res['http_code'];
                }
                $this->session->set_flashdata('error', 'WhatsApp not sent to ' . $mobile . ': ' . $msg);
            }
        }
    }
...
//        $content = $this->input->post('sms_body');
    ...
    /*------------- Add Single WhatsApp Message (Dashboard) ---------------*/
    public function addSingleWhatsapp()
    {
        $this->form_validation->set_rules('hiddencust_whatsapp', lang("Customer Contact"), 'required');
        $this->form_validation->set_rules('wa_body', lang("Message"), 'required');

        if ($this->form_validation->run() == false) {
            exit(json_encode(array('error' => validation_errors())));
        }

        $contactArr = $this->ContactParse($this->input->post('hiddencust_whatsapp'));
        $mobile_arr = isset($contactArr['mobile']) && is_array($contactArr['mobile']) ? $contactArr['mobile'] : array();

        if (empty($mobile_arr)) {
            exit(json_encode(array('error' => 'Mobile no not found.')));
        }

        $content = $this->input->post('wa_body');

        $this->SendWhatsappDashboard($mobile_arr, $content);
        ...
        exit(json_encode(array('success' => 'WhatsApp message sent successfully')));
    }

    /*------------- Add Group WhatsApp Message (Dashboard) ---------------*/
    public function addGroupWhatsapp()
    {
        $this->form_validation->set_rules('group_id', lang("Customer Group"), 'required');
        $this->form_validation->set_rules('wa_body', lang("Message"), 'required');
        ...
        if (count($mobile_arr) == 0) {
            exit(json_encode(array('error' => 'Unable to send WhatsApp message (no mobile numbers found).')));
        }
        ...
        exit(json_encode(array('success' => 'WhatsApp messages sent successfully')));
    }
