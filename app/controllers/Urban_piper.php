        ...
        if (is_array($upOrders)) {
            foreach ($upOrders as $key => $order) {
                ...
                $tablelist .='<tr>
                                ...
                                <td>' . $this->sma->formatMoney($up_response->order->details->order_total) . '</td>
                                <td>';
        ...
        foreach ($up_response->order->items as $items) {
            ...
            if (!empty($items->options_to_add)) {
               ...
               /* foreach ($items->options_to_add as $options) {
                    $optionAddName[] = $options->title . ' - ' . $this->sma->formatMoney($options->price);
                    $price += $options->price;
                }//end for   */                 
            }//end if
            ...
            if (!empty($items->charges)) {
                foreach ($items->charges as $charge) {
                    $chargeArr[] = $charge->title . ' - ' . $this->sma->formatMoney($charge->value);
                }//end for 
            }//end if
