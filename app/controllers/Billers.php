...
class Billers extends MY_Controller
{
        ...
        if ($this->form_validation->run('supplier/add') == true) {
            ...
            if (($this->input->post('state') == 'other' || $this->input->post('state') == '') && ($this->input->post('statecode') != '' && $this->input->post('statename') != '' )) {
                ...
                if ($this->input->post('state') == 'other') {
                    ...
                    // Value can be "Name" or "Name~CODE"
                    $state_value = $this->input->post('state');
                    if (strpos($state_value, '~') !== false) {
                        $parts = explode('~', $state_value);
                        $state = $parts[0];
                        $state_code = $parts[1];
                    } else {
                        $state = $state_value;
            ...
            // Derive GST state code from cleaned state name (for India GST)
            $gst_state_code = $state ? $this->site->getGstStateCodeFromName($state) : NULL;
            ...
            $data = array('name' => $this->input->post('name'),
                ...
                'pan_card' => strtoupper(trim($this->input->post('pan_card'))),
    ...
    public function getLogoList()
    {
        ...
        $dirname = "assets/mdata/$this->Customer_assets/uploads/logos";
