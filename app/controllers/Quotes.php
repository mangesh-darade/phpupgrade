...
class Quotes extends MY_Controller
{
    ...
    {
        parent::__construct();
        $authentication_methods = array('pdf_view','pdfview');
    ...
    public function getQuotes($warehouse_id = null)
    {
        ...
        $sms_link = anchor('reciept/quote_sms/$1', '<i class="fa fa-send"></i> ' . lang('SMS Quotation'), 'class="send_quote_sms" quote_id="$1"');
    ...
    public function pdfview($code = NULL, $view = NULL, $save_bufffer = NULL){
         $this->pdf_view($code, $view, $save_bufffer);
    }
    
    public function pdf_view($code = NULL, $view = NULL, $save_bufffer = NULL)
    {
    ...
    public function email($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if(!empty($inv->attachment)):
           	 $attachment[] =  $this->digital_upload_path. $inv->attachment  ; 
            endif;
...
public function add(){
    ...
    if ($this->form_validation->run() == true) {
        ...
        if ($this->Owner || $this->Admin ||  $this->GP['quotes-date']) {
        ...
        for ($r = 0; $r < $i; $r++) {
            ...
            $item_option_color = isset($_POST['product_option_color'][$r]) ? $_POST['product_option_color'][$r] : null;
            ...
            $item_mrp = $this->sma->formatDecimal($_POST['mrp'][$r]);
            // ADDED: Capture shade_id from post data
            //$item_shade_id = isset($_POST['shade_id'][$r]) ? $_POST['shade_id'][$r] : null;
            ...
            if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                ...
                $products[] = array(
                    ...
                    // ADDED: Include shade_id in products array
                    'shade_id' => $item_option_color,
    ...
    public function edit($id = null)
    {
        ...
        if (!empty($item->shade_id)) {
            ...
            $item->shade_name = $color ? $color->name : '';
        } else {
            $item->shade_name = '';
        }
    }
            ...
            foreach ($inv_items as $item) {
                ...
                if (isset($item->shade_name)) {
                    $row->option_color_name = $item->shade_name;
                    $row->shade_name = $item->shade_name;
                }
                if ((empty($row->option_color_name) || $row->option_color_name === null) && !empty($item->shade_id)) {
                    $cdata = $this->quotes_model->getProductOptionByID($item->shade_id);
                    $row->option_color_name = $cdata ? $cdata->name : '';
                    $row->shade_name = $row->option_color_name;
                }
                
                $options = $this->quotes_model->getProductSizes($row->id);
                $options_color = $this->quotes_model->getProductColors($row->id);
                ...
                $row_id = $row->id . ($row->option ? '_' . $row->option : '') . ($row->option_color ? '_' . $row->option_color : '');
                $ri = $this->Settings->item_addition ? $row_id : $c;

                // Provide a selected_color object for easier color name fallback on the client
                $selected_color = null;
                if (!empty($row->option_color) || !empty($row->option_color_name)) {
                    $selected_color = (object) array(
                        'id' => isset($row->option_color) ? $row->option_color : null,
                        'name' => isset($row->option_color_name) ? $row->option_color_name : ''
                    );
                }
    ...
    public function suggestions()
    {
        ...
        // ADDED: Get color variant ID from analyzed term
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $row->qty = $this->input->get('quantity', true) ? $this->input->get('quantity', true) : 1;
                $row->subtotal = $this->input->get('subtotal', true);
                $row->discount = '0';
                
                // Get product variants
                $options = $this->quotes_model->getProductVariantsWithoutcolor($row->id, $variant_popup, 1);
                // ADDED: Get color variants (group_id = 2 for colors)
                ...
                // ADDED: Set selected color variant if exists
                $selected_color = null;
                if ($options_color) {
                    if ($option_color_id) {
                        foreach ($options_color as $color) {
                            if ($color->id == $option_color_id) {
                                $selected_color = $color;
                                $row->option_color = $color->id;
                                $row->option_color_name = $color->name;
                                break;
                            }
                        }
                    }
                    // If no color selected but colors exist, use first one
                    if (!$selected_color && !empty($options_color)) {
                        $selected_color = $options_color[0];
                        $row->option_color = $selected_color->id;
                        $row->option_color_name = $selected_color->name;
                    }
                }
                ...
                $unitData = $this->quotes_model->getUnitById($row->unit);
                $row->unit_lable = $unitData ? $unitData->name : '';
                $row->image = $row->image ? $row->image : 'no_image.png';
                ...
                $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                $row_id = $row->id . ($row->option ? '_' . $row->option : '') . ($row->option_color ? '_' . $row->option_color : '');
                $has_bom = $this->quotes_model->getBomDetailsByProductId($row->id);
                ...
                $pr[] = array(
                    ...
                    'image' => $row->image, 
                    'label' => $row->name . " (" . $row->code . ")", 
                    // ADDED: Include color information
                    'color_label' => $selected_color ? 'Color: ' . $selected_color->name : '',
                    ...
                    'units' => $units, 
                    'options' => $options,
                    // ADDED: Include color variants
                    'options_color' => $options_color,
                    // ADDED: Include selected color
                    'selected_color' => $selected_color,
                    'has_bom' => $has_bom ? true : false
                );
                $r++;
            }
            ...
            // RAW product block
            $raw_check = $this->quotes_model->isRawProduct($sr);
            if ($raw_check) {
                $this->sma->send_json([
                    'status' => 'raw_error',
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A1', 'Quotations');
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('G2', lang('status'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $qu->supplier);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $qu->total);
    ...
    public function get_bom_materials()
    {
        $product_id = $this->input->get('product_id');
        $warehouse_id = $this->input->get('warehouse_id');
        $bom = $this->quotes_model->getBomDetailsByProductId($product_id);
        if ($bom && isset($bom[0]->id)) {
            $items = $this->quotes_model->getBomMaterialItems($product_id, $warehouse_id);
            $final_items = [];
            foreach ($items as $item) {
                $product = $this->site->getProductByID($item['material_id']);
                if ($product) {
                    $item['details'] = $product;
                    $item['net_price'] = $product->price;
                    $item['tax_rate'] = $product->tax_rate;
                    $item['tax_method'] = $product->tax_method;
                    $item['tax_rate_details'] = $this->site->getTaxRateByID($product->tax_rate);
                    $item['units'] = $this->site->getUnitsByBUID($product->unit);
                    // Retain unit_name from bom_items instead of overriding it with product's unit
            ...
            $this->sma->send_json($final_items);
        } else {
            $this->sma->send_json([]);
        }
    }
