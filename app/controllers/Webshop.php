...
class Webshop extends MY_Controller
{
    ...
    public function webshop_request()
    {
        ...
        switch ($action) {
            ...
            case "get_product_suggestions":
                ...
                $this->get_product_suggestions($postData);
    ...
    public function search_products()
    {
        ...
        if ($this->webshop_settings->webshop_theme == 'restaurant') {
            if (empty($products) && empty($otherProductArray)) {
                $this->session->set_flashdata('message', 'Product not found for "' . $keyword . '". Please try searching for another product.');
                redirect('webshop/index');
                return;
            }           
        $hash = md5($products[0]['id']);
            redirect('webshop/product_details/' . $hash);
            return;
        }
