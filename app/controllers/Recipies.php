...
class Recipies extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        if (!$this->Owner && !$this->Admin) {
            $has_mk_recipies = !empty($this->GP['Recipies']);
            $has_ok_recipies = !empty($this->GP['OK_Recipies']);
            if (!$has_mk_recipies && !$has_ok_recipies) {
    ...
    function index($warehouse_id = NULL)
    {
        ...
        // Handle comma-separated warehouses: first = primary, rest = secondary
        $user_warehouses_raw   = (string)$user_data->warehouse_id;

        // Locations used elsewhere on the page (no change)
        ...
        $this->data['categories'] = $this->DineIn_Bill_of_material_model->getProductCategoriesList();
        $this->data['outletName'] = $location_data;
        // Show only warehouses from user's warehouse_id list whose location_type is "Workstation"
        $this->data['workstations'] = $this->DineIn_Bill_of_material_model->getWorkstations($user_warehouses_raw);

        $bc = array(array('link' => base_url(), 'page' => lang('Production Unit')), array('link' => '#', 'page' => lang('Bill of Materials')));
        $meta = array('page_title' => lang('Bill of Materials'), 'bc' => $bc);

        $this->page_construct('production_unit/dinein_bill_of_material', $meta, $this->data);
    }
    public function GetProductsbyCategoriesID()
    {
        ...
        $location = $this->DineIn_Bill_of_material_model->getWarehouseByID($location_id);

        $productsByCategories = '';
        if ($categoriesId || $subcategoryId) {
            $productsByCategories = $this->DineIn_Bill_of_material_model->getProductByCategories($categoriesId, $subcategoryId);
        }
        if ($productsByCategories) {
            $i = 0;
            foreach ($productsByCategories as $row) {

                $unitData = $this->DineIn_Bill_of_material_model->getUnitById($row->unit);
    ...
    public function getProcurementOrderList()
    {
        ...
        if ($status) {
            ...
            $getOrderData = $this->DineIn_Bill_of_material_model->getProcurmentOrders($status, $order_dispatch);
    ...
    public function GetRawProducts()
    {
        ...
        $Rawproducts = '';
        // if ($categoriesId || $subcategoryId) {
        $Rawproducts = $this->DineIn_Bill_of_material_model->getRawProducts();
        // }
        if ($Rawproducts) {
            $i = 0;
    ...
    public function addProddUnitBomData()
    {
        ...
        $workstation_id = isset($postData['workstation_id']) ? (int)$postData['workstation_id'] : null;

        // Get version number per workstation
        $version = $this->DineIn_Bill_of_material_model->get_bom_version_no($productId, $workstation_id);
        ...
        $bom_id = $this->DineIn_Bill_of_material_model->addProddUnitBom($bomData);
        ...
        if (!empty($postData['batchData'])) {
            ...
            foreach ($postData['batchData'] as $item) {
                ...
                $this->DineIn_Bill_of_material_model->addproducts_in_batches($puItem);
    ...
    public function GetBomDetailsByProductId()
    {
        ...
        // Fetch BOM details for all workstations (no user-location filtering)
        $bomDetails = $this->DineIn_Bill_of_material_model->getBomDetailsByProductId($productId);
        if ($bomDetails) {
    ...
    public function getBomMaterialDetails()
    {
        ...
        // Fetch BOM material headers for all workstations (no user-location filtering)
        $bomDetails = $this->DineIn_Bill_of_material_model->getBomMaterialDetails($productId);
        if ($bomDetails) {
    ...
    public function getBomMaterialItems()
    {
        ...
        $bomItems = $this->DineIn_Bill_of_material_model->getBomMaterialItems($bom_id);
        if ($bomItems) {
    ...
    {
        $productId = $this->input->get('productId');
        $workstation_id = $this->input->get('workstation_id') ? (int)$this->input->get('workstation_id') : null;
