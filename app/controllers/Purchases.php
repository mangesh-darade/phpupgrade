...
class Purchases extends MY_Controller {
    ...
    public function index($warehouse_id = null) {
        ...
        if (preg_match('/android|webos|iphone|ipad|ipod|mobile|blackberry|opera mini/i', $ua)) {
            redirect('Purchases_Mobile' . ($warehouse_id ? '/' . $warehouse_id : ''));
        }
    ...
    //     if ($warehouse_id) {
    //         $getwarehouse = trim(str_replace("_", ",", $warehouse_id), ',');
    //         /* $this->datatables
    ...
    //         $this->datatables
    //                 ->select("{$this->db->dbprefix('purchases')}.id as id, DATE_FORMAT({$this->db->dbprefix('purchases')}.date, '%Y-%m-%d %T') as date, {$this->db->dbprefix('purchases')}.reference_no, CONCAT(" . $this->db->dbprefix('companies') . ".company, '(', {$this->db->dbprefix('purchases')}.supplier,')') as supplier, {$this->db->dbprefix('purchases')}.status, ({$this->db->dbprefix('purchases')}.grand_total+{$this->db->dbprefix('purchases')}.rounding), {$this->db->dbprefix('purchases')}.paid, (({$this->db->dbprefix('purchases')}.grand_total+{$this->db->dbprefix('purchases')}.rounding)-{$this->db->dbprefix('purchases')}.paid) as balance, {$this->db->dbprefix('purchases')}.payment_status, {$this->db->dbprefix('purchases')}.attachment")
    //                 ->from('purchases')
    //                 ->where("{$this->db->dbprefix('purchases')}.warehouse_id IN (" . $getwarehouse . ")")
    //                 ->join('companies', "{$this->db->dbprefix('companies')}.id = {$this->db->dbprefix('purchases')}.supplier_id", 'left');
    //     } else {
    //         /* $this->datatables
    //           ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, supplier, status, (grand_total+rounding), paid, ((grand_total+rounding)-paid) as balance, payment_status, attachment")
    //           ->from('purchases'); */
    ...
    //         $this->datatables->where("{$this->db->dbprefix('purchases')}.created_by", $this->session->userdata('user_id'));
    //     } elseif ($this->Supplier) {
    //         $this->datatables->where("{$this->db->dbprefix('purchases')}.supplier_id", $this->session->userdata('user_id'));
    //     }
        
    //     // Apply supplier and status filters ONLY when Job-Work feature is enabled
    //     if (!empty($this->Settings->display_job_work)) {
    //     // Supplier filter from purchases list toggle form
    //     $supplier_id = $this->input->post('supplier');
    //     $supplier_all = $this->input->post('supplier_all');
    //     if (!empty($supplier_id)) {
    //         // filter by selected supplier
    //         $this->datatables->where($this->db->dbprefix('purchases') . '.supplier_id', (int) $supplier_id);
    //     } elseif ((string) $supplier_all !== '1' && $supplier_id != '') {
    //         // apply vendor filter only when supplier dropdown is used
    //         $this->datatables->join($this->db->dbprefix('warehouses') . ' wv', 'wv.id = ' . $this->db->dbprefix('companies') . '.location_id', 'left');
    //         $this->datatables->where('wv.location_type', 'Vendor');
    //     }

    //     // All purchase toggle: if not selected, show only non-complete statuses
    //     $all_purchase = $this->input->post('all_purchase');
    //     if ((string) $all_purchase !== '1') {
    //         // Only show pending, ordered, partial
    //         $this->datatables->where_in($this->db->dbprefix('purchases') . '.status', ['pending', 'ordered', 'partial']);
    //     }
    //     // else: show all statuses
    //     }
        
    //     $this->datatables->where("{$this->db->dbprefix('purchases')}.status !=", 'deleted');
    ...
    public function getPurchases($warehouse_id = null) {
        ...
        $edit_link = anchor('purchases/edit/$1', '<i class="fa fa-edit"></i>', 'title="' . lang('edit_purchase') . '" class="tip link_edit_$2"');
        $detail_link_icon = anchor('purchases/view/$1', '<i class="fa fa-eye"></i>', 'title="' . lang('purchase_details') . '" class="tip link_view_$2"');
        ...
        $delete_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_purchase") . "</b>' data-content=\"<p>"
                ...
                . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
                . lang('delete_purchase') . "</a>";            
        $action = '<div class="text-center purchase-list-actions row_status_$2">'
                . $edit_link
                . $detail_link_icon
                . anchor('purchases/add/?purchase_id=$1', '<i class="fa fa-clone"></i>', 'title="' . lang('duplicate_purchase') . '" class="tip link_duplicate_$2"')
                . anchor('products/print_barcodes/?purchase=$1', '<i class="fa fa-print"></i>', 'title="' . lang('print_barcodes') . '" class="tip link_print_barcode_$2"')
                . '</div>';
        ...
        if ($warehouse_id) {
            $getwarehouse = trim(str_replace("_", ",", $warehouse_id), ',');
            $this->datatables
                    ->select("{$this->db->dbprefix('purchases')}.id as id, {$this->db->dbprefix('purchases')}.id as inward_po, DATE_FORMAT({$this->db->dbprefix('purchases')}.date, '%Y-%m-%d %T') as date, {$this->db->dbprefix('purchases')}.reference_no, CONCAT(" . $this->db->dbprefix('companies') . ".company, '(', {$this->db->dbprefix('purchases')}.supplier,')') as supplier, {$this->db->dbprefix('purchases')}.status as status, ({$this->db->dbprefix('purchases')}.grand_total+{$this->db->dbprefix('purchases')}.rounding), {$this->db->dbprefix('purchases')}.paid, (({$this->db->dbprefix('purchases')}.grand_total+{$this->db->dbprefix('purchases')}.rounding)-{$this->db->dbprefix('purchases')}.paid) as balance, {$this->db->dbprefix('purchases')}.payment_status, {$this->db->dbprefix('purchases')}.attachment")
                    ->from('purchases')
                    ->where("{$this->db->dbprefix('purchases')}.warehouse_id IN (" . $getwarehouse . ")")
                    ->join('companies', "{$this->db->dbprefix('companies')}.id = {$this->db->dbprefix('purchases')}.supplier_id", 'left');
        } else {
        ...
        $this->datatables->edit_column('inward_po', '$1', 'Purchases::inward_po_btn(id, status)');
        if (!$this->Customer && !$this->Supplier && !$this->Owner && !$this->Admin && !$this->session->userdata('view_right')) {
            $this->datatables->where("{$this->db->dbprefix('purchases')}.created_by", $this->session->userdata('user_id'));
        } elseif ($this->Supplier) {
            $this->datatables->where("{$this->db->dbprefix('purchases')}.supplier_id", $this->session->userdata('user_id'));
    ...
    public function modal_view($purchase_id = null) {
        ...
        //     $this->load->view($this->theme . 'purchases/modal_view_print', $this->data);
        // }
        ...
        // }
        // Get GRN data for this purchase
        $this->data['grn_data'] = $this->purchases_model->getGRNDataByPurchaseID($purchase_id);
    ...
    public function view($purchase_id = null) {
        ...
        $this->data['taxItems'] = $this->purchases_model->getAllTaxItemsGroup($purchase_id, $inv->return_id);
        // Get GRN data for this purchase
        $this->data['grn_data'] = $this->db->get_where('sma_good_received_notes', array('purchase_id' => $purchase_id))->result();
    ...
    public function add($quote_id = null) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if($this->Settings->product_batch_setting == 2) { //(Batch settings : Add Batch while transactions)
                    if($batchData == NULL){
                        // if($batchData == NULL && $item_batch_number != NULL ){ //existing
                        // Auto generate batch number while creating purchase(Batch settings : Add Batch while transactions)
                        if (empty($item_batch_number)) {
                ...
                foreach ($items as $item) {
                    ...
                    if ($row->type == 'combo') {
                        ...
                        foreach ($combo_items as $citem) {
                            ...
                            $crow->expiry = '';
                            $row->batch_number = '';
                            $options = $this->purchases_model->getProductOptionsBygroupId($crow->id, 1);
                            $options_color = $this->purchases_model->getProductOptionsBygroupId($crow->id, 2);
                            if ($options_color) {
                            ...
                            $tax_rate = $this->site->getTaxRateByID($crow->tax_rate);
                             $crow->option_color = $citem->shade_id;
                            $ri = $this->Settings->item_addition ? $crow->id : $c;

                            $pr[$ri] = array('id' => $c, 'item_id' => $crow->id, 'image' => $crow->image, 'label' => $crow->name . " (" . $crow->code . ")", 'row' => $crow, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'options_color' => $options_color);
                            $c++;
                        }
                    } elseif ($row->type == 'Bundle') {
                        $combo_items = $this->site->getProductComboItems($row->id, $item->warehouse_id);
                        foreach ($combo_items as $citem) {
                            $crow = $this->site->getProductByID($citem->id);
                            if (!$crow) {
                    ...
                    } elseif ($row->type == 'standard' || $row->type == 'raw' || $row->type == 'Intermediate') {
                        if (!$row) {
                            $row = json_decode('{}');
                            $row->quantity = 0;
                        } else {
                        ...
                        $options = $this->purchases_model->getProductOptionsBygroupId($row->id, 1);
                        $options_color = $this->purchases_model->getProductOptionsBygroupId($row->id, 2);
                        if ($options_color) {
                            $opt_color = current($options_color);
                            if (!empty($opt_color)) {
                            $option_color_id = $opt_color->id;
                        ...
                        $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'image' => $row->image, 'label' => $row->name . " (" . $row->code . ")",
                            'row' => $row, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'options_color' => $options_color);
                        $c++;
                    }
                }
...
//            else 
            ...
            foreach ($inv_items as $item) {
                ...
                $options = ($row->storage_type == 'packed') ? $this->purchases_model->getProductOptionsBygroupId($row->id, 1) : FALSE;
            ...
            if($this->Settings->product_batch_setting > 0) {     
                ...
                if ($batch) {
                    $firstKey = key($batch);
                    $batchoption = $batch;
                    $row->batch = $item->batch_number?$item->batch_number:$batchoption[$firstKey]->id;
                    $row->batch_number = $item->batch_number?$item->batch_number:$batchoption[$firstKey]->batch_no;                    
            ...
            **/
            ...
            $this->data['warehouses']   = $this->site->getAllActiveWarehouses();
            // Get GRN data for this purchase
            $this->data['grn_data'] = $this->db->get_where('sma_good_received_notes', array('purchase_id' => $id))->result();
            $this->load->helper('string');
            $value = random_string('alnum', 20);
            $this->session->set_userdata('user_csrf', $value);
            $this->session->set_userdata('remove_pols', 1);
    ...
    public function purchase_by_csv() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $cols = ['code', 'net_unit_cost', 'quantity', 'variant', 'item_tax_rate', 'discount', 'expiry', 'batch_number', 'price', 'mrp'];
              ...
              //  echo "arrResult: ".count($arrResult);
               ...
               //  echo "<br/>final: ".count($final);           
                ...
                foreach ($final as $csv_pr) {
                    ...
                    $net_unit_cost       = isset($csv_pr['net_unit_cost'])   && $csv_pr['net_unit_cost'] !== '' ? trim($csv_pr['net_unit_cost']) : 0;
                    //$quantity            = isset($csv_pr['quantity'])        ? trim($csv_pr['quantity'])      : 0;
                    $quantity            = isset($csv_pr['quantity'])   && $csv_pr['quantity'] !== '' ? trim($csv_pr['quantity']) : 0;
                    ...
                    $item_price          = isset($csv_pr['price'])           ? trim($csv_pr['price'])  : NULL;
                    $mrp                 = isset($csv_pr['mrp'])             ? trim($csv_pr['mrp'])  : NULL;
                    ...
                    if ( $item_code) {
                        ...
                        if ($product_details = $this->purchases_model->getProductByCode( $item_code )) {
                            ...
                            if (!empty($csv_pr['batch_number'])) {
                                $batch = $this->db
                                    ->where('product_id', $product_id)
                                    ->where('batch_no', $csv_pr['batch_number'])
                                    ->where('option_id', $variant ? $item_option->id : 0)
                                    ->get('product_batches')
                                    ->row();
                            ...
                            $qty_before_transaction = $this->site->getQtyBeforeTransaction(
                                $product_details->id,
                                $warehouse_id,
                                $item_option_id
                            );
                            if ($pr_item_tax) {
                                //For Impementation of Tax
                                // Determine if this tax is GST-family before splitting into CGST/SGST/IGST
                                $is_gst = false;
                                if (isset($tax_details)) {
                                    $tn = strtoupper((isset($tax_details->name) ? $tax_details->name : '') . ' ' . (isset($tax_details->code) ? $tax_details->code : ''));
                                    if (strpos($tn, 'GST') !== false) {
                                        $is_gst = true;
                                    }
                                }
                                ...
                                if ($is_gst) {
                                    ...
                                    // Non-GST (e.g., VAT) should not populate GST fields
                                    $cgst = 0; $sgst = 0; $igst = 0; $gst_rate = 0;
                                }
                            }
                            
                            // Ensure batch_number is properly set from CSV data
                            $batch_number = !empty($item_batch_number) ? $item_batch_number : 
                                         (isset($csv_pr['batch_number']) ? trim($csv_pr['batch_number']) : '');
                            ...
                            {
                                $pm=[
                                    'price'             =>$this->sma->formatDecimal($item_price),
        ...
        if ($this->form_validation->run() == true ) {
            ...
            if($products) {
                ...
                if( $this->purchases_model->addPurchaseItemsByCsv($items,$price_mrp)){
    ...
    public function suggestions() {
        ...
        $quick_product_quantity = $this->input->get('quick_product_quantity', true); // product variant entered quantity when we add product via add quick product popup
        $add_quick_product = $this->input->get('add_quick_product', true); // set flag for add quick product
       

        
        $supplier_id = $this->input->get('supplier_id', true);

        if (strlen($termstring) < 3 || !$termstring) {
        ...
        foreach ($termdata as $term) {
            ...
            if ($term != '') {
                ...
                if (strpos($term, '-') !== false) {
                    $rows = $this->purchases_model->getProductNames($qty_value);
                }else {
                ...
                if ($rows) {
                    ...
                    foreach ($rows as $row) {
                        ...
                        $option = isset($exp[1]) ? $exp[1] : false;
                        ...
                        if ($options) {
                            ...
                            if ($add_quick_product == 1) {
                                if (!empty($quick_product_quantity)) {
                                    $quantity = $quick_product_quantity;
                                } elseif (isset($opt->unit_quantity) && $opt->unit_quantity !== null && $opt->unit_quantity !== '') {
                                    $quantity = $opt->unit_quantity;
                                }
                        ...
                        if (!empty($options_color)) {
                            $opt_color = $option_color_id && $r == 0 ? $this->purchases_model->getProductOptionByID($option_color_id) : $options_color[0];
                        ...
                        // Always use variant cost if variant is selected, regardless of whether it's zero or not
                        if ($options && $opt && isset($opt->cost)) {
                            $row->cost = $opt->cost;
                            $row->base_unit_cost = $row->cost;
                            $row->real_unit_cost = $row->cost;
                        }
                        ...
                        $row->base_quantity =($quantity) ? $quantity :  (float) $product_qty;
                        ...
                        $row->qty = ($quantity) ? $quantity :  $row->qty ;
                        $subtotal = $this->sma->formatDecimal($subtotal);
                        if ($subtotal != '' && $subtotal > 0) {
                            $row->real_unit_cost = $subtotal;
                            $row->cost = $subtotal;
                            $row->base_unit_cost = $subtotal;
                        } else {
                            // Use the already set cost from variant or product
                            $row->real_unit_cost = $row->cost;
                        }
                        ...
                        $pr[] = ['id' => ($c + $r), 'item_id' => $row_id, 'image' => $product->image, 'label' => $label,
                            'row' => $row, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'option_batches'=> $productbatches, 'batchs' => $batchoption, 'primary_variant'=> $product->primary_variant, 'options_color' => $options_color];
                    }
                }else {
...
//                        $this->excel->getActiveSheet()->SetCellValue('J' . $row, $price);
    ...
    public function getExpenses() {
        ...
        $this->datatables
                ->select($this->db->dbprefix('expenses') . ".id as id, date, reference,{$this->db->dbprefix('warehouses')}.name as warehouse , {$this->db->dbprefix('expense_categories')}.name as category, esc.name as subcategory, amount, note, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as user, attachment", false)
                ->from('expenses')
                ...
                ->join('expense_categories esc', 'esc.id=expenses.subcategory_id', 'left')
    ...
    public function add_expense() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'subcategory_id' => $this->input->post('subcategory', true) ? $this->input->post('subcategory', true) : NULL,
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addExpense($data)) {
            ...
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['categories'] = $this->purchases_model->getExpenseParentCategories();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function edit_expense($id = null) {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updateExpense($id, $data)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['categories'] = $this->purchases_model->getExpenseParentCategories();
            $this->data['subcategories'] = $this->data['expense']->category_id ? $this->purchases_model->getExpenseSubCategoriesByParent($this->data['expense']->category_id) : FALSE;
            $this->load->view($this->theme . 'purchases/edit_expense', $this->data);
        }
    }

    public function get_expense_subcategories() {
        $this->sma->checkPermissions('expenses', true);
        $category_id = $this->input->get('category_id', true);
        $subcategories = array();
        if ($category_id) {
            $rows = $this->purchases_model->getExpenseSubCategoriesByParent($category_id);
            if (!empty($rows)) {
                foreach ($rows as $row) {
                    $subcategories[] = array('id' => $row->id, 'name' => $row->name);
                }
            }
        }
        $this->output
            ->set_content_type('application/json')
            ->set_output(json_encode($subcategories));
    }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E2', ucfirst(lang('subcategory')));
                    $this->excel->getActiveSheet()->SetCellValue('F2', lang('amount'));
                    $this->excel->getActiveSheet()->SetCellValue('G2', lang('note'));
                    $this->excel->getActiveSheet()->SetCellValue('H2', lang('created_by'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $getsubcategory = NULL;
                        if (!empty($expense->subcategory_id)) {
                            $getsubcategory = $this->db->select('name')->where('id', $expense->subcategory_id)->get('expense_categories')->row();
                        }
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $getsubcategory ? $getsubcategory->name : '');
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, $this->sma->formatMoney($expense->amount));
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                if ($_POST['quantity'][$r] > 0) {
                    ...
                    $item_tax_method = $_POST['tax_method'][$r];
                    $product_option_color = isset($_POST['product_option_color'][$r]) && !empty($_POST['product_option_color'][$r]) && $_POST['product_option_color'][$r] != 'false' ? $_POST['product_option_color'][$r] : 0;
                    if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                        $product_details = $this->purchases_model->getProductByCode($item_code);
...
//                    $notexitProduct[]= '04760673';
    ...
    //////////////////////////// Generate Variant PO ////////////////////////////
    /**
     * Generate Variant PO screen
     *
     * Business logic (why):
     * - Variant_Order page lets users pick variants and quantities, then stages a payload in session.
     * - Here we read that staged payload to prefill the Purchase screen without breaking existing add/edit flows.
     * - We keep Supplier selection user-driven, so items are only added to the grid after supplier is selected.
     * - We inject defaults (status=ordered, note, warehouse per role) to reduce user inputs and enforce consistency.
     * - We also pass a lightweight JS payload (window.__dummyItems) so the client can fetch item details lazily via item_details.
     * - Order items: if job_work_id has a suffix, resolve to child SKUs (main_suffix_variant); if suffix empty, payload is main product + variant options (RM_Calculator). Raw Materials Supplied still uses ingredients/BOM from the payload.
     *
     * Inputs:
     * - Session: variant_po_payload { variants[], ingredients[], note, job_work_id }
     * - GET: optional quote_id for consistency with other purchase routes
     *
     * Outputs:
     * - Renders job_works/generate_variant_po with:
     *   - vpo_variants, vpo_ingredients, vpo_note (for Raw Materials Supplied panel)
     *   - Injected JS to set window.__dummyItems and defaults in localStorage
     */
    public function generate_variant_po($quote_id = null) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $rm_reqs = $this->input->post('vpo_request');
            if (is_array($rm_reqs)) {
                foreach ($rm_reqs as $req_qty) {
        ...
        if ($this->form_validation->run() == true) {
            $vpo_transfer_qty = $this->input->post('vpo_transfer_qty');
            $rm_ids           = $this->input->post('vpo_material_id'); // raw material product ids
            $rm_reqs          = $this->input->post('vpo_request');     // required qty per index
            $rm_units         = $this->input->post('vpo_material_unit'); // unit per index

            $this->db->trans_start();
            $purchase_id = $this->purchases_model->addPurchase($data, $products, null, $flag = "1");

            // Insert one row per raw material into sma_vendor_rm_stock
            if ($purchase_id && is_array($rm_ids)) {
                foreach ($rm_ids as $idx => $rm_product_id) {
                    $rm_product_id = (int) $rm_product_id;
                    if ($rm_product_id <= 0) {
                        continue;
                    }

                    // Required quantity for this raw material
                    $req_qty = 0;
                    if (is_array($rm_reqs) && isset($rm_reqs[$idx])) {
                        $req_qty = (float) $rm_reqs[$idx];
                    }

                    // Unit for this raw material
                    $rm_unit = '';
                    if (is_array($rm_units) && isset($rm_units[$idx])) {
                        $rm_unit = $rm_units[$idx];
                    }

                    // Total supplied quantity for this raw material from all locations
                    $supplied_qty = 0;
                    if (is_array($vpo_transfer_qty)
                        && isset($vpo_transfer_qty[$rm_product_id])
                        && is_array($vpo_transfer_qty[$rm_product_id])) {
                        foreach ($vpo_transfer_qty[$rm_product_id] as $location_id => $qty) {
                            $qty = (float) $qty;
                            if ($qty > 0) {
                                $supplied_qty += $qty;
                            }
                    ...
                    // Skip if both requested and supplied are zero
                    if ($req_qty <= 0 && $supplied_qty <= 0) {
                        continue;
                    }

                    $vendor_rm_data = array(
                        'purchase_id'    => $purchase_id,
                        'po_reference_no' => $reference,
                        'total_req_qty'  => $req_qty,
                        'total_supplied' => $supplied_qty,
                        'supplier_id'    => $supplier_id,
                        'rm_product_id'  => $rm_product_id,
                        'rm_unit'        => $rm_unit,
                    );
                    $this->db->insert('sma_vendor_rm_stock', $vendor_rm_data);
                }
            }
            
            // Check if all transfer quantities are 0 across all materials and locations
            // Data structure: $vpo_transfer_qty[material_id][location_id] = qty
            // Example: [2553] => Array([5] => 0, [8] => 0)
            $all_transfer_qty_zero = true;
            if (is_array($vpo_transfer_qty) && !empty($vpo_transfer_qty)) {
                foreach ($vpo_transfer_qty as $material_id => $locations) {
                    if (is_array($locations)) {
                        foreach ($locations as $location_id => $qty) {
                            if ((float)$qty > 0) {
                                $all_transfer_qty_zero = false;
                                break 2; // Break out of both loops - found at least one qty > 0
                            }
                        }
                    }
                }
            } else {
                // If no transfer qty data posted, treat as no transfers (direct supplier fulfillment)
                $all_transfer_qty_zero = true;
            }
            
            // If all transfer quantities are 0, complete transaction and redirect to show only PO
            // No transfer request or transfer will be created - supplier provides directly
            if ($all_transfer_qty_zero && $purchase_id) {
                $this->db->trans_complete();
                $this->session->set_userdata('remove_pols', 1);
                redirect('purchases/variant_po_post_success?purchase_id='.(int)$purchase_id);
            }
            
            $req_ok = false;
            $transfer_id = false;
            if ($purchase_id) {
                // Create Request (pending) and Transfer (sent) based on the PO
                $req_ok = $this->create_request_from_po($data, $products,$purchase_id);
                $transfer_id = $this->create_transfer_from_po($data, $products,$flag ="1");
            }

            if ($purchase_id && $req_ok && $transfer_id) {
                $this->db->trans_complete();
                ...
                // Get all transfer IDs if multiple were created
                $all_transfer_ids = $this->session->userdata('vpo_transfer_ids');
                if ($all_transfer_ids) {
                    // Pass all transfer IDs for printing
                    redirect('purchases/variant_po_post_success?purchase_id='.(int)$purchase_id.'&transfer_ids='.$all_transfer_ids);
                } else {
                    // Fallback to single transfer
                    redirect('purchases/variant_po_post_success?purchase_id='.(int)$purchase_id.'&transfer_id='.(int)$transfer_id);
                }
            } else {
            ...
            $this->data['categories'] = $this->site->getAllCategories();
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $this->data['warehouses'] = $this->site->getNonVendorActiveWarehouses(); // Only non-Vendor locations
            $this->data['ponumber'] = $this->site->getReference('po');
            $this->load->helper('string');
            $value = random_string('alnum', 20);
            $this->session->set_userdata('user_csrf', $value);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            // Inject Variant Order payload if available
            $payload = $this->session->userdata('variant_po_payload');
            $variant_po_js = '';
            if (!empty($payload)) {
                $variants_json = isset($payload['variants']) ? $payload['variants'] : '[]';
                $ingredients_json = isset($payload['ingredients']) ? $payload['ingredients'] : '[]';
                $note = isset($payload['note']) ? $payload['note'] : '';

                // Determine warehouse default: admin -> default warehouse, else user's assigned warehouse
                $user_id = $this->session->userdata('user_id');
                $user = $this->site->getUser($user_id);
                $default_wh_id = ($this->Owner || $this->Admin) ? (int)$this->Settings->default_warehouse : (int)$user->warehouse_id;

                // Build __dummyItems from RM Calculator payload:
                // - Job work with suffix: order lines = child SKUs main_<suffix>_<variant> (BOM rows may stay upstream, e.g. _Stiched_).
                // - Job work without suffix: payload already normalized to main product + product_variants id (RM_Calculator).
                $items = array();
                $decoded_variants = json_decode($variants_json, true);
                $jw_po_id = isset($payload['job_work_id']) ? (int) $payload['job_work_id'] : 0;
                if (is_array($decoded_variants)) {
                    $resolved_items = null;
                    if ($jw_po_id > 0) {
                        $this->load->model('Variant_Order_Model');
                        $resolved_items = $this->Variant_Order_Model->resolve_jobwork_variant_po_dummy_items($jw_po_id, $decoded_variants);
                    }
                    if (is_array($resolved_items) && !empty($resolved_items)) {
                        foreach ($resolved_items as $row) {
                            $items[] = array(
                        ...
                        foreach ($decoded_variants as $v) {
                            $vid = isset($v['id']) ? $v['id'] : 0;
                            $items[] = array(
                                'product_id' => isset($v['product_id']) ? (int) $v['product_id'] : 0,
                                'variant_id' => is_numeric($vid) ? (int) $vid : 0,
                                'tax_id'     => 0,
                                'qty'        => isset($v['required_quantity']) ? (float) $v['required_quantity'] : 1,
                            );
                        }
                    }
                }
                $items_js = json_encode($items);
                $note_js  = json_encode($note);
                // Fresh staged lines from RM Calculator: wipe persisted cart first so generate_variant_po.js
                // loadItems() does not merge old main+variant rows with newly resolved child SKUs.
                $variant_po_js = "try{localStorage.removeItem('variantpoitems');}catch(e){}; window.__dummyItems = $items_js; localStorage.setItem('postatus','ordered'); localStorage.setItem('ponote', $note_js); localStorage.setItem('powarehouse', '" . $default_wh_id . "');";
                
                // If supplier_id is available from RM Calculator, set it in localStorage
                $supplier_id = $this->session->userdata('variant_po_supplier_id');
                if (!empty($supplier_id)) {
                    $variant_po_js .= " localStorage.setItem('posupplier', '" . $supplier_id . "');";
                }
                
                // Clear payload after reading
                $this->session->unset_userdata('variant_po_payload');
                $this->session->unset_userdata('variant_po_supplier_id');
                $this->data['vpo_variants'] = json_decode($variants_json, true);
                $this->data['vpo_ingredients'] = json_decode($ingredients_json, true);
                $this->data['vpo_note'] = $note;

                // Create Vendor Stock Details if supplier and ingredients are present
                if (!empty($supplier_id) && !empty($this->data['vpo_ingredients'])) {
                    $this->load->model('Variant_bill_of_materials_model');
                    $rm_ids = [];
                    foreach ($this->data['vpo_ingredients'] as $ing) {
                        // Check for product_id or id or raw_material_id
                        $pid = 0;
                        if (isset($ing['raw_material_id'])) {
                            $pid = $ing['raw_material_id'];
                        } elseif (isset($ing['product_id'])) {
                            $pid = $ing['product_id'];
                        } elseif (isset($ing['id'])) {
                            $pid = $ing['id'];
                        }
                    ...
                    if (!empty($rm_ids)) {
                        $rm_ids = array_unique($rm_ids);
                        // Fetch stock list for these RMs from selected supplier only
                        $stock_list = $this->Variant_bill_of_materials_model->getVendorStockList(NULL, $supplier_id, $rm_ids);

                        if ($stock_list) {
                            foreach ($stock_list as &$row) {
                                // Fetch details for each row
                                $row->details = $this->Variant_bill_of_materials_model->getVendorRMDetails($row->supplier_id, $row->product_id);
                            }
                        }
                        $this->data['vendor_stock_details'] = $stock_list;
                    }
                }
            }
            $this->data['variant_po_js'] = $variant_po_js;
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => 'Generate Variant PO'));
            $meta = array('page_title' => 'Generate Variant PO', 'bc' => $bc);

            $this->page_construct('job_works/generate_variant_po', $meta, $this->data);
        }
    }
    
  
    // Lightweight item details endpoint for Generate Variant PO grid
    /**
     * item_details
     *
     * Business logic (why):
     * - The client posts only product_id and/or variant_id plus qty.
     * - If product_id is missing but variant_id is present, we resolve product_id from the variant to keep the client simple.
     * - We return a normalized item payload compatible with add_purchase_item(),
     *   minimizing coupling between the Variant PO flow and the generic purchase grid code.
     *
     * Inputs: GET product_id, variant_id, tax_id, qty
     * Output: JSON { status, item }
     */
    public function get_vendor_stock_table()
    {
        $this->load->model('Variant_bill_of_materials_model');
        $rm_ids     = $this->input->post('rm_ids');
        $supplier_id = $this->input->post('supplier_id') ?: NULL;

        if (!empty($rm_ids) && !is_array($rm_ids)) {
            $rm_ids = array($rm_ids);
        }
        ...
        if (!empty($rm_ids) && is_array($rm_ids)) {
            ...
            // Filter by selected supplier only
            $stock_list = $this->Variant_bill_of_materials_model->getVendorStockList(NULL, $supplier_id, $rm_ids);
            ...
            $this->data['vendor_stock_details'] = $stock_list;
            $this->load->view($this->theme . 'job_works/vendor_stock_partial', $this->data);
        } else {
            echo '<div class="alert alert-warning">No raw materials selected.</div>';
        }
    }
    public function item_details() {
        ...
        // If product_id not provided, try resolving from variant_id
        if (!$product_id && $variant_id) {
            $optTmp = $this->purchases_model->getProductOptionByID($variant_id);
            if ($optTmp && isset($optTmp->product_id)) {
                $product_id = (int)$optTmp->product_id;
            }
        }

        if (!$product_id) {
            return $this->sma->send_json(array('status' => 'error', 'message' => 'Missing product_id'));
        }

        $product = $this->purchases_model->getProductByID($product_id);
        if (!$product) {
            return $this->sma->send_json(array('status' => 'error', 'message' => 'Product not found'));
        }

        $option = null;
        if ($variant_id) {
            $option = $this->purchases_model->getProductOptionByID($variant_id);
        }
        ...
        $tax_rate = null;
        if (!empty($tax_id)) {
            $rate = $this->site->getTaxRateByID($tax_id);
            if ($rate) { $tax_rate = $rate; $tax_rate_id = (int)$rate->id; }
        }
        if (!$tax_rate && !empty($product->tax_rate)) {
            $rate = $this->site->getTaxRateByID($product->tax_rate);
            if ($rate) { $tax_rate = $rate; $tax_rate_id = (int)$rate->id; }
        }

        // Cost calculation: Use supplier rate from sma_vendor_rates if available
        // Otherwise show 0 (don't use product master table values)
        $vendor_rate = 0;
        if ($supplier_id) {
            // Priority: variant-specific vendor rate -> product-level vendor rate -> 0
            $vendor_rate = $this->getSupplierCostFromVendorRates($supplier_id, $product_id, $variant_id);
        }
        
        if ($vendor_rate > 0) {
            // Use vendor rate as the primary cost
            $row_cost = $vendor_rate;
        } else {
            // Show 0 if no vendor rate available (don't use product master table values)
            $row_cost = 0;
        }
        
        $real_unit_cost = $row_cost;
        $tax_method = isset($product->tax_method) ? (int)$product->tax_method : $product->tax_method ; // 1 exclusive, 0 inclusive

        // Units info
        $base_unit_id = isset($product->unit) ? (int)$product->unit : 0;
        $units = $base_unit_id ? $this->site->getUnitsByBUID($base_unit_id) : array();

        $image_url = isset($product->image) && $product->image ? base_url() . $this->thumbs_path . $product->image : '';
        $payload = array(
            'id' => (int)$product->id,
            'code' => (string)$product->code,
            'name' => (string)$product->name,
            'image_url' => $image_url,
            'product_unit_id' => $base_unit_id,
            'tax_rate_id' => $tax_rate_id,
            'tax_rate' => $tax_rate ? array(
                'id' => (int)$tax_rate->id,
                'rate' => (float)$tax_rate->rate,
                'type' => (int)$tax_rate->type,
                'name' => (string)$tax_rate->name,
            ) : null,
            'option_id' => $option ? (int)$option->id : 0,
            'option_name' => $option ? (string)$option->name : '',
            'units' => $units,
            'row' => array(
                'id' => (int)$product->id,
                'code' => (string)$product->code,
                ...
                'cost' => (float)$row_cost,
                'real_unit_cost' => (float)$real_unit_cost,
                'base_unit_cost' => (float)$row_cost,
                'tax_method' => $tax_method,
                'tax_rate' => $tax_rate_id,
                'option' => $option ? (int)$option->id : 0,
                'option_color' => $option && isset($option->shade_id) ? (int)$option->shade_id : 0,
                'option_color_name' => '',
                'storage_type' => isset($product->storage_type) ? $product->storage_type : '',
                'hsn_code' => isset($product->hsn_code) ? (string)$product->hsn_code : '',
                'unit' => $base_unit_id,
            ),
        );

        return $this->sma->send_json(array('status' => 'success', 'item' => $payload));
    }
    public function getSupplierCostFromVendorRates($supplier_id, $product_id, $variant_id = 0) {
        if (!$supplier_id || !$product_id) {
            return 0;
        }

        // Use unprefixed table name: dbprefix (sma_) is applied by Query Builder.
        // Using 'sma_vendor_rates' here would incorrectly resolve to sma_sma_vendor_rates.
        // Prefer newest row when multiple exist (e.g. different job_work) — matches "newly added" vendor rate.

        // 1) Try variant-specific vendor rate first (when variant exists)
        $variant_id = (int) $variant_id;
        if ($variant_id > 0) {
            $this->db->select('rate_per_item');
            $this->db->from('vendor_rates');
            $this->db->where('vendor', (int) $supplier_id);
            ...
            if ($query && $query->num_rows() > 0) {
                $row = $query->row();
                return (float) $row->rate_per_item;
            }
        }

        // 2) Fallback to product-level rate (variant_id = 0 or NULL)
        $this->db->select('rate_per_item');
        $this->db->from('vendor_rates');
        $this->db->where('vendor', (int) $supplier_id);
        $this->db->where('product_id', (int) $product_id);
        $this->db->group_start();
        $this->db->where('variant_id', 0);
        $this->db->or_where('variant_id IS NULL', null, false);
        $this->db->group_end();
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
    ...
    /***********************************************************
     * Helper: Create a Transfer from a just-created Purchase Order
     ***********************************************************/
    private function create_transfer_from_po($poHeader, $poItems,$flag = null) {
        // ========== TRANSFER LOGIC ==========
        // FROM: Our warehouse locations (Pune, Kitchen, etc.) - set per item based on vpo_transfer_qty
        // TO: Supplier's assigned location_id
        // ====================================
        
        // Default FROM warehouse (used only if no location-specific data)
        $from_wid = isset($poHeader['warehouse_id']) ? (int)$poHeader['warehouse_id'] : (int)$this->Settings->default_warehouse;
        
        // Resolve TO warehouse = Supplier's assigned location
        $to_wid = 0;
        $to = null;
        if (!empty($poHeader['supplier_id'])) {
            $supplier = $this->site->getCompanyByID($poHeader['supplier_id']);
            if ($supplier && !empty($supplier->location_id)) {
                $to_wid = (int)$supplier->location_id; // TO = Supplier Location
                $to = $this->production_unit_model->getwarehousesById($to_wid);
            }
        }

        // Get default FROM details (used for validation and fallback)
        $fromDetailsArr = $this->site->getWarehouseByID($from_wid);
        $from = isset($fromDetailsArr[$from_wid]) ? $fromDetailsArr[$from_wid] : null;

        // Validation: require supplier has assigned location
        if (empty($to_wid) || empty($to)) {
            $this->session->set_flashdata('error', 'Supplier has no location assigned. Please set a Location for this supplier and try again.');
            return false;
        }

        $total = 0; $product_tax = 0;
        $titems = [];

        // Get location-wise transfer quantities
        $vpo_transfer_qty = $this->input->post('vpo_transfer_qty'); // [material_id][location_id] = qty
        
        // Prefer building transfer from Raw Materials if posted
        $rm_ids = $this->input->post('vpo_material_id');
        $rm_qtys = $this->input->post('vpo_supplied'); // Total Supplied Qty
        $rm_variant_ids = $this->input->post('vpo_material_variant_id'); // Variant option id per RM row

        if (is_array($rm_ids) && is_array($rm_qtys)) {
            foreach ($rm_ids as $idx => $pid) {
                $pid = (int)$pid;
                $total_qty = isset($rm_qtys[$idx]) ? (float)$rm_qtys[$idx] : 0;
                $rm_option_id = (is_array($rm_variant_ids) && isset($rm_variant_ids[$idx])) ? (int)$rm_variant_ids[$idx] : 0;
                
                if ($pid > 0 && $total_qty > 0) {
                    $prod = $this->site->getProductByID($pid);
                    if (!$prod) { continue; }
                    $unit = $this->site->getUnitByID($prod->unit);
                    $unit_code = $unit ? $unit->code : '';
                    
                    // Check if we have location-wise quantities for this material
                    $has_location_data = isset($vpo_transfer_qty[$pid]) && is_array($vpo_transfer_qty[$pid]);
                    
                    if ($has_location_data) {
                        // Create separate transfer items for each source location
                        foreach ($vpo_transfer_qty[$pid] as $source_location_id => $qty) {
                            $qty = (float)$qty;
                            if ($qty > 0) {
                                $net_cost = 0; $line_tax = 0; $subtotal = 0;
                                ...
                                $titems[] = [
                                    ...
                                    'date'              => date('Y-m-d', strtotime($poHeader['date'])),
                                    'shade_id'          => null,
                                    'batch_number'      => null,
                                    '_source_location_id' => $source_location_id, // Temp: track source for grouping
                                ];
                            }
                        }
                    } else {
                        // Fallback: single transfer item with total quantity (old behavior)
                        $net_cost = 0; $line_tax = 0; $subtotal = 0;
                        ...
                        $titems[] = [
                            ...
                            'product_unit_id'   => $prod->unit,
                            'product_unit_code' => $unit_code,
                            'unit_quantity'     => $total_qty,
                            'quantity_balance'  => $total_qty,
                            'warehouse_id'      => $to_wid,
                            'item_tax'          => $line_tax,
                            'tax_rate_id'       => 0,
                            'tax'               => '',
        ...
        // Fallback to original PO items if no RM posted
        if (empty($titems)) {
            foreach ((array)$poItems as $it) {
                $unit_qty = isset($it['unit_quantity']) ? (float)$it['unit_quantity'] : (float)$it['quantity'];
                $net_cost = isset($it['net_unit_cost']) ? (float)$it['net_unit_cost'] : 0;
                $line_tax = isset($it['item_tax']) ? (float)$it['item_tax'] : 0;
                $subtotal = ($net_cost * $unit_qty) + $line_tax;
                $total += ($net_cost * $unit_qty);
                ...
                $titems[] = [
                    ...
                    'option_id'         => isset($it['option_id']) ? $it['option_id'] : 0,
                    'net_unit_cost'     => $net_cost,
                    'unit_cost'         => isset($it['unit_cost']) ? $it['unit_cost'] : ($net_cost + $line_tax),
                    'quantity'          => isset($it['quantity']) ? $it['quantity'] : $unit_qty,
                    'product_unit_id'   => $it['product_unit_id'],
                    'product_unit_code' => $it['product_unit_code'],
                    'unit_quantity'     => $unit_qty,
                    'quantity_balance'  => isset($it['quantity_balance']) ? $it['quantity_balance'] : $unit_qty,
                    'warehouse_id'      => $to_wid,
                    'item_tax'          => $line_tax,
                    'tax_rate_id'       => isset($it['tax_rate_id']) ? $it['tax_rate_id'] : 0,
                    'tax'               => isset($it['tax']) ? $it['tax'] : '',
                    'subtotal'          => $this->sma->formatDecimal($subtotal),
                    'expiry'            => isset($it['expiry']) ? $it['expiry'] : null,
                    'real_unit_cost'    => isset($it['real_unit_cost']) ? $it['real_unit_cost'] : $net_cost,
                    'date'              => date('Y-m-d', strtotime($poHeader['date'])),
                    'shade_id'          => isset($it['shade_id']) ? $it['shade_id'] : null,
                    'batch_number'      => isset($it['batch_number']) ? $it['batch_number'] : null,
                ];
            }
        }

        // If we built from RM (costless), totals remain zero; else totals computed above
        $grand_total = $this->sma->formatDecimal(($total + $product_tax), 4);
        // Compose note from Generate Variant PO screen if provided
        $posted_note = $this->input->post('vpo_note');
        $posted_note = is_string($posted_note) ? trim($posted_note) : '';
        $auto_note_t = 'Auto-created from PO '.$poHeader['reference_no'];
        $final_note_t = $posted_note !== '' ? $posted_note : $auto_note_t;
        
        // Check if we have location-specific transfers
        $has_source_locations = false;
        foreach ($titems as $item) {
            if (isset($item['_source_location_id'])) {
                $has_source_locations = true;
                break;
            }
        }
        ...
        if ($has_source_locations) {
            ...
            // Create separate transfer for each source location (FROM)
            // Each transfer goes FROM: Our Location → TO: Supplier Location
            // ==============================================
            
            // Group items by source warehouse (our locations: Pune, Kitchen, etc.)
            $items_by_source = [];
            foreach ($titems as $item) {
                $source_id = isset($item['_source_location_id']) ? $item['_source_location_id'] : $from_wid;
                if (!isset($items_by_source[$source_id])) {
                    $items_by_source[$source_id] = [];
                }
                // Remove temporary field before adding to group
                $clean_item = $item;
                unset($clean_item['_source_location_id']);
                $items_by_source[$source_id][] = $clean_item;
            }
            
            // Create separate transfers for each source location
            $transfer_ids = [];
            foreach ($items_by_source as $source_wid => $source_items) {
                // Get FROM warehouse details (our location)
                $sourceDetailsArr = $this->site->getWarehouseByID($source_wid);
                $source = isset($sourceDetailsArr[$source_wid]) ? $sourceDetailsArr[$source_wid] : null;
                
                if (!$source) continue; // Skip if source warehouse not found
                // Guard: FROM and TO cannot be the same
                if ((int)$source_wid === (int)$to_wid) {
                    // Skip creating an invalid transfer (same source & destination)
                    // Optionally accumulate a warning for UI if needed
                    continue;
                }
                
                // Create transfer: FROM our location → TO supplier location
                $transfer_note = $final_note_t . "\n\n" . 
                                'Transfer FROM: ' . $source->name . ' (' . $source->code . ')' . "\n" .
                                'Transfer TO: ' . $to->name . ' (' . $to->code . ')';
                
                $tdata = [
                    'transfer_no'                => $this->site->getReference('to'),
                    'date'                       => $poHeader['date'],
                    // FROM = Our warehouse (Pune, Kitchen, etc.)
                    'from_warehouse_id'          => $source_wid,
                    'from_warehouse_code'        => $source->code,
                    'from_warehouse_name'        => $source->name,
                    'from_warehouse_state_code'  => $source->state_code,
                    // TO = Supplier's assigned location
                    'to_warehouse_id'            => $to_wid,
                    'to_warehouse_code'          => $to->code,
                    'to_warehouse_name'          => $to->name,
                    'to_warehouse_state_code'    => $to->state_code,
                    'note'                       => $transfer_note,
                    'total_tax'                  => 0, // RM transfers have no tax
                    'total'                      => 0,
                    'grand_total'                => 0,
                    'created_by'                 => $this->session->userdata('user_id'),
                    'status'                     => 'completed',
                ...
                $transfer_id = $this->transfers_model->addTransfer($tdata, $source_items, $flag = "1");
                if ($transfer_id) {
                    $transfer_ids[] = $transfer_id;
                }
            }
            
            // Return array of all transfer IDs (or comma-separated for single return)
            if (!empty($transfer_ids)) {
                // Store all transfer IDs in session for success page
                $this->session->set_userdata('vpo_transfer_ids', implode(',', $transfer_ids));
                return $transfer_ids[0]; // Return first for backward compatibility
            }
            return false;
            
        } else {
            // Original single transfer logic
            // Guard: FROM and TO cannot be the same
            if ((int)$from_wid === (int)$to_wid) {
                $this->session->set_flashdata('error', 'Cannot create transfer: Source and destination locations are the same. Please choose a different delivery location or supplier location.');
                return false;
            }
            $tdata = [
                'transfer_no'                => $this->site->getReference('to'),
                ...
                'from_warehouse_code'        => $from->code,
                'from_warehouse_name'        => $from->name,
                'from_warehouse_state_code'  => $from->state_code,
                'to_warehouse_id'            => $to_wid,
                'to_warehouse_code'          => $to->code,
                'to_warehouse_name'          => $to->name,
                'to_warehouse_state_code'    => $to->state_code,
            ...
            return $this->transfers_model->addTransfer($tdata, $titems,$flag = "1");
        }
    }

    /**
     * Show confirmation and print actions after Generate Variant PO submit
     * Expects GET: purchase_id, transfer_ids (comma-separated) or transfer_id (single)
     */
    public function variant_po_post_success() {
        $purchase_id = (int)$this->input->get('purchase_id');
        
        // Check for multiple transfer IDs (comma-separated)
        $transfer_ids_str = $this->input->get('transfer_ids');
        $transfer_id_single = (int)$this->input->get('transfer_id');
        
        $transfer_ids = [];
        if ($transfer_ids_str) {
            // Multiple transfers
            $ids = explode(',', $transfer_ids_str);
            foreach ($ids as $tid) {
                if ((int)$tid > 0) {
                    $transfer_ids[] = (int)$tid;
                }
            }
        } elseif ($transfer_id_single > 0) {
            // Single transfer (backward compatibility)
            $transfer_ids[] = $transfer_id_single;
        }
        
        
        $this->data['purchase_id'] = $purchase_id;
        $this->data['transfer_ids'] = $transfer_ids;
        $this->data['transfer_id'] = $transfer_ids[0]; // First transfer for backward compatibility
        $this->data['page_title'] = 'Generate Variant PO - Success';
        $this->load->view($this->theme.'purchases/variant_po_post_success', $this->data);
    }

    /***********************************************************
     * Helper: Create Transfer Request(s) from a just-created Purchase Order
     * Creates ONE request per source location (our warehouse)
     ***********************************************************/
    private function create_request_from_po($poHeader, $poItems,$purchase_id) {
        // ========== REQUEST LOGIC ==========
        // FROM: Our warehouse locations (Pune, Kitchen, etc.) - set per item based on vpo_transfer_qty
        // TO: Supplier's assigned location_id
        // request_quantity = Total Req (from vpo_request)
        // sent_quantity = Transfer qty from that location (from vpo_transfer_qty)
        // ====================================
        ...
        $total = 0; $product_tax = 0;
        $ritems = [];
        $total_requested_qty = 0; // accumulate total quantity across all request items
        $any_sent_gt0 = false; $any_partial = false; $all_full = true; // for header status

        // ========== MULTI-LOCATION REQUEST CREATION ==========
        // If location-wise transfer quantities are posted, create multiple requests
        // One request per "Our Location" with transfer quantities
        $vpo_transfer_qty = $this->input->post('vpo_transfer_qty'); // [product_id][source_wid] => transfer qty
        $vpo_request_qty = $this->input->post('vpo_request'); // [index] => total required qty
        $rm_ids = $this->input->post('vpo_material_id'); // [index] => product_id
        $rm_variant_ids = $this->input->post('vpo_material_variant_id'); // [index] => variant option id
        
        if (is_array($vpo_transfer_qty) && !empty($vpo_transfer_qty)) {
            // Build product_id => total_required map
            $product_total_required = [];
            $product_variant_map = [];
            if (is_array($rm_ids) && is_array($vpo_request_qty)) {
                foreach ($rm_ids as $idx => $pid) {
                    $pid = (int)$pid;
                    $req_qty = isset($vpo_request_qty[$idx]) ? (float)$vpo_request_qty[$idx] : 0;
                    $vid = (is_array($rm_variant_ids) && isset($rm_variant_ids[$idx])) ? (int)$rm_variant_ids[$idx] : 0;
                    if ($pid > 0) {
                        $product_total_required[$pid] = $req_qty;
                        if (!isset($product_variant_map[$pid])) {
                            $product_variant_map[$pid] = $vid;
                        }
                    }
                }
            }
            ...
            $items_by_source = [];
            foreach ($vpo_transfer_qty as $pid => $locMap) {
                $pid = (int)$pid;
                if ($pid <= 0 || !is_array($locMap)) { continue; }
                $prod = $this->site->getProductByID($pid);
                if (!$prod) { continue; }
                $unit = $this->site->getUnitByID($prod->unit);
                $unit_code = $unit ? $unit->code : '';
                
                // Get total required quantity for this product
                $total_req_qty = isset($product_total_required[$pid]) ? $product_total_required[$pid] : 0;
                
                foreach ($locMap as $source_wid => $transfer_qty) {
                    $source_wid = (int)$source_wid;
                    $transfer_qty = (float)$transfer_qty;
                    if ($transfer_qty <= 0) { continue; }
                    if ($source_wid === (int)$to_wid) { continue; } // avoid same from/to
                    
                    // For location-wise requests: request_quantity should equal transfer_qty (same as sent_quantity)
                    // This ensures that in the add request, the request quantity shows the same as transfer qty
                    $request_qty = $transfer_qty; // Request qty = Transfer qty for this location
                    
                    // Calculate quantity balance (should be 0 since request = sent)
                    $qty_balance = $request_qty - $transfer_qty;
                    if ($qty_balance < 0) { $qty_balance = 0; }
                    
                    // Determine item status
                    // Since request_quantity = transfer_qty, status should be 'completed' when transfer_qty > 0
                    $item_status = 'pending';
                    if ($transfer_qty > 0) { 
                        // request_quantity equals transfer_qty, so if transfer_qty > 0, it's completed
                    ...
                    $line = [
                        ...
                        'product_code'      => $prod->code,
                        'product_name'      => $prod->name,
                        'option_id'         => isset($product_variant_map[$pid]) ? (int)$product_variant_map[$pid] : 0,
                        'quantity'          => $request_qty, // Same as transfer qty for this location
                        'unit_quantity'     => $request_qty, // Same as transfer qty for this location
                        'request_quantity'  => $request_qty, // Same as transfer qty (not total req)
                        'sent_quantity'     => $transfer_qty, // Transfer qty from this location
                    ...
                    if (!isset($items_by_source[$source_wid])) { $items_by_source[$source_wid] = []; }
                    $items_by_source[$source_wid][] = $line;
                }
            }

            // Create requests per source
            $any_request_created = false;
            foreach ($items_by_source as $source_wid => $source_items) {
                if (empty($source_items)) { continue; }
                // Source details
                $srcDetailsArr = $this->site->getWarehouseByID($source_wid);
                $src = isset($srcDetailsArr[$source_wid]) ? $srcDetailsArr[$source_wid] : null;
                if (!$src) { continue; }
                // Guard: FROM != TO
                if ((int)$source_wid === (int)$to_wid) { continue; }
                
                // Calculate request status based on items
                $req_status = 'pending';
                $has_sent = false;
                $has_partial = false;
                $all_completed = true;
                ...
                foreach ($source_items as $item) {
                    if ($item['sent_quantity'] > 0) { $has_sent = true; }
                    if ($item['sent_quantity'] > 0 && $item['sent_quantity'] < $item['request_quantity']) { $has_partial = true; }
                    if (!($item['request_quantity'] > 0 && $item['sent_quantity'] >= $item['request_quantity'])) { $all_completed = false; }
                }
                
                if ($all_completed && $has_sent) { $req_status = 'completed'; }
                elseif ($has_partial || $has_sent) { $req_status = 'partial'; }
                
                $posted_note = $this->input->post('vpo_note');
                $posted_note = is_string($posted_note) ? trim($posted_note) : '';
                $auto_note = 'Auto-created request from PO ' . $poHeader['reference_no'] . ' (From: ' . $src->name . ')';
                $final_note = $posted_note !== '' ? $posted_note : $auto_note;

                $rdata = [
                    'transfer_request_no'        => $this->site->getReference('treq'),
                    'date'                       => $poHeader['date'],
                    'from_warehouse_id'          => $source_wid,
                    'from_warehouse_code'        => $src->code,
                    'from_warehouse_name'        => $src->name,
                    'from_warehouse_state_code'  => $src->state_code,
                    'to_warehouse_id'            => (int)$to->id,
                    'to_warehouse_code'          => $to->code,
                    'to_warehouse_name'          => $to->name,
                    'to_warehouse_state_code'    => $to->state_code,
                    'note'                       => $final_note,
                ...
                if ($this->transfers_model->addRequest($rdata, $source_items)) {
                    $any_request_created = true;
                }
            }

            return $any_request_created;
        }

        // Prefer building request from Raw Materials if posted
        $rm_ids   = $this->input->post('vpo_material_id');
        $rm_qtys  = $this->input->post('vpo_supplied');   // Supplied Qty -> sent_quantity
        $rm_reqs  = $this->input->post('vpo_request');    // Request Qty  -> request_quantity
        $rm_variant_ids = $this->input->post('vpo_material_variant_id'); // Variant option id per RM row
    
        if (is_array($rm_ids) && (is_array($rm_qtys) || is_array($rm_reqs))) {
            foreach ($rm_ids as $idx => $pid) {
                $pid = (int)$pid;
                $sent_qty = isset($rm_qtys[$idx]) ? (float)$rm_qtys[$idx] : 0;   // what will be sent now
                $req_qty  = isset($rm_reqs[$idx]) ? (float)$rm_reqs[$idx] : 0;   // total requested
                $rm_option_id = (is_array($rm_variant_ids) && isset($rm_variant_ids[$idx])) ? (int)$rm_variant_ids[$idx] : 0;
                // keep building item even if sent is 0, as long as requested > 0
                if ($pid > 0 && ($req_qty > 0 || $sent_qty > 0)) {
                    $prod = $this->site->getProductByID($pid);
                    if (!$prod) { continue; }
                    $unit = $this->site->getUnitByID($prod->unit);
                    $unit_code = $unit ? $unit->code : '';
                    $net_cost = 0; // requests for RM: cost not required
                    $line_tax = 0;
                    $subtotal = 0;
                    $qty_balance = $req_qty - $sent_qty; if ($qty_balance < 0) { $qty_balance = 0; }

                    // compute item status
                    $item_status = 'pending';
                    if ($sent_qty > 0 && $sent_qty < $req_qty) { $item_status = 'partial'; }
                    if ($req_qty > 0 && $sent_qty >= $req_qty) { $item_status = 'complited'; }
                    // header status flags
                    if ($sent_qty > 0) { $any_sent_gt0 = true; }
                    if ($sent_qty > 0 && $sent_qty < $req_qty) { $any_partial = true; }
                    if (!($req_qty > 0 && $sent_qty >= $req_qty)) { $all_full = false; }
                    ...
                    $ritems[] = [
                        ...
                        'product_name'      => $prod->name,
                        'option_id'         => 0,
                        // For requests: quantity = unit_quantity = requested qty
                        'quantity'          => $req_qty,
                        'unit_quantity'     => $req_qty,
                    ...
                    $total_requested_qty += $req_qty;
                }
            }
        }

        // Fallback: derive from PO items as before
        if (empty($ritems)) {
            foreach ((array)$poItems as $it) {
                $unit_qty = isset($it['unit_quantity']) ? (float)$it['unit_quantity'] : (float)$it['quantity'];
                $net_cost = isset($it['net_unit_cost']) ? (float)$it['net_unit_cost'] : 0;
                ...
                $ritems[] = [
                    ...
                    'quantity'          => $unit_qty,
                    'unit_quantity'     => $unit_qty,
                    'request_quantity'  => $total_requested_qty,
                    'sent_quantity'     => 0,
                    'quantity_balance'  => $unit_qty,
                    'tax_rate_id'       => isset($it['tax_rate_id']) ? $it['tax_rate_id'] : 0,
                    'tax'               => isset($it['tax']) ? $it['tax'] : '',
                    'item_tax'          => $line_tax,
                    'net_unit_cost'     => $net_cost,
        ...
        $grand_total = $this->sma->formatDecimal(($total + $product_tax), 4);
        // Determine overall request status from items
        $request_status = 'pending';
        if ($all_full && !empty($ritems)) { $request_status = 'completed'; }
        elseif ($any_partial || ($any_sent_gt0 && !$all_full)) { $request_status = 'partial'; }

        // Determine destination (to) warehouse from selected supplier's location
        $to = null; $to_wid = null;
        if (!empty($poHeader['supplier_id'])) {
            $supplier = $this->site->getCompanyByID($poHeader['supplier_id']);
            if ($supplier && !empty($supplier->location_id)) {
                $to_wid = (int) $supplier->location_id;
                $to = $this->production_unit_model->getwarehousesById($to_wid);
            }
        }
        // Validation: require supplier to have a location to create request
        if (empty($to_wid) || empty($to)) {
            $this->session->set_flashdata('error', 'Selected supplier has no location assigned. Please set a Location for this supplier and try again.');
            return false; // skip creating transfer request
        }
        // Compose note: prefer dynamic note from Generate Variant PO screen
        $posted_note = $this->input->post('vpo_note');
        $posted_note = is_string($posted_note) ? trim($posted_note) : '';
        $auto_note = 'Auto-created request from PO ' . $poHeader['reference_no'] . ' | Total Qty: ' . $total_requested_qty;
        $final_note = $posted_note !== '' ? $posted_note : $auto_note;
        $rdata = [
            'transfer_request_no'        => $this->site->getReference('treq'),
            'date'                       => $poHeader['date'],
        ...
        return $this->transfers_model->addRequest($rdata, $ritems);
    }
    /*************************************************************
     * End Purchase Notification
     *************************************************************/
    ...
    /**
     * Get Material Availability for Purchase Order
     * Returns availability quantities from:
     * - All warehouses (Total Available)
     * - Supplier's warehouse location (Supplier Available Qty)
     * - Delivery location warehouse (Delivery Location Qty)
     */
    public function get_material_availability() {
        $material_ids = $this->input->post('material_ids');
        $supplier_id = $this->input->post('supplier_id');
        $delivery_warehouse_id = $this->input->post('delivery_warehouse_id');
        ...
        // Get supplier's warehouse/location
        $supplier_warehouse_id = null;
        if (!empty($supplier_id)) {
            $supplier = $this->site->getCompanyByID($supplier_id);
            if ($supplier && !empty($supplier->location_id)) {
                $supplier_warehouse_id = $supplier->location_id;
            }
        }
        
        // Use production_unit_model to get material availability
        $data = $this->production_unit_model->getMaterialsAvailability(
            $material_ids,
    ...
    public function inward_po($id = null) {
        // $this->sma->checkPermissions();
        $PreUrl = '';
        if ($this->input->post('previous_url')) {
            $previous_url = $this->input->post('previous_url');
        ...
        // $this->session->unset_userdata('csrf_token'); // Removed - this was causing CSRF validation to fail
        if ($this->form_validation->run() == true) {

            $reference = $this->input->post('reference_no');
            // For inward PO, always use current date since date field is disabled
            $date = date('Y-m-d H:i:s');
            $warehouse_id       = $this->input->post('warehouse');
            $supplier_id        = $this->input->post('supplier');
            $status             = $this->input->post('status');
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $items)) {
            ...
            // Insert data into sma_good_received_notes after successful purchase update
            $this->insertGoodReceivedNotes($id, $items, $date, $reference);
            ...
            **/
            ...
            $this->data['warehouses']   = $this->site->getAllActiveWarehouses();
            // Get GRN data for this purchase
            $this->data['grn_data'] = $this->purchases_model->getGRNDataByPurchaseID($id);
            $this->load->helper('string');
            $value = random_string('alnum', 20);
            $this->session->set_userdata('user_csrf', $value);
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('Inward_PO')));
            $meta = array('page_title' => lang('Inward_PO'), 'bc' => $bc);
            $this->page_construct('purchases/inward_po', $meta, $this->data);
        }
    }

    private function insertGoodReceivedNotes($purchase_id, $items, $date, $reference) {
        // Check if table exists first
        if (!$this->db->table_exists('sma_good_received_notes')) {
            error_log('sma_good_received_notes table does not exist');
            return; // Silently exit if table doesn't exist
        }
        
        // Fix date issue - use current date if date is empty or invalid
        $date = !empty($date) ? date('Y-m-d', strtotime($date)) : date('Y-m-d');
        if ($date == '1970-01-01') {
            $date = date('Y-m-d');
        }
        
        error_log('Final date to be used: ' . $date);
        
        // echo"<pre>";
        // print_r($items);exit;
        if (!empty($items)) {
            foreach ($items as $item) {
                // Insert all items without GRN number
                $grn_data = [
                    'date' => $date,
                    'purchase_id' => $purchase_id,
                    'product_id' => $item['product_id'],
                    'option_id' => isset($item['option_id']) ? $item['option_id'] : null,
                    'po_number' => $reference,
                    'received_quantity' => $item['quantity_received'] > 0 ? $item['quantity_received'] : $item['quantity']
                ];
                
                error_log('Inserting GRN data: ' . print_r($grn_data, true));
                
                try {
                    $this->db->insert('sma_good_received_notes', $grn_data);
                    error_log('GRN insert successful');
                } catch (Exception $e) {
                    error_log('GRN insert failed: ' . $e->getMessage());
                }
            }
        } else {
    ...
    /**
     * Inward PO button for purchases list (hidden when status is received or returned).
     */
    public static function inward_po_btn($id, $status) {
        $status = is_string($status) ? strtolower(trim($status)) : $status;
        if (in_array($status, array('received', 'returned'), true)) {
            return '<div class="text-center">N/A</div>';
        }
        $CI =& get_instance();
        return anchor('purchases/inward_po/' . $id, '<i class="fa fa-edit"></i> ' . $CI->lang->line('Inward_PO'), 'class="btn btn-primary btn-xs inward-po-btn"');
    }
