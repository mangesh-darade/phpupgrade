	...
	*********************************************************/
	public function addSaleFromEshopOrder( $data = array(), $items = array(), $extrasPara = array() )
    {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            if($order_id) {
                //Update sale_invoice_no after convert order into sales. 
               $this->db->where(['id'=>$order_id])->update('orders', ['sale_invoice_no' => $invoice_no, 'sale_id' => $sale_id ]); 
               //Update order payment if exists
               $this->db->where(['order_id'=>$order_id])->update('payments', ['sale_id' => $sale_id]);
            }
            // End Invoice No
            
            if ($this->site->getReference('eshop') == $data['reference_no']) {
...
public function getOrderDetails($id)
   ...
   public function getShipingAdress($id){
    ...
    public function updateOrderStatus($order_id, $new_status){
        ...
        return $this->db->update('sma_orders', ['sale_status' => $new_status]);
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////// 

     public function add_3p_order_payments($data = array(), $customer_id = null, $challan = NULL) {
        if ($this->db->insert('payments', $data)) {
            ...
            if ($challan == 'chalan') {
                ...
                // $this->site->syncOrderPayments($data['order_id']);
                $this->site->syncordersalepayments($data['order_id']);
            } else {
    ...
    public function get_sale_by_order_no($order_no) {
         return $this->db->get_where('sma_sales', ['order_no' => $order_no])->row();
    }
    ...
    public function insert_schedule($data) {
        $this->db->insert('sma_product_schedule', $data);
        return $this->db->insert_id(); // Get the last inserted schedule ID
    }
    public function insert_schedule_product($schedule_id, $product_id) {
       $data = array(
            'product_id' => $product_id,
        );

        $this->db->where('id', $schedule_id);
        $this->db->update('sma_product_schedule', $data);
        // Assuming a join table 'schedule_products'
    }
    public function insert_schedule_weekday($schedule_id, $weekday) {
        $data = array(
             'weekday' => $weekday,
        );
    ...
    public function insert_holiday($data) {
        return $this->db->insert('sma_holiday', $data);
    }
    public function get_all_product_schedule() {
        // Fetch all records from sma_product_schedule
        $query = $this->db->get('sma_product_schedule');
        return $query->result(); // Returns the result as an array of objects
    }
    public function get_all_holidays() {
        // Fetch all records from sma_holiday
        $query = $this->db->get('sma_holiday');
        return $query->result(); // Returns the result as an array of objects
    }
    public function delete_holiday($id) {
        return $this->db->delete('sma_holiday', array('id' => $id));
    }
    public function delete_schedule($id) {
        return $this->db->delete('sma_product_schedule', array('id' => $id));
    }
    public function get_holiday_by_id($id) {
        return $this->db->get_where('sma_holiday', array('id' => $id))->row();
    }

    public function update_holiday($id, $data) {
        return $this->db->where('id', $id)->update('sma_holiday', $data);
    }
    public function get_all_schedules() {
        return $this->db->select('sma_product_schedule.id, sma_categories.name as category_name, sma_products.name as product_name, order_lead_time, start_time, end_time')
            ->from('sma_product_schedule')
            ->join('sma_categories', 'sma_categories.id = sma_product_schedule.category_id', 'left')
            ->join('sma_schedule_products', 'sma_schedule_products.schedule_id = sma_product_schedule.id', 'left')
            ->join('sma_products', 'sma_products.id = sma_schedule_products.product_id', 'left')
            ->get()->result();
    }
    public function get_schedule_by_id($id) {
        return $this->db->get_where('sma_product_schedule', array('id' => $id))->row();
    }

    public function get_schedule_products($id) {
        return $this->db->select('sma_product_schedule.*, sma_categories.name as category_name, sma_products.name as product_name')
            ->from('sma_product_schedule')
            ->join('sma_categories', 'sma_categories.id = sma_product_schedule.category_id', 'left')
            ->join('sma_products', 'sma_products.id = sma_product_schedule.product_id', 'left')
            ->where('sma_product_schedule.id', $id)
            ->get()->row(); // << return single row
    }
    
    
    
    public function get_schedule_weekdays($id) {
        $this->db->select('weekday');
        $this->db->where('id', $id);
        $query = $this->db->get('sma_product_schedule');
    
        if ($query->num_rows() > 0) {
            return $query->row('weekday');
        } else {
            return null;
        }
    }
    ...
    public function get_all_categories() {
        return $this->db->order_by('name', 'ASC')->get('sma_categories')->result_array();
    }
    public function update_schedule($id, $data) {
        return $this->db->where('id', $id)->update('sma_product_schedule', $data);
    }
    public function schedule_exists_combined($product_id, $category_id) {
    ...
    {
        $this->db->select('sma_product_schedule.*, products.name AS product_name, categories.name AS category_name');
        $this->db->from('sma_product_schedule');
        $this->db->join('products', 'products.id = sma_product_schedule.product_id', 'left');
        $this->db->join('categories', 'categories.id = sma_product_schedule.category_id', 'left');
        $this->db->order_by('sma_product_schedule.weekday', 'ASC');
        $query = $this->db->get();
        return $query->result();
    }
