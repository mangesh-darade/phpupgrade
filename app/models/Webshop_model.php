...
class Webshop_model extends CI_Model {
        ...
        if ($q->num_rows() > 0) {
            ...
            foreach ($q->result() as $row) {
                ...
                if ((int) $row->parent_id > 0) {
                    ...
                    $data[$row->parent_id][$row->id]->categoryActive = $categoryActive;
                    $data[$row->parent_id][$row->id]->categoryInfoText = $categoryInfoText;
                } else {
    ...
    public function get_product_suggestions($keyword, $limit = 10) {
        ...
        $this->db->select('p.id, p.name, p.image, p.price, p.code');
        $this->db->from('products p');
        $this->db->where('p.in_eshop', '1');
        $this->db->where('p.is_active', '1');
        $this->db->where("(p.name LIKE '%" . $this->db->escape_like_str($keyword) . "%' OR p.code LIKE '%" . $this->db->escape_like_str($keyword) . "%')");
