...
class Auth_model extends CI_Model
{
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $this->identity_column2 = 'phone';
        ////////////////////////// Mrp , Price , Cost//////////////////////////////////
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, view_right, edit_right, allow_discount, show_cost, show_price, show_mrp')
            ->where($this->identity_column, $this->db->escape_str($identity))
            ->or_where($this->identity_column2, $this->db->escape_str($identity)) 
            ->limit(1)
            ->get($this->tables['users']);
   ...
   public function getGroup($id = NULL){
        ...
        $this->db->limit(1);
         return $this->db->get('sma_groups')->row();
    }
    ...
    public function getuserinfobyphone($phone)
    {
        ...
        $sql = 'SELECT users.`id`, users.`username`, users.`email`, users.`first_name`, users.`last_name`, users.`company`, users.`phone`, users.`gender`, users.`group_id`, groups.`name` as user_type, users.`warehouse_id`, warehouses.`name` as warehouse, users.`biller_id`, companies.`name` as biller, users.`company_id`, users.`show_cost`, users.`show_price`, users.`show_mrp`, users.`award_points`, users.`view_right`, users.`edit_right`, users.`allow_discount`, users.`active`, users.`offline_mobile_app_access`, users.`offline_windows_app_access` '; 
        $sql .= ' FROM sma_users as users ';
        $sql .= ' LEFT JOIN sma_groups as groups ON  groups.id=users.group_id ';
        $sql .= ' LEFT JOIN sma_warehouses as warehouses ON warehouses.id=users.warehouse_id ';
        $sql .= ' LEFT JOIN sma_companies as companies ON companies.id=users.biller_id ';
        $sql .= " WHERE users.phone='$phone' LIMIT 1";
         
         $qp = $this->db->query($sql);        
    ...
    public function updatePosAccessStatus($user_phone,$source,$status=0)
    {
        
        if($source == "mobileOfflineApp"){
           $rec = $this->db->update("users", ['offline_mobile_app_access' => $status], ['phone' => $user_phone]);
        }
        elseif($source == "windowsOfflineApp"){
           $rec = $this->db->update("users", ['offline_windows_app_access' => $status], ['phone' => $user_phone]);
        }
        else{
          $rec =  $this->db->update("users", ['active' => $status], ['phone' => $user_phone]);
        }
