...
class Reports_model_new extends CI_Model {
    ...
    public function getSalesItemAsGst($id, $type) {
        ...
        // echo $res->sum;
        if ($res->sum != " " && $res->sum != 0.0000) {
            $myQuery = "SELECT  DISTINCT CONCAT('(',gst_rate, '%) ',sum($type))as sumgst, gst_rate as gstrrate , sum($type) as totalgst  FROM  sma_sale_items WHERE id = $id  Group By gst_rate";
        } else {
            $myQuery = "SELECT  DISTINCT CONCAT('',sum($type))as sumgst  FROM  sma_sale_items WHERE id = $id  Group By gst_rate";
        }
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function getPurcahseAsGst($purchaseid, $type) {
        ...
        // echo $saleid;
        if ($res->sum != " " && $res->sum != 0.0000) {
            $myQuery = "SELECT  DISTINCT CONCAT('(',gst_rate, '%) ',sum($type))as sumgst, gst_rate as gstrate, sum($type) as totalgst  FROM  sma_purchase_items WHERE purchase_id = $purchaseid  Group By gst_rate";
        } else {
            $myQuery = "SELECT  DISTINCT CONCAT('',sum($type))as sumgst  FROM  sma_purchase_items WHERE purchase_id = $purchaseid  Group By gst_rate";
        }
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function getDailyPurchaseItemsTaxes($date) {
        ...
        //     WHERE `purchase_id` IN ( SELECT  `id`  FROM  "
        //      . $this->db->dbprefix('purchases') . "  WHERE DATE( `date` ) =  '$date' ) 
        //     GROUP BY `gst_rate` ";
        $query = "SELECT 
                pi.gst_rate, 
                pi.product_name, 
                ...
                SUM(pi.sgst) AS SGST,
                SUM(pi.igst) AS IGST,
                COALESCE(pv.`name`, '-') AS variant_name  -- Show variant name or '-' if no variant
            FROM  
                " . $this->db->dbprefix('purchase_items') . " pi
            LEFT JOIN 
                " . $this->db->dbprefix('product_variants') . " pv 
            ON pi.`option_id` = pv.`id`  -- Assuming `option_id` is the linking column
            WHERE  
                pi.`purchase_id` IN (
                    SELECT  `id`  
            ...
            GROUP BY  
                pi.gst_rate, pv.`name`  -- Group by gst_rate and variant name
            ORDER BY 
                pi.gst_rate";
    ...
    public function getDailyPurchaseItems($date) {
        ...
        $query = "SELECT  
                pi.`product_id`, 
                COALESCE(pv.`name`, '-') AS `name`,  -- Show '-' if pv.name is NULL
                pi.`product_code`,  
                pi.`product_name`,  
                pi.`net_unit_cost`,  
                pi.`product_unit_code` AS unit,
                SUM(pi.`quantity`) AS qty, 
                SUM(pi.`item_tax`) AS tax, 
                pi.`tax` AS tax_rate, 
                SUM(pi.`item_discount`) AS discount, 
                SUM(pi.`subtotal`) AS total
            FROM  
                " . $this->db->dbprefix('purchase_items') . " pi
            LEFT JOIN  -- Use LEFT JOIN to include records where the option_id may be NULL
                " . $this->db->dbprefix('product_variants') . " pv 
            ON pi.`option_id` = pv.`id`  -- Join condition
            WHERE  
                pi.`purchase_id` IN (
                    SELECT  `id`  
                    FROM  " . $this->db->dbprefix('purchases') . " 
            ...
            GROUP BY  
                pv.`name`  -- Group by product_code to prevent aggregation issues
    ...
    public function getVatCess($id, $attr_code) {
        $sql = "SELECT GROUP_CONCAT( CONCAT( ' (' ,`attr_per`, '%) ', format(`tax_amount`,2) ) ) AS $attr_code
...
public function getPaymentMethod($sale_id){
    ...
    public function getSaleGstData_new($year = NULL, $month = NULL, $warehouse = NULL, $biller = NULL, $sale_items = FALSE, $customer = NULL, $start_date = NULL, $end_date = NULL, $customer_group = NULL) {
        ...
        if($sale_items === TRUE){
            $select_sales_items = ", si.id AS item_id, si.product_id, si.product_code, si.product_name, si.option_id, pv.name AS option_name, si.quantity , si.item_discount, si.tax AS tax_rate, si.hsn_code, si.net_unit_price, ((si.`subtotal`) - (COALESCE(sit_cgst.tax_amount, si.cgst) + COALESCE(sit_sgst.tax_amount, si.sgst) + COALESCE(sit_igst.tax_amount, si.igst) + COALESCE(sit_cgst.surcharge, 0) + COALESCE(sit_sgst.surcharge, 0) + COALESCE(sit_igst.surcharge, 0)) ) taxable_amount, COALESCE(sit_cgst.attr_per, sit_sgst.attr_per, sit_igst.attr_per, si.gst_rate) AS gst_rate, COALESCE(sit_cgst.tax_amount, si.cgst) cgst, COALESCE(sit_sgst.tax_amount, si.sgst) sgst, COALESCE(sit_igst.tax_amount, si.igst) igst, (COALESCE(sit_cgst.surcharge, 0) + COALESCE(sit_sgst.surcharge, 0) + COALESCE(sit_igst.surcharge, 0)) AS surcharge ";
        } else {
            $select_sales_items = ", count(si.`id`) total_items, (SUM(si.`subtotal`) - SUM(COALESCE(sit_cgst.tax_amount, si.cgst) + COALESCE(sit_sgst.tax_amount, si.sgst) + COALESCE(sit_igst.tax_amount, si.igst) + COALESCE(sit_cgst.surcharge, 0) + COALESCE(sit_sgst.surcharge, 0) + COALESCE(sit_igst.surcharge, 0)) ) taxable_amount, COALESCE(sit_cgst.attr_per, sit_sgst.attr_per, sit_igst.attr_per, si.gst_rate) AS gst_rate, SUM(COALESCE(sit_cgst.tax_amount, si.cgst)) cgst, SUM(COALESCE(sit_sgst.tax_amount, si.sgst)) sgst, SUM(COALESCE(sit_igst.tax_amount, si.igst)) igst, si.tax AS tax_rate, SUM(COALESCE(sit_cgst.surcharge, 0) + COALESCE(sit_sgst.surcharge, 0) + COALESCE(sit_igst.surcharge, 0)) AS surcharge ";
        }
        //GROUP_CONCAT(p.`paid_by`) as payment_method,
        $qry = "SELECT s.id sale_id, s.`invoice_no`, s.`reference_no`, s.`date`, s.`reference_no`, s.`customer_id`, s.`customer`, s.`note`, c.`gstn_no`, c.`customer_group_name`, c.state as `state_code`, s.`biller_id`, s.`biller`, 
                s.`warehouse_id`, s.`sale_status`, s.`payment_status`, s.`payment_term`, s.`total_tax`, s.`shipping`, s.`grand_total`, s.`total_discount`, 
            ...
            RIGHT JOIN " . $this->db->dbprefix('sale_items') . " si ON si.`sale_id` = s.`id` 
            RIGHT JOIN " . $this->db->dbprefix('companies') . " c ON c.id = s.`customer_id`
            LEFT JOIN " . $this->db->dbprefix('sales_items_tax') . " sit_cgst ON sit_cgst.item_id = si.id AND sit_cgst.attr_code = 'CGST'
            LEFT JOIN " . $this->db->dbprefix('sales_items_tax') . " sit_sgst ON sit_sgst.item_id = si.id AND sit_sgst.attr_code = 'SGST'
            LEFT JOIN " . $this->db->dbprefix('sales_items_tax') . " sit_igst ON sit_igst.item_id = si.id AND sit_igst.attr_code = 'IGST'";  
          //  LEFT JOIN " . $this->db->dbprefix('payments') . " p ON p.sale_id = s.`id` ";
       ...
       $qry .= " WHERE s.`date` BETWEEN ('$fromDate') AND ('$toDate') ";
        ...
        if ($sqlrs->num_rows() > 0) {
            $CI =& get_instance();
            $results = $sqlrs->result();
            $sale_ids = array_unique(array_column($results, 'sale_id'));
            
            // Call dynamic tax details global helper function
            ...
            // Fetch the item rates to map surcharges correctly by rate
            $item_rate_map = [];
            if (!empty($sale_ids)) {
                $items_query = $this->db->select('id, gst_rate')->where_in('sale_id', $sale_ids)->get('sale_items')->result();
                foreach ($items_query as $it) {
                    $item_rate_map[$it->id] = $it->gst_rate;
                }
            }
            ...
            if (!empty($tax_details)) {
                foreach ($tax_details as $td) {
                    $is_vat = !in_array($td->attr_code, ['CGST', 'SGST', 'IGST']);

                    if (!isset($surcharge_by_item[$td->item_id])) {
                        $surcharge_by_item[$td->item_id] = 0.0;
                    }
                    $surcharge_by_item[$td->item_id] += (float)$td->surcharge;

                    if ($is_vat) {
                        $has_vat_by_item[$td->item_id] = true;
                        if (!isset($vat_by_item[$td->item_id])) {
                            $vat_by_item[$td->item_id] = 0.0;
                            $vat_rate_by_item[$td->item_id] = 0.0;
                        }
                        $vat_by_item[$td->item_id] += (float)$td->tax_amount;
                        $vat_rate_by_item[$td->item_id] += (float)$td->attr_per;
                    }

                    $item_gst_rate = isset($item_rate_map[$td->item_id]) ? $item_rate_map[$td->item_id] : 0;
                    $rate_key = number_format((float)$item_gst_rate, 4, '.', '');
                    if (!isset($surcharge_by_group[$td->sale_id][$rate_key])) {
                        $surcharge_by_group[$td->sale_id][$rate_key] = 0.0;
                    }
                    $surcharge_by_group[$td->sale_id][$rate_key] += (float)$td->surcharge;

                    if ($is_vat) {
                        $has_vat_by_group[$td->sale_id][$rate_key] = true;
                        if (!isset($vat_by_group[$td->sale_id][$rate_key])) {
                            $vat_by_group[$td->sale_id][$rate_key] = 0.0;
                            $vat_rate_by_group[$td->sale_id][$rate_key] = 0.0;
                        }
                        $vat_by_group[$td->sale_id][$rate_key] += (float)$td->tax_amount;
                        $vat_rate_by_group[$td->sale_id][$rate_key] += (float)$td->attr_per;
                    }
                }
            }

            foreach ($results as $row_rs) {
                $row_rs->payment_method = $this->getPaymentMethod($row_rs->sale_id);
                // Attach dynamic tax details maps if available
                $row_rs->tax_details = isset($tax_map[$row_rs->sale_id][$row_rs->item_id]) ? $tax_map[$row_rs->sale_id][$row_rs->item_id] : null;
                
                $sql_surcharge = (float)$row_rs->surcharge;
                if ($sale_items === TRUE) {
                    $row_rs->surcharge = isset($surcharge_by_item[$row_rs->item_id]) ? $surcharge_by_item[$row_rs->item_id] : 0.0;
                    
                    $is_vat = isset($has_vat_by_item[$row_rs->item_id]) ? true : false;
                    if ($is_vat) {
                        $row_rs->cgst = 0.0;
                        $row_rs->sgst = 0.0;
                        $row_rs->igst = 0.0;
                        $row_rs->vat = isset($vat_by_item[$row_rs->item_id]) ? $vat_by_item[$row_rs->item_id] : 0.0;
                        $row_rs->vat_rate = isset($vat_rate_by_item[$row_rs->item_id]) ? $vat_rate_by_item[$row_rs->item_id] : 0.0;
                    } else {
                        $row_rs->vat = 0.0;
                        $row_rs->vat_rate = 0.0;
                    }
                } else {
                    $rate_key = number_format((float)$row_rs->gst_rate, 4, '.', '');
                    $row_rs->surcharge = isset($surcharge_by_group[$row_rs->sale_id][$rate_key]) ? $surcharge_by_group[$row_rs->sale_id][$rate_key] : 0.0;
                    
                    $is_vat = isset($has_vat_by_group[$row_rs->sale_id][$rate_key]) ? true : false;
                    if ($is_vat) {
                        $row_rs->cgst = 0.0;
                        $row_rs->sgst = 0.0;
                        $row_rs->igst = 0.0;
                        $row_rs->vat = isset($vat_by_group[$row_rs->sale_id][$rate_key]) ? $vat_by_group[$row_rs->sale_id][$rate_key] : 0.0;
                        $row_rs->vat_rate = isset($vat_rate_by_group[$row_rs->sale_id][$rate_key]) ? $vat_rate_by_group[$row_rs->sale_id][$rate_key] : 0.0;
                    } else {
                        $row_rs->vat = 0.0;
                        $row_rs->vat_rate = 0.0;
                    }
                ...
                // Adjust taxable_amount based on correct surcharge
                $diff = (float)$row_rs->surcharge - $sql_surcharge;
                $row_rs->taxable_amount -= $diff;
    ...
    public function getPurchaseGstData_new($year = NULL, $month = NULL, $warehouse = NULL, $supplier = NULL, $start_date = NULL, $end_date = NULL) {
        ...
        if ($sqlrs->num_rows() > 0) {
            $CI =& get_instance();
            $results = $sqlrs->result();
            $purchase_ids = array_unique(array_column($results, 'purchase_id'));
            $tax_details = $CI->sma->getSalesItemsTaxDetails($purchase_ids, NULL, NULL, NULL, FALSE, 'purchase');
            ...
            if (!empty($purchase_ids)) {
                $items_query = $this->db->select('id, gst_rate')->where_in('purchase_id', $purchase_ids)->get('purchase_items')->result();
                foreach ($items_query as $it) {
                    $purchase_rate_map[$it->id] = $it->gst_rate;
                }
            }

            if (!empty($tax_details)) {
                foreach ($tax_details as $td) {
                    $tax_map[$td->purchase_id][$td->item_id][$td->attr_code] = $td;
                    
                    $is_vat = !in_array($td->attr_code, ['CGST', 'SGST', 'IGST']);
                    if ($is_vat) {
                        $item_gst_rate = isset($purchase_rate_map[$td->item_id]) ? $purchase_rate_map[$td->item_id] : 0;
                        $rate_key = number_format((float)$item_gst_rate, 4, '.', '');
                        $has_vat_by_group[$td->purchase_id][$rate_key] = true;
                        if (!isset($vat_by_group[$td->purchase_id][$rate_key])) {
                            $vat_by_group[$td->purchase_id][$rate_key] = 0.0;
                            $vat_rate_by_group[$td->purchase_id][$rate_key] = 0.0;
                        }
                        $vat_by_group[$td->purchase_id][$rate_key] += (float)$td->tax_amount;
                        $vat_rate_by_group[$td->purchase_id][$rate_key] += (float)$td->attr_per;
                    }
                }
            }
            ...
            foreach ($results as $row_rs) {
                // Attach dynamic tax details maps if available
                $row_rs->tax_details = isset($tax_map[$row_rs->purchase_id]) ? $tax_map[$row_rs->purchase_id] : null;
                
                $rate_key = number_format((float)$row_rs->gst_rate, 4, '.', '');
                $is_vat = isset($has_vat_by_group[$row_rs->purchase_id][$rate_key]) ? true : false;
                if ($is_vat) {
                    $row_rs->cgst = 0.0;
                    $row_rs->sgst = 0.0;
                    $row_rs->igst = 0.0;
                    $row_rs->vat = isset($vat_by_group[$row_rs->purchase_id][$rate_key]) ? $vat_by_group[$row_rs->purchase_id][$rate_key] : 0.0;
                    $row_rs->vat_rate = isset($vat_rate_by_group[$row_rs->purchase_id][$rate_key]) ? $vat_rate_by_group[$row_rs->purchase_id][$rate_key] : 0.0;
                } else {
                    $row_rs->vat = 0.0;
                    $row_rs->vat_rate = 0.0;
                }
