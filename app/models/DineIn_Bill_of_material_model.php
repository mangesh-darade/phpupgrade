<?php
class DineIn_Bill_of_material_model extends CI_Model
{
    ...
    public function getWorkstations($warehouse_ids = null)
    {
        // First get the location_type id for "Workstation"
        $type_q = $this->db->select('id')
            ->from('location_type')
            ->where('type', 'Workstation')
            ->limit(1)
            ->get();

        if ($type_q->num_rows() === 0) {
            return [];
        }

        $type_id = (int) $type_q->row()->id;

        // Then fetch warehouses matching that location_type
        $this->db->select('id, name');
        $this->db->from('warehouses');
        $this->db->where('location_type', $type_id);
        
        // If warehouse_ids provided (comma-separated), filter by those IDs
        if ($warehouse_ids && !empty($warehouse_ids)) {
            $warehouse_ids_array = array_filter(array_map('trim', explode(',', $warehouse_ids)));
            if (!empty($warehouse_ids_array)) {
                $this->db->where_in('id', $warehouse_ids_array);
            }
        }
    ...
    public function getBomDetailsByProductId($product_id, $secondary_location_ids = null)
    {
        if ($product_id) {
            ...
            // Optional filter: restrict by specific workstation IDs if provided
            if (is_array($secondary_location_ids) && !empty($secondary_location_ids)) {
                $this->db->where_in('b.workstation_id', $secondary_location_ids);
            }
            ...
            if ($q->num_rows() > 0) {
                ...
                foreach ($boms as &$bom) {
                    ...
                    $this->db->from('sma_produnit_bill_of_material_items i');
    ...
    public function get_bom_version_no($product_id, $workstation_id = null)
    {
        $this->db->select('MAX(CAST(version_no AS UNSIGNED)) AS max_version');
        $this->db->from('sma_produnit_bill_of_materials');
        $this->db->where('product_id', $product_id);
        
        // Filter by workstation_id to maintain version sequence per workstation
        // if ($workstation_id !== null) {
        //     $this->db->where('workstation_id', $workstation_id);
        // }
    ...
    public function getBomMaterialDetails($product_id, $secondary_location_ids = null)
    {
    ...
    public function getBomMaterialItems($bom_id, $location_id = null)
    {
        $this->db->select('
            ...
            sma_produnit_bill_of_materials.workstation_id AS workstation_id,
            bom_items.is_alternative
        ');
        $this->db->from('sma_produnit_bill_of_material_items AS bom_items');
        $this->db->join('sma_products', 'bom_items.material_id = sma_products.id', 'left');
        $this->db->join('sma_products AS alternative_product', 'bom_items.primary_product_id = alternative_product.id', 'left');
        ...
        $this->db->join(
            '(
                SELECT pb1.bom_id, pb1.is_batch_only, pb1.min_batch_qty, pb1.batch_uom, pb1.sales_units
                FROM sma_produnit_products_in_batches pb1
                INNER JOIN (
                    SELECT bom_id, MAX(id) AS max_id
                    FROM sma_produnit_products_in_batches
                    GROUP BY bom_id
                ) pb2 ON pb1.bom_id = pb2.bom_id AND pb1.id = pb2.max_id
            ) AS pb',
            'pb.bom_id = bom_items.bom_id',
            'left'
        );
        $this->db->join('sma_units AS batch_unit', 'pb.batch_uom = batch_unit.id', 'left');
        $this->db->where('bom_items.bom_id', (int) $bom_id);
        $this->db->group_by('bom_items.id');
        $this->db->order_by('COALESCE(bom_items.primary_product_id, bom_items.material_id)', 'ASC', false);
        $this->db->order_by('bom_items.is_alternative', 'ASC');
        $this->db->order_by('bom_items.material_id', 'ASC');
