...
//     }
    ...
    public function getProductNames($term, $warehouse_id, $limit = 50, $warehouseRes = NULL, $product_Id = null, $overselling = null) {
        if ($overselling === null) {
            $overselling = $this->Settings->overselling;
        }
        $wp = "( SELECT product_id, warehouse_id, quantity as quantity from {$this->db->dbprefix('warehouses_products')} ) FWP";

        $this->db->select('products.*, FWP.quantity as wp_quantity, categories.id as category_id, categories.name as category_name, sma_combo_items.quantity as comboquantity, ' . (int)$product_Id . ' as product_id', FALSE)
            ->join($wp, 'FWP.product_id=products.id' . ($warehouse_id ? ' AND FWP.warehouse_id = ' . (int)$warehouse_id : ''), 'left')
            ->join('categories', 'categories.id=products.category_id', 'left')
            ->join('categories as subcategories', 'subcategories.id=categories.parent_id', 'left')
            ->join('brands', 'brands.id=products.brand', 'left')
            ->join('seasons', 'seasons.id=products.season_id', 'left')
            ->join('sma_combo_items', 'sma_combo_items.product_id = products.id', 'left')
            ->group_by('products.id');
        ...
        if ($overselling) {
           /* $this->db->where("({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.article_code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%')"); */
            ...
            //    $this->db->where("(IF({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%',{$this->db->dbprefix('products')}.name LIKE '%" . $term . "%', Replace(coalesce({$this->db->dbprefix('products')}.name,''), ' ','') LIKE '%".str_replace(" ","",$term)."%') OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.article_code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%')");
            $this->db->where("(IF({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%', {$this->db->dbprefix('products')}.name LIKE '%" . $term . "%', REPLACE(COALESCE({$this->db->dbprefix('products')}.name,''), ' ','') LIKE '%" . str_replace(" ", "", $term) . "%') 
            OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' 
            OR {$this->db->dbprefix('products')}.article_code LIKE '%" . $term . "%' 
            OR CONCAT({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%'
            OR brands.name LIKE '%" . $term . "%'
            OR categories.name LIKE '%" . $term . "%'
            OR seasons.SeasonName LIKE '%" . $term . "%'
            OR subcategories.name LIKE '%" . $term . "%') AND {$this->db->dbprefix('products')}.flag_visible = 1");


             if((int)$warehouse_id > 0 && !empty($warehouseRes)):
                // Filtered by join now
            endif;
            
        } else {
            /* $this->db->where("(products.track_quantity = 0 OR FWP.quantity > 0) AND FWP.warehouse_id = '" . $warehouse_id . "' AND "
                . "({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.article_code LIKE '%" . $term . "%')"); */
            ...
            $this->db->where("(products.track_quantity = 0 OR FWP.quantity > 0) AND FWP.warehouse_id = '" . $warehouse_id . "' AND "
            . "(IF({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%', {$this->db->dbprefix('products')}.name LIKE '%" . $term . "%', REPLACE(COALESCE({$this->db->dbprefix('products')}.name,''), ' ','') LIKE '%" . str_replace(" ", "", $term) . "%') 
            OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' 
            OR CONCAT({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%'
            OR brands.name LIKE '%" . $term . "%'
            OR categories.name LIKE '%" . $term . "%'
    ...
    public function syncQuantity($sale_id = NULL, $purchase_id = NULL, $oitems = NULL, $product_id = NULL, $order_id = NULL, $batch_number = NULL) {
        if ($sale_id) {
            $sale_items = $this->getAllInvoiceItems($sale_id);
            foreach ($sale_items as $item) {
                // Sync product quantity
                $this->site->syncProductQty($item->product_id, $item->warehouse_id, $item->batch_number);
                
                // If variant exists, sync variant quantity with batch number
                if (isset($item->option_id) && !empty($item->option_id)) {
                    $this->site->syncVariantQty($item->option_id, $item->warehouse_id, $item->product_id, $item->batch_number);
                }
            }
        } 
    }
    ...
    public function getProductVariants($product_id, $variant_popup = null) {
    ...
    public function getProductoptionData($product_id) {
    ...
    public function addSale($data = [], $items = [], $payments = [], $si_return = [], $extrasPara = []) {
        ...
        $status_field = ($sale_action == 'chalan') ? 'challan_status' : 'sale_status';
        if ($data[$status_field] != 'pending' && ($data[$status_field] != 'returned' || $sale_action == 'chalan')) {
            $cost = $this->site->costing($items);
        }
        $order_id = $extrasPara['order_id'] ? $extrasPara['order_id'] : null;
        $syncQuantity = $extrasPara['sale_action'] ? $extrasPara['sale_action'] : 'sale';

        if ($sale_action == 'chalan') {
            $sma_sales = 'delivery_challan';
            $sma_sales_items = 'delivery_challan_items';
            $sma_sales_items_tax = 'delivery_challan_items_tax';
            $saleRefKey = 'ordr';
            $ReturnSaleRefKey = 're_ordr';
            $data['sale_as_chalan'] = 1;
            
            // Convert sale_status to challan_status for challans
            if (isset($data['sale_status'])) {
                $data['challan_status'] = $data['sale_status'];
        ...
        $_tax_type = ($sale_action == 'chalan' ? 'dc' : NULL);
        ...
        if ($this->db->insert($sma_sales, $data)) {
            ...
            if ($biller_invoice_prefix) {
                if ($sale_action == 'chalan') {
                    ...
                    $formatted_no = $this->sma->biller_invoice_format($sale_id, $todaydate,$biller_invoice_prefix);
                    $invoice_no = $biller_invoice_prefix .'/'. $formatted_no;
                }
            }else{
                $invoice_no = ($sale_action == 'chalan') ? $sale_id : $this->sma->invoice_format($sale_id, $todaydate);
            }
            ////////////////////////////// Multiple Biller Invoice No End ////////////////////////////////////////////////////
          
            //Update formated invoice no
            $update_data = ['invoice_no' => ($sale_action == 'chalan' ? NULL : $invoice_no)];
            if ($sale_action == 'chalan') {
                $update_data['challan_no'] = $invoice_no;
            }
            $this->db->where(['id' => $sale_id])->update($sma_sales, $update_data);

            if ($order_id) {
                //Update sale_invoice_no after convert order into sales. 
                $this->db->where(['id' => $order_id])->update('orders', ['sale_invoice_no' => $invoice_no]);
                $this->db->where(['id' => $order_id])->update('delivery_challan', ['invoice_no' => $invoice_no]);
            }
            ...
            foreach ($items as $item) {
                ...
                if ($sale_action == 'chalan') {
                    $item['challan_id'] = $sale_id;
                    unset($item['sale_id']); // delivery_challan_items has no sale_id column
                }
                ...
                $status_field = ($sale_action == 'chalan') ? 'challan_status' : 'sale_status';
                if ($data[$status_field] == 'completed') {
            ...
            $status_field = ($sale_action == 'chalan') ? 'challan_status' : 'sale_status';
            if ($data[$status_field] == 'completed' || ($data[$status_field] == 'returned' && $sale_action == 'chalan')) {
                $this->site->syncPurchaseItems($cost);
            }

            if (!empty($si_return) && $sale_action != 'chalan') {
                foreach ($si_return as $return_item) {
                    
                    $purchase_item_id = null;
                   
                    $effective_return_qty = $return_item['quantity'];
                    if (isset($return_item['packing_size']) && $return_item['packing_size'] > 0) {
                        $effective_return_qty = $return_item['quantity'] * $return_item['packing_size'];
                    }
                    ...
                    if ($product->type == 'combo') {
                        ...
                        foreach ($combo_items as $combo_item) {
                            if ($sale_action == 'chalan') {
                                $this->orders_model->updateCostingLine($return_item['id'], $combo_item->id, $effective_return_qty);
                                $this->orders_model->updatePurchaseItem($purchase_item_id, ($effective_return_qty * $combo_item->qty), NULL, $combo_item->id, $return_item['warehouse_id']);
                            } else {
                                $this->updateCostingLine($return_item['id'], $combo_item->id, $effective_return_qty);
                                $this->updatePurchaseItem($purchase_item_id, ($effective_return_qty * $combo_item->qty), NULL, $combo_item->id, $return_item['warehouse_id']);
                            }
                        }
                        ...
                        if ($sale_action == 'chalan') {
                            $this->orders_model->updateCostingLine($return_item['id'], $return_item['product_id'], $effective_return_qty);
                            $this->orders_model->updatePurchaseItem($purchase_item_id, $effective_return_qty, $return_item['id']);
                        } else {
                            $this->updateCostingLine($return_item['id'], $return_item['product_id'], $effective_return_qty);
                            $this->updatePurchaseItem($purchase_item_id, $effective_return_qty, $return_item['id'],$return_item['product_id'], $return_item['warehouse_id'],$return_item['option_id']);
                        }
                    }
                }
            ...
            if (($data['sale_status'] == 'completed' || $data['sale_status'] == 'returned') && $sale_action != 'chalan') {
                $this->site->syncQuantity($sale_id);
            } elseif ($sale_action == 'chalan') {
                $this->site->syncQuantity(NULL, NULL, NULL, NULL, NULL, NULL, $sale_id);
            }

            // Urbanpiper Stock Manage
            if($this->Settings->pos_type == 'restaurant' && $data['sale_status'] == 'completed'){
            ...
            if ($data['sale_status'] == 'returned') {
                $action = ($sale_action == 'chalan') ? 'return_challan' : 'return_sale';
            } else {
                $action = ($sale_action == 'chalan') ? 'add_challan' : 'add_sale';
            }
          
            $this->site->syncProductTransactionHistory($action, array(($sale_action == 'chalan' ? 'challan' : 'sale') => $data, 'items' => $items));
            return $sale_id;
        }
    ...
    public function updateSale($id, $data, $items = array()) {
      ...
      if($data['sale_status'] == 'completed') {
        ...
        if ($this->db->update('sales', $data, array('id' => $id))) {
            ...
            $data['id'] = $id;
            $this->site->syncProductTransactionHistory('edit_sale', array('sale' => $data, 'items' => $items));
            return true;
        }
        return false;
    }
    ...
    public function updateStatus($id, $status, $note) {
        ...
        if ($this->db->update('sales', array('sale_status' => $status, 'note' => $note), array('id' => $id))) {
            ...
            $sale_data = (array) $this->getInvoiceByID($id);
            $this->site->syncProductTransactionHistory('edit_sale', array(
                'sale'  => $sale_data,
                'items' => $items
            ));
        ...
        if ($sale = $this->getInvoiceByID($id)) {
            ...
            if ($sale->sale_status == 'completed') {
                ...
                foreach ($items as $item) {
                    ...
                    $effective_unit_qty = $item->unit_quantity;
                    if (isset($item->packing_size) && $item->packing_size > 0) {
                        $effective_unit_qty = $item->unit_quantity * $item->packing_size;
                    }
                    ...
                    if ($item->product_type == 'combo') {
                        ...
                        foreach ($combo_items as $combo_item) {
                            if ($combo_item->type == 'standard') {
                                $qty = ($effective_unit_qty * $combo_item->qty);
                        ...
                        $this->updatePurchaseItem($purchase_item_id, $effective_unit_qty, $item->id, $item->product_id, $item->warehouse_id, $option_id, $batch_number);
                    }
                }
                if ($sale->return_id || $return_id) {
                    ...
                    foreach ($returned_items as $item) {
                        ...
                        if ($item->product_type == 'combo') {
                            ...
                            $option_id = isset($item->option_id) && !empty($item->option_id) ? $item->option_id : 0;
                            $this->updatePurchaseItem($purchase_item_id, $effective_unit_qty, $item->id, $item->product_id, $item->warehouse_id, $option_id);
                        }
                    }
                }
 ...
 // Urbanpiper Stock Manage 
        ...
        if ($id) {
            ...
            if ($sale_item_id) {
                if ($sale_item = $this->getSaleItemByID($sale_item_id)) {
                    ...
                    $option_id = ($sale_item->option_id) ? $sale_item->option_id : 0;
    ...
    public function getSaleItemByID($id) {
        ...
        $q = $this->db->get_where('delivery_challan_items', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getPaymentsForSale($sale_id) {
        $this->db->select('payments.date, payments.paid_by,payments.cc_holder, payments.amount,payments.transaction_id, payments.cc_no, payments.cheque_no, payments.reference_no, users.first_name, users.last_name, type')
    ...
    public function addPayment($data = array(), $customer_id = null, $challan = NULL) {
        if ($this->db->insert('payments', $data)) {
            ...
            if ($challan == 'chalan') {
                $this->site->syncChallanPayments($data['challan_id']);
    ...
    public function updatePayment($id, $data = array(), $customer_id = null) {
        ...
        if ($this->db->update('payments', $data, array('id' => $id))) {
            if ($data['sale_id'] == 0) {
                if (isset($data['challan_id']) && $data['challan_id']) {
                    $this->site->syncChallanPayments($data['challan_id']);
                } else {
                    $this->site->syncOrderPayments($data['order_id']);
                }
    ...
    public function deletePayment($id) {
        ...
        if ($this->db->delete('payments', array('id' => $id))) {
            ...
            } elseif ($opay->challan_id) {
                $this->site->syncChallanPayments($opay->challan_id);
            }
            if ($opay->paid_by == 'gift_card') {
                $gc = $this->site->getGiftCardByNO($opay->cc_no);
                $this->db->update('gift_cards', array('balance' => ($gc->balance + $opay->amount)), array('card_no' => $opay->cc_no));
    ...
    public function addSaleFromChallan($data = array(), $items = array(), $extrasPara = array()) {
        ...
        $sale_action = $extrasPara['sale_action'] ? $extrasPara['sale_action'] : null;
        $challan_id = isset($extrasPara['challan_id']) ? $extrasPara['challan_id'] : (isset($extrasPara['order_id']) ? $extrasPara['order_id'] : null);
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            if ($biller_invoice_prefix) {
                ...
                $invoice_no = $this->sma->invoice_format($sale_id, $todaydate);
            }
            ...
            if ($challan_id) {
                //Update sale_invoice_no and status after convert order into sales. 
                $this->db->where(['id' => $challan_id])->update('delivery_challan', [
                    'invoice_no' => $invoice_no,
                    'challan_status' => 'sale_created'
                ]);
                //Update order payment if exists
                $this->db->where(['challan_id' => $challan_id])->update('payments', ['sale_id' => $sale_id]);
            }
            // End Invoice No
    ...
    public function addSaleReturnFromChallanReturn($data = array(), $items = array(), $extrasPara = array()) {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            if ($challan_id) {
                //Update sale_invoice_no after convert challan into sales return. 
                $this->db->where(['id' => $challan_id])->update('delivery_challan', ['invoice_no' => $data['invoice_no']]);
                //Update challan payment if exists
                $this->db->where(['challan_id' => $challan_id])->update('payments', ['sale_id' => $sale_return_id]);
            }
            // End Invoice No
            ...
            if ($items) {
                foreach ($items as $item) {
                    ...
                    $this->db->where(['challan_id' => $challan_id, 'product_id' => $item['product_id']])->update('costing', ['sale_id' => $sale_return_id, 'sale_item_id' => $sale_item_id]);
                }
    ...
    public function getChallanByCode($code) {
        if (empty($code)) {
            return false;
        }
        ...
        $this->db->where("MD5(CONCAT('Reciept'," . $this->db->dbprefix('delivery_challan') . ".`reference_no`," . $this->db->dbprefix('delivery_challan') . ".`id`))", $code);
    ...
    public function getAllChallanItems($sale_id, $return_id = NULL) {
        $this->db->select('delivery_challan_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.image, products.details as details, product_variants.name as variant, product_variants.price as variant_price, products.hsn_code as hsncode, delivery_challan.rounding as rounding')
                ...
                ->order_by('id', 'asc');
        if ($sale_id && !$return_id) {
            $this->db->where('delivery_challan_items.challan_id', $sale_id);
        } elseif ($return_id) {
            $this->db->where('delivery_challan_items.challan_id', $return_id);
        }
        ...
        if ($sale = $this->getChallanByID($id)) {
            ...
            if ($sale->sale_status == 'completed') {
                ...
                foreach ($items as $item) {
                    ...
                    if (isset($item->packing_size) && $item->packing_size > 0) {
                        $effective_qty = $item->quantity * $item->packing_size;
                    }
                    ...
                    if ($item->product_type == 'combo') {
                        ...
                        foreach ($combo_items as $combo_item) {
                            if ($combo_item->type == 'standard') {
                                $qty = ($effective_qty * $combo_item->qty);
                                $this->updatePurchaseItem(NULL, $qty, NULL, $combo_item->id, $item->warehouse_id);
                            }
                        }
                    } else {
                        $option_id = isset($item->option_id) && !empty($item->option_id) ? $item->option_id : NULL;
                        $this->updatePurchaseItem(NULL, $effective_qty, $item->id, $item->product_id, $item->warehouse_id, $option_id);
                    }
                }
                if ($sale->return_id || $return_id) {
                    $rid = $return_id ? $return_id : $sale->return_id;
    ...
    public function getDelivaryPerson() {
        return $this->db->select('id,name,phone')->where(['group_name' => 'delivery'])->get('sma_companies')->result();
    }
   ...
   /**
    ...
    public function getAllInvoiceItems1($sale_id, $return_id = NULL) {
        $this->db->select('sale_items.*,companies.*,sales.invoice_no,sales.date as s_date,sales.customer_id,sales.biller_id,sales.total as s_total,sales.grand_total as s_grand_total, sales.sgst as s_sgst, sales.cgst as s_cgst, sales.igst as s_igst ,sales.sale_status ')
                ->join('products', 'products.id=sale_items.product_id', 'left')
                ->join('product_variants', 'product_variants.id=sale_items.option_id', 'left')
                ->join('sales', 'sales.id=sale_items.sale_id', 'left')
    ...
    public function getItemsByInv($id)
    {
    ...
    public function getProductUnit($pid)
    {
        // select sma_units.name from sma_products,sma_units where  sma_products.unit = sma_units.id and sma_products.id = 55
        $res = $this->db->query("select sma_units.name from sma_products,sma_units where  sma_products.unit = sma_units.id and sma_products.id =".$pid);
    ...
    //counter 
    public function getProductOptionsByAttr($product_id, $warehouse_id, $GroupId='')
    {
		 $this->db->select('product_variants.*');
		if($GroupId!='')
			$this->db->where(array('product_variants.group_id' => $GroupId));
		$this->db->where(array('product_variants.product_id' => $product_id));
		$this->db->where(array('warehouses_products_variants.warehouse_id' => $warehouse_id));
		$this->db->group_by('warehouses_products_variants.option_id');
		$this->db->join('warehouses_products_variants', 'warehouses_products_variants.option_id=product_variants.id', 'inner');
		$q = $this->db->get('product_variants');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getLooseProductOptionsByAttr($product_id, $warehouse_id, $GroupId='')
    {
		 $this->db->select('product_variants.*');
		if($GroupId!='')
			$this->db->where(array('product_variants.group_id' => $GroupId));
    ...
    public function getCategoryTax($categoryId){
     ...
     public function getInvoiceByID_bulkDiscount($id)
     {
         if (is_array($id)) {
             // $q = $this->db->where_in('id', $id)->get('sales');
             $this->db->select('id,total,product_discount,total_discount, grand_total, order_tax_id');
             $this->db->from('sales');
             $this->db->where_in('id', $id);
             $q = $this->db->get();
             if ($q->num_rows() > 0) {
                 // return $q->row(); // or $q->result() if you expect multiple rows
                 return $q->result_array();
             }        
             return FALSE;
         } else {
     ...
     public function updateSale_bulkDiscount($data, $items = array())
     {
         ...
         //   $customer_state_code = $this->sma->getstatecode($customer_id);
         //   $biller_state_code   = $this->sma->getstatecode($biller_id);
         //   $GSTType = ($customer_state_code == $biller_state_code)?'GST':'IGST'; 
         // // End GST Type          
         ...
         $DatalogArr = array('data'=>$data, 'sale_items'=>$items);
         $DataLog = array(
             'action_type'=>'Edit',
             'product_id'=>'',
             'quantity'=>'',
             'action_reff_id'=>$id,
             'action_affected_data'=>json_encode($DatalogArr),
             'action_comment'=>'Edit sales',
         );
         $this->sma->setUserActionLog($DataLog);
         $update_data = [];
         
         if ($this->db->update_batch('sales', $data, 'id')) {
             ...
             //     $_taxSaleID = $id;
             //     $taxAtrr = $this->sma->taxClassification($item['tax_rate_id'],$item['net_unit_price'],$item['unit_quantity'],$sale_item_id,$_taxSaleID,$GSTType);
                 
             //       /*Add New field to Sale_items Code cgst,igst,sgst 17-1-2020*/
             //     $tax_ItemAtrr = $this->sma->taxArr_rate($item['tax_rate_id'],$item['net_unit_price'],$item['unit_quantity'],$sale_item_id,$_taxSaleID,$GSTType);
             //      /*End */  
             ...
             //     }
             //     /*New Field Add to Sales cgst,igst,sgst 17-1-2020 */
             //         $total_cgst  = $total_cgst + $tax_ItemAtrr['CGST'];
             //         $total_sgst  = $total_sgst + $tax_ItemAtrr['SGST'];
             //         $total_igst  = $total_igst + $tax_ItemAtrr['IGST'];
             //     }
 
             //     $this->db->update('sales', array('cgst' => $total_cgst, 'sgst' => $total_sgst,'igst' => $total_igst), array('id' => $id));  // Using for update GST columns
             // }
             // if ($data['sale_status'] == 'completed') {
             //     $this->site->syncPurchaseItems($cost);
             // }
     ...
     public function getPreviousSaleNo(){
        $this->db->select('*');
        $this->db->from('sales');
        $this->db->where('sale_status', 'returned');
        $this->db->order_by('id', 'DESC'); // Replace 'id' with your actual column name
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            return $q->row(); // or $q->result() if you expect multiple rows
        }        
    ...
    public function getProductBybundelId($pid, $warehouse_id = NULL) {
    ...
    public function getProductComboItem($pid, $warehouse_id = NULL) {
    ...
    public function getProductName($term, $warehouse_id, $limit = 50, $warehouseRes=NULL)
    {
        ...
        if ($this->Settings->overselling) {
           /* $this->db->where("({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.article_code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.name, ' (', {$this->db->dbprefix('products')}.code, ')') LIKE '%" . $term . "%')"); */
            ...
            OR categories.name LIKE '%" . $term . "%'
            OR seasons.SeasonName LIKE '%" . $term . "%'
            OR subcategories.name LIKE '%" . $term . "%')");
    ...
    public function getProductid($term, $warehouse_id, $limit = 50, $warehouseRes=NULL)
    {
    ...
    public function getProductBundelItems($pid, $warehouse_id = NULL) {
        $this->db->select('
                products.id as id, 
                combo_items.item_code as code, 
                combo_items.quantity as qty, 
                combo_items.unit_price, 
                products.name as name, 
                products.type as type,
                combo_items.option_id, 
                warehouses_products.quantity as quantity,
                ' . (int)$pid . ' as product_id');  // Add $pid as product_id in the select clause
        ...
        // if ($q->num_rows() > 0) {
        //     foreach ($q->result() as $row) {
        //         // Include the product_id (pid) in the result
        //         $data[] = (object) array_merge((array)$row, ['product_id' => $pid]);
        //     }
        //     return $data;
        // }
        ...
        if ($q->num_rows() > 0) {
            ...
            foreach ($q->result() as $row) {
                if (!empty($row->option_id)) {
                    // Fetch quantity from warehouse_product_variant table based on product.id and combo_items.option_id
                    ...
                    if ($variant->num_rows() > 0) {
                        ...
                        $row->quantity = $variant_row->quantity;  
                        if ($row->quantity <= 0) {
                            continue; 
                        }
                    } 
    ...
    // fetch bundle products without warehouse
    public function getProductComboItemsdata($pid) {
        $this->db->select('products.id as id, combo_items.item_code as code, combo_items.quantity as qty, combo_items.unit_price, products.name as name, products.type as type, ' . (int)$pid . ' as product_id, combo_items.option_id')
                 ->join('products', 'products.code = combo_items.item_code', 'left');
    ...
    public function updatePaymentsWithGrandTotal($all_invoices)
    {
        if (!empty($all_invoices)) {
            foreach ($all_invoices as $invoice) {
                if (!empty($invoice['id']) && isset($invoice['total'])) {
                    $this->db->where('sale_id', $invoice['id']);
                    $this->db->update('payments', [
                        'amount'   => $invoice['grand_total'],
                        'pos_paid' => $invoice['grand_total']
                    ]);
                }
            }
            return true;
    ...
    public function getBatchByNumber($batch_number, $product_id) {
        $this->db->where('batch_no', $batch_number);
        $this->db->where('product_id', $product_id);
        // if ($warehouse_id) {
        //     $this->db->where('warehouse_id', $warehouse_id);
        // }
        $q = $this->db->get('product_batches');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function getWarehouse($product_id, $warehouse_id = null) {
        $this->db->select('option_id, SUM(quantity) as total_qty');
        $this->db->from('sma_warehouses_products_variants');
        $this->db->where('product_id', $product_id);

        if ($warehouse_id !== null && $warehouse_id !== '') {
            if (strpos($warehouse_id, ',') !== false) {
                $this->db->where_in('warehouse_id', explode(',', $warehouse_id));
            } else {
                $this->db->where('warehouse_id', $warehouse_id);
            }
        } else {
            $warehouse_id = $this->session->userdata('warehouse_id');
            if (!$this->Owner && !$this->Admin && $warehouse_id !== null && $warehouse_id !== '') {
                if (strpos($warehouse_id, ',') !== false) {
                    $this->db->where_in('warehouse_id', explode(',', $warehouse_id));
                } else {
                    $this->db->where('warehouse_id', $warehouse_id);
        ...
        if ($q->num_rows() > 0) {
            $data = [];
            foreach ($q->result() as $row) {
                $data[$row->option_id] = $row->total_qty;
            }
            return $data;
        }
    ...
    /**
     * Billing/shipping address text for standard sale receipts (non-POS, non-Eshop, non-Offline).
     *
     * @param object $inv Sale invoice row
     ...
     */
    public function getInvoiceReceiptAddresses($inv, $customer) {
        if (!$inv || !$customer) {
            return null;
        }
        if ((int) $inv->pos !== 0 || (int) $inv->eshop_sale !== 0 || (int) $inv->offline_sale !== 0) {
            return null;
        }
        ...
        if (!empty($inv->billing_address_id)) {
            $billing_addr = $CI->site->getCustomerAddressById($inv->billing_address_id);
        ...
        if (!empty($inv->shipping_address_id)) {
            $shipping_addr = $CI->site->getCustomerAddressById($inv->shipping_address_id);
