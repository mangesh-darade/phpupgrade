...
class Db_model extends CI_Model
{
    ...
    public function getLatestCustomers()
    {
     ...
     public function getLatestApinotify()
    {
         $this->db->order_by('id', 'desc');
         $q = $this->db->get_where("apinotify", array('status' => 'pending'), 10);
    ...
    public function getAcceptedApinotify()
    {
         $this->db->order_by('id', 'desc');
         $q = $this->db->get_where("apinotify", array('status' => 'accepted'), 10);
    	//$q = $this->db->get("apinotify", array('status' => 'declined'), 5);
       //$q = $this->db->get("apinotify", 5);
        if ($q->num_rows() > 0) {
            $data = array();
