  ...
  public function openRegister($data, $data_coinage, $data_coinage_transactions, $coinage_master_id = null) {
    ...
    foreach ($items as $item) {
        $suspended_item = [
            ...
            'surcharge'                   => isset($item['surcharge']) ? $item['surcharge'] : NULL,
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL) {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            foreach ($items as $item) {
                ...
                if ($this->Settings->mother_kitchen) {
                    $this->load->model('DineIn_Bill_of_material_model');
                    $raw_materials = $this->DineIn_Bill_of_material_model->getRawMaterials($item['product_id'], $data['warehouse_id']);
                    if ($raw_materials) {
                        $seen_materials = [];
                        foreach ($raw_materials as $rm) {
                            // Deduplicate: skip if this material was already processed
                            if (isset($seen_materials[$rm->material_id])) {
                                continue;
                            }
                            $seen_materials[$rm->material_id] = true;
                            $total_deduct_qty = $rm->quantity_required * $item['quantity'];
                            $stock_check = $this->updaterawproductstock($rm->material_id, $data['warehouse_id'], $total_deduct_qty);
                            if ($stock_check !== true) {
                                $msg = lang($stock_check) . ' (Product is out of stock)';
                    ...
                    if (!empty($payment) && isset($payment['amount']) && $payment['amount'] != 0) {
                        ...
                        if ($payment['paid_by'] == 'ppp') {
                            ...
                            if ($payment['paid_by'] == 'award_point') {
                                $paid_by_award_point = true;
                                $payment_amount = (float) $payment['amount'];
                                $customer = $this->site->getCompanyByID($data['customer_id']);
                                // Same as earn: floor((amount / each_spent) * ca_point)
                                $redeemPoints = $this->calculateAwardPointsFromAmount($payment_amount);
                                $redeemPoints = min($redeemPoints, (int) $customer->award_points);
                                if ($redeemPoints > 0) {
                                    if ($this->db->table_exists('award_point_log')) {
                                        $this->db->insert('award_point_log', array(
                                            'sale_id' => $sale_id,
                                            'award_point' => $redeemPoints,
                                            'customer_id' => $data['customer_id'],
                                        ));
                                    }
                                    $this->db->update('companies', array(
                                        'award_points' => ((int) $customer->award_points - $redeemPoints),
                            ...
                            unset($payment['cc_cvv2']);
                            if ((strpos($payment['paid_by'], 'other') === 0 || $payment['paid_by'] == 'NEFT' ) && !empty($_POST['other_tran'][0])) {
 ...
 $printerSettings = $this->site->defaultPrinterOption($this->Settings->default_printer);
    ...
    public function getAllOrderItems($order_id) {
        ...
        if ($this->pos_settings->item_order == 0) {
            ...
            // ->order_by('id', 'asc');
            if (isset($this->pos_settings->display_category) && $this->pos_settings->display_category == 1)
                $this->db->order_by('categories.id', 'desc');
            else
    ...
    public function suspendSale($data = array(), $items = array(), $did = NULL) {
        ...
        foreach ($items as $key => $item) {
            if (!array_key_exists('surcharge', $item)) {
                $items[$key]['surcharge'] = NULL;
            }
        }
    ...
    /**
     * Award points for an amount — same as Sma::update_award_points() when earning.
     * Formula: floor((amount / each_spent) * ca_point)
     */
    public function calculateAwardPointsFromAmount($amount) {
        $amount = (float) $amount;
        $each_spent = (float) $this->Settings->each_spent;
        $ca_point = (float) $this->Settings->ca_point;
        if ($each_spent <= 0 || $ca_point <= 0 || $amount < $each_spent) {
            return 0;
        }
        return (int) floor(($amount / $each_spent) * $ca_point);
    }

    /**
     * Resolve award-point redemption (mirror of earn logic; subtract on save).
     *
     * @return array{redeem_point:float,invoice_amount:float,error:string}
     */
    public function resolveAwardPointRedeem($customer_id, $bill_amt) {
        $bill_amt = (float) $bill_amt;
        $result = array('redeem_point' => 0, 'invoice_amount' => $bill_amt, 'error' => '');
        if ($bill_amt <= 0) {
            $result['error'] = 'invalid_bill';
            return $result;
        }

        $each_spent = (float) $this->Settings->each_spent;
        $ca_point = (float) $this->Settings->ca_point;
        if ($each_spent <= 0 || $ca_point <= 0) {
            $result['error'] = 'AwardPointNotAdded';
            return $result;
        }

        $data = $this->getAwardPointByCustomer($customer_id);
        if (!$data || (float) $data['award_points'] <= 0) {
            $result['error'] = 'AwardPointNotFound';
            return $result;
        }

        $customer_award_point = (int) $data['award_points'];

        if ($bill_amt < $each_spent) {
            $result['error'] = 'AwardPointNotValid';
            return $result;
        }

        $points_needed = $this->calculateAwardPointsFromAmount($bill_amt);
        if ($points_needed <= 0) {
            $result['error'] = 'AwardPointNotValid';
            return $result;
        }

        if ($customer_award_point >= $points_needed) {
            $result['redeem_point'] = $points_needed;
            $result['invoice_amount'] = $bill_amt;
        } else {
            $max_coverable = ($customer_award_point / $ca_point) * $each_spent;
            $result['invoice_amount'] = min($bill_amt, $max_coverable);
            $result['redeem_point'] = $this->calculateAwardPointsFromAmount($result['invoice_amount']);
            if ($result['redeem_point'] <= 0) {
                $result['error'] = 'AwardPointNotValid';
            }
        }
    ...
    public function calculateAwardPointRedeem($customer_id, $bill_amt) {
        return (float) $this->resolveAwardPointRedeem($customer_id, $bill_amt)['redeem_point'];
    }
...
//                $this->db->group_by('date(date)');
   ...
   public function getCategoryIdByName($category_id) {
    ...
    public function get_active_payment_methods() {
        $table = null;
        foreach (array('payment_methods', 'sma_payment_methods') as $candidate) {
        ...
        $codeCol = null;
        foreach (array('code', 'method_code', 'type', 'paid_by') as $col) {
        ...
        $nameCol = null;
        foreach (array('name', 'display_name', 'title', 'method_name') as $col) {
            if (in_array($col, $fields, true)) {
                $nameCol = $col;
                break;
            }
        ...
        if ($codeCol === null || $nameCol === null) {
            log_message('error', 'sma_payment_methods: missing code/type and name/display_name columns.');
            return array();
        }

        $select = array('id', $codeCol . ' AS method_key', $nameCol . ' AS name', 'display_order', 'is_default');
        foreach (array(
            'logo' => array('logo'),
            'icon' => array('icon', 'image'),
            'field_type' => array('field_type', 'payment_field_type'),
            'balance_type' => array('balance_type'),
            'show_denomination' => array('show_denomination'),
        ...
        $query = $this->db->get();
        if ($query === false) {
            log_message('error', 'sma_payment_methods query failed: ' . json_encode($this->db->error()));
            return array();
        }

        $rows = array();
        foreach ($query->result_array() as $row) {
            $methodKey = isset($row['method_key']) ? trim((string) $row['method_key']) : '';
            $displayName = isset($row['name']) ? (string) $row['name'] : $methodKey;
            $row['code'] = $this->resolvePaymentMethodPaidByCode($methodKey, $displayName);

            $row['name'] = $displayName;
            $row['is_default'] = isset($row['is_default']) ? (int) $row['is_default'] : 0;
            $row['icon_class'] = (strtolower($row['code']) === 'cash') ? 'whitebgredB' : 'whitebgblueB';

            $row['icon_url'] = $this->paymentMethodIconUrl(
    ...
    public function resolve_coinage_preferred_payment_method($customer_id, array $methods = null) {
        if ($methods === null) {
        ...
        if ($customer_id > 0) {
            $balances = array('deposit' => 0, 'advance' => 0, 'wallet' => 0, 'gift_card' => 0, 'award_point' => 0, 'credit' => 0);
            $companyFields = $this->db->list_fields('companies');
            $companySelect = array('deposit_amount', 'award_points');
            foreach (array('advance_amount', 'advance_balance', 'advance_payment', 'credit_balance', 'credit_amount', 'store_credit') as $col) {
                if (in_array($col, $companyFields, true)) {
                    $companySelect[] = $col;
                }
            }
            $company = $this->db->select(implode(', ', array_unique($companySelect)))->where('id', $customer_id)->get('companies')->row();
            if ($company) {
                $balances['deposit'] = (float) $company->deposit_amount;
                $balances['award_point'] = (float) $company->award_points;
                foreach (array('advance_amount', 'advance_balance', 'advance_payment') as $col) {
                    if (isset($company->$col) && (float) $company->$col > 0) {
                        $balances['advance'] = (float) $company->$col;
                        break;
                    }
                }
                foreach (array('credit_balance', 'credit_amount', 'store_credit') as $col) {
                    if (isset($company->$col) && (float) $company->$col > 0) {
                        $balances['credit'] = (float) $company->$col;
                        break;
                    }
                }
            }
            $this->load->model('companies_model');
            $gift = $this->companies_model->getGiftCardAmt($customer_id);
            if ($gift && isset($gift->giftbalance)) {
                $balances['gift_card'] = (float) $gift->giftbalance;
            }
            if ($this->db->table_exists('sma_customer_wallet_transactions')) {
                $walletRow = $this->db->select('closing_balance')->where('customer_id', $customer_id)->order_by('id', 'DESC')->limit(1)->get('sma_customer_wallet_transactions')->row();
                if ($walletRow && $walletRow->closing_balance !== null) {
                    $balances['wallet'] = (float) $walletRow->closing_balance;
                }
            }

            foreach (array('deposit', 'advance', 'wallet', 'gift_card', 'award_point', 'credit') as $balanceType) {
                if ((float) $balances[$balanceType] <= 0) {
                    continue;
                }
                $balanceType = strtolower($balanceType);
                foreach ($methods as $method) {
                    $methodBalance = isset($method['balance_type']) ? strtolower(trim((string) $method['balance_type'])) : '';
                    $code = isset($method['code']) ? strtolower(trim((string) $method['code'])) : '';
                    if ($methodBalance === $balanceType || $code === $balanceType
                        || ($balanceType === 'gift_card' && in_array($code, array('gift_card', 'giftcard'), true))
                        || ($balanceType === 'award_point' && in_array($code, array('award_point', 'award_points'), true))) {
                        return $method['code'];
                    }
                }
            }
            ...
            if (!empty($codes)) {
                $q = $this->db->select('p.paid_by')->from('payments p')->join('sales s', 's.id = p.sale_id', 'inner')
                    ->where('s.customer_id', $customer_id)->where_in('p.paid_by', $codes)
                    ->order_by('p.date', 'DESC')->order_by('p.id', 'DESC')->limit(1)->get();
                if ($q->num_rows() > 0) {
                    return $q->row()->paid_by;
                }
            }
        }
        ...
        foreach ($methods as $method) {
            if (!empty($method['code']) && strtolower($method['code']) === 'cash') {
                return $method['code'];
            }
        }
    ...
    //for finalize sale screen
    public function get_coinage() {
        return $this->db
            ->select('currency_value, type')
            ->from('sma_coinage')
            ->where('is_active', 1)
            ->order_by('currency_value', 'ASC')
            ->get()
            ->result();
            
    }
    public function updaterawproductstock($product_id, $warehouse_id, $quantity) {
        if (!$this->Settings->mother_kitchen) {
            return true;
        }
        if ($quantity <= 0) return true;
        
        $affected_batches = []; // [warehouse_id => [batch_no1, batch_no2, ...]]

        // Step 1: Find stock across ALL warehouses, prioritizing the sale warehouse
        $this->db->select('id, warehouse_id, quantity_balance, batch_number');
        $this->db->where('product_id', $product_id);
        $this->db->where('quantity_balance >', 0);
        $this->db->group_start()
                 ->where('status', 'received')
                 ->or_where('status', 'partial')
                 ->or_where('status', 'returned')
                 ->group_end();
        
        // Custom order: primary warehouse first, then by date/id (FIFO)
        $this->db->order_by("CASE WHEN warehouse_id = " . (int)$warehouse_id . " THEN 0 ELSE 1 END", '', FALSE);
        ...
        if ($q->num_rows() > 0) {
            foreach ($q->result() as $item) {
                ...
                $deduct = min((float)$quantity, (float)$item->quantity_balance);
                if ($deduct > 0) {
                    $new_balance = (float)$item->quantity_balance - $deduct;
                    $this->db->update('purchase_items', ['quantity_balance' => $new_balance], ['id' => $item->id]);
                    
                    if (!isset($affected_batches[$item->warehouse_id])) {
                        $affected_batches[$item->warehouse_id] = [];
                    }
                    $affected_batches[$item->warehouse_id][] = $item->batch_number;
        ...
        // Step 2: If still short, handle Negative Inventory (Overselling)
        if ($quantity > 0) {
            // If overselling is disabled, block the deduction and signal an error back to addSale
            if (!$this->Settings->overselling) {
                $pr = $this->site->getProductByID($product_id);
                return $pr ? $pr->name : 'Product ID: ' . $product_id;
            }
            // Apply the remaining deficit to the LATEST record in the primary warehouse
            $this->db->select('id, batch_number, quantity_balance');
            $this->db->where('product_id', $product_id)->where('warehouse_id', $warehouse_id);
            $this->db->order_by('date', 'desc')->order_by('id', 'desc');
            $qlatest = $this->db->get('purchase_items', 1);

            if ($qlatest->num_rows() > 0) {
                $last_item = $qlatest->row();
                $new_neg_balance = (float)$last_item->quantity_balance - $quantity;
                $this->db->update('purchase_items', ['quantity_balance' => $new_neg_balance], ['id' => $last_item->id]);
                
                if (!isset($affected_batches[$warehouse_id])) $affected_batches[$warehouse_id] = [];
                $affected_batches[$warehouse_id][] = $last_item->batch_number;
            } else {
                // If NO purchase record exists, create a deficit entry for synchronization
                $pr = $this->site->getProductByID($product_id);
                if ($pr) {
                    $deficit_item = [
                        'product_id' => $product_id,
                        ...
                        'net_unit_cost' => (float)$pr->cost,
                        'unit_cost' => (float)$pr->cost,
                        'real_unit_cost' => (float)$pr->cost,
                        'quantity' => 0,
                    ...
                    $this->db->insert('purchase_items', $deficit_item);
                    $affected_batches[$warehouse_id][] = null;
                }
            }
        }

        // Always ensure the primary sale warehouse is synced
        if (!isset($affected_batches[$warehouse_id])) {
            $affected_batches[$warehouse_id] = [null];
        }

        // Step 3: Sync each touched warehouse and its affected batches
        foreach ($affected_batches as $wh_id => $batches) {
            $unique_batches = array_unique($batches);
            foreach ($unique_batches as $batch_no) {
                $this->site->syncProductQty($product_id, $wh_id, $batch_no);
            }
        }
        return true;
    }
    ...
    public function getPaymentMethods($id = null) {
        $tbl = 'sma_payment_methods';
        if ($id !== null) {
            $q = $this->db->get_where($tbl, array('id' => (int) $id), 1);
    ...
    public function getPaymentMethodsFormatted() {
        $list = array();
        foreach ($this->getPaymentMethods() as $row) {
            $list[] = $this->formatPaymentMethodForList($row);
        }
        return $list;
    }

    /** @return int|false */
    public function savePaymentMethod($data, $id = 0) {
        $id = (int) $id;
        if ($id > 0) {
            $existing = $this->getPaymentMethods($id);
            if ($existing && in_array(strtolower(trim((string) $existing->display_name)), array('award point', 'deposit', 'gift card'), true)) {
                unset($data['display_name'], $data['type']);
            }
            $this->db->where('id', $id);
            return $this->db->update('sma_payment_methods', $data) ? $id : false;
        }
        if (!isset($data['display_order'])) {
            $is_default_new = isset($data['is_default']) && (int) $data['is_default'] === 1;
            $default_row = $this->db->select('id')->from('sma_payment_methods')->where('is_default', 1)->order_by('id', 'ASC')->limit(1)->get()->row();
            if (!$is_default_new && $default_row) {
                // Put every new non-default method right after default.
                $this->db->set('display_order', 'display_order + 1', false);
                $this->db->where('id !=', (int) $default_row->id);
                $this->db->update('sma_payment_methods');
                $data['display_order'] = 2;
            } else {
                $row = $this->db->select_max('display_order')->get('sma_payment_methods')->row();
                $data['display_order'] = ($row && $row->display_order !== null) ? ((int) $row->display_order + 1) : 1;
            }
        }
        return $this->db->insert('sma_payment_methods', $data) ? $this->db->insert_id() : false;
    }

    public function deletePaymentMethod($id) {
        $row = $this->getPaymentMethods($id);
        if ($row && !empty($row->logo)) {
            $this->deletePaymentMethodLogo($row->logo);
        }
        $this->db->where('id', (int) $id);
        return $this->db->delete('sma_payment_methods');
    }

    public function uploadPaymentMethodLogo($file, $customer_assets) {
        ...
        $relativeDir = "assets/mdata/{$customer_assets}/uploads/payment_method_logos/";
        $uploadPath = FCPATH . $relativeDir;
        if (!is_dir($uploadPath)) {
            @mkdir($uploadPath, 0777, true);
        }
        $filename = md5(uniqid('', true) . $file['name']) . '.' . $ext;
        return move_uploaded_file($file['tmp_name'], $uploadPath . $filename) ? $relativeDir . $filename : false;
    }

    public function deletePaymentMethodLogo($path) {
        if (empty($path)) {
            return false;
        }
    ...
    /**
     * Coinage/checkout icon URL: uploaded logo if file exists, else theme SVG by code.
     */
    public function paymentMethodIconUrl($logo = null, $code = null) {
        $logo = trim((string) $logo);
        $code = trim((string) $code);
        $themeRel = 'themes/default/assets/pos/images/';
        $themeDir = FCPATH . $themeRel;

        if ($logo !== '') {
            if (strpos($logo, '://') !== false) {
                return $logo;
            }
            $logoNorm = ltrim(str_replace('\\', '/', $logo), '/');
            $pathCandidates = array($logoNorm);
            $base = basename($logoNorm);
            if (preg_match('#^https?://[^/]+/(.+)$#i', $logoNorm, $m)) {
                $pathCandidates[] = $m[1];
            }
            if (preg_match('#^pos/images/(.+)$#i', $logoNorm, $m)) {
                $pathCandidates[] = $themeRel . $m[1];
            }
            if ($base && preg_match('/\.(svg|png|jpe?g|gif|webp)$/i', $base)) {
                $pathCandidates[] = $themeRel . $base;
            }
            $CI =& get_instance();
            if (!empty($CI->Customer_assets)) {
                $tenantDir = 'assets/mdata/' . $CI->Customer_assets . '/uploads/payment_method_logos';
                if (stripos($logoNorm, 'payment_method_logos') !== false) {
                    $pathCandidates[] = $logoNorm;
                } elseif ($base) {
                    $pathCandidates[] = $tenantDir . '/' . $base;
                }
            }
            foreach (array_unique($pathCandidates) as $relative) {
                if ($relative !== '' && is_file(FCPATH . $relative)) {
        ...
        $svgCandidates = array();
        if ($logo !== '') {
            $logoHint = ltrim(str_replace('\\', '/', $logo), '/');
            $hintBase = basename($logoHint);
            if ($hintBase !== '') {
                $svgCandidates[] = $hintBase;
                if (preg_match('/\.(jpe?g|png|gif|webp)$/i', $hintBase)) {
                    $stem = preg_replace('/\.[^.]+$/', '', $hintBase);
                    $svgCandidates[] = $stem . '.svg';
                    $svgCandidates[] = ucfirst($stem) . '.svg';
                    $svgCandidates[] = 'N' . ucfirst($stem) . '.svg';
                }
            }
            if (preg_match('#(?:^|/)pos/images/([^/]+)$#i', $logoHint, $m)) {
                $svgCandidates[] = $m[1];
            }
        }
        ...
        $key = strtolower($code);
        $byCode = array(
            'cash' => 'NCash.svg', 'cheque' => 'NCheque.svg', 'deposit' => 'NDeposit.svg',
            'gift_card' => 'NGift.svg', 'award_point' => 'NAwardPoints.svg', 'cc' => 'NCredit.svg',
            'dc' => 'NDebit.svg', 'neft' => 'NNeft.svg', 'upi_qrcode' => 'NQR.svg',
            'googlepay' => 'NGPay.svg', 'razorpay' => 'NRazorpay.svg', 'paytm' => 'NPaytm.svg',
            'ccavenue' => 'NCCAvenue.svg', 'stripe' => 'NStripe.svg', 'ppp' => 'NPaypal.svg',
            'paypal' => 'NPaypal.svg', 'instamojo' => 'NInstamojo.svg', 'paynear' => 'NPayNearMe.svg',
            'payumoney' => 'NPayUMoney.svg', 'other' => 'NOther.svg', 'complimentary' => 'NComplimentary.svg',
            'pos' => 'PosNew.svg', 'payswiff' => 'NPaySwiff.svg', 'magicpin' => 'NMagicPin.svg',
            'authnet' => 'NAuthNet.svg',
        );
        if (isset($byCode[$key])) {
            $svgCandidates[] = $byCode[$key];
        }
        foreach (array_unique($svgCandidates) as $file) {
            if ($file !== '' && is_file($themeDir . $file)) {
                return base_url($themeRel . $file);
            }
        }
        if (is_file($themeDir . 'NCash.svg')) {
            return base_url($themeRel . 'NCash.svg');
        }
    ...
    /**
     * Paid-by code for POS/coinage from DB type + display name.
     */
    public function resolvePaymentMethodPaidByCode($methodKey, $displayName) {
        $key = strtolower(trim((string) $methodKey));
        $label = strtolower(trim((string) $displayName));
        $paidByMap = array(
            'cash' => 'cash', 'credit_card' => 'CC', 'debit_card' => 'DC', 'upi' => 'UPI_QRCODE',
            'cheque' => 'Cheque', 'deposit' => 'deposit', 'gift_card' => 'gift_card',
            'award_point' => 'award_point', 'neft' => 'NEFT', 'googlepay' => 'Googlepay',
            'google_pay' => 'Googlepay', 'paytm' => 'PAYTM', 'razorpay' => 'razorpay',
            'ccavenue' => 'ccavenue', 'stripe' => 'stripe', 'other' => 'other',
        );
        if (isset($paidByMap[$key])) {
            return $paidByMap[$key];
        }
        if ($key === 'payment_gateway' || $key === 'payment_method') {
            if (strpos($label, 'cash') !== false) {
                return 'cash';
            }
            if (strpos($label, 'cheque') !== false || strpos($label, 'check') !== false) {
                return 'Cheque';
            }
            if (strpos($label, 'deposit') !== false) {
                return 'deposit';
            }
            if (strpos($label, 'gift') !== false) {
                return 'gift_card';
            }
            if (strpos($label, 'award') !== false || strpos($label, 'point') !== false) {
                return 'award_point';
            }
            if (strpos($label, 'neft') !== false) {
                return 'NEFT';
            }
            if (strpos($label, 'debit') !== false) {
                return 'DC';
            }
            if (strpos($label, 'credit') !== false) {
                return 'CC';
            }
            if (strpos($label, 'upi') !== false || strpos($label, 'qr') !== false) {
                return 'UPI_QRCODE';
            }
            if (strpos($label, 'google') !== false) {
                return 'Googlepay';
            }
            if (strpos($label, 'razor') !== false) {
                return 'razorpay';
            }
            if (strpos($label, 'paytm') !== false) {
                return 'PAYTM';
            }
            if (strpos($label, 'ccavenue') !== false || strpos($label, 'cc avenue') !== false) {
                return 'ccavenue';
            }
            if (strpos($label, 'stripe') !== false) {
                return 'stripe';
            }
            if (strpos($label, 'paypal') !== false) {
                return 'ppp';
            }
            if (strpos($label, 'instamojo') !== false) {
                return 'instamojo';
            }
            if (strpos($label, 'paynear') !== false) {
                return 'paynear';
            }
            if (strpos($label, 'payumoney') !== false || strpos($label, 'payu') !== false) {
                return 'payumoney';
            }
            if (strpos($label, 'tabby') !== false) {
                return 'Tabby';
            }
            if (strpos($label, 'tamara') !== false) {
                return 'Tamara';
            }
            if ($label === 'pos' || strpos($label, ' pos') !== false) {
                return 'Pos';
            }
            $sanitized = strtolower(preg_replace('/[^a-zA-Z0-9_]/', '', str_replace(' ', '_', $displayName)));
            if ($sanitized === '' || $sanitized === 'payment_gateway') {
                return 'other';
            }
            return 'other_' . $sanitized;
        }
        return $methodKey !== '' ? $methodKey : ($displayName !== '' ? $displayName : '');
    }

    public function setDefaultPaymentMethod($id) {
        $id = (int) $id;
        if (!$this->getPaymentMethods($id)) {
            return false;
        }
        $this->db->update('sma_payment_methods', array('is_default' => 0));
        $this->db->where('id', $id);
        $this->db->update('sma_payment_methods', array('is_default' => 1, 'updated_at' => date('Y-m-d H:i:s')));
        $order = array($id);
        foreach ($this->db->select('id')->from('sma_payment_methods')->order_by('display_order', 'ASC')->order_by('id', 'ASC')->get()->result() as $row) {
            if ((int) $row->id !== $id) {
                $order[] = (int) $row->id;
            }
        }
        return $this->updatePaymentMethodsDisplayOrder($order, false);
    }

    public function updatePaymentMethodsDisplayOrder($order, $pin_default_first = true) {
        if (!is_array($order)) {
            return false;
        }
        ...
        if ($pin_default_first) {
            $default_row = $this->db->select('id')->where('is_default', 1)->order_by('id', 'ASC')->limit(1)->get('sma_payment_methods')->row();
            if ($default_row) {
                $default_id = (int) $default_row->id;
                $order = array_values(array_diff($order, array($default_id)));
                array_unshift($order, $default_id);
            }
        }
        $now = date('Y-m-d H:i:s');
        foreach ($order as $pos => $id) {
            $this->db->where('id', $id);
            $this->db->update('sma_payment_methods', array('display_order' => $pos + 1, 'updated_at' => $now));
        }
        return true;
    }

    public function paymentMethodTypeLabel($type) {
        static $labels = array(
    ...
    public function paymentGatewayJson($fields) {
        $gw = array();
        foreach (array('merchant_id', 'api_path', 'api_key', 'secret_key', 'access_code') as $k) {
            $gw[$k] = isset($fields[$k]) ? trim($fields[$k]) : '';
        }
        return json_encode($gw);
    }

    public function formatPaymentMethodForList($row) {
        static $icons = array(
            'payment_gateway' => 'fa-credit-card',
            'payment_method' => 'fa-credit-card-alt',
            'offline_payment' => 'fa-money',
            'net_banking' => 'fa-university',
            'upi' => 'fa-mobile',
            'cash_on_delivery' => 'fa-shopping-bag',
        );
        $emptyGw = array('merchant_id' => '', 'api_path' => '', 'api_key' => '', 'secret_key' => '', 'access_code' => '');
        $raw = !empty($row->pgateway_production) ? $row->pgateway_production : (isset($row->pgateway_testing) ? $row->pgateway_testing : '');
        $decoded = $raw ? json_decode($raw, true) : array();
        $gw = array_merge($emptyGw, is_array($decoded) ? $decoded : array());
        $type_label = $this->paymentMethodTypeLabel($row->type);
        $provider = '';
        if (!empty($gw['merchant_id'])) {
            $provider = $gw['merchant_id'];
        } elseif ($row->type === 'payment_gateway' && !empty($gw['api_path'])) {
            $provider = parse_url($gw['api_path'], PHP_URL_HOST) ?: $gw['api_path'];
        }
        $ts = (!empty($row->updated_at) && $row->updated_at !== '0000-00-00 00:00:00') ? $row->updated_at : (isset($row->created_at) ? $row->created_at : '');
        $updated = $ts ? date('d M Y, h:i A', strtotime($ts)) : '';
        $logo = isset($row->logo) ? $row->logo : '';
        $code = $this->resolvePaymentMethodPaidByCode(isset($row->type) ? $row->type : '', isset($row->display_name) ? $row->display_name : '');
        $logo_url = $this->paymentMethodIconUrl($logo, $code);
        return array(
            'id' => (int) $row->id,
            'display_name' => $row->display_name,
            'type' => $row->type,
            'type_label' => $type_label,
            'subtype_line' => $type_label . ($provider ? ' • ' . $provider : ''),
            ...
            'logo' => $logo,
            'logo_url' => $logo_url,
            'icon_class' => isset($icons[$row->type]) ? $icons[$row->type] : 'fa-credit-card-alt',
            'merchant_id' => $gw['merchant_id'],
            'api_path' => $gw['api_path'],
            'api_key' => $gw['api_key'],
            'secret_key' => $gw['secret_key'],
            'access_code' => $gw['access_code'],
        );
    }
    public function paymentMethodNameExists($display_name, $exclude_id = 0) {
        $display_name = trim($display_name);
        if ($display_name === '') {
            return false;
        }
        $escaped = $this->db->escape_str(strtolower($display_name));
        $this->db->from('sma_payment_methods');
        $this->db->where("LOWER(display_name) = '{$escaped}'", null, false);
        if ((int) $exclude_id > 0) {
