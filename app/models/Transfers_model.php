...
class Transfers_model extends CI_Model {
    ...
    public function getProductNames($term, $warehouse_id, $limit = 20) {
        ...
        if ($this->Settings->overselling) {
            ...
            //  allow RAW
            $this->db->where("(name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR article_code LIKE '%" . $term . "%' 
            OR concat(name, ' (', code, ')') LIKE '%" . $term . "%')");

        } else {

            // allow RAW
            $this->db->where("warehouses_products.warehouse_id = '" . $warehouse_id . "' AND (name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%'
            OR concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
    ...
    public function addTransfer($data = [], $items = [],$flag = null) {
        ...
        if ($this->db->insert('transfers', $data)) {
            ...
            /*if(!empty($productsids)){
             ...
             if($flag == "1"){
                $data['id'] = $transfer_id;
                $this->site->syncProductTransactionHistory('add_transfer', array('transfer' => $data, 'items' => $items));
                return $transfer_id; // return inserted ID for downstream printing
            }
             /////////////////////////// End Generate po///////////////////////////////////////
            $data['id'] = $transfer_id;
            $this->site->syncProductTransactionHistory('add_transfer', array('transfer' => $data, 'items' => $items));
    ...
    public function updateTransfer($id, $data = [], $items = []) {
        ...
        if ($this->db->update('transfers', $data, ['id' => $id])) {            
            ...
            $data['id'] = $id;
            $this->site->syncProductTransactionHistory('edit_transfer', array('transfer' => $data, 'items' => $items));
            return true;
        }
    ...
    public function updateStatus($id, $status, $note) {
       ...
       $productsids = array();
        ...
        if ($this->db->update('transfers', array('status' => $status, 'note' => $note), array('id' => $id))) {
            ...
            $transfer_data = (array) $this->getTransferByID($id);
            $this->site->syncProductTransactionHistory('edit_transfer', array(
    ...
    public function getAllTransferItems($transfer_id, $status) {
        ...
        if ($status == 'completed') {
            $this->db->select("sma_purchase_items.*, 
                               sma_product_variants.name as variant, 
                               sma_products.unit,categories.name as category_name, 
                               IF(sma_purchase_items.option_id IS NOT NULL AND sma_purchase_items.option_id != 0, 
                                  sma_product_variants.price, 
                                  sma_products.price) AS mrp", FALSE) 
                     ...
                     ->join('sma_product_variants', 'sma_product_variants.id = sma_purchase_items.option_id', 'left')
                     ->group_by('sma_purchase_items.id')
                     ->where('sma_purchase_items.transfer_id', $transfer_id);
        } else {
            $this->db->select("sma_transfer_items.*, 
                               sma_product_variants.name as variant, 
                               sma_products.unit,categories.name as category_name,
                               IF(sma_transfer_items.option_id IS NOT NULL AND sma_transfer_items.option_id != 0, 
                                  sma_product_variants.price, 
                                  sma_products.price) AS mrp", FALSE) 
                     ->from('sma_transfer_items')
                     ->join('sma_products', 'sma_products.id = sma_transfer_items.product_id', 'left')
                     ->join('categories', 'categories.id=sma_products.category_id', 'left')
                     ->join('sma_product_variants', 'sma_product_variants.id = sma_transfer_items.option_id', 'left')
                     ->group_by('sma_transfer_items.id')
                     ->where('sma_transfer_items.transfer_id', $transfer_id);
        }
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            return $q->result();
...
//                    $this->db->insert('purchase_items', $clause);
   ...
   /* public function deleteTransfer($id) {
    ...
    public function syncTransderdItem($product_id, $warehouse_id, $quantity, $option_id = NULL, $batch_number = NULL, $Status = NULL) {
        ...
        $this->site->syncQuantity(NULL, NULL, NULL, $product_id, NULL, $batch_number);
    ...
    public function getTransferProductList($warehouse_id, $werehouse_id_2) {
        ...
        //->group_by('products.id');
        $this->db->where_in("products.type", array('standard', 'raw', 'Intermediate'))->where("warehouses_products.warehouse_id = $warehouse_id ")->order_by('products.name', 'ASC');
    ...
    {
        // Distinct from-warehouses ordered alphabetically by name
        $quary="SELECT DISTINCT(from_warehouse_name), from_warehouse_id
                FROM sma_transfers
                ORDER BY from_warehouse_name ASC";
    ...
    {
        // Distinct categories used in transfers ordered alphabetically
        $quary = "SELECT DISTINCT sma_categories.name AS name, category_id
                  FROM sma_categories
                  JOIN sma_products ON sma_products.category_id = sma_categories.id
                  JOIN sma_transfer_items ON sma_transfer_items.product_id = sma_products.id
    ...
    {
        // Distinct brands used in transfers ordered alphabetically
        $quary = "SELECT DISTINCT sma_brands.name AS name, sma_products.brand
                  FROM sma_brands
    ...
    public function getProductVariantColor($product_id, $GroupId= '') {
