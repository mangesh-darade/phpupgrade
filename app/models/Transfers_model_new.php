...
class Transfers_model_new extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $limit = 20)
    {
        ...
        if ($this->Settings->overselling) {
            $this->db->where("type IN ('standard', 'raw', 'Intermediate') AND (IF(name LIKE '%" . $term . "%', name LIKE '%" . $term . "%', Replace(coalesce(name,''), ' ','') LIKE '%".str_replace(" ","",$term)."%') OR code LIKE '%" . $term . "%' OR article_code LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
        } else {
            $this->db->where("type IN ('standard', 'raw', 'Intermediate') AND warehouses_products.warehouse_id = '" . $warehouse_id . "' AND " 
