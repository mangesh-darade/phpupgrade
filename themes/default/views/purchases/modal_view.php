...
<td style="width: 120px; text-align:center; vertical-align:middle;"><?= $row->batch_number; ?></td>
                        ...
                        <?php 
                        // tax implementation with condition check
                        if (!empty($itemTaxes) && !empty($default_printer->tax_classification_view)) {

                            $dynamic = $this->sma->taxAttrTBL_csi_dynamic($row, 'purchase', $offset);

                            $purchase_tax_prdWise = !empty($dynamic) 
                                ? $dynamic 
                                : $this->sma->taxAttrTBL_csi(
                                    $row->gst_rate, 
                                    $row->cgst, 
                                    $row->sgst, 
                                    $row->igst, 
                    ...
                    if ($return_rows) {
                        ...
                        ?>
                            ...
                            <?php 
                              ...
                              $purchase_tax_prdWise = !empty($dynamic) 
                                  ? $dynamic 
                                  : $this->sma->taxAttrTBL_csi($row->gst_rate, $row->cgst, $row->sgst, $row->igst, $offset);
                    ...
                    <tfoot>
                    <?php
                    if ($Owner || $Admin || ($GP['products-cost']=='1')) { 
                    $col = 5;
                    } else {
                        $col = 3;
                    }
                 ...
                 <?php
                    ...
                    if ($Settings->invoice_view_purchase== 1) { 
                        ...
                        //  echo $this->sma->purchaseTaxInvoiceTableCSI($tax_summary,$inv,$return_purchase,$Settings);
                            echo $this->sma->getTaxpurchaseReport($inv->id, $Settings);
                        }?>
                    <div class="well well-sm">
                        <p>
            ...
            <?php if (!$Supplier || !$Customer) { ?>
                <div class="buttons">
                    ...
                    <div class="btn-group btn-group-justified">
                        ...
                        <div class="btn-group"> 
                            <a href="<?= site_url('purchases/payments/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal2" class="tip btn btn-primary" title="<?= lang('view_payments') ?>">
                                <i class="fa fa-dollar"></i>
                                <span class="hidden-sm hidden-xs"><?= lang('view_payments') ?></span>
                            </a>
                        </div>
                        <div class="btn-group">
                            <a href="<?= site_url('reciept/purchase_sms/' . $inv->id) ?>" class="tip btn btn-primary send_quote_sms" purchase_id="<?= $inv->id ?>" title="<?= lang('SMS') ?>">
                                <i class="fa fa-envelope-o"></i>
                                <span class="hidden-sm hidden-xs"><?= lang('SMS') ?></span>
                            </a>
                        </div>
                        <div class="btn-group">
