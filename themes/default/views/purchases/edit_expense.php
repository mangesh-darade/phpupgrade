        ...
        <div class="modal-body">
            ...
            <?php if ($Owner || $Admin) { ?>
                ...
                <div class="form-group">
                    <?= lang("date", "date"); ?>
                    <?= form_input('date', (isset($_POST['date']) ? $_POST['date'] : $this->sma->hrld($expense->date)), 'class="form-control datetime" id="date" required="required" readonly'); ?>
                </div>
            <?php } ?>
            ...
            <div class="form-group">
                <label for="subcategoryexp"><strong><?= ucfirst(lang('subcategory')); ?></strong></label>
                <?php
                $sct[''] = lang('select') . ' ' . lang('subcategory');
                ...
                ?>
                <?= form_dropdown('subcategory', $sct, set_value('subcategory', $expense->subcategory_id), 'class="form-control tip" id="subcategoryexp"'); ?>
            </div>
...
<script type="text/javascript" charset="UTF-8">
    $(document).ready(function () {
        ...
        $('#category').on('change', function () {
            ...
            var $subcategory = $('#subcategoryexp');
            var selectedSubcategory = '<?= set_value('subcategory', $expense->subcategory_id); ?>';
            $subcategory.html('<option value=""><?= lang('select') . ' ' . lang('subcategory'); ?></option>');
            if (!categoryId) {
                return;
            }
            $.get('<?= site_url("purchases/get_expense_subcategories"); ?>', {category_id: categoryId}, function (res) {
                if (res && res.length) {
                    $.each(res, function (_, row) {
                        var selected = (selectedSubcategory && selectedSubcategory == row.id) ? ' selected="selected"' : '';
                        $subcategory.append('<option value="' + row.id + '"' + selected + '>' + row.name + '</option>');
                    });
                }
            }, 'json');
        });
