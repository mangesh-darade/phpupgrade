...
function product_option_model_call(product) {
		...
		$.each(product.options, function (index, element) {
			...
			if (element.name.toLowerCase() == 'note') {
				...
				product_options += '<button onclick="addProductToVarientProduct(\'' + element.id + '\',\'' + element.name + '\')" type="button"  title="' + element.name + '" class="btn-prni btn-info pos-tip" tabindex="-1"><img src="assets/mdata/'+ $Customer_assets +'/uploads/thumbs/no_image.png" alt="' + element.name + '" style="width:33px;height:33px;" class="img-rounded"><span>' + element.name + '</span></button>';
			}
		});
