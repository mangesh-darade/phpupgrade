...
//                    echo '</pre>';
       ...
       <?php if(count($cart)) { ?> 
        ...
        <div class="alert alert-success"><?= count($cart)?> product(s) in cart.</div>  
                    ...
                    <tbody>
                        <?php
                            ...
                            foreach ( $cart as $key => $item) {
                                ...
                                $taxname = $itemTax[$item['tax_rate']]['name'];
                        ...
                        <tr id="item_<?= $key?>">
                            <td class="product-image">
                                ...
                                <img src="<?= $baseurl;?>assets/mdata/<?= $Customer_assets ?>/uploads/thumbs/<?= $item['image']?>" alt="<?= $item['code']?>" style="max-width:60px;" />
                            </td>
                            <td>
                            ...
                            <td class="product-tax">
                                <p><?= $taxname?><br/><i class="text-warning"><?= $taxType?></i></p>
                            </td>
                            <td class="product-name">
                                <p><span class="mobile-show text-right">Price</span><i class="fa fa-inr" aria-hidden="true"></i> <?= round($item['price'])?><?= $inclusiveInfo?></p>
                            </td>
                            <td class="product-total">
                                <p class="item_total_price text-right" >
                                    <span class="mobile-show">Items Price</span> <i class="fa fa-inr" aria-hidden="true"></i> <span id="show_total_<?= $item['id']?>"><?= round($item_price_total)?></span>
                                    <br/><small class="text-info">(Tax: <i class="fa fa-inr" aria-hidden="true"></i> <i id="show_tax_total_<?= $item['id']?>"><?php echo round($item_tax_total);?></i> )</small>
                                </p>
                            </td>
                        ...
                        <?php 
                            ...
                            <tr class="total-count">                                     
                                <td colspan="4"></td>
                                <td>Sub Total:</td>
                                <td class="tot-price text-right"><i class="fa fa-inr" aria-hidden="true"></i> <b id="cart_sub_total_show"><?= number_format($cart_sub_total,0)?></b></td>
                                <td></td>
                            </tr>
                            <tr class="total-count">                                     
                                <td colspan="4"></td>
                                <td>Total Tax</td>
                                <td class="tot-price text-right"><i class="fa fa-inr" aria-hidden="true"></i> <b id="cart_tax_total_show"><?= number_format($cart_tax_total,0)?></b></td>
                                <td></td>
                            </tr>
                            <tr class="total-count">                                    
                                <td colspan="4"></td>
                                <td>Gross Total</td>
                                <td class="tot-price text-right"><i class="fa fa-inr" aria-hidden="true"></i> <b id="cart_gross_total_show"><?= number_format($cart_gross_total,0)?></b></td>
                                <td></td>
                            </tr>
                    </tbody>
		</table>
