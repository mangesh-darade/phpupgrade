                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                if (!empty($inv_items)) {
                                    $items_obj = json_decode($inv_items);
                                    if (is_object($items_obj) || is_array($items_obj)) {
                                        foreach ($items_obj as $item) {
                                            if (isset($item->row->tax_rate) && $item->row->tax_rate) {
                                                $default_tax_percentage = $item->row->tax_rate;
                                                break;
                                            }
                                        }
                                    }
                                ...
                                <select name="tax_percentage" id="tax_percentage" class="form-control select">
                                    ...
                                    <?php foreach($tax_rates as $tax){ ?>
                                        <option value="<?= $tax->id; ?>" <?= ($tax->id == $default_tax_percentage) ? 'selected' : ''; ?>><?= $tax->name; ?> (<?= (float)$tax->rate; ?>%)</option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data !== null) {
                    var item = data[0];
                    var default_tax_id = $('#tax_percentage').val();
                    if (default_tax_id && item.tax_method_type !== 'category') {
                        var selected_tax = null;
                        $.each(tax_rates, function() {
                            if (this.id == default_tax_id) {
                                selected_tax = this;
                        ...
                        if (selected_tax) {
                            item.tax_rate = selected_tax;
                            item.row.tax_rate = selected_tax.id;
                        }
                    }
