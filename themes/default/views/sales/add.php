...
/* Scroll only inline page modals — not #myModal (customer view/edit/add loads here) */
#customerAddressModal .modal-body,
#prModal .modal-body,
#mModal .modal-body,
#gcModal .modal-body,
#scan_barcode_camera .modal-body,
.modalvarient .modal-body {
    max-height: 70vh;
    overflow-y: auto;
}
...
echo form_open_multipart("sales/add" . ($current_action != 'sales' ? '?sale_action=' . $current_action : ''), $attrib);
                ...
                <style type="text/css">
                    @media (min-width: 992px) {
                        .sale-add-top-fields > .sale-add-top-col {
                            float: left;
                            width: 20%;
                            position: relative;
                            min-height: 1px;
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="row sale-add-top-fields">
                            <?php if ($Owner || $Admin || $GP['sales-date']) { ?>
                            <div class="col-xs-12 sale-add-top-col">
                            ...
                            <?php } ?>
                            <div class="col-xs-12 sale-add-top-col" id="tax_percentage_container" style="<?= ($sale_action != 'chalan' && $sale_action != 'challan') ? 'display: none;' : ''; ?>">
                                <div class="form-group">
                                        ...
                                        <table id="slTable"
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="col-md-1"><?= lang("Packing Size"); ?></th>
                                                    <?php } ?>
...
<script>
    ...
    function autoPopulateCustomerAddresses() {
        ...
        // Keep current behavior: fetch from server and apply defaults
        getCustomerAddresses(customer_id);
    }

    function getSelectedCustomerIdForAddress() {
        var cid = $('#quick_custome_nm').val();
        if (!cid) {
    ...
    $(document).ready(function() {
        ...
        // Ensure addresses are auto-filled even when customer is pre-selected
        autoPopulateCustomerAddresses();
        // Retry once because select2/customer state can settle slightly later on load
        setTimeout(autoPopulateCustomerAddresses, 350);

        // Also refresh when customer select changes from any script flow
...
<script>
$(document).ready(function() {
    function syncSalesAddCustomerModalLink(billerId) {
    ...
    syncSalesAddCustomerModalLink($('#slbiller').val());
    ...
    $('#slbiller').on('change', function() {
        ...
        syncSalesAddCustomerModalLink(biller_name);
    });
