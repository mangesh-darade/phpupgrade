...
<script>
    $(document).ready(function () {
        var oTable = $('#SLData').dataTable({
            ...
            'sAjaxSource': '<?=site_url('sales/getChallans' . $warehouseIds )?>',
            ...
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                ...
                nRow.id = aData[0];
                nRow.setAttribute('data-return-id', aData[0]);
                nRow.className = "challan_link re"+aData[0];
                if (aData[4]) {
                    $(nRow).find('.sledit').parent().remove();
                    $(nRow).find('.create-sale-link').parent().remove();
                }
                return nRow;
            },
            ...
            {column_number: 3, filter_default_label: "[<?=lang('Challan_No');?>]", filter_type: "text", data: []},
            ...
            {column_number: 7, filter_default_label: "[<?=lang('challan_status');?>]", filter_type: "text", data: []},
...
<?php if ($Owner || $GP['bulk_actions']) {
	    echo form_open('sales/challan_actions', 'id="action-form"');
	}
?>
<div class="box">
...
<?= lang('sales_challan') . ' (' . (!empty($warehouse_id) && is_numeric($warehouse_id) ? $warehouse[$warehouse_id]->name : lang('all_warehouses')) . ')'; ?>
        </h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?=site_url('sales/add?sale_action=chalan')?>">
                        ...
                        <li>
                            <a href="#" class="bpo"
                                title="<b><?=lang("delete_challan")?></b>"
                                data-content="<p><?=lang('r_u_sure')?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?=lang('i_m_sure')?></a> <button class='btn bpo-close'><?=lang('no')?></button>"
                                data-html="true" data-placement="left">
                                <i class="fa fa-trash-o"></i> <?=lang('delete_challans')?>
                            </a>
                        </li>
                    </ul>
                </li>
                <?php if (!empty($warehouses)) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=site_url('sales'). '/challans'?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
                                $permisions_werehouse = explode(",", $this->session->userdata('warehouse_id'));
                            	foreach ($warehouses as $warehouse) {
                            	     if($Owner || $Admin  ){
                            	        	echo '<li><a href="' . site_url('sales/challans/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';		
                ...
                <div class="table-responsive">
                    <table id="SLData" class="table table-bordered table-hover table-striped">
                        ...
                        <tr>
                            ...
                            <th><?= lang("date"); ?></th>
                            <th><?= lang("Packing_no"); ?></th>
                            <th><?= lang("Challan_No"); ?></th>
                            <th><?= lang("Invoice_no"); ?></th>
                            <th><?= lang("biller"); ?></th>
                            <th><?= lang("customer"); ?></th>
                            <th><?= lang("Challan_Status"); ?></th>
