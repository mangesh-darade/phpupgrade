                            ...
                            <div style="clear:both;height:15px;"></div>
                                <div id="receiptData" style="border:1px solid #DDD; padding:10px; text-align:center;">
                                    ...
                                    <table width="100%" style="margin:15px 0;">
                                        <thead>
                                            <tr>
                                                <th style="border-bottom:1px solid #DDD;"><?= lang("product_name", "name") ?></th>
                                                <th style="border-bottom:1px solid #DDD;">Quantity x  (<?= lang("product_price", "price") ?>+Tax )</th>
                                                <th style="border-bottom:1px solid #DDD;text-align:right"><?=lang("total");?></th> 
                                            </tr>
                                        </thead>
                                        <tbody>
                                        <?php
                                        $r = 1;
                                        foreach ($rows as $row) {
                                            echo '<tr><td style="border-bottom:1px solid #DDD;">' . $row->product_name . ($row->variant ? ' (' . $row->variant . ')' : '') . '</td>'
                                                    . '<td style="border-bottom:1px solid #DDD;">' . $this->sma->formatQuantity($row->quantity) . ' x ';
                                            if ($row->item_discount != 0) {
                                                echo '<del>' . $this->sma->formatMoney($row->net_unit_price + ($row->item_discount / $row->quantity) + ($row->item_tax / $row->quantity)) . '</del> ';
                                            }
                                            echo $this->sma->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)).' ('.$this->sma->formatMoney($row->net_unit_price).' + '.$this->sma->formatMoney($row->item_tax / $row->quantity) . ')</td>'
                                                    . '<td style="border-bottom:1px solid #DDD;text-align:right">'.$this->sma->formatMoney($row->subtotal).'</td>'
                                                    . '</tr>';
                                            $r++;
                                        }
                                        ?>
                                        ...
                                        <tfoot>
                                        <tr>
                                            <th style="text-align:left;border-bottom:1px solid #DDD;" colspan="2"><?= lang("total"); ?></th>
                                            <th style="text-align:right;border-bottom:1px solid #DDD;" colspan="2"><?= $this->sma->formatMoney($inv->total); ?></th>
                                        </tr>
                                        <?php
                                        if ($inv->order_tax != 0 && $Settings->tax2) {
                                            echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;" colspan="2">' . lang("tax") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->sma->formatMoney($inv->order_tax) . '</th></tr>';
                                        }
                                        if ($inv->total_discount != 0) {
                                            echo '<tr><th style="text-align:left;border-bottom:1px solid #DDD;" colspan="2">' . lang("discount") . '</th><th style="text-align:right;border-bottom:1px solid #DDD;">' . $this->sma->formatMoney($inv->total_discount) . '</th></tr>';
                                        }
                                        ?>
                                        <tr>
                                            <th style="text-align:left;border-bottom:1px solid #DDD;" colspan="2"><?= lang("grand_total"); ?></th>
                                            <th style="text-align:right;border-bottom:1px solid #DDD;"><?= $this->sma->formatMoney($inv->grand_total); ?></th>
                                        </tr>
                                        <?php if ($inv->rounding) { ?>
                                            <tr>
                                                <th style="text-align:left;border-bottom:1px solid #DDD;" colspan="2"><?=lang("rounding");?></th>
                                        ...
                                        <?php } if ($inv->paid < $inv->grand_total) { ?>
                                            <tr>
                                                <th style="text-align:left;border-bottom:1px solid #DDD;" colspan="2"><?=lang("paid_amount");?></th>
                                                <th style="text-align:right;border-bottom:1px solid #DDD;"><?=$this->sma->formatMoney($inv->paid);?></th>
                                            </tr>
                                            <tr>
                                                <th style="text-align:left;border-bottom:1px solid #DDD;" colspan="2"><?=lang("due_amount");?></th>
                                    ...
                                    if ($payments) {
                                        ...
                                        foreach ($payments as $payment) {
                                            ...
                                            if(in_array($payment->paid_by,array('instomojo','ccavenue')) &&  $payment->amount) {
                                                echo '<td>' . lang("amount") . ': ' . $payment->amount . '</td>';
                                                echo '<td>' . lang("Transaction No.") . ': ' .$payment->transaction_id . '</td>';
                                            }
                                            echo '</tr>';
                                        }
                                        echo '</tbody></table>';
