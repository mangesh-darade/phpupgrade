...
<script>
    ...
    "aaSorting": [[2, "asc"], [3, "asc"]],
            ...
            nRow.id = aData[0];
            favClass = 'make_fav';
            // Name column is now rendered with hyperlink via productNameLink function, so we don't set it here
            // $("td:eq(2)", nRow).text(aData[2]);

            // is_featured is taken to check for is fav or not
             ...
             * Note: Name column is at index 2 (td:eq(2)) for raw materials screen
             */
            ...
            {"bVisible": false, "bSortable": false}  // Actions column - hidden from table
            ]
    }).fnSetFilteringDelay().dtFilter([
    {column_number: 2, filter_default_label: "[<?= lang('Products_name'); ?>]", filter_type: "text", data: []},
...
$col = 5;
if ($Owner || $Admin) {
    echo '{column_number : 6, filter_default_label: "[' . lang('Avg Cost') . ']", filter_type: "text", data: [] },';
...
$col++;
echo $col;
?>, filter_default_label: "[<?= lang('Expiry Date'); ?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
        <h2 class="blue">
            <i class="fa-fw fa fa-barcode"></i><?= lang('Raw_Materials') . ' (' . (!empty($warehouse_id) && is_numeric($warehouse_id) ? $warehouse[$warehouse_id]->name : lang('all_warehouses')) . ')'; ?>
        </h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        ...
                        <?php } ?>
                        <!-- for raw material :- print barcod and label  not required  -->
                        <!-- <li>
                ...
                <?php if (!empty($warehouses)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?= site_url('products/rawMaterials') ?>"><i class="fa fa-building-o"></i> <?= lang('all_warehouses') ?></a></li>
                            <li class="divider"></li>
                            <?php
                            $permisions_werehouse = explode(",", $this->session->userdata('warehouse_id'));
                            foreach ($warehouses as $warehouse) {
                                if ($Owner || $Admin) {
                                    echo '<li><a href="' . site_url('products/rawMaterials/' . $warehouse->id . $alertqty1) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
