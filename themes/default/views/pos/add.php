...
<head>
    ...
    #search_customer::-webkit-inner-spin-button, 
    #search_customer::-webkit-outer-spin-button { 
                                    ...
                                    <div id="left-bottom">
                                        ...
                                        <div id="botbuttons" class="col-xs-12 text-center">
                                            ...
                                            <div class="row">
                                                        ...
                                                        <div class="col-xs-4 padding0"<?php if ($single_btn) { ?> style="padding-left:10px !important;"<?php } ?>>
                                                            <div class="btn-group-vertical btn-block">
                                                                ...
                                                                <button type="button" class="btn btn-info btn-block"
                                                                    id="print_order" <?php if ($single_btn) { ?>style="height: 67px;"<?php } ?>>
                                                                    <?= lang('order'); ?>
                                                                </button>
                                                                <?php } ?>
                                                                <?php if ($show_bill_btn) { ?>
                                                                <button type="button" class="btn btn-primary btn-block"
                                                                    id="print_bill" style="margin-left: 0px;margin-bottom: 2px;<?php if ($single_btn) { ?>height: 65px;px;<?php } ?>">
                                                                    <?= lang('bill'); ?>
                                                                </button>
                                                                <?php } ?>
                                                            </div>
                                                ...
                                                <?php } ?>
                                                <?php if (!in_array((int)$pos_settingss->display_seller, [0, 1, 2], true)) { ?>
                                                    <div class="col-xs-4 padding0">
                                                        <?php if ($Owner || $Admin || $GP['checkout']) { ?>
                                                        <?php
                                                        $no_print_buttons = (!$show_order_btn && !$show_bill_btn);
                                            ...
                                            <div class="<?php
                                                ...
                                                <div class="input-group mob">
                                                    <!-- <?php
                                                            ...
                                                            ?> -->
                                                    <input type="hidden" class="nodisplay" name="customer" id="poscustomer" value="<?= ($_SESSION['quick_customerid'] ) ? $_SESSION['quick_customerid'] : $customer->id; ?>" />
                                                    <input type="text" placeholder="Customer Name" name="customer_name" class="form-control kb-text txt-box ui-keyboard-input ui-widget-content ui-corner-all ui-keyboard-autoaccepted" id="customer_name" value="<?= ($_SESSION['quick_customername'] ) ? $_SESSION['quick_customername'] . '(' . $_SESSION['quick_customerphone'] . ')' : $customer->name; ?>" />
                                                    ...
                                                    <div class="col-sm-12 ">
                                                        <div class="select-group">
                                                            ...
                                                            <div class="form-control" id="sales_person_trigger_mobile"
                                                                ...
                                                                <input type="hidden" name="sales_person_mobile"
                                                                    id="sales_person" value="0">
                                                            </div>
                                                            <?php } else { ?>
...
?>
        ...
        if (KB) {
            ...
            $('#poscustomer').on('select2-close', function() {
                ...
                $('#test').click();
                $('select, .select').not('.crm-state-native').select2('destroy');
                $('select, .select').not('.crm-state-native').select2({
                    minimumResultsForSearch: 7
                });
            });
...
?>
        ...
        $('#payment').click(function() {
            ...
            paymentMethodManuallySelected = false;
            amount_1 = 0;
            <?php
        ...
        function depositGiftAmt() {
            ...
            $.ajax({
                ...
                success: function(result) {
                    console.log('success');
                    if (result.deposit) {
                        if (result.deposit > 0 && !paymentMethodManuallySelected) {
                            _paymentAutoSelect = true;
                            $('input[value="deposit"]').trigger('click');
                            _paymentAutoSelect = false;
                        }
...
?>
        ...
        $('select, .select').not('#multiselect1, #multiselect1_to, .crm-state-native').select2({
            minimumResultsForSearch: 7
        });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data && data.status === 'error') {
                            if (data.type === 'combo') {
                                bootbox.alert('The following products are out of stock: ' + data.product_name);
                            } else {
                                bootbox.alert('<?= lang('no_match_found') ?>');
                            }
                        return false;
...
?>
    ...
    var paymentMethodManuallySelected = false;
    var _depositValidateSeq = 0;
    var _giftCardValidateSeq = 0;
    var _paymentAutoSelect = false;

    $(document).ready(function() {

        $('.custom_payment_icon').click(function() {
            if (!_paymentAutoSelect) {
                paymentMethodManuallySelected = true;
            }
            $('.final-submit-btn').prop('disabled', false);
    ...
    function isGiftCardPaymentSelected() {
        var pm = $("input[name='colorRadio']:checked").val() || $('#paid_by_val_1').val() || $('#paid_by_1').val();
    ...
    function isDepositPaymentSelected() {
        var pm = $("input[name='colorRadio']:checked").val() || $('#paid_by_1').val();
        return pm === 'deposit';
    }
    ...
    function setCustomerDeposit(ba, sel) {
        ...
        window._depositValidateXhr = $.ajax({
            ...
            dataType: "json",
            success: function(data) {
                if (reqId !== _depositValidateSeq || !isDepositPaymentSelected()) {
                    return;
                }
                ...
                if (data.balanceamt > 0) {
                    $('#depositdetails_1').html('<small>Value: ' + data.value + ' - Balance: ' + data
                        .balanceamt + '</small>');
                    if (billAmt > parseFloat(data.balanceamt)) {
