...
$resOutput = $this->sma->posBillTable_webshop($default_printer, $inv, $return_sale, $rows, $return_rows);
            ...
            <div id="receiptData">
                ...
                <div id="receipt-data" style="font-size: <?php echo $font_size; ?>pt !important">
                    ...
                    <div class="text-center">
                        ...
                        if ($Settings->invoice_view == 1) {
                            ...
                            <div class="col-sm-12 text-center">
                                <h4 style="font-weight:bold; margin: 0px;"><?= lang('Order_Receipt'); ?></h4>
                            </div>
                            <?php
                        }
                        ?>
                    <div class="row">
                        <div class="col-sm-12">                            
                            <div class="pull-left">
                                <b><?php echo lang("Order"); ?></b><?php echo " #&nbsp" . $inv->id; ?>
                            </div>
                            <div class="pull-right">
                                <?php echo "<b>" . lang("Order Date") . ":</b> " . $this->sma->hrld($inv->date); ?>
                            </div>
                        </div>
                    </div> 
                    <div class="row">
                        <div class="col-sm-12"> 
                            <!-- <div><?php echo lang("Channel") . ":&nbsp" . $inv->up_channel; ?></div> -->
                            <div>
                                <?php
                                    $channel = $inv->up_channel ? $inv->up_channel : 'webshop';
                                    $ref = isset($upOrder->order->details->merchant_ref_id) && $upOrder->order->details->merchant_ref_id
                                        ? ' / ' . $upOrder->order->details->merchant_ref_id
                                        : '';
                                    echo '<b>' . lang("Channel") . '</b>:&nbsp;' . $channel . $ref;
                                ?>
                            </div>
                            <!-- <div><?php echo lang("Channel ID") . ":&nbsp" . $upOrder->order->details->merchant_ref_id; ?></div> -->
                        <?php
                            echo "<b>" . lang("customer") . ":</b> " . $customer->name;
                            if (!empty($address_to_show->phone)) {
                                echo "<br/><b>" . lang("address") . ":</b>&nbsp;" . $address_to_show->line1;
                                if (!empty($address_to_show->line2)) {
                                    echo " " . $address_to_show->line2;
                                }
                                if (!empty($address_to_show->city)) {
                                    echo ", " . $address_to_show->city;
                                }
                                if (!empty($address_to_show->postal_code)) {
                                    echo " - " . $address_to_show->postal_code;
                                }
                            }
                        ?>
                        ...
                        <div class="pull-left mr-0">
                            <?php echo '<b>Shop</b> <b>' . lang("reference_no") . '</b>: ' . $inv->reference_no; ?>
                        </div>
                    <div style="clear:both;"></div> 
                        ...
                        ?>
                                            <?php 
                            if ($inv->coupon_code && $inv->product_discount > 0) {
                        ?>
                            <p>
                                <?php 
                                    echo $inv->coupon_code 
                                        ? 'Coupon Applied : ' . $inv->coupon_code . ' (' . $this->sma->formatMoney($inv->product_discount) . ')' 
                                        : ''; 
                                ?> 
                            </p>
                        <?php
                        ...
                        <?php }//close if 
                        ?>
                        <?php if (!empty($inv->note) && strtolower(trim($inv->note)) !== 'null'): ?>
                            <p class="text-center"><?= $this->sma->decode_html($inv->note); ?></p>
                        <?php endif; ?>
                        <?php if (!empty($inv->staff_note) && strtolower(trim($inv->staff_note)) !== 'null'): ?>
                            <p class="no-print"><strong><?= lang('staff_note'); ?>:</strong> <?= $this->sma->decode_html($inv->staff_note); ?></p>
                        <?php endif; ?>
