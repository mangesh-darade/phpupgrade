...
<div class="modal-dialog modal-lg no-modal-header">
    <div class="modal-content">
        <div class="modal-body">
			...
			<?= $this->Settings->order_receipt_label ? $this->Settings->order_receipt_label : 'Order' ?> No. : <?= $inv->invoice_no;?>
                    </p>
                    </div>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="vertical-align:middle;">
                                ...
                                <?php 
                                // Get tax details for this order item
                                $itemTaxes = $this->db->select('attr_code, attr_per, tax_amount')
                                            ->where('item_id', $row->id)
                                            ->where('order_id', $inv->id)
                                            ->get('sma_orders_items_tax')
                                            ->result();
                                
                                if (!empty($itemTaxes) && $default_printer->tax_classification_view) {
                                    echo '<div style="margin-top:5px; color:#666; font-size:0.9em;">';
                                    foreach ($itemTaxes as $tax) {
                                        echo $tax->attr_code . ' ' . $tax->attr_per . '%: ' . $this->sma->formatMoney($tax->tax_amount) . '<br>';
                                    }
                                    echo '</div>';
                                }
                                ?>
...
<!--                        <td style="text-align:right; width:100px;"><?= $this->sma->formatMoney($row->mrp); ?></td>-->
                        ...
                        ?>
                            ...
                            <td style="vertical-align:middle;">
                               ...
                               <?= $row->product_code.' - '.$row->product_name . ($row->variant ? ' (' . $row->variant . ')' : ''). ($row->shade_name ? ' (' . $row->shade_name . ')' : ''); ?>
                                ...
                                <?php 
                                // Get tax details for this return item
                                $itemTaxes = $this->db->select('attr_code, attr_per, tax_amount')
                                            ->where('item_id', $row->id)
                                            ->where('order_id', $inv->id)
                                            ->get('sma_orders_items_tax')
                    ...
                    <?php if ($inv->grand_total != $inv->total) { ?>
                        <tr>
                            ...
                            <?php
                             ...
                             echo '<td style="text-align:right;">' . $this->sma->formatMoney($return_sale ? ($inv->total+$return_sale->total) : $inv->total) . '</td>';
                    ...
                    <?php
                    if ($return_sale) {
                        echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;">' . lang("return_total") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->sma->formatMoney($return_sale->grand_total+$return_sale->rounding) . '</td></tr>';
                    }
                    if ($inv->surcharge != 0) {
            ...
            <?php if (!$Supplier || !$Customer) { ?>
                <div class="buttons">
                    <div class="btn-group btn-group-justified">
                        ...
                        <?php } ?>
                        <div class="btn-group">
                            <a href="<?= site_url('sales/emailchallan/' . $inv->id) ?>" data-toggle="modal" data-target="#myModal2" class="tip btn btn-primary" title="<?= lang('email') ?>">
...
<!--                        <div class="btn-group">
                        ...
                        <div class="btn-group del_btn_group">
                            <a href="#" class="tip btn btn-danger bpo" title="<b><?= $this->lang->line("delete") ?></b>"
                                data-content="<div style='width:150px;'><p><?= lang('r_u_sure') ?></p><a class='btn btn-danger' href='<?= site_url('sales/delete_order/' . $inv->id) ?>'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button></div>"
