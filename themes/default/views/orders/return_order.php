    ...
    function loadItems() {
        ...
        if (localStorage.getItem('reitems')) {
            ...
            $.each(reitems, function () {
                ...
                var a_item_qty = item.row.quantity;
...
//                             item_price = item_price + (parseFloat(this.price));
...
//                     }); 
                        ...
                        $.each(optionArr, function (k, opt) {
                            ...
                            if (this_options.id == opt) {     
                               ...
                               if (this_options.price != 0 && this_options.price != '' && this_options.price != null) {
                                    ...
                                    item_price = formatDecimal(parseFloat(item_price) + parseFloat(this_options.price), 6);
                                    unit_price = item_price;
                ...
                product_discount += formatDecimal((item_discount * a_item_qty), 4); //item_qty
                ...
                if (site.settings.tax1 == 1) {
                    if (pr_tax !== false) {
                        ...
                        product_tax += pr_tax_val * a_item_qty; //item_qty
                    }
                }
                ...
                tr_html = '<td><input name="sale_item_id[]" type="hidden" class="rsiid" value="' + sale_item_id + '"><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><input name="product_option_color[]" type="hidden" class="roption_color" value="' + item_option_color + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+ (item_option_color_name ? ' (' + item_option_color_name + ')' : '') + '</span></td>';
                ...
                tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(show_unit_price ) + '</span></td>';
                ...
                tr_html +='<td class="text-right">'+formatMoney(parseFloat(unit_price)  * parseFloat(a_item_qty) )+'</td>';
                if (site.settings.product_discount == 1) {
                    tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + item_ds + '"><span class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">' + formatMoney(0 - (item_discount * a_item_qty)) + '</span></td>';
                }
                if (site.settings.tax1 == 1) {
                    tr_html += '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + pr_tax.id + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (pr_tax_rate ? '(' + pr_tax_rate + ')' : '') + ' ' + formatMoney(pr_tax_val * a_item_qty) + '</span></td>';
                }
                tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(a_item_qty))) + '</span></td>';
                ...
                total += parseFloat((item_price + parseFloat(pr_tax_val)) * parseFloat(a_item_qty));
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-minus-circle"></i><?= lang('return_order'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("orders/return_order/" . $inv->id, $attrib)
                ?>
