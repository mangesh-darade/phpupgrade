                        ...
                        <div id="search_product">
                            ...
                            <div class="col-md-12">
                                ...
                                <div  class="from-group">
                                    ...
                                    <?php echo form_submit('add_transfer', $this->lang->line("submit"), 'id="add_transfer" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;" onclick="submit_form()"'); ?>
...
function product_option_model_call(product) {
    ...
    var product_options = '<table class="table table-striped table-border"><thead><tr><th>Variant Name</th><th>Quantity</th><th>Net Cost</th><th>Subtotal (INR)</th></tr></thead><tbody>';
    var optionsArr = [];
    if (product.options) { optionsArr = Object.keys(product.options).map(function(k){ return product.options[k]; }); }
    optionsArr.sort(function(a, b){
        var am = (a.manage_variant_pk && parseInt(a.manage_variant_pk)) ? parseInt(a.manage_variant_pk) : 1000000000;
        var bm = (b.manage_variant_pk && parseInt(b.manage_variant_pk)) ? parseInt(b.manage_variant_pk) : 1000000000;
        if (am !== bm) { return am - bm; }
        var an = (a.name || '').toString();
        var bn = (b.name || '').toString();
        var ap = an.split('/').map(function(p){ var v = parseFloat(p); return isNaN(v)? Number.MAX_SAFE_INTEGER : v; });
        var bp = bn.split('/').map(function(p){ var v = parseFloat(p); return isNaN(v)? Number.MAX_SAFE_INTEGER : v; });
        var len = Math.min(ap.length, bp.length);
        for (var i=0;i<len;i++){ if (ap[i]!==bp[i]) return ap[i]-bp[i]; }
        return an.localeCompare(bn);
    });
    $.each(optionsArr, function(index, element) {
        ...
        if (element.name.toLowerCase() == 'note') {
            ...
            variantRow = '<tr>' +
                ...
                '<td class="w25-right"><input type="text" min="0" value="' + formatMoney(0) + '" class="form-control input-sm  subtotals width-setting" data-variant-id="' + element.id + '" id="subtotals"></td>' +
                '</tr>';
        }
    ...
    if (ColorOption) { modalTitle += '<br>Color: ' + ColorOption; }
...
$(document).on('change', '.quantity_input', function() {
    ...
    $(this).closest('tr').find('.subtotals').val(formatMoney(netCost));
...
$(document).on('change', '.net-cost-input', function() {
    ...
    $(this).closest('tr').find('.subtotals').val(formatMoney(subtotal));
});
$(document).on('change', '.subtotals', function() {
    ...
    row.find('.net-cost-input').val(netCost);
});
$('#submitBtn').on('click', function() {
    $('.quantity-input').each(function(index) {
        setTimeout(function() {
            ...
            var netCostText = $this.closest('tr').find('.net-cost').text();
            var updatedCost = netCostText.replace(/[^0-9.-]+/g, "");
            var inputValue = $this.closest('tr').find('.subtotals').val();
...
function addProductToVarientProduct(option_id, option_name,unitcost) {
    var Product_color_el = $('#Product_color');
    var Product_color = Product_color_el.length ? Product_color_el.val() : '';
    $('.color-form-group').removeClass('has-error');
    if (Product_color_el.length && Product_color == '0') {
        $('.color-form-group').addClass('has-error');
        return false;
    }
    var note = '';
    if (typeof option_name === 'string' && option_name.toLowerCase() == 'note') {
    ...
    var term = $(".modalvarient").find('.product_term').val() + "<?= $this->Settings->barcode_separator; ?>" + option_id + "<?= $this->Settings->barcode_separator; ?>" + Product_color;
    var wh = $('#from_warehouse').val();
    var wh2 = $("#to_warehouse").val();
    $.ajax({
        type: "get",
        url: "<?= site_url('transfers/suggestions') ?>",
        data: {
...
function modalClose(modalClass) { $('.' + modalClass).hide(); }
$('.modalvarient').hide();
</script>
