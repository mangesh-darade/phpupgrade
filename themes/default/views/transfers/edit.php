...
<?php /* Seed cart before footer (transfers.js); avoids painting previous transfer from localStorage */ ?>
<?php if (!empty($transfer) && isset($transfer_items)) { ?>
<script type="text/javascript">
(function () {
    try {
        localStorage.setItem('toitems_transfer_id', '<?= (int) $transfer->id ?>');
        localStorage.setItem('toitems', <?= json_encode($transfer_items) ?>);
...
<?php if ($transfer) { ?>
            ...
            // Avoid JSON.stringify(embedded PHP): use JSON string from server (handles escaping, UTF-8, edge cases)
            localStorage.setItem('toitems', <?= json_encode($transfer_items) ?>);
<?php } ?>
...
<?php if ($transfer) { ?>
        // transfers.js may call loadItems() before this block runs; run once after localStorage is seeded for THIS transfer
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                ?>
                <input type="hidden" name="pageAction" id="pageAction" value="edit_transfer">
                <input type="hidden" id="transfer_server_id" value="<?= (int) $transfer->id ?>">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                if ($transfer->status == 'request') {
                                    $post = ['request' => lang('Request'), 'partial' => lang('Partial Sent'), 'sent' => lang('Sent All')];
                                }
                                elseif ($transfer->status == 'partial' ) {
                                    $post = ['partial' => lang('Partial sent'), 'partial' => lang('Partial sent'), 'partial_completed' => lang('Partial Completed'), 'sent_balance' => lang('Sent Balance')];
                                }
                                elseif ($transfer->status == 'partial_completed') {
                                    $post = ['partial_completed' => lang('Partial Completed'), 'partial' => lang('Partial sent'), 'partial_completed' => lang('Partial Completed'), 'sent_balance' => lang('Sent Balance')];
                                }
                                elseif ($transfer->status == 'sent_balance') {
                                    $post = ['sent_balance' => lang('Sent Balance'), 'partial_completed' => lang('Partial Completed'), 'completed' => lang(' Completed')];
                                }
                                ...
                                <div class="panel-body" style="padding: 5px;">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            // Variant PO / supplier location may not be in user's warehouse list — still show on edit
                                            if (!empty($transfer->to_warehouse_id) && !isset($whto[$transfer->to_warehouse_id])) {
                                                $whto[$transfer->to_warehouse_id] = !empty($transfer->to_warehouse_name)
                                        ...
                                        <table id="toTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
                                            <thead>
                                                <tr>
                                                    ...
                                                    ?>
                                                    <th class="col-md-1"><?= lang("net_unit_cost"); ?></th>                                                    
                                                    <?php if ($transfer->status == 'request' || $transfer->status == 'partial' || $transfer->status == 'partial_completed' || $transfer->status == 'sent_balance') { ?>
    ...
    $(document).ready(function () {
        ...
        setTimeout(function(){
            ...
            if(tostatus !== 'edit_pending_request') {
                ...
                $('#display_product').attr('readonly', true);
...
$(document).on('change', '.quantity_input', function() {
    ...
    $(this).closest('tr').find('.net-cost').text(formatMoney(netCosts));
});
