    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("transfers/jw_edit_request/" . $transfer_request->id, $attrib);
                $wh[''] = '';
                                    ...
                                    <table id="" class="table table-bordered table-condensed table-hover table-striped dataTable">
                                        <thead>
                                            <tr>
                                                ...
                                                <?php
                                                // Expiry date column hidden for JW edit transfer
                                                // if ($Settings->product_expiry) {
                                                //     echo '<th class="col-md-2">' . $this->lang->line("expiry_date") . '</th>';
                                                // }
                                                ?>
                                                <!-- Net unit cost column hidden for JW edit transfer -->
                                        ...
                                        <table id="toTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="col-md-1"><?= lang("Vendor Quantity") ?><span id="stock_2"></span></th> 
                                                    <!-- <?php
                                                    if ($Settings->product_expiry) {
                                                        // echo '<th class="col-md-2">' . $this->lang->line("expiry_date") . '</th>';
                                                    }
...
//            }    
    ...
    //                                            pass_variant= passvariant ;  
                            ...
                            // Hidden fields for form submission - not displayed but required for backend processing
                            htmlset += '<input class="form-control input-sm text-right rcost allcheck" name="net_cost[]" disabled="ture" type="hidden" id="net_cost_' + result[i].item.id + '' + variant_id + '" value="' + item_cost + '">';
                            ...
                            htmlset += '<input class="realucost allcheck" name="real_unit_cost[]" disabled="ture" id="real_unit_cost_' + result[i].item.id + '' + variant_id + '"  type="hidden" value="' + result[i].item.cost + '">';
                            ...
                            <?php if ($Settings->tax1) { ?>
                            htmlset += '<td class="text-right">(' + formatDecimal(result[i].item.rate) + '%)<br/><span id="total_tax_' + result[i].item.id + '' + variant_id + '">' + formatMoney(product_tax) + '</span></td>';
                                <?php } else { ?>
                                    htmlset += '<td class="text-right"></td>';
                                <?php } ?>
                    ...
                    // ENFORCE: Disable ALL DataTables alerts permanently
