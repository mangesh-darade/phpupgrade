...
<style>
    ...
    thead th:nth-child(1) .yadcf-filter-wrapper input
    {
    text-align: left;
    ...
    $(document).ready(function () {
        ...
        var oTable = $('#SlRData').dataTable({
            ...
            "aoColumns": [
                ...
                { "bSearchable": false, "mRender": formatQuantity},
                { "bSearchable": false, "mRender": formatQuantity},
                {"mRender": currencyFormat,  "bSearchable": false},
                {"mRender": currencyFormat,  "bSearchable": false},
                 { "bSearchable": false},
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                function parseMoney(val) {
                    ...
                    // Convert to string & remove everything except digits, minus & dot
                    var num = val.toString()
                        .replace(/[^0-9.-]/g, '');

                    // Prevent invalid parse
                ...
                for (var i = 0; i < aiDisplay.length; i++) {
                    ...
                    tot_sales_amount += parseMoney(aaData[aiDisplay[i]][2]);
                    tot_total_sales  += parseFloat(aaData[aiDisplay[i]][3]) || 0;
                    tot_items        += parseFloat(aaData[aiDisplay[i]][4]) || 0;
                    tot_discount     += parseMoney(aaData[aiDisplay[i]][5]);
                    tot_tax          += parseMoney(aaData[aiDisplay[i]][6]);
                }

                var nCells = nRow.getElementsByTagName('th');

                nCells[1].innerHTML = '<div style="text-align:right;">' + currencyFormat(tot_sales_amount) + '</div>';
                nCells[2].innerHTML = '<div style="text-align:center;">' + formatQuantity(tot_total_sales) + '</div>';
                nCells[3].innerHTML = '<div style="text-align:center;">' + formatQuantity(tot_items) + '</div>';
                nCells[4].innerHTML = '<div style="text-align:right;">' + currencyFormat(tot_discount) + '</div>';
                nCells[5].innerHTML = '<div style="text-align:right;">' + currencyFormat(tot_tax) + '</div>';
            }
        });
    });
</script>
