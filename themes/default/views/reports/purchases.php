...
$(document).ready(function () {
    ...
    var columns = [
        ...
        null,               // 2 - Warehouse
        null,                // 3 - Supplier
        {"mRender": pqFormat},          // Product + Qty (iname)
    ];

    if (show_batch) {
        columns.push(null); // 4 - Batch No
    }

    columns.push(
        {"mRender": currencyFormat},    // Grand Total
        {"mRender": currencyFormat},    // Paid
        {"mRender": currencyFormat},    // Balance
        {"mRender": row_status}         // Status
    );
    ...
    var oTable = $('#PoRData').dataTable({
        ...
        "fnRowCallback": function (nRow, aData) {
            ...
            var grandTotalIndex = show_batch ? 6 : 5;
            nRow.className = (parseFloat(aData[grandTotalIndex]) > 0) ? "purchase_link2" : "purchase_link2 warning";
            return nRow;
        },
        "aoColumns": columns,
        "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
            ...
            for (var i = 0; i < aaData.length; i++) {
                var gIndex = show_batch ? 6 : 5;
                var pIndex = show_batch ? 7 : 6;
                var bIndex = show_batch ? 8 : 7;

                gtotal += parseFloat(aaData[aiDisplay[i]][gIndex]);
                paid += parseFloat(aaData[aiDisplay[i]][pIndex]);
                balance += parseFloat(aaData[aiDisplay[i]][bIndex]);
            }

            var nCells = nRow.getElementsByTagName("th");
            var gt = show_batch ? 6 : 5;
            nCells[gt].innerHTML = currencyFormat(gtotal);
            nCells[gt + 1].innerHTML = currencyFormat(paid);
            nCells[gt + 2].innerHTML = currencyFormat(balance);
        }
    }).fnSetFilteringDelay().dtFilter([
        {column_number: 0, filter_default_label: "[<?=lang('date');?> (yyyy-mm-dd)]", filter_type: "text"},
        {column_number: 1, filter_default_label: "[<?=lang('ref_no');?>]", filter_type: "text"},
        {column_number: 2, filter_default_label: "[<?=lang('warehouse');?>]", filter_type: "text"},
        {column_number: 3, filter_default_label: "[<?=lang('supplier');?>]", filter_type: "text"},
        {column_number: show_batch ? 9 : 8, filter_default_label: "[<?=lang('status');?>]", filter_type: "text"}
    ], "footer");
});
</script>
    ...
    <div class="box-header">
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                ...
                <li class="dropdown"><a href="<?=site_url('reports/view_image_format/?v=1'.$v)?>" id="image" class="tip" title="<?= lang('save_image') ?>"><i
                ...
                <div id="form">
                    ...
                    <div class="form-group">
                        <div class="controls">
                            ...
                            <!--<input type="reset" id="report_reset" data-value="<?=base_url('reports/purchases');?>" name="submit_report" value="Reset" class="btn btn-warning input-xs">-->
