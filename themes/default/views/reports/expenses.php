...
<script>
    $(document).ready(function () {
        ...
        var oTable = $('#EXPData').dataTable({
            ...
            "aoColumns": [{"mRender": fld}, null, null, null, {"mRender": currencyFormat}, null, null, null, {"bSortable": false, "mRender": attachment}],
            ...
            {column_number: 3, filter_default_label: "[<?=ucfirst(lang('subcategory'));?>]", filter_type: "text", data: []},
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        function resetSubcategoryToCategoryPrompt() {
            $("#subcategory").select2("destroy")
                .html('<option value="">Please select category to load</option>')
                .val('')
                .select2({
                    allowClear: true,
        ...
        function bindSubcategories(scdata) {
            var options = '<option value=""><?= lang('select_subcategory') ?></option>';
            $.each(scdata, function (_, row) {
        ...
        resetSubcategoryToCategoryPrompt();
        $('#category').change(function () {
            ...
            if (v) {
                ...
                $.ajax({
                    type: "get",
                    url: "<?= site_url('reports/getExpenseSubCategories') ?>/" + v,
                    dataType: "json",
                    success: function (scdata) {
                        if ($.isArray(scdata) && scdata.length) {
                            bindSubcategories(scdata);
                        } else {
        ...
        $.ajax({
            type: "get",
            url: "<?= site_url('reports/getExpenseSubCategories') ?>/" + <?= $_POST['category'] ?>,
            dataType: "json",
            success: function (scdata) {
                if ($.isArray(scdata) && scdata.length) {
                    bindSubcategories(scdata);
                    <?php if (isset($_POST['subcategory']) && !empty($_POST['subcategory'])) { ?>
                    $("#subcategory").val("<?= $_POST['subcategory'] ?>").trigger('change');
                    <?php } ?>
                }
            }
        });
            ...
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="subcategory"><?= ucfirst(lang("subcategory")); ?></label>
                                <?php
                                $sct[""] = "Please select category to load";
                                echo form_dropdown('subcategory', $sct, (isset($_POST['subcategory']) ? $_POST['subcategory'] : ""), 'class="form-control" id="subcategory" data-placeholder="Please select category to load"');
                                ?>
                            </div>
                        </div>
                        <div class="col-sm-4">
                ...
                <div class="table-responsive">
                    <table id="EXPData" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <tr class="active">
                            ...
                            <th class="col-xs-2"><?= ucfirst(lang("subcategory")); ?></th>
