...
$(document).ready(function () {
    // ✅ Batch column should show if setting == 1 OR 2
    var showBatch = <?= (($Settings->product_batch_setting == 1 || $Settings->product_batch_setting == 2) ? 'true' : 'false') ?>;
    ...
    // ✅ Column order must match table header order
    var aoColumns = [
        {"mRender": fld},        // Date
        null,                    // Reference No
        null,                    // Warehouse
        null,                    // Created By
        {"mRender": decode_html},// Note
        {"bSortable": false, "mRender": pqFormat}, // Products
        {"bSortable": false, "mRender": pqFormat}  // Variant Name
    ];

    // ✅ Add batch column only if batch setting is enabled
    if (showBatch) {
        aoColumns.push({"bSortable": false, "mRender": pqFormat}); // Batch No
    }
    ...
    oTable = $('#dmpData').dataTable({
        ...
        'bProcessing': true,
        'bServerSide': true,
        'sAjaxSource': '<?= site_url('reports/getAdjustmentReport/?v=' .$permisions_werehouse[0]. $v); ?>',
        ...
        'fnRowCallback': function (nRow, aData) {
            ...
            nRow.id = aData[aData.length - 1]; // ✅ sets ID from last column
            return nRow;
        },
        ...
        {column_number: 3, filter_default_label: "[<?= lang('created_by'); ?>]", filter_type: "text"},
        {column_number: 4, filter_default_label: "[<?= lang('note'); ?>]", filter_type: "text"}
        // ✅ No need for filter for products/variant/batch
    ], "footer");
});
</script>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <div class="form-group">
                        <div class="controls"> <?php echo form_submit('submit_report', $this->lang->line("submit"), 'class="btn btn-primary"'); ?>
                        <a href="reports/restbutton" class="btn btn-success">Reset</a> </div>
                        </div>
                    </div>
                    <?php echo form_close(); ?>
                ...
                <div class="table-responsive">
                    <table id="dmpData" class="table table-bordered table-condensed table-hover table-striped">
                        ...
                        <tr>
                            ...
                            <th class="col-xs-2"><?= lang('Variant_Name'); ?></th>
                            <?php if ($Settings->product_batch_setting == 1 || $Settings->product_batch_setting == 2) { ?>
                                <th class="col-xs-2"><?= lang('Batch_No.'); ?></th>
                            <?php } ?>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        <tr class="active">
                            ...
                            <th class="col-xs-2"><?= lang('products'); ?></th>
                            <th class="col-xs-2"><?= lang('Varient_Name'); ?></th>
                            <?php if ($Settings->product_batch_setting == 1 || $Settings->product_batch_setting == 2) { ?>
                                <th class="col-xs-2"><?= lang('Batch_No.'); ?></th>
                            <?php } ?>
                        </tr>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#image').click(function (event) {
            event.preventDefault();
			window.location.href = "<?=site_url('reports/getAdjustmentReport/0/0/img/?v=1'.$v)?>";
            /*html2canvas($('.box'), {
