...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        ...
        <?php
        $attrib = array('data-toggle' => 'validator', 'role' => 'form', 'id' => 'edit-warehouse-form');
            ...
            <div class="row">
                <div class="col-sm-6">
                    ...
                    <div class="form-group">
                        ...
                        $ct['other'] = 'Other';
                        $selected_country = isset($_POST['country']) ? $_POST['country'] : ($warehouse->country ? $warehouse->country : 'India');
                    ...
                    <div class="form-group">
                        ...
                        if ($selected_state === '' && !empty($warehouse->state) && !empty($warehouse->state_code)) {
                            $selected_state = $warehouse->state . '~' . $warehouse->state_code;
                        }
                ...
                <div class="col-sm-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="email" value="<?= isset($warehouse->email) ? htmlspecialchars($warehouse->email, ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-control" id="email" title="<?= htmlspecialchars(lang('email_address'), ENT_QUOTES, 'UTF-8'); ?>" />
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="location_radius_m" id="location_radius_m" class="form-control" min="10" step="1" value="<?= isset($warehouse->location_radius_m) && (int) $warehouse->location_radius_m > 0 ? (int) $warehouse->location_radius_m : 100; ?>" title="Geofence for attendance check-in/out">
                    </div>
                    <div class="form-group">
                        <label class="control-label" for="latitude">Latitude</label>
                        <?php echo form_input('latitude', isset($warehouse->latitude) ? $warehouse->latitude : '', 'class="form-control" id="latitude" placeholder="e.g. 22.7196"'); ?>
                    </div>
                    <div class="form-group">
                        <label class="control-label" for="longitude">Longitude</label>
                        <div style="display:flex; gap:8px;">
                            <?php echo form_input('longitude', isset($warehouse->longitude) ? $warehouse->longitude : '', 'class="form-control" id="longitude" placeholder="e.g. 75.8577"'); ?>
                    ...
                    <div class="form-group">
                        ...
                        <?php echo form_input('state_code', $warehouse->state_code, 'class="form-control" readonly="readonly" id="state_code"'); ?>
                    </div>
                        ...
                        <?php
                            ...
                            if (isset($warehouses_list) && !empty($warehouses_list)) {
                                foreach ($warehouses_list as $warehouse_item) {
                                    // Exclude current warehouse from the list
                                    if ($warehouse_item->id != $warehouse->id) {
                                        $warehouse_list[$warehouse_item->id] = $warehouse_item->name;
                                    }
                                }
                            }
                            $selected_workstations = array();
                            if (isset($warehouse->workstations) && !empty($warehouse->workstations)) {
                                $selected_workstations = explode(',', $warehouse->workstations);
                            }
                            echo form_dropdown('workstations[]', $warehouse_list, $selected_workstations, 'class="form-control tip select" id="workstations" multiple="multiple" style="width:100%;"');
                        ?>
                    </div>
...
$(document).ready(function() {
    var warehouseCountryPhoneMeta = <?= json_encode($warehouse_country_phone_meta, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT); ?>;

    function parseCountryPhoneMeta() {
        return warehouseCountryPhoneMeta || null;
    }
    ...
    function syncEditWarehouseSubmitState() {
        var phoneOk = validateWarehousePhone(false);
        var emailOk = validateWarehouseEmail(false);
        var $submitBtn = $('#edit-warehouse-form').find('button[type="submit"], input[type="submit"]');
        $submitBtn.prop('disabled', !(phoneOk && emailOk));
    }
    ...
    function getStates(country, selectedState) {
        $.ajax({
            type: 'ajax',
            dataType: 'json',
            method: 'get',
    ...
    setTimeout(function() {
        ...
        var initialState = <?= json_encode(isset($_POST['state']) ? $_POST['state'] : (!empty($warehouse->state) && !empty($warehouse->state_code) ? $warehouse->state . "~" . $warehouse->state_code : "")); ?>;
        if (initialCountry && initialCountry !== 'other') {
            getStates(initialCountry, initialState);
        } else {
