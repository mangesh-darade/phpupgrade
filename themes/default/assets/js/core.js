...
jQuery(document).ready(function (e) {
    ...
    e(".dropmenu").click(function (t) {
        t.preventDefault();
        var $a = e(this), $sub = $a.parent().find("ul").first(), $chev = $a.find(".chevron");
        if (!$sub.length) return;
        if (e("#sidebar-left").hasClass("minified") && $a.hasClass("open")) return;

        var wasOpen = $sub.is(":visible") || ($sub[0] && $sub[0].style.display === "block");
        if ($sub[0].style.display) $sub[0].style.display = "";

        if (wasOpen) {
            $sub.stop(true, true).slideUp();
            $a.removeClass("open");
            $chev.removeClass("opened").addClass("closed");
        } else {
            $sub.stop(true, true).slideDown();
            $chev.removeClass("closed").addClass("opened");
        }
    });
...
function singlecellmultirecord(x, format = '') {
    if (x == null) return format === '' ? '<div class="text-right">0</div>' : '0';

    // Detect if multiple values separated by <br> or newline
    var separator = x.includes('<br>') ? '<br>' : (x.includes('\n') ? '\n' : null);

    if (separator) {
        var parts = x.split(separator).map(function (part) {
            part = part.trim();
            return part ? formatMoney(part) : '0';
        });
        var joined = parts.join(separator);
        return format === '' ? '<div class="text-right">' + joined + '</div>' : joined;
    } else {
        return format === '' ? '<div class="text-right">' + formatMoney(x) + '</div>' : formatMoney(x);
    }
}
...
function img_hl(x) {
    ...
    return '<div class="text-center"><a href="' + site.base_url + 'assets/mdata/'+ subdomain + '/uploads/' + image_link + '" data-toggle="lightbox"><img src="' + site.base_url + 'assets/mdata/'+ subdomain + '/uploads/thumbs/' + image_link + '" alt="" style="width:30px; height:30px;" /></a></div>';
}
...
$(document).ready(function () {
    ...
    /**
     * Row click handler for product list screens
     * Opens modal_view when clicking on row cells (except excluded columns)
     * 
     * EXCLUDED COLUMNS:
     * - :first-child (checkbox column)
     * - :nth-child(2) (image column)
     * - :nth-child(5) (name column - has hyperlink, should navigate directly)
     * - :last-child (action column)
     * 
     * This allows product name link to work independently without triggering modal
     */
    $('body').on('click', '.product_link td:not(:first-child, :nth-child(2), :nth-child(5), :last-child)', function () {
