  ...
  */
    ...
    $("#reset").click(function (e) {
      bootbox.confirm(lang.r_u_sure, function (result) {
        if (result) {
          ...
          if (localStorage.getItem("new_req_qty")) {
            localStorage.removeItem("new_req_qty");
          }
    ...
    $(document)
      ...
      .on("change", ".reqqty", function () {
        ...
        var item_id = row.attr("data-item-id");
        toitems[item_id].row.request_quantity = new_req_qty;
        localStorage.setItem("toitems", JSON.stringify(toitems));
  ...
  $(document).on("change", "#poption", function () {
    ...
    toitems[item_id].row.stockwarehouse2 = qtyw2 =
      Items.options[option_id].quantity2;
  ...
  function loadItems() {
    ...
    if (localStorage.getItem("toitems")) {
      ...
      $.each(sortedItems, function () {
        ...
        var item_request_id = item.row.transfer_request_id
          ? item.row.transfer_request_id
          : false;
        ...
        if (item.options) {
          ...
          $.each(item.options, function () {
            ...
            if (oi == 1) {
              first_opt = this.id;
              first_sel_opt = this.name;
            }
            if (primary_variant == this.id) {
              primary_variant_name = this.name;
            }
          });
        ...
        tr_html =
          ...
          item_name +
          '"><input type="hidden" id="PrItemId_' +
          row_no +
          '" value="' +
          item.item_id +
          '"><input name="product_option[]" type="hidden" class="roption " id="ItemOption_' +
          row_no +
          '" value="' +
          item_option +
        ...
        tr_html +=
          '<td  class="text-right stock_2_' +
          row_no +
          '">' +
          formatDecimal(getstock_2) +
          "</td>";
        // Hidden expiry field for form submission
        if (site.settings.product_expiry == 1) {
            tr_html += '<input class="form-control date rexpiry" name="expiry[]" type="hidden" value="' + item_expiry + '" data-id="' + row_no + '" data-item="' + item_id + '" id="expiry_' + row_no + '">';
        }
        // Hidden cost fields for form submission - required by backend
        tr_html += '<input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + formatDecimal(item_cost) + '">';
        tr_html += '<input class="rucost" name="unit_cost[]" type="hidden" value="' + unit_cost + '">';
        tr_html += '<input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_cost + '">';
  
        var qty_readonly = "",
          req_readonly = "";
        if (page_action == "edit_request") {
          req_readonly =
            tostatus == "edit_pending_request" ? "" : 'readonly="readonly" ';
          qty_readonly =
            tostatus == "edit_pending_request" ? 'readonly="readonly" ' : "";
  
          item_bqty = item_bqty == 0 ? item_reqqty - item_sqty : item_bqty;
          tr_html +=
            '<td><input name="transfer_request_id[]" type="hidden" value="' +
            item_request_id +
            '"><input name="request_quantity[]" type="text" class="form-control text-center reqqty" value="' +
            formatDecimal(item_reqqty ? item_reqqty : 1, 2) +
            '"  data-id="' +
            row_no +
            '" data-item="' +
            item_id +
            '" id="request_quantity_' +
            row_no +
            '" onClick="this.select();" ' +
            req_readonly +
            " ></td>";
          tr_html +=
            '<td><input name="sent_quantity[]" type="text" class="form-control text-center sqty" value="' +
            formatDecimal(item_sqty, 2) +
            '"  data-id="' +
            row_no +
            '" data-item="' +
            item_id +
            '" id="sent_quantity_' +
            row_no +
            '" onClick="this.select();" readonly="readonly" ></td>';
          tr_html +=
            '<td><input name="quantity_balance[]" type="text" class="form-control text-center rbqty" value="' +
            formatDecimal(item_bqty, 2) +
            '"  data-id="' +
            row_no +
            '" data-item="' +
            item_id +
            '" id="quantity_balance_' +
            row_no +
            '" onClick="this.select();" readonly="readonly"  ></td>';
          var pqty = item_bqty;
        } else {
        ...
        tr_html +=
          ...
          '">' +
          '<input name="quantity[]" type="text" class="form-control text-center rquantity" tabindex="' +
        ...
        } else if (base_quantity > item_aqty && base_quantity > 0) {
      ...
      var tfoot =
        ...
        formatNumber(parseFloat(count) - 1) +
        "</th>";
