...
// save and load the fields in and/or from localStorage
    ...
    $(document).on('change', '.rbtach_no', function () {
        ...
        // If batch is cleared, also clear expiry and related batch info in poitems
        if (!batch) {
            poitems[item_id].row.batch_number = '';
            poitems[item_id].row.batch = '';
            poitems[item_id].row.expiry = '';
            poitems[item_id].row.select_batch = '';
            localStorage.setItem('poitems', JSON.stringify(poitems));
            loadItems();
            return;
        }
...
// Order discount calcuation
    ...
    $(document).on('click', '.edit', function () {
        ...
        if (!item && poitems) {
            // Some server environments serialize keys differently (string/number); fallback scan.
            $.each(poitems, function (key, value) {
                if (item) {
                    return false;
                }
                var value_item_id = value && (value.item_id || value.id);
                if ((value_item_id + '') === (item_id + '')) {
                    item = value;
                    item_id = key;
                }
            });
...
//                        }
        ...
        if (item.options_color !== false && item.options_color) {
            var o = 1;
    ...
    $(document).on('click', '#editItem', function () {
        ...
        var row = $('#' + $('#row_id').val());
        var item_id = row.attr('data-item-id');
        var hidden_item_id = $('#item_id').val();
        var new_pr_tax = $('#ptax').val(), new_pr_tax_rate = {};

        if (!item_id && hidden_item_id) {
            item_id = hidden_item_id;
        }
        if ((!poitems[item_id] || !poitems[item_id].row) && poitems) {
            // Fallback: recover the keyed entry if ids changed type/stringification.
            $.each(poitems, function (key, value) {
                if (poitems[item_id] && poitems[item_id].row) {
                    return false;
                }
                var value_item_id = value && (value.item_id || value.id);
                if ((value_item_id + '') === (item_id + '') || (key + '') === (item_id + '')) {
                    item_id = key;
                }
            });
        }
...
function updateBatchNumber(input) {
    ...
    if (poitems[item_id]) {
        poitems[item_id].row.batch_number = batch_number;
        localStorage.setItem('poitems', JSON.stringify(poitems));
    }
}
