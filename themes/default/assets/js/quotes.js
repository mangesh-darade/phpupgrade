    ...
    if (localStorage.getItem('quitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            if (!unit_price || isNaN(parseFloat(unit_price)) || parseFloat(unit_price) <= 0) {
                unit_price = (item.row.unit_price && !isNaN(parseFloat(item.row.unit_price)) && parseFloat(item.row.unit_price) > 0)
                    ? parseFloat(item.row.unit_price)
                    : ((item.row.base_unit_price && !isNaN(parseFloat(item.row.base_unit_price)) && parseFloat(item.row.base_unit_price) > 0)
                        ? parseFloat(item.row.base_unit_price)
                        : 0);
            }
            var unit_lable = item.row.unit_lable;
            var mrp = item.row.mrp;
            if (!mrp || isNaN(parseFloat(mrp)) || parseFloat(mrp) <= 0) {
                // Prefer real_unit_price (base), then price, then unit_price
                if (item.row.real_unit_price && !isNaN(parseFloat(item.row.real_unit_price)) && parseFloat(item.row.real_unit_price) > 0) {
                    mrp = parseFloat(item.row.real_unit_price);
                } else if (item.row.price && !isNaN(parseFloat(item.row.price)) && parseFloat(item.row.price) > 0) {
                    mrp = parseFloat(item.row.price);
                } else {
                    mrp = unit_price;
                }
            }

            // Ensure item_price has a sensible fallback for edit screen
            if (!item_price || isNaN(parseFloat(item_price)) || parseFloat(item_price) <= 0) {
            ...
            // Handle color variants with strong preference for explicit name
            var sel_opt_color = '';
            if (item.row && item.row.option_color_name) {
                sel_opt_color = item.row.option_color_name;
            } else if (item.selected_color && item.selected_color.name) {
                sel_opt_color = item.selected_color.name;
            } else if (item.options_color && item.row && item.row.option_color) {
                $.each(item.options_color, function () {
                    if (String(this.id) == String(item.row.option_color)) {
                        sel_opt_color = this.name;
                    }
                });
            }

            // Final display color name
            var display_color = '';
            if (item.row && item.row.option_color_name) {
                display_color = item.row.option_color_name;
            } else if (item.row && item.row.shade_name) {
                display_color = item.row.shade_name;
            } else if (sel_opt_color) {
                display_color = sel_opt_color;
            } else if (item.color_label) {
                // Fallback: extract name from a label like "Color: Red"
                var lbl = String(item.color_label);
                var idx = lbl.indexOf(':');
                display_color = idx !== -1 ? lbl.substring(idx + 1).trim() : lbl;
            } else if (item.options_color && item.options_color.length === 1) {
                // Display-only fallback: single available color
                display_color = item.options_color[0].name;
            }

            // Build variant display string
            var variant_display = '';
            if (sel_opt) variant_display += ' (' + sel_opt + ')';
            if (sel_opt_color) variant_display += ' (' + sel_opt_color + ')';
            ...
            // Ensure displayed unit price is not zero for variant-priced items
            var show_item_cost_num = (item.row.real_unit_price && !isNaN(parseFloat(item.row.real_unit_price))) ? parseFloat(item.row.real_unit_price) : 0;
            if (!show_item_cost_num || show_item_cost_num === 0) {
                // Fallback to computed item_price (variant-adjusted, pre-tax for inclusive pricing)
                show_item_cost_num = parseFloat(item_price);
            }
            var show_item_cost = formatDecimal(show_item_cost_num, 4);
            mrp = formatDecimal(mrp, 4);
            
            var row_no = (new Date).getTime();
            var newTr = $('<tr id="row_' + row_no + '" class="row_' + item_id + '" data-item-id="' + item_id + '"></tr>');
            
            // Build table row HTML with modified variant display
            tr_html = '<td>';
            if (site.settings.quotation_image == 1) {
                var subdomain = new URL(window.location.href).hostname.split('.')[0];
                tr_html += '<img src="assets/mdata/'+ subdomain + '/uploads/thumbs/'+product_img+'" alt="'+product_img+'" style="width:30px; height:30px;" /> ';
            }
            
            // Inside your loadItems() function where you build each table row:

            // Replace the product name display section with this:
            tr_html += '<input name="product_id[]" type="hidden" class="rid" value="' + product_id + '">' +
                '<input name="hsn_code[]" type="hidden" class="rid" value="' + hsn_code + '">' +
                ...
                '<input name="product_option[]" type="hidden" class="roption" value="' + item_option + '">' +
                 '<input name="product_option_color[]" type="hidden" class="roption_color" value="' + ((item.row.option_color && item.row.option_color !== '0') ? item.row.option_color : '') + '">' +
                '<span class="sname" id="name_' + row_no + '">' + 
                    item_code + ' - ' + item_name + 
                    (sel_opt != '' ? ' (' + sel_opt + ')' : '') + 
                    (display_color != '' ? ' (' + display_color + ')' : '') + 
                '</span> ' + 
                '<i class="pull-right fa fa-edit tip pointer edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';


            
            // Rest of the table columns (unchanged)
