...
// Quantity decimal formatter using qty_decimals setting (may add thousand separators — use for display only)
...
// Safe numeric string for type="number" inputs — NO thousand separators (commas break number inputs)
function toRawDecimal(x) {
    var n = parseFloat(x);
    if (isNaN(n)) return '';
    var decimals = (site && site.settings && site.settings.qty_decimals != null) ? parseInt(site.settings.qty_decimals) : 2;
    return n.toFixed(decimals);
}
var buttonText = '';
var buttonText_changed = '';
    // Universal fix for workstation dropdown placeholder
    function ensureWorkstationPlaceholder() {
        const $workstationSelect = $('#workstation_select');
        if ($workstationSelect.length && $workstationSelect.val() === '') {
            $workstationSelect.val('');
            $workstationSelect.find('option:first').prop('selected', true);
        }
    }

    // Run the fix whenever the edit section becomes visible
    $(document).on('click', '.edit-icon', function() {
        setTimeout(ensureWorkstationPlaceholder, 200);
    });
$(document).ready(function () {
    // Always start with a clean slate
    localStorage.removeItem('addBomItemClicked');
    localStorage.removeItem('addBomItemClickTime');
    $('.right_section').removeClass('is-visible');
    ...
    $(document).on('click', '.subcategory', function () {
        ...
        $(document).ready(function () {
            ...
            $('#sale_unit').val(toRawDecimal(1.00));
            $('#sale_unit_uom').val('');
            $('#uom_select').val('');
            $('#min_batch_qty').val(toRawDecimal(1.00));
    ...
    $(document).on('click', '.edit-icon', async function (e) {
        ...
        localStorage.setItem('addBomItemClickTime', new Date().toISOString());
            // $.ajax({
            //         url: site.base_url + "Recipies/getLatestBomVersion",
            ...
            $.ajax({
                url: site.base_url + "Recipies/getBomMaterialItems",
                type: 'POST',
                data: {
                    bom_id: id,
                ...
                success: function (response) {
                    ...
                    // Set workstation dropdown based on saved BOM header
                    if (items.length > 0 && items[0].workstation_id && items[0].workstation_id !== '0' && items[0].workstation_id !== 0) {
                        $('#workstation_select').val(items[0].workstation_id);
                    } else {
                        // Show "Select Workstation" placeholder when no workstation is selected
                        $('#workstation_select').val('');
                        
                        // Force the placeholder to be visible
                        setTimeout(function() {
                            $('#workstation_select').val('');
                            // Ensure the first option (placeholder) is selected
                            $('#workstation_select option:first').prop('selected', true);
                        }, 100);
                    }
                    ...
                    if (items.length > 0) {
                        ...
                        // Set is_batch_only from server response
                        const isBatchOnly = parseInt(items[0].is_batch_only) === 1;
                        $('#is_batch_only_toggle').prop('checked', isBatchOnly);
                        $('#is_batch_only').val(isBatchOnly ? 1 : 0);
                        
                        // Show/hide min qty container based on batch_only setting
                        if (isBatchOnly) {
             ...
             $('.min_batch_qty').val(toRawDecimal(items[0]['min_batch_qty']));
             $('#uom_select').val(items[0].batch_uom || '');
             $('.sale_unit').val(toRawDecimal(items[0]['sales_units'].split(" ")[0]));
    ...
    $('#addBomItemBtn').on('click', function () {
        ...
        // Reset workstation dropdown and version for new recipe
        $('#workstation_select').val('');
        $('#saveRawData').show();                     
        $('#add_note').closest('div').show(); 
        ...
        if (selectedBomProduct.id) {
            ...
            // Fetch the latest version from server (per workstation)
            const workstationId = $('#workstation_select').val();
            $.ajax({
                url: site.base_url + "Recipies/getLatestBomVersion",
                data: { 
    ...
    function getProducts(CategoryID, SubCategoryID) {
        $.ajax({
            url: site.base_url + "Recipies/GetProductsbyCategoriesID",
            method: 'GET',
            data: {
    ...
    function getBomdetailsByProductId(productId, productName) {
        $.ajax({
            url: site.base_url + "Recipies/GetBomDetailsByProductId",
            method: 'GET',
            data: {
                productId: productId,
                    ...
                    if (productName) {
                        ...
                        if (!bom.id || bom.id === null) {
                            ...
                            if (clicked != 'true') {
                                ...
                                $('#created_date').text(new Date().toLocaleDateString());
                                $("#requestDeliveryDate").datepicker("setDate", new Date());
                                // Fetch the latest version to display next version number (per workstation)
                                const workstationId = $('#workstation_select').val();
                                $.ajax({
                                    url: site.base_url + "Recipies/getLatestBomVersion",
                                    data: { 
                        ...
                        if (clicked === 'true') {
                            ...
                            $('#created_date').text(new Date().toLocaleDateString());
                            $("#requestDeliveryDate").datepicker("setDate", new Date());
                            // Show next version number (per workstation)
                            const workstationId = $('#workstation_select').val();
                            $.ajax({
                                url: site.base_url + "Recipies/getLatestBomVersion",
                                data: { 
    ...
    $(document).on('click', '.view-icon', function () {
        ...
        $.ajax({
            ...
            `);
                ...
                const batchUnitName = items[0].batch_unit_name || '';
                $('#min_batch_qty_new').text(quantityDecimal(items[0]['min_batch_qty']) + ' ' + batchUnitName);
                $('#sale_unit_view ').text(quantityDecimal(items[0]['sales_units'].split(" ")[0]) + ' ' + (items[0]['sales_units'].split(" ")[1] || ''));
    ...
    function updateBomUI(bom) {
        ...
        // Restore workstation selection when loading an existing BOM header

        if (bom.workstation_id && bom.workstation_id !== '0' && bom.workstation_id !== 0) {
            $('#workstation_select').val(bom.workstation_id);
        } else {
            // Show "Select Workstation" placeholder when no workstation is selected
            $('#workstation_select').val('');
        ...
        var sale_uom = bom.sales_units ? bom.sales_units.match(/^([\d.]+)\s*(.*)$/) : null;
        if (sale_uom) {
           var qty = sale_uom[1];
           var unit_name = sale_uom[2].trim();
            $('#sale_unit').val(toRawDecimal(qty) || '');
        ...
        // Min Qty
        $('#min_batch_qty').val(toRawDecimal(bom.min_batch_qty || 0));
    }
    ...
    function getRowProducts() {
        $.ajax({
            url: site.base_url + "Recipies/GetRawProducts",
            method: 'GET',
            dataType: 'json',
            success: function (response) {
    ...
    storedItems.forEach((item, index) => {
      ...
      const row = `
        ...
        item.primary_product_id ? "alt-row" : ""
                ...
                <td class="text-end">
                    <input type="number" step="${site && site.settings && site.settings.qty_decimals ? (site.settings.qty_decimals == 0 ? '1' : '0.' + '0'.repeat(site.settings.qty_decimals - 1) + '1') : '0.01'}" name="quantity[]" class="form-control text-end quantity-input" value="${toRawDecimal(
                      item.quantity
                ...
                <td class="text-end">
                    <input type="number" step="${site && site.settings && site.settings.qty_decimals ? (site.settings.qty_decimals == 0 ? '1' : '0.' + '0'.repeat(site.settings.qty_decimals - 1) + '1') : '0.01'}" name="wastage[]" class="form-control text-end wastage-input" value="${toRawDecimal(
    ...
    $('#dynamicTable tbody').on('change', 'input.quantity-input, select.unit-select, input.wastage-input, select.alt-select', function () {
        ...
        const qtyVal = $row.find('input.quantity-input').val();
        const wastageVal = $row.find('input.wastage-input').val();
        const unitVal = $row.find('select.unit-select').val();
        const updatedPrimaryProductId = $row.find('select.alt-select').val();

        // Only update if the value is not empty (prevents overwriting with 0 mid-typing)
        if (qtyVal !== '') {
            const parsedQty = parseFloat(qtyVal);
            if (!isNaN(parsedQty)) storedItems[index].quantity = parsedQty;
        }
        if (wastageVal !== '') {
            const parsedWastage = parseFloat(wastageVal);
            if (!isNaN(parsedWastage)) storedItems[index].wastage_percent = parsedWastage;
        }
        if (unitVal !== '') {
            storedItems[index].unitId = Number(unitVal);
        }
    ...
    $('#saveRawData').on('click', function () {
        // Prevent multiple clicks by disabling the button for 1 minute
        var $saveButton = $(this);
        ...
        // Disable button for 1 minute and change text to show processing
        ...
        $.ajax({
            url: site.base_url + "Recipies/addProddUnitBomData",
            type: 'POST',
            data: {
    ...
    function getProcurementOrderList(selectedHorizonatlTabValue) {
        $.ajax({
            url: site.base_url + "Recipies/getProcurementOrderList",
            method: 'GET',
            data: {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $(document).on('blur', '.quantity-input, .wastage-input, .minqtyset, .Saleunitset', function () {
        ...
        // Don't reformat if field is empty — leave it for the user
        ...
        if (!isNaN(val)) {
            ...
            // Use toRawDecimal (no commas) — safe for type="number" inputs
            $(this).val(toRawDecimal(val));
            // Sync the formatted value back to localStorage immediately on blur
            const $row = $(this).closest('tr');
            const index = $row.data('index');
            if (index !== undefined) {
                let storedItems = JSON.parse(localStorage.getItem('productRawDetailsList')) || [];
                if (storedItems[index]) {
                    if ($(this).hasClass('quantity-input')) storedItems[index].quantity = val;
                    if ($(this).hasClass('wastage-input')) storedItems[index].wastage_percent = val;
                    localStorage.setItem('productRawDetailsList', JSON.stringify(storedItems));
                }
            }
        }
    ...
    // Update version number when workstation changes (only in add mode, not edit mode)
    $(document).on('change', '#workstation_select', function () {
        const productId = $('#product_id').val();
        const workstationId = $(this).val();
        const isEditMode = localStorage.getItem('editMode') === 'true';
        
        // Only update version in add mode, not edit mode
        // In edit mode, version should remain as the current BOM version
        if (!isEditMode && productId && workstationId) {
            $.ajax({
                url: site.base_url + "Recipies/getLatestBomVersion",
                data: { 
                    productId: productId,
        ...
        } else if (!isEditMode && productId && !workstationId) {
            // If workstation is cleared in add mode, show version 1
            $('#version_bom').text('1');
        }
    });
