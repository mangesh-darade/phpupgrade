...
$(document).ready(function () {
           ...
           <tr>
            ...
            if (selectedHorizonatlTab == 'Received') {
                ...
                response.forEach(function (Open_order) {
                    ...
                    var previous_order =
                        '<tr>' +
                        '<td><a href="" id="orderAction" class="Received-Item-link" data-order-id="' + Open_order.orderId + '" data-order-no="' + Open_order.orderNo + '">' + Open_order.orderNo + '</a></td>'+
                        // '<td>' + Open_order.orderNo + '</td>' +
                        '<td class="text-center">' + Open_order.itemCount + '</td>' +
                        '<td>' + Open_order.placedOn + '</td>' +
                        '<td>' + Open_order.createdByFirstName + '</td>' +
                        // '<td>' + receiveddate + '</td>' +
                        '<td>' + Open_order.orderStatus + '</td>' ;
            ...
            if (selectedHorizonatlTab == 'Received_orders') {
                ...
                response.forEach(function (Open_order) {
                    ...
                    var receivedQty = Open_order.received_quantity;
                    
                   var isReceived = Open_order.orderStatus === "Received";
                   var inputValue = isReceived ? receivedQty : dispatchQty;
                    ...
                    var Received_orders =
                        ...
                        '<td class="subtotal text-center" data-order_quantity="' + Open_order.order_quantity + '" data-order-id="' + Open_order.unit_price + '" data-dispatch-data ="' + Open_order.allot_quantity + '">' + formatQuantity(Open_order.allot_quantity) + '</td>' +
                        ...
                        '<td style="width: 20px; height: 30px; white-space: nowrap;">' +'<input type="number" name="order_quantity[]" ' + (isReceived ? 'disabled' : '') + ' style="width: 50px; height: 30px; display: inline-block;" class="text-end received_qtys" value="' + inputValue + '">' +'</td>' +
                    ...
                    $("#update_order").hide();
                    if(Open_order.orderStatus === "Received"){
                        $("#received_order").hide();
                        $("#reject").hide();
                    }else{
                        $('#received_order').attr('data-id', Open_order.itemsId);
                        $('#reject').attr('data-id', Open_order.itemsId);
                    }
                    // $('#received_order').attr('data-id', Open_order.itemsId);
                    // $('#reject').attr('data-id', Open_order.itemsId);
    ...
    $(document).ready(function () {
        $('#Received .status').click(); // This line triggers the click event on page load
    });

    selectedHorizonatlTabValue = 'Received'
    ...
    $(document).on('change', '.received_qtys', function () {
        ...
        if (!isNaN(quantity) && !isNaN(unitPrice) && !isNaN(dispatch_qty)) {
            ...
            var adjustments = (price - adjustment_price);
    
            row.find('.price').text(formatMoney(price)); 
            var adjustmentCell = row.find('.adjustment');
            row.find('.adjustment').text(formatMoney(adjustments)); 
            if (dispatch_qty > quantity) {
                adjustmentCell.addClass('negative'); 
                if (adjustments < 0) {
                    adjustmentCell.addClass('negative'); 
                }
            }else if (adjustments < 0) {
                adjustmentCell.addClass('negative'); 
            }else if (dispatch_qty < quantity) {
                // adjustmentCell.addClass('negative'); 
            }
             else{
                adjustmentCell.removeClass('negative');
            }
                    ...
                    if (selectedHorizonatlTabValue) {
                        switch (selectedHorizonatlTabValue) {
                            ...
                            case 'Received':
                                ...
                                $(".right_section").hide();
                                $(".clickMe").hide();
                                $('#order_status').text(('Receive Delivery For : '));
                                $("#add_note").hide();
                                $(".repeat_order").hide();
                                response.forEach(function (orderItem) {
    ...
    $('.TabClick').click(function () {
        ...
        switch (selectedHorizonatlTabValue) {  
            case 'received_order':
                if (!confirm("Are you sure, You want to Accept the Order?")) {
            ...
            success: function (response) {
                if (selectedHorizonatlTabValue == 'reject') {
                    Toastify({
                        ...
                        duration: 700,  // Duration in milliseconds (1 seconds)
                        gravity: 'top-right', // Display position: 'bottom-left', 'bottom-right', 'top-left', 'top-right'
                        close: true,  // Whether to add a close button
                        callback: function () {
                            $("#received_order").hide();
                    ...
                    Toastify({
                        ...
                        duration: 3000,  // Duration in milliseconds (1 seconds)
                        gravity: 'top-right', // Display position: 'bottom-left', 'bottom-right', 'top-left', 'top-right'
                        close: true,  // Whether to add a close button
                        callback: function () {
                            $("#received_order").hide();
