...
function quantityDecimal(x) {
    if (x == null || x === '') return '';
    var s = String(x).trim().replace(/,/g, '').replace(/\s/g, '');
    var n = parseFloat(s);
    if (isNaN(n)) return '0';
    var decimals = 2;
...
function quantityInputStep() {
    var d = 2;
    if (typeof site !== 'undefined' && site && site.settings && site.settings.qty_decimals != null)
...
$(document).ready(function () {
    ...
    function loadOrderItems(procurmentOrderId, tabsInMiddleSection) {
        ...
        orderItemsDetails.forEach(function (orderItems) {
            ...
            OrderItemsData.forEach(function (Items) {
                ...
                var orderDisplay = quantityDecimal(Items.order_quantity);
                var stockDisplay = Items.stock_quantity != null && Items.stock_quantity !== '' ? quantityDecimal(Items.stock_quantity) : '';
                var allotDisplay = (allotQuantity === '' || allotQuantity == null) ? '' : quantityDecimal(allotQuantity);
                var allotStep = quantityInputStep();
                var row = $("<tr class='text-center red'>" +
                    "<td class='cen-set'><p class='circle-set'>" + orderDisplay + "</p></td>" +
                    "<td class='textleft'>" + Items.product_name + "</td>" +
                    "<td>" + stockDisplay + "</td>" +
                    "<td><input type='number' class='allot-set' name='quantity[]' step='" + allotStep + "' style='width:20%; text-align:right;' value='" + allotDisplay + "' " + Items.itemId + "\")'>" +
    ...
    $('#tablebody').on('click', 'input[type="checkbox"]', function () {
        ...
        allotQuantityInput = $(this).closest('tr').find('input[name="quantity[]"]').val();
        var qty = (allotQuantityInput === '' || allotQuantityInput == null) ? '' : quantityDecimal(allotQuantityInput);
        $(this).closest('tr').find('input[name="quantity[]"]').val(qty);
        var procurmentOrderId = localStorage.getItem('procurmentOrderId');

        if (isChecked === true) {
            localStorage.setItem('checkbox_' + orderItemId, isChecked);
            localStorage.setItem('allotQuantity_' + orderItemId, qty);
            updateOrderItemDetails(procurmentOrderId, orderItemId, "Checked", qty);
        } else {
            localStorage.removeItem('checkbox_' + orderItemId);
            localStorage.removeItem('allotQuantity_' + orderItemId);
            updateOrderItemDetails(procurmentOrderId, orderItemId, "Unchecked", qty);
        }
    });
    ...
    $('#selectAll').on('click', function () {
        ...
        $('#tablebody tr').each(function () {
            ...
            if (!checkbox.prop('disabled')) {
                var production_unit_id = localStorage.getItem('production_unit_id_' + procurmentOrderId);

                var allotQuantityInput = $(this).closest('tr').find('input[name="quantity[]"]').val();
                var qty = (allotQuantityInput === '' || allotQuantityInput == null) ? '' : quantityDecimal(allotQuantityInput);
                $(this).closest('tr').find('input[name="quantity[]"]').val(qty);
                ...
                if (bulkAllotCheck === 'allChecked' && singleItemChecked !== 'true') {
                    localStorage.setItem('checkbox_' + orderItemId, allChecked.toString());
                    localStorage.setItem('bulkAllotCheck_' + procurmentOrderId, bulkAllotCheck);
                    localStorage.setItem('allotQuantity_' + orderItemId, qty);

                    itemData.push({ orderItemId: orderItemId, allotQuantity: qty });
                } else if (bulkAllotCheck === 'allUnChecked') {
                    var existingAllotQuantityInput = localStorage.getItem('allotQuantity_' + orderItemId);
                    itemData.push({ orderItemId: orderItemId, allotQuantity: existingAllotQuantityInput });
