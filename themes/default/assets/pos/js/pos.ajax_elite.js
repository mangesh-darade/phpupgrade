...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            tr_html += '<input name="mrp[]" type="hidden" class="rmrp" value="' + (item.row.mrp ? item.row.mrp : 0) + '">';
            tr_html += '<input name="discount_on_mrp[]" type="hidden" class="rdmrp" value="' + (item.row.discount_on_mrp ? item.row.discount_on_mrp : '0%') + '">';
            tr_html += '<input name="customer_group_discount[]" type="hidden" class="rcgd" value="' + (item.row.customer_group_discount ? item.row.customer_group_discount : 0) + '">';
            ...
            tr_html += '<td>';
            tr_html += '<table style="border: none; display: flex; justify-content: space-around;"><tr ><td style="border-bottom: 0px !important;"> ';
            ...
            tr_html += '<td class="text-center litever"><small>' + item_sale_unit + '</small></td>';
            //}            
...
function addOfferFreeItems(itemId, itemQty) {
    ...
    $.ajax({
        type: "get",
        url: base_url + 'pos_elite/getProductDataById',
...
function applyRepeateDiscount(discountApply) {
    ...
    $.each(poscart_items, function () {
        ...
        $.ajax({
            ...
            url: site.base_url + 'pos_elite/RepeateDiscount',
            async: false,
...
// Add missing autocomplete functionality for product search
$(document).ready(function() {
        ...
        source: function(request, response) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var exp = request.term.split("_"); // Using barcode scanning
                    if (exp[1]) { // Using barcode scanning
                        if (data[0].id !== 0) {
    ...
    // Add missing payment button click handler with cart validation
    $('#payment').click(function() {
        var salesPerson = document.getElementById('sales_person').value;
        var displaySeller = pos_settings.display_seller;
        ...
        // Check if cart is empty (count == 1 means only the empty row exists)
        if (count == 1) {
