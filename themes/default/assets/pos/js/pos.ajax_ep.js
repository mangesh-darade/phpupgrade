var discountOnMrpEnabled = (typeof window.discountOnMrpEnabled !== 'undefined') ? !!window.discountOnMrpEnabled : true;
function getPrModalSelector() {
    return discountOnMrpEnabled ? '#prModal' : '#prModalPrice';
}
function getPrModalLabelSelector() {
    return discountOnMrpEnabled ? '#prModalLabel' : '#prModalPriceLabel';
}
function getPrDiscountFieldId() {
    return discountOnMrpEnabled ? 'mrpdiscount' : 'pricediscount';
}

function getPrTaxRateObj(pr_tax, tax_rate_id) {
    if (tax_rate_id && typeof tax_rates !== 'undefined') {
        var pr_tax_rate_obj = null;
        $.each(tax_rates, function () {
            if (this.id == tax_rate_id) {
...
function getCartItemUnitPrice(unit_price, mrp, item) {
    if (unit_price !== null && unit_price !== undefined && unit_price !== '' && parseFloat(unit_price) > 0) {
        return parseFloat(unit_price);
    }
    if (item && item.row) {
        var taxMethod = item.row.tax_method;
        if (taxMethod == '1' || taxMethod == 1) {
            if (parseFloat(item.row.real_unit_price) > 0) {
                return parseFloat(item.row.real_unit_price);
            }
            if (parseFloat(item.row.base_unit_price) > 0) {
                return parseFloat(item.row.base_unit_price);
            }
            if (parseFloat(item.row.price) > 0) {
                return parseFloat(item.row.price);
            }
            if (parseFloat(item.row.unit_price) > 0) {
                return parseFloat(item.row.unit_price);
            }
        }
    }
    if (parseFloat(mrp) > 0) {
...
function getPrTaxFinalVal(selector, pr_tax_rate_obj) {
    var pr_tax_val = getPrTaxBaseVal(selector);
    var pr_tax_surcharge = calcPrTaxSurcharge(pr_tax_rate_obj, pr_tax_val, 1);
    return pr_tax_surcharge.has_pr_tax_surcharge ? parseFloat(pr_tax_surcharge.pr_tax_final_val) : pr_tax_val;
}

function getPrTaxLineWithSurcharge(pr_tax, tax_rate_id, pr_tax_val, item_qty) {
    var pr_tax_rate_obj = getPrTaxRateObj(pr_tax, tax_rate_id);
    var pr_tax_surcharge = calcPrTaxSurcharge(pr_tax_rate_obj, pr_tax_val, item_qty);
    return {
        pr_tax_rate_obj: pr_tax_rate_obj,
        pr_tax_surcharge: pr_tax_surcharge,
        pr_tax_line: pr_tax_surcharge.has_pr_tax_surcharge ? parseFloat(pr_tax_surcharge.pr_tax_final_line) : (parseFloat(pr_tax_val) * parseFloat(item_qty))
    };
}

function getSuspendedItemStoredTax(item, item_qty) {
    if (!item || !item.row || item.row.unit_tax === undefined || item.row.unit_tax === null || item.row.unit_tax === '' || !(parseFloat(item.row.net_unit_price) > 0)) {
        return null;
    }
    var unit_tax = parseFloat(item.row.unit_tax) || 0;
    var surcharge_line = parseFloat(item.row.surcharge) || 0;
    var surcharge_unit = item_qty > 0 ? (surcharge_line / item_qty) : 0;
    var tax_line = formatDecimal((unit_tax * item_qty) + surcharge_line, 6);
    return {
        pr_tax_val: unit_tax,
        pr_tax_surcharge_val: surcharge_unit,
        pr_tax_final_val: unit_tax + surcharge_unit,
        pr_tax_line: parseFloat(tax_line),
        pr_tax_surcharge: {
            has_pr_tax_surcharge: surcharge_line != 0,
            pr_tax_surcharge_val: surcharge_unit,
            pr_tax_surcharge_line: surcharge_line,
            pr_tax_final_val: unit_tax + surcharge_unit,
            pr_tax_final_line: parseFloat(tax_line)
        }
    };
}
...
$(document).ready(function () {
    ...
    $(document).on('click', '#updateOrderDiscount', function () {
        ...
        let discountValue = isPercentage ? parseFloat(ds.slice(0, -1)) : parseFloat(ds);
        ...
        // Only check if ds > Total when it's a flat amount (not a percentage)
        if (!isPercentage && discountValue > Total) {
            alert("Order discount cannot be greater than total payable amount")
            return;
        }
        // Validate percentage discount
        if (isPercentage && (discountValue > 100 || isNaN(discountValue))) {
            bootbox.alert(lang.unexpected_value);
            return;
        }
    ...
    $(document).on('click', '.posdel', function () {
        ...
        var suspendId = parseInt($('#is_suspend_id').val(), 10) || 0;
        var requirePinForDelete = (
            protect_delete == 1 &&
            suspendId > 0 &&
            suspendProtectedItemIds.hasOwnProperty(String(item_id))
        );
                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            if (pos_pin == pos_settings.pin_code) {
                                ...
                                delete suspendProtectedItemIds[String(item_id)];
                                row.remove();
    ...
    $(document).on('click', '.edit', function () {
        ...
        var itemsalesperson = row.children().children('.ritemsalesperson').val();
        
        var manualedit = (item.row.manualedit) ? item.row.manualedit : '';
        var posdiscountVal = localStorage.getItem('posdiscount') || $('#posdiscount').val();
        if (!posdiscountVal || posdiscountVal === '0' || posdiscountVal === '0%' || discountOnMrpEnabled) {
            posdiscountVal = '';
        }

        // Order discount overrides existing customer/product discount in edit popup (price mode only)
        if (posdiscountVal) {
            discount_on_mrp = (discount && discount !== '0' && discount !== '0%') ? discount : posdiscountVal;
        } else if (editpopup == 'edititems') {
            if (item.row.discount_on_mrp !== undefined && item.row.discount_on_mrp !== null && String(item.row.discount_on_mrp) !== '') {
                discount_on_mrp = item.row.discount_on_mrp;
            }
        } else if (customer_group_discount == '1' && discount && manualedit != '1') {
            discount_on_mrp = discount;
        }
        if (!discountOnMrpEnabled && editpopup == 'edititems' && !posdiscountVal) {
            if (manualedit == '1') {
                if (item.row.tax_method == '0') {
                    unit_price = formatDecimal(parseFloat(item.row.unit_price) > 0 ? item.row.unit_price : item.row.price);
                } else {
                    unit_price = formatDecimal(parseFloat(item.row.price) > 0 ? item.row.price : item.row.real_unit_price);
                }
        ...
        $(getPrModalLabelSelector()).text(item.row.name + ' (' + item.row.code + ')');
        ...
        if (discountOnMrpEnabled) {
            ...
            if(manualedit == '1') {
                net_price += item_discount_on_mrp; 
                unit_price += item_discount_on_mrp; 
                var flat_discount = mrp - net_price;
                discount_on_mrp = flat_discount.toFixed(0);  
            }
        } else {
            var popup_catalog_price = parseFloat(item.row.base_unit_price) > 0 ? parseFloat(item.row.base_unit_price) : (parseFloat(mrp) > 0 ? parseFloat(mrp) : (parseFloat(item.row.real_unit_price) > 0 ? parseFloat(item.row.real_unit_price) : 0));
            if (posdiscountVal && popup_catalog_price > 0) {
                unit_price = popup_catalog_price;
                net_price = popup_catalog_price;
            } else {
                net_price = unit_price;
            }
            if ((editpopup !== 'edititems' || posdiscountVal) && discount_on_mrp && manualedit != '1') {
                var mds = discount_on_mrp;
                var item_discount_on_price = 0;
                var disc_base_price = (posdiscountVal && popup_catalog_price > 0) ? popup_catalog_price : unit_price;
                if (mds.indexOf("%") !== -1) {
                    var pricediscountPct = mds.split("%");
                    if (!isNaN(pricediscountPct[0])) {
                        item_discount_on_price = parseFloat(((disc_base_price) * parseFloat(pricediscountPct[0])) / 100);
                    } else {
                ...
                net_price = disc_base_price - item_discount_on_price;
                unit_price = net_price;
            }
        }

        // calculate order discount on price (MRP mode legacy only; price mode uses cart hidden discount_on_mrp)
        if (discountOnMrpEnabled && discount && discount != '0' && discount != '0%' && customer_group_discount == 0) {
            var mds = discount;
            var item_discount_on_price = 0;
            if (mds.indexOf("%") !== -1) {
                var mrpdiscount = mds.split("%");
                if (!isNaN(mrpdiscount[0])) {
                    item_discount_on_price = parseFloat(((unit_price) * parseFloat(mrpdiscount[0])) / 100);
                } else {
                    item_discount_on_price = parseFloat(mds);
                }
            } else {
            ...
            discount_on_mrp = parseFloat(mrp) - parseFloat(unit_price);
            // net_price -= item_discount_on_price; 
            // unit_price -= item_discount_on_price;
        }

        var pr_tax_val = 0;
        var suspended_popup_tax = getSuspendedItemStoredTax(item, 1);

        if (suspended_popup_tax) {
            pr_tax_val = suspended_popup_tax.pr_tax_val;
            net_price = parseFloat(item.row.net_unit_price);
            if (parseFloat(item.row.unit_price) > 0) {
                unit_price = parseFloat(item.row.unit_price);
            }
            if (site.settings.tax1 && item.row.tax_rate) {
                $('#ptax').select2('val', item.row.tax_rate);
                $('#old_tax').val(item.row.tax_rate);
            }
        } else if (site.settings.tax1) {
            if (item.fixtax) {
        ...
        var pr_tax_rate_obj = getPrTaxRateObj(null, item.row.tax_rate);
        if (!suspended_popup_tax && positems[item_id].row.tax_method == 0) {
            var pr_tax_surcharge_popup = calcPrTaxSurcharge(pr_tax_rate_obj, pr_tax_val, 1);
            if (pr_tax_surcharge_popup.has_pr_tax_surcharge) {
                net_price -= parseFloat(pr_tax_surcharge_popup.pr_tax_surcharge_val);
            }
        }
        ...
        $('#' + getPrDiscountFieldId()).val((discount_on_mrp));
        ...
        var pr_tax_final_val_popup = parseFloat(pr_tax_val);
        if (pr_tax_rate_obj) {
            var pr_tax_surcharge_popup2 = calcPrTaxSurcharge(pr_tax_rate_obj, pr_tax_val, 1);
            if (pr_tax_surcharge_popup2.has_pr_tax_surcharge) {
                pr_tax_final_val_popup = parseFloat(pr_tax_surcharge_popup2.pr_tax_final_val);
            }
        }
        $('#punit_price').val(formatDecimal(parseFloat(unit_price) + pr_tax_final_val_popup));
        $('#poption').select2('val', item.row.option);
        var old_price_val = unit_price;
        if (!discountOnMrpEnabled) {
            var catalog_price = parseFloat(item.row.base_unit_price) > 0 ? parseFloat(item.row.base_unit_price) : (parseFloat(item.row.real_unit_price) > 0 ? parseFloat(item.row.real_unit_price) : parseFloat(old_price_val) || 0);
            if (editpopup == 'edititems') {
                if (posdiscountVal) {
                    old_price_val = catalog_price;
                } else if (item.row.tax_method == '0') {
                    old_price_val = parseFloat(item.row.unit_price) > 0 ? parseFloat(item.row.unit_price) : parseFloat(item.row.price);
                } else {
                    old_price_val = parseFloat(item.row.price) > 0 ? parseFloat(item.row.price) : parseFloat(item.row.real_unit_price);
                }
                if (customer_group_discount == '1' && !posdiscountVal) {
                    var saved_disc_op = String(item.row.discount_on_mrp != null ? item.row.discount_on_mrp : (discount_on_mrp || '0'));
                    var disc_pct_op = saved_disc_op.indexOf('%') !== -1 ? (parseFloat(saved_disc_op.split('%')[0]) || 0) : (parseFloat(saved_disc_op) || 0);
                    if (disc_pct_op === 0) {
                        old_price_val = parseFloat(item.row.price) > 0 ? parseFloat(item.row.price) : catalog_price;
                    } else {
                        old_price_val = catalog_price;
                    }
                } else if (discount_on_mrp) {
                    if (posdiscountVal) {
                        old_price_val = catalog_price;
                    } else {
                        var mds_op = String(discount_on_mrp);
                        if (mds_op.indexOf("%") !== -1) {
                            var pct_op = parseFloat(mds_op.split("%")[0]) || 0;
                            if (pct_op > 0 && pct_op < 100) {
                                old_price_val = old_price_val / (1 - pct_op / 100);
                            }
                        } else if (parseFloat(mds_op) > 0) {
                            old_price_val = old_price_val + (parseFloat(mds_op) || 0);
                        }
                    }
                }
            } else {
                if (customer_group_discount == '1' && !posdiscountVal) {
                    ...
                    old_price_val = catalog_price;
                    if (!posdiscountVal && discount_on_mrp && discount_on_mrp !== '0' && discount_on_mrp !== '0%') {
                        var mds_op = String(discount_on_mrp);
                        if (mds_op.indexOf("%") !== -1) {
                            var pct_op = parseFloat(mds_op.split("%")[0]) || 0;
                            if (pct_op > 0 && pct_op < 100) {
                                old_price_val = parseFloat(unit_price) / (1 - pct_op / 100);
                            }
                        } else if (parseFloat(mds_op) > 0) {
                            old_price_val = parseFloat(unit_price) + (parseFloat(mds_op) || 0);
                        }
                    }
                }
            }
        }
        $('#old_price').val(formatDecimal(old_price_val));
        $('#row_id').val(row_id);
        $('#item_id').val(item_id);
        $('#pserial').val(row.children().children('.rserial').val());
        ...
        $('#itemsalesperson').select2({placeholder: 'Select Sales Person',allowClear: true,width: '100%'});
        $('#itemsalesperson').val(itemsalesperson || '').trigger('change');
  
        // $('#mrpdiscount').val(discount_on_mrp);
        $(getPrModalSelector()).appendTo("body").modal('show');

    });

    $(getPrModalSelector()).on('shown.bs.modal', function (e) {
		$('#pmrp').focus();
    ...
    $(document).on('change', '#ptax, #pmrp, #mrpdiscount, #pricediscount, #selling, #pquantity', function () { 
        ...
        // Price mode: qty change must not recalculate selling price, net unit price, or tax
        if (!discountOnMrpEnabled && triggerId === 'pquantity') {
        ...
        var listMrp = parseFloat($('#pmrp').val()) || 0;

        if (!discountOnMrpEnabled) {
            if (triggerId === 'pmrp') {
                if (listMrp < 0) {
                    alert("MRP cannot be negative.");
                    $('#pmrp').val(formatDecimal(Math.abs(listMrp)));
                }
                return;
            }

            if (triggerId === 'selling') {
                var sellingPrice = parseFloat($('#selling').val());
                ...
                if (sellingPrice < 0) {
                    ...
                    $('#selling').val(formatDecimal(sellingPrice));
                }
                if (listMrp > 0 && sellingPrice > listMrp) {
                    alert("Selling price cannot be greater than MRP");
                    sellingPrice = listMrp;
                    $('#selling').val(formatDecimal(sellingPrice));
                }
                $('#pricediscount').val('0%');
                mrp = sellingPrice;
            } else if (triggerId === 'pricediscount') {
                var priceDiscountInput = $('#pricediscount').val() ? String($('#pricediscount').val()).trim() : '0';
                if (priceDiscountInput === '%' || priceDiscountInput === '') {
                    priceDiscountInput = '0%';
                    $('#pricediscount').val(priceDiscountInput);
                }
                var isPercentage = priceDiscountInput.indexOf('%') !== -1;
                var discountValue = isPercentage ? parseFloat(priceDiscountInput.replace('%', '')) : parseFloat(priceDiscountInput);
                if (isNaN(discountValue)) {
                    discountValue = 0;
                }
                var basePrice = parseFloat($('#old_price').val());
                if (isNaN(basePrice) || basePrice <= 0) {
                    basePrice = parseFloat(item.row.base_unit_price) > 0 ? parseFloat(item.row.base_unit_price) : (parseFloat(item.row.real_unit_price) > 0 ? parseFloat(item.row.real_unit_price) : (parseFloat($('#pmrp').val()) || 0));
                }
                ...
                if (isPercentage && discountValue > 100) {
                    ...
                    $('#pricediscount').val('0%');
                    mrp = basePrice;
                } else if (!isPercentage && discountValue > basePrice) {
                    alert("Discount cannot be more than price");
                    $('#pricediscount').val('0');
                    mrp = basePrice;
                } else if (basePrice > 0) {
                    if (isPercentage) {
                        item_discount_on_mrp = parseFloat(((basePrice) * discountValue) / 100);
                        mrp = basePrice - item_discount_on_mrp;
                    } else {
                        item_discount_on_mrp = discountValue;
                        mrp = Math.max(0, basePrice - discountValue);
                    }
                } else {
                    mrp = 0;
                }
                $('#selling').val(formatDecimal(mrp));
            } else {
                mrp = parseFloat($('#selling').val());
                if (isNaN(mrp)) {
                    mrp = 0;
                }
            }
            ...
            if (triggerId === 'selling') {
                ...
                if (unit_price < 0) {
                    alert("Selling price cannot be negative.");
                    unit_price = Math.abs(unit_price);
                    $('#selling').val(formatDecimal(unit_price));
                }
                mrp = unit_price;
                var pmrp = $('#pmrp').val();
                if (unit_price > pmrp) {
        ...
        var pr_tax_rate_obj_chg = getPrTaxRateObj(null, $('#ptax').val());
        if (item_tax_method == 0) {
            var pr_tax_surcharge_chg = calcPrTaxSurcharge(pr_tax_rate_obj_chg, pr_tax_val, 1);
            if (pr_tax_surcharge_chg.has_pr_tax_surcharge) {
                mrp -= parseFloat(pr_tax_surcharge_chg.pr_tax_surcharge_val);
            }
        }
        $('#selling').val(formatDecimal(mrp)); // Set it to the selling price input
        $('#net_price').text(formatMoney(mrp));
        setPrTaxDisplay('#pro_tax', pr_tax_val, pr_tax_rate_obj_chg);

        if (discountOnMrpEnabled) {
            if (triggerId === 'selling') {
                var unit_price = parseFloat($('#selling').val());
                var pr_tax_final_chg = getPrTaxFinalVal('#pro_tax', pr_tax_rate_obj_chg);
                var selling_price = parseFloat(unit_price) + parseFloat(pr_tax_final_chg);
                var discount = 0;
                var listMrpVal = $('#pmrp').val();

                if (listMrpVal > 0) {
                    discount = parseFloat(listMrpVal) - parseFloat(unit_price);
                    if (item_tax_method == '0') {
                        discount = parseFloat(listMrpVal) - parseFloat(selling_price);
                    }
                    if (unit_price == 0) {
                        discount = 0;
                    }
    ...
    $(document).on('change', '#pprice, #ptax, #pdiscount', function () {
        ...
        var pr_tax_rate_obj_p = getPrTaxRateObj(null, pr_tax);
        if (item_tax_method == 0) {
            var pr_tax_surcharge_p = calcPrTaxSurcharge(pr_tax_rate_obj_p, pr_tax_val, 1);
            if (pr_tax_surcharge_p.has_pr_tax_surcharge) {
                unit_price -= parseFloat(pr_tax_surcharge_p.pr_tax_surcharge_val);
            }
        }

        $('#net_price').text(formatMoney(unit_price));
        setPrTaxDisplay('#pro_tax', pr_tax_val, pr_tax_rate_obj_p);
    });
     ...
     * Edit Row Method
     ...
     $(document).on('click', '#editItem', function () {
        ...
        // set these price for manualedit price in cart
        var net_unit_price = parseFloat($('#net_price').text().trim().replace(/,/g, ''));
        var pr_tax_rate_obj_edit = getPrTaxRateObj(null, $('#ptax').val());
        var pro_tax = getPrTaxFinalVal('#pro_tax', pr_tax_rate_obj_edit);
        var unit_price = parseFloat(net_unit_price) + parseFloat(pro_tax);
        if (!discountOnMrpEnabled) {
            var add_tax_cart_ep = ($('#add_tax_in_cart_unit_price').val() == 1);
            if (add_tax_cart_ep) {
                var edit_tax_method = positems[item_id].row.tax_method;
                if (edit_tax_method == 0 || edit_tax_method == '0') {
                    price = unit_price;
                }
            }
        }
        ...
        if (item.options !== false) {
            ...
            $.each(item.options, function () {
                if (this.id == opt && this.price != 0 && this.price != '' && this.price != null) {
                    // price = price - parseFloat(this.price); //check later
        ...
        // positems[item_id].row.manualedit = 1;
        var orderDiscount = localStorage.getItem('posdiscount') || $('#posdiscount').val();
        if (!orderDiscount || orderDiscount === '0' || orderDiscount === '0%' || discountOnMrpEnabled) {
            orderDiscount = '';
        }
        var catalogPrice = parseFloat($('#old_price').val());
        positems[item_id].row.fup = 1,
            ...
            positems[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : '', // order discount
            positems[item_id].row.discount_on_mrp = ($('#' + getPrDiscountFieldId()).val() || '0%'),
            positems[item_id].row.mrp = $('#pmrp').val() ? $('#pmrp').val() : '';
            positems[item_id].row.itemsalesperson = $('#itemsalesperson').val() ? $('#itemsalesperson').val() : '';
        if (orderDiscount) {
            if (!isNaN(catalogPrice) && catalogPrice > 0) {
                positems[item_id].row.real_unit_price = catalogPrice + parseFloat(opt_price);
            } else {
                positems[item_id].row.real_unit_price = (parseFloat(price) + parseFloat(opt_price));
            }
            positems[item_id].row.discount = ($('#' + getPrDiscountFieldId()).val() || $('#pdiscount').val() || orderDiscount);
            positems[item_id].row.discount_on_mrp = ($('#' + getPrDiscountFieldId()).val() || orderDiscount);
            positems[item_id].row.edit_after_order_discount = '1';
            localStorage.setItem('posdiscount', orderDiscount);
            $('#posdiscount').val(orderDiscount);
        } else {
            positems[item_id].row.real_unit_price = (parseFloat(price) + parseFloat(opt_price));
            delete positems[item_id].row.edit_after_order_discount;
        }
        ...
        if (item.options !== false) {
            $.each(item.options, function () {
                if (this.id == opt && this.price != 0 && this.price != '' && this.price != null) {
                    ...
                    $('#pmrp').val(formatDecimal((parseFloat(this.mrp)))).trigger('change');
                }
            });
        }
    });
    ...
    $(document).on('change', '#mprice, #mtax, #mdiscount', function () {
        ...
        var pr_tax_rate_obj_m = getPrTaxRateObj(null, pr_tax);
        if (item_tax_method == 0) {
            var pr_tax_surcharge_m = calcPrTaxSurcharge(pr_tax_rate_obj_m, pr_tax_val, 1);
            if (pr_tax_surcharge_m.has_pr_tax_surcharge) {
                unit_price -= parseFloat(pr_tax_surcharge_m.pr_tax_surcharge_val);
            }
        }

        $('#mnet_price').text(formatMoney(unit_price));
        setPrTaxDisplay('#mpro_tax', pr_tax_val, pr_tax_rate_obj_m);
    });
    ...
    $(document).on("focus", '.rquantity', function () {
        ...
        var new_qty = parseFloat($(this).val()),
            ...
            if(positems[item_id].row.storage_type != 'loose'){
            ...
            $.each(positems[item_id].units, function () {
                if (this.id == positems[item_id].row.unit) {
                    if(positems[item_id].row.storage_type != 'loose'){
                        ...
                        positems[item_id].row.qty = unitToBaseQty(new_qty, this);
                    }
                }
            });
        }
    ...
    $(document).on("focus", '.userprice', function () {
        ...
        if (positems[item_id].row.storage_type == 'loose' && changeQtyAsPerPrice == true) {
            ...
            var cartOrderDiscount = localStorage.getItem('posdiscount') || $('#posdiscount').val();
            if (!cartOrderDiscount || cartOrderDiscount === '0' || cartOrderDiscount === '0%' || discountOnMrpEnabled) {
                cartOrderDiscount = '';
            }
            if (cartOrderDiscount) {
                var cartCatalogPrice = parseFloat(positems[item_id].row.base_unit_price) > 0 ? parseFloat(positems[item_id].row.base_unit_price) : (parseFloat(mrp) > 0 ? parseFloat(mrp) : parseFloat(positems[item_id].row.real_unit_price));
                if (cartCatalogPrice > 0) {
                    positems[item_id].row.real_unit_price = cartCatalogPrice;
                }
            } else {
            ...
            if (discountOnMrpEnabled) {
                positems[item_id].row.tax_method = 0; // MRP mode: manual cart price edit uses inclusive tax method
            } else if (!cartOrderDiscount) {
                positems[item_id].row.discount_on_mrp = '0%';
            }
...
// Item ids present when suspended sale is first loaded.
var suspendProtectedItemIds = {};
var suspendProtectedItemsInitialized = false;

function lockPosCustomerFieldsOnCart(itemCount) {
    if (!site || !site.settings || site.settings.theme !== 'theme_three') {
        return;
    }
    var disableCustomerControls = (itemCount > 0);
    $('#search_customer').prop('disabled', disableCustomerControls);
    $('#customer_name').prop('disabled', disableCustomerControls);
    $('#toogle-customer-read-attr').toggleClass('disabled', disableCustomerControls);
}
...
function ItemAddtoCart() {
    ...
    if ($('#posdiscount').length && !discountOnMrpEnabled) {
        var posItemsCount = 0;
        if (localStorage.getItem('positems')) {
            var poscart_items = JSON.parse(localStorage.getItem('positems'));
            posItemsCount = Object.keys(poscart_items).length;
        }
        if (posItemsCount == 0) {
            localStorage.removeItem('posdiscount');
            $('#posdiscount').val('0');
        } else {
            var hidOrderDiscount = $('#posdiscount').val();
            if (hidOrderDiscount && hidOrderDiscount !== '0' && hidOrderDiscount !== '0%') {
                localStorage.setItem('posdiscount', hidOrderDiscount);
            } else {
                localStorage.removeItem('posdiscount');
            }
        }
    }
    var activeOrderDiscount = discountOnMrpEnabled ? '' : localStorage.getItem('posdiscount');
    if (!activeOrderDiscount || activeOrderDiscount === '0' || activeOrderDiscount === '0%') {
        activeOrderDiscount = '';
    }
    ...
    if (localStorage.getItem('positems')) {
        ...
        // Only items loaded from suspended sale require PIN for deletion.
        var suspendId = parseInt($('#is_suspend_id').val(), 10) || 0;
        if (suspendId > 0 && !suspendProtectedItemsInitialized) {
            suspendProtectedItemIds = {};
            $.each(positems, function (k, item) {
                var protectedItemId = site.settings.item_addition == 1 ? item.item_id : item.id;
                suspendProtectedItemIds[String(protectedItemId)] = true;
            });
            suspendProtectedItemsInitialized = true;
        }
        ...
        lockPosCustomerFieldsOnCart(posItemsCount);
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_desc = item.row.description, item_option = item.row.option,item_option_color = item.row.option_color, item_code = item.row.code, item_article_code = item.row.article_code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            var suspended_tax_info = getSuspendedItemStoredTax(item, item_qty);
            ...
            var price_mode_edit_from_popup = (item.row.editpopup == 'edititems' && !discountOnMrpEnabled);
            var manualedit = (item.row.manualedit) ? item.row.manualedit : ''; // 05-09-19
            var price_mode_edit_manual_disc = ((price_mode_edit_from_popup || (manualedit == '1' && !discountOnMrpEnabled)) && customer_group_discount != '1');
            var price_mode_edit_after_order_discount = (item.row.editpopup == 'edititems' && item.row.edit_after_order_discount == '1');
            ...
            if (suspended_tax_info && parseFloat(item.row.unit_price) > 0) {
                var unit_price = parseFloat(item.row.unit_price);
            } else if (!warehouse_price_group_id) {
            ...
            if (!discountOnMrpEnabled && manualedit == "1") {
                if (item_tax_method == '0' || item_tax_method == 0) {
                    if (parseFloat(item.row.unit_price) > 0) {
                        unit_price = parseFloat(item.row.unit_price);
                    } else if (parseFloat(item.row.price) > 0) {
                        unit_price = parseFloat(item.row.price);
                    }
                } else {
                    unit_price = parseFloat(item.row.price) > 0 ? parseFloat(item.row.price) : parseFloat(item.row.unit_price);
                }
            }
            if (item.row.editpopup != 'edititems' && manualedit != "1") {
                if (discountOnMrpEnabled) {
                    unit_price = mrp;
                } else if (suspended_tax_info && parseFloat(item.row.unit_price) > 0) {
                    unit_price = parseFloat(item.row.unit_price);
                } else if (!activeOrderDiscount && parseFloat(item.row.price) > 0) {
                    unit_price = parseFloat(item.row.price);
                }
            }
            unit_price = getCartItemUnitPrice(unit_price, mrp, item);
            ...
            if (item.row.fup != 1 && product_unit != item.row.base_unit) {
                $.each(item.units, function () {
                    if (this.id == product_unit) {
                        ...
                        var converted_unit_price = formatDecimal((parseFloat(item.row.base_unit_price) * (unitToBaseQty(1, this))), 6);
                        if (parseFloat(converted_unit_price) > 0) {
                            ...
                            $.each(optionArr, function (k, opt) {
                                ...
                                if (this_options.id == opt) {
                                    if (this_options.price != 0 && this_options.price != '' && this_options.price != null) {
                                        if (manualedit == '') {
                                            item_price = formatDecimal(parseFloat(item.row.price) + parseFloat(discountOnMrpEnabled ? this_options.mrp : this_options.price), 6);
            ...
            var base_price = unit_price; // preserve actual selling price before any coupon reset
            // Order level discount distributed in each items as item discount.
            var posdiscount = discountOnMrpEnabled ? '' : activeOrderDiscount;

            if (posdiscount && !discountOnMrpEnabled && !price_mode_edit_after_order_discount) {
                var catalog_base_price = parseFloat(item.row.base_unit_price) > 0 ? parseFloat(item.row.base_unit_price) : (parseFloat(mrp) > 0 ? parseFloat(mrp) : (parseFloat(item.row.real_unit_price) > 0 ? parseFloat(item.row.real_unit_price) : 0));
                if (catalog_base_price > 0) {
                    unit_price = catalog_base_price;
                    base_price = catalog_base_price;
                }
            }

            if (posdiscount && !price_mode_edit_after_order_discount) {
                ...
                // Order discount overrides customer/product discount when applied
                if (ods.indexOf("%") !== -1) {
                    var pds = ods.split("%");
                    if (!isNaN(pds[0])) {
                        item_order_discount = formatDecimal((parseFloat(((base_price) * parseFloat(pds[0])) / 100)), 6);
                        item_ds = ods;
                    } else {
                ...
                // Restore actual selling price so sub-total is never replaced by DB mrp
                ...
                if(manualedit == "1"){
                    if (discountOnMrpEnabled) {
                        var flat_discount = mrp - unit_price;
                        ...
                        item_discount = formatDecimal(0, 6);
                        if (item.row.discount_on_mrp !== undefined && item.row.discount_on_mrp !== null && String(item.row.discount_on_mrp) !== '') {
                            discount_on_mrp = item.row.discount_on_mrp;
                        }
                    }
            ...
            if(item.row.editpopup == 'edititems' && (!posdiscount || price_mode_edit_after_order_discount)) {
                
                if (manualedit == "1") {
                    if (!discountOnMrpEnabled && (item_tax_method == '1' || item_tax_method == 1)) {
                        unit_price = parseFloat(item.row.price) > 0 ? parseFloat(item.row.price) : parseFloat(item.row.unit_price);
                    } else {
                        unit_price = item.row.unit_price;
                    }
                } else if (discountOnMrpEnabled) {
                    unit_price = mrp;
                } else {
                    if (item_tax_method == '0') {
                        unit_price = parseFloat(item.row.unit_price) > 0 ? parseFloat(item.row.unit_price) : parseFloat(item.row.price);
                    } else {
                        unit_price = parseFloat(item.row.price) > 0 ? parseFloat(item.row.price) : parseFloat(item.row.real_unit_price);
                    }
                }
                ...
                if(manualedit == "1"){
                    ...
                    // Discount on mrp / discount on price
                    if(discount_on_mrp){
                        var ds = discount_on_mrp ? String(discount_on_mrp) : '0';
                        ...
                        if (ds.indexOf("%") !== -1) {
                            var pds = ds.split("%");
                            if (!isNaN(pds[0])) {
                                var pct_ed = parseFloat(pds[0]) || 0;
                                if (!discountOnMrpEnabled && customer_group_discount == '1') {
                                    var base_for_disc = parseFloat(item.row.base_unit_price) > 0 ? parseFloat(item.row.base_unit_price) : (parseFloat(item.row.real_unit_price) > 0 ? parseFloat(item.row.real_unit_price) : unit_price);
                                    item_discount = formatDecimal((parseFloat(base_for_disc) * pct_ed) / 100, 6);
                                } else if (!discountOnMrpEnabled && pct_ed > 0 && pct_ed < 100) {
                                    item_discount = formatDecimal((unit_price / (1 - pct_ed / 100)) - unit_price, 6);
                                } else {
                                    item_discount = formatDecimal((parseFloat(((unit_price) * parseFloat(pds[0])) / 100)), 6);
                                }
                            } else {
           ...
           if(posdiscount && !price_mode_edit_after_order_discount){
            ...
            if(manualedit == '' && !price_mode_edit_from_popup){
                unit_price = formatDecimal(unit_price - item_discount, 6);
            }
            if(posdiscount && !price_mode_edit_after_order_discount){ // for calculate unit_price after apply order level discount
            ...
            // }
            var pr_tax = item.tax_rate;
            var pr_tax_val = 0, pr_tax_rate = 0, pr_tax_line_info = null, pr_tax_surcharge_val = 0, pr_tax_final_val = 0;
            if (suspended_tax_info) {
                pr_tax_val = suspended_tax_info.pr_tax_val;
                pr_tax_surcharge_val = suspended_tax_info.pr_tax_surcharge_val;
                pr_tax_final_val = suspended_tax_info.pr_tax_final_val;
                pr_tax_line_info = {
                    pr_tax_surcharge: suspended_tax_info.pr_tax_surcharge,
                    pr_tax_line: suspended_tax_info.pr_tax_line
                };
                product_tax += suspended_tax_info.pr_tax_line;
            } else if (site.settings.tax1 == 1) {
                if (pr_tax !== false) {
                    if (pr_tax.type == 1) {
                        if (item_tax_method == '0') {
                            if (fixtax) {
                    ...
                    pr_tax_line_info = getPrTaxLineWithSurcharge(pr_tax, null, pr_tax_val, item_qty);
                    product_tax += pr_tax_line_info.pr_tax_line;
                    pr_tax_final_val = parseFloat(pr_tax_val);
                    pr_tax_surcharge_val = 0;
                    if (pr_tax_line_info.pr_tax_surcharge && pr_tax_line_info.pr_tax_surcharge.has_pr_tax_surcharge) {
                        pr_tax_surcharge_val = parseFloat(pr_tax_line_info.pr_tax_surcharge.pr_tax_surcharge_val) || 0;
                        pr_tax_final_val = parseFloat(pr_tax_line_info.pr_tax_surcharge.pr_tax_final_val) || pr_tax_final_val;
                    }
                }
            }
            // if(item.row.editpopup == 'edititems') {
            ...
            // }
            if(posdiscount && !price_mode_edit_after_order_discount){ // for adding order level discount in price again
                unit_price = formatDecimal((unit_price), 6) + formatDecimal((item_order_discount), 6);
            }
            if (suspended_tax_info) {
                item_price = formatDecimal(parseFloat(item.row.net_unit_price), 6);
            } else {
                item_price = item_tax_method == 0 ? formatDecimal((unit_price - pr_tax_val - pr_tax_surcharge_val), 6) : formatDecimal(unit_price, 6);
            }
            // unit_price = formatDecimal((unit_price), 6) + formatDecimal((item_discount), 6);
            if(manualedit == '' && !price_mode_edit_manual_disc){
                unit_price = formatDecimal((unit_price), 6) + formatDecimal((item_discount), 6);
            }
            /********************************************/
            if (item_tax_method == 0) {
                offerCartItems[item.row.id] = JSON.parse('{"item_id":"' + item.row.id + '", "price_with_tax":"' + unit_price + '", "price_without_tax":"' + (parseFloat(unit_price) - parseFloat(pr_tax_val) - pr_tax_surcharge_val) + '", "qty":"' + item_qty + '", "category":"' + item.row.category_id + '", "discount":"' + item.row.discount + '"}');
            } else {
                offerCartItems[item.row.id] = JSON.parse('{"item_id":"' + item.row.id + '", "price_with_tax":"' + (parseFloat(unit_price) + parseFloat(pr_tax_final_val)) + '", "price_without_tax":"' + unit_price + '", "qty":"' + item_qty + '", "category":"' + item.row.category_id + '", "discount":"' + item.row.discount + '"}');
            }
            /************************************************/
            ...
            tr_html += '<input class="rtaxmethod" name="tax_method[]" type="hidden" value="' + item_tax_method + '">';
            ...
            if (!discountOnMrpEnabled && manualedit == '1' && (item_tax_method == '0' || item_tax_method == 0)) {
                cart_item_price = parseFloat(item.row.unit_price) > 0 ? parseFloat(item.row.unit_price) : parseFloat(item.row.price);
            } else if (add_tax_in_cart_unit_price == true && add_discount_in_cart_unit_price == true) {
                cart_item_price = parseFloat(item_price) + parseFloat(pr_tax_final_val) + (price_mode_edit_manual_disc ? 0 : parseFloat(item_discount)); //item_ds
            } else if (add_tax_in_cart_unit_price == true) {
                cart_item_price = parseFloat(item_price) + parseFloat(pr_tax_final_val);
            } else if (add_discount_in_cart_unit_price == true) {
                cart_item_price = parseFloat(item_price) + (price_mode_edit_manual_disc ? 0 : parseFloat(item_discount));
            ...
            if (permission_admin || permission_owner || per_cartpriceedit) {
                if (changeQtyAsPerPrice == true && item.row.storage_type == 'loose') {
                    tr_html += '<input type="number" maxlength="10" name="item_user_price[]" id="suserprice_' + row_no + '" value="' + ((cart_user_price > 0) ? parseInt(cart_user_price) : parseInt(cart_item_price)) + '"  class="form-control input-sm kb-pad text-center userprice" />';
                    ...
                    tr_html += '<input type="number" maxlength="10" name="item_price[]" id="sprice_' + row_no + '" value="' + ((cart_item_price)) + '"  ' + item_disabled + '  class="form-control input-sm kb-pad text-center item_price userprice" />';
                }
            } else {
            ...
            if (oldProductSearch(item_id)) {
                tr_html += '<button onclick="qtyMinus(\'' + item_id + '\')" type="button" style="border: 0; background: none; top:2px!important; padding:4px;" ><i class="fa fa-minus"></i> </button>';
            }
            tr_html += ' &nbsp;  </td>';
            ...
            if (permission_admin || permission_owner || per_cartpriceedit) {
                ...
                if (item.row.storage_type == 'packed') {
                    ...
                    tr_html += '<input style="width: 43px; float: right;" class="form-control input-sm kb-pad text-center returnquantity rquantity" maxlength="6" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" ' + item_disabled + ' type="number"   value="' + item_qty + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
                } else {
                    if (changeQtyAsPerPrice == true && cart_user_price > 0) {
                        ...
                        tr_html += '<input style="width: 43px; float: right;" class="form-control input-sm kb-pad text-center rquantity" maxlength="6" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" ' + item_disabled + ' type="number"    value="' + formatDecimal(item_qty, 3) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
                    }
                }

            } else {
                tr_html += '<input readonly="readonly" style="width: 43px; float: right;" class="form-control input-sm kb-pad text-center rquantity" maxlength="6" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" ' + item_disabled + '  type="number" value="' + item_qty + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
            }
            ...
            //tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_final_val)) * parseFloat(item_qty))) + '</span></td>';
            ...
            invoice_total_withtax += formatDecimal(((parseFloat(item_price) + parseFloat(pr_tax_final_val)) * parseFloat(item_qty)), 6);
            invoice_total_withouttax += formatDecimal((parseFloat(item_price) * parseFloat(item_qty)), 6);

            if (changeQtyAsPerPrice == true && item.row.storage_type == 'loose' && cart_user_price > 0) {
                total += formatDecimal(cart_user_price, 6);
            } else {
                total += formatDecimal(((parseFloat(item_price) + parseFloat(pr_tax_final_val)) * parseFloat(item_qty)), 6);
            }
            ...
            if (java_applet == 1) {
                ...
                bill_data += printLine(item_qty + " x " + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_final_val)) + ": " + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_final_val)) * parseFloat(item_qty)))) + "\n";
                ...
                var bprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td> ' + item_code + ' </td><td class="no-border">  ' + item_name + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + (item.options ? '(' + item.row.option + ')' : '') + '</td><td>' + formatDecimal(item_qty) + ' ' + item.row.unit_lable + '</td> <td>' + (item_discount != 0 ? '<del>' + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_final_val) + item_discount) + '</del>' : '') + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_final_val)) + '</td><td style="text-align:right;">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_final_val)) * parseFloat(item_qty))) + '</td></tr>';
                //var bprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td colspan="2" class="no-border">#'+(an-1)+' '+ item_name + ' (' + item_code + ')</td></tr>';
                //bprTr += '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td>(' + formatDecimal(item_qty) + ' x ' + (item_discount != 0 ? '<del>'+formatMoney(parseFloat(item_price) + parseFloat(pr_tax_final_val) + item_discount)+'</del>' : '') + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_final_val))+ ')</td><td style="text-align:right;">'+ formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_final_val)) * parseFloat(item_qty))) +'</td></tr>';
        ...
        $('#total').text(formatMoney(total + product_discount));
...
function scanItemAddToCart(items) {
    ...
    if (items.length > 0 && items[0].SuspendIds) {
        var suspendIdsVal = items[0].SuspendIds;
        var firstSuspendId = Array.isArray(suspendIdsVal) ? suspendIdsVal[0] : suspendIdsVal;
        if (firstSuspendId) {
            $('#is_suspend_id').val(firstSuspendId);
            suspendProtectedItemsInitialized = false;
        }
    }
...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        if (java_applet == 1) {
            ...
            $("#order_span").empty();
            $("#bill_span").empty();
            var styles = '<style>table, th, td { border-collapse:collapse; border-bottom: 1px solid #CCC; font-size: 14px; } .no-border { border: 0; } .bold { font-weight: bold; }</style>';
            // var pos_head1 = '<span style="text-align:center;"><h3>' + site.settings.site_name + '</h3><h4>';
            //var pos_head2 = '</h4><h5> Token No.: ' + tokan_no + ' </h5><h5>' + $('#select2-chosen-1').text() + '<br>' + hrld() + '</h5></span>';
            //$("#order_span").prepend(styles + pos_head1 + ' Order ' + pos_head2);
        ...
        $.each(sortedItems, function () {
            ...
            item.order = item.order ? item.order : new Date().getTime();
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_desc = item.row.description, item_option = item.row.option,item_option_color = item.row.option_color,item_option_color_name = item.row.option_color_name, item_code = item.row.code, item_article_code = item.row.article_code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            var suspended_tax_info = getSuspendedItemStoredTax(item, item_qty);
            var product_unit = item.row.unit;
            var item_weight = 0;
            if(item.row.storage_type == 'loose'){
           ...
           if(posdiscount && !price_mode_edit_after_order_discount){
            ...
            tr_html += '<input name="return_ref_no[]" type="hidden" class="return_ref_no" value="' + item.row.return_ref_no + '">';
            tr_html += '<input  name="customerRefNo" type="hidden" class="customerRefNo" value=" ' + scanValue + '">';
  tr_html += '<input  name="cat_id[]" type="hidden" id="cat_id" class="rcategory_id" value=" ' + item.row.category_id + '">';  /////////////// Multi[ple category ////////////////////////]
            // tr_html += '<input  name="customerRefNo" type="hidden" class="customerRefNo" value=" ' + scanValue +'">';
            ...
            tr_html += '<i class="pull-right fa fa-edit tip pointer edit iconsize" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i>';
            tr_html += '<td>';  
            tr_html += '<span class="sname" id="name_' + row_no + '">' + productCodes  + item_name + (sel_opt ? ' (' + sel_opt + (item.note ? ': ' + item.note : '') + ')' : '') + (item_option_color_name ? ' (' + item_option_color_name + ')' : '') + '</span>';
            tr_html += '</td>'; 
            if (item.note == 'Free Items') {
                var item_disabled = ' readonly="readonly" ';
                tr_html += '</td>';  
            ...
            tr_html += '<input class="reditpopup reditpopup" name="editpopup[]" type="hidden" value="' +  item.row.editpopup + '">';
            tr_html += '<input class="rcustomer_group_discount rcustomer_group_discount" name="customer_group_discount[]" type="hidden" value="' +  customer_group_discount + '">';
            tr_html += '<input class="ritemsalesperson" name="itemsalesperson[]" type="hidden" id="itemsalesperson_' + row_no + '" value="' + item.row.itemsalesperson + '">';
            ...
            tr_html += '<input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '">';
            tr_html += '<input name="product_base_quantity[]" maxlength="6" type="hidden" class="rbase_quantity product_base_quantity" value="' + base_quantity + '">';
            tr_html += '<input name="product_option_color[]" type="hidden" class="product_option_color" value="' + item_option_color + '">';
            ...
            if (permission_admin || permission_owner || per_cartpriceedit) {
                ...
                if (item.row.storage_type == 'packed') {
                    ...
                    if (changeQtyAsPerPrice == true && cart_user_price > 0) {
                        tr_html += formatDecimal(item_qty, 3) + '<input style="width: 43px; float: right;" class="form-control input-sm kb-pad text-center returnquantity rquantity" maxlength="6" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" ' + item_disabled + ' type="hidden" value="' + formatDecimal(item_qty, 3) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
                    } else {
                        tr_html += '<input style="width: 43px; float: right;" class="form-control input-sm kb-pad text-center returnquantity rquantity" maxlength="6" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" ' + item_disabled + ' type="number"    value="' + formatDecimal(item_qty, 3) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
                    }
                }

            } else {
                tr_html += '<input readonly="readonly" style="width: 43px; float: right;" class="form-control input-sm kb-pad text-center returnquantity rquantity" maxlength="6" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" ' + item_disabled + '  type="number" value="' + item_qty + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
            }
            tr_html += ' </td><td style="border-bottom: 0px !important;">';
            tr_html += '&nbsp;  <button type="button" onclick="qtyPlus(\'' + item_id + '\')"  style="border: 0; background: none; bottom:7px!important; padding:4px;"> <i class="fa fa-plus"></i> </button> ';
            tr_html += '</td></table>';
            ...
            if (java_applet == 1) {
                ...
                if (pos_settings.show_current_stock == 1) {
                    if ($("#order-table .receipt-head").length === 0) {
                        $("#order-table").append( '<tr class="receipt-head"><th>S.no</th><th>Name</th><th>QTY</th><th>Current stock</th></tr>');
                    }
                    $("#order-table .total-row").remove();   // Remove old total row before appending new item
                    var oprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '">' +'<td>' + (an - 1) + '</td>' +'<td>' + item_name +  (sel_opt != '' ? ' (' + sel_opt + ')' : '') +  (item.options ? '(' + item.row.option + ')' : '') + '</td>' +'<td class="qty-cell">' + formatDecimal(item_qty) + '</td>' +'<td>' + ((item.row.current_stock != null) ? formatDecimal(item.row.current_stock) : '-') + '</td>' +'</tr>';
                    $("#order-table").append(oprTr);
                    let totalQty = 0;
                    $("#order-table .qty-cell").each(function () {
                        totalQty += parseFloat($(this).text()) || 0;
                    });

                    $("#order-table").append(
                        '<tr class="total-row">' +
                            '<td colspan="2"><strong>Total</strong></td>' +
                            '<td class="total-qty"><strong>' + formatDecimal(totalQty) + '</strong></td>' +
                            '<td></td>' +
                        '</tr>'
                    );
                }else {
...
function resetCartItems() {
    ...
    if ($('#posdiscount').length && !discountOnMrpEnabled) {
        var posItemsCount = (typeof positems === 'object' && positems !== null) ? Object.keys(positems).length : 0;
        if (posItemsCount == 0 && localStorage.getItem('positems')) {
            var poscart_items = JSON.parse(localStorage.getItem('positems'));
            posItemsCount = Object.keys(poscart_items).length;
        }
        if (posItemsCount == 0) {
            ...
            var orderDiscount = $('#posdiscount').val();
            if (orderDiscount && orderDiscount !== '0' && orderDiscount !== '0%') {
                localStorage.setItem('posdiscount', orderDiscount);
            } else {
                localStorage.removeItem('posdiscount');
            }
            ...
            case 'BUY_X_GET_Y_FREE':
                $.each(activeOffers, function () {
                    ...
                    $.each(poscart_items, function () {
                        ...
                        if (objOffer.offer_on_products == objCart.row.id) {
                            ...
                            if (objCart.row.qty >= objOffer.offer_on_products_quantity) {
                                ...
                                var freeItemData = { item_id: freeItemIds, item_qty: freeItemQty };

                                if (objOffer.offer_free_products == objOffer.offer_on_products && objCart.row.option) {
                                    freeItemData.item_option = objCart.row.option;
                                }

                                offerFreeItems.push(freeItemData);
            ...
            case 'FREE_ITEM_ON_GROUPING_QTY':
                ...
                $.each(activeOffers, function () {
                    ...
                    $.each(poscart_items, function () {
                        ...
                        if (offer_on_products.indexOf(objCart.row.id) != -1) {
                            items_count++;
                    ...
                    if (items_qty >= objOffer.offer_on_products_quantity && items_count >= objOffer.offer_items_condition) {
                        ...
                        offerFreeItems.push(JSON.parse('{"item_id":' + freeItemIds + ', "item_qty":' + freeItemQty + '}'));
            ...
            case 'FREE_ITEM_ON_GROUPING_AMOUNTS':
                ...
                $.each(activeOffers, function () {
                    ...
                    $.each(poscart_items, function () {
                        ...
                        if (offer_on_products.indexOf(objCart.row.id) != -1) {
                            items_count++;
                            amount_total_withouttax += formatDecimal(parseFloat(offerCartItems[objCart.row.id].price_without_tax) * parseFloat(objCart.row.qty));
                            amount_total_withtax += formatDecimal(parseFloat(offerCartItems[objCart.row.id].price_with_tax) * parseFloat(objCart.row.qty));
                        }
                    });
...
function applyFreeItems(offerFreeItems) {
    ...
    $.each(offerFreeItems, function () {
        ...
        addOfferFreeItems(objItems.item_id, objItems.item_qty, objItems.item_option);
    });

    loadItems();
}

function addOfferFreeItems(itemId, itemQty, itemOption) {

    itemQty = itemQty ? itemQty : 1;
    var wh = $('#poswarehouse').val();
    var cu = $('#poscustomer').val();

    var requestData = { code: itemId, warehouse_id: wh, customer_id: cu };

    if (itemOption) {
        requestData.option_id = itemOption;
    }
...
function exchange_all_products() {
    ...
    for (const itemId in positems) {
        if (positems.hasOwnProperty(itemId)) {
            ...
            console.log('existingItem:', existingItem);
            if (existingItem.row.qty > 0) {
                if (existingItem.row.storage_type == 'loose') {
                    positems[itemId] = {
                        ...existingItem,
                        row: {
                            ...existingItem.row,
                            qty: -existingItem.row.qty,
                            quantity: -existingItem.row.quantity,
                            base_quantity: existingItem.row.base_quantity,
                        },
                    };
    ...
    $.each(activeCoupon, function () {
        ...
        if (objCoupon.status == 'active') {
            ...
            if (objCoupon.max_coupons > 0) {
                if (usedCoupon >= objCoupon.max_coupons) {
                    bootbox.alert('Sorry, Coupon usage limit has been exceeded.');
                    return false;
                }
            }
            ...
            if (today > offer_end_date) {
                bootbox.alert('Sorry, Coupon is expired.');
                return false;
            }
                ...
                if (localStorage.getItem('positems')) {
                    ...
                    $.each(poscart_items, function () {
                        var objCart = this;
                        items_qty = objCart.row.qty;
                        cart_items_price += (objCart.row.mrp * items_qty);
                    });

                    if (CouponDiscount.includes('%')) {
                        var splitc = CouponDiscount.split('%');
                        ApplyDiscount = (parseFloat(splitc[0]) / 100) * cart_items_price;
                    } else {
                        ApplyDiscount = parseFloat(CouponDiscount);
                    }
                    if (cart_items_price >= minimum_cart_value) {
                        ...
                        localStorage.setItem('posdiscount', offerDiscount);
                        // Show the coupon message
                        $('#coupon-message')
                            .text('Coupon is successfully applied.')
...
function couponValidate(objOffer) {
    ...
    // Initialize both checks
    var matchesCustomer = true;
    var matchesGroup = true;

    // Check for customer match
    if (objOffer.customer_id && objOffer.customer_id != 0) {
        var allowedCustomers = objOffer.customer_id.toString().split(',').map(id => id.trim());
        var selectedCustomer = $('#poscustomer').val();
        console.log('Allowed customers:', allowedCustomers, 'Selected customer:', selectedCustomer);
        if (!selectedCustomer || !allowedCustomers.includes(selectedCustomer.toString().trim())) {
            matchesCustomer = false;
        }
    }

    // Check for group match
    if (objOffer.customer_group_id && objOffer.customer_group_id != 0) {
        var allowedGroups = objOffer.customer_group_id.toString().split(',').map(id => id.trim());
        var selectedGroup = $('#customer_group').val();
        console.log('Allowed groups:', allowedGroups, 'Selected group:', selectedGroup);
        if (!selectedGroup || !allowedGroups.includes(selectedGroup.toString().trim())) {
            matchesGroup = false;
        }
    }

   // Require both customer and group to match (if specified)
    if (!matchesCustomer || !matchesGroup) {
         console.warn('Customer or group does not match. Invalid coupon.');
         return false;
    }
...
function qtyMinus(item_id) {
    ...
    if (new_qty > 0) {
        positems[item_id].row.qty = new_qty;
        positems[item_id].row.qty = new_qty;
        if(data.row.storage_type != 'loose'){
