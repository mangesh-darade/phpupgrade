...
$(document).ready(function () {
    ...
    $(document).on('click', '.edit', function () {
        ...
        var pr_tax_val = 0, pr_tax_rate_obj = null;
        ...
        if (site.settings.tax1) {
            ...
            pr_tax_rate_obj = getPrTaxRateObj(null, item.row.tax_rate);
            if (positems[item_id].row.tax_method == 0) {
                var pr_tax_surcharge = calcPrTaxSurcharge(pr_tax_rate_obj, pr_tax_val, 1);
                if (pr_tax_surcharge.has_pr_tax_surcharge) {
                    net_price -= parseFloat(pr_tax_surcharge.pr_tax_surcharge_val);
                }
            }
        }
        ...
        var pr_tax_final_val = parseFloat(pr_tax_val);
        if (pr_tax_rate_obj) {
            var pr_tax_surcharge = calcPrTaxSurcharge(pr_tax_rate_obj, pr_tax_val, 1);
            if (pr_tax_surcharge.has_pr_tax_surcharge) {
                pr_tax_final_val = parseFloat(pr_tax_surcharge.pr_tax_final_val);
            }
        }
    ...
    $(document).on('change', '#pprice, #ptax, #pdiscount', function () {
        ...
        if (item_tax_method == 0) {
            var pr_tax_surcharge = calcPrTaxSurcharge(pr_tax_rate_obj, pr_tax_val, 1);
            if (pr_tax_surcharge.has_pr_tax_surcharge) {
                unit_price -= parseFloat(pr_tax_surcharge.pr_tax_surcharge_val);
            }
        }
    ...
    $(document).on('change', '#mprice, #mtax, #mdiscount', function () {
        ...
        setPrTaxDisplay('#mpro_tax', pr_tax_val, pr_tax_rate_obj);
    });
...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        // Disable customer-related controls when cart has items, enable when empty
        var disableCustomerControls = (posItemsCount > 0);
        ...
        $.each(sortedItems, function () {
            ...
            if (item_tax_method == 0) {
                ...
                offerCartItems[item.row.id] = JSON.parse('{"item_id":"' + item.row.id + '", "price_with_tax":"' + (parseFloat(unit_price) + pr_tax_final_val) + '", "price_without_tax":"' + unit_price + '", "qty":"' + item_qty + '", "category":"' + item.row.category_id + '", "discount":"' + item.row.discount + '"}');
            }
            /************************************************/
            ...
            tr_html += '<span class="sname" id="name_' + row_no + '">' + item_code + ' - ' + item_name + (sel_opt != '' ? ' (' + sel_opt + ((item.note == '') ? item.note : ': ' + item.note) + ')' : '') + (item_option_color_name ? ' (' + item_option_color_name + ')' : '') + '</span>';
            ...
            if (site.settings.tax1 == 1) {
                tr_html += '<input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + pr_tax.id + '"><input type="hidden" class="sproduct_tax" id="sproduct_tax_' + row_no + '" value="' + formatMoney(pr_tax_line_info ? pr_tax_line_info.pr_tax_line : (pr_tax_val * item_qty)) + '">';
            }
            ...
            if (add_tax_in_cart_unit_price == true && add_discount_in_cart_unit_price == true) {
                cart_item_price = parseFloat(item_price) + parseFloat(pr_tax_final_val) + parseFloat(item_discount); //item_ds
            ...
            if (permission_admin || permission_owner || per_cartpriceedit) {
                if (changeQtyAsPerPrice == true && item.row.storage_type == 'loose') {
                    ...
                    tr_html += '<input type="number" maxlength="10" name="item_price[]" id="sprice_' + row_no + '" value="' + ((cart_item_price)) + '"  ' + item_disabled + '  class="form-control input-sm kb-pad text-center userprice" />';
                }
            } else {
            ...
            if (permission_admin || permission_owner || per_cartpriceedit) {
                ...
                if (item.row.storage_type == 'packed') {
                    ...
                    tr_html += '<input style="width: 43px; float: right;" class="form-control input-sm kb-pad text-center rquantity" maxlength="6" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" ' + item_disabled + ' type="number"   value="' + item_qty + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
                } else {
                    ...
                    $.each(poscart_items, function () {
                        ...
                        if (objOffer.offer_on_products == objCart.row.id) {
                            ...
                            if (objCart.row.qty >= objOffer.offer_on_products_quantity) {
                                ...
                                freeItemQty = objOffer.offer_free_products_quantity ? (objOffer.offer_free_products_quantity * offerTerm).toFixed(3) : (1).toFixed(3);
                                localStorage.setItem('offer_category', pos_settings.active_offer_category);
                                localStorage.setItem('offer_description', objOffer.offer_invoice_descriptions);
...
function add_free_invoice_item(item, itemQty) {
    ...
    if (offer_free_items[itemtype]) {
        ...
        offer_free_items[itemtype].row.base_quantity = formatDecimal(itemQty, 3);
    } else {
