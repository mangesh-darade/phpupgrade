...
$(document).ready(function () {
    ...
    $(document).on('change', '#ptax, #pmrp, #mrpdiscount, #selling, #pquantity', function () {
        var row = $('#' + $('#row_id').val());
        var item_id = row.attr('data-item-id');
        ...
        if ($(this).attr('id') === 'selling') {
            ...
            if (unit_price === 0 || isNaN(discount)) {
                discount = '0%';
            }
     ...
     * Edit Row Method
     ...
     $(document).on('click', '#editItem', function () {
        ...
        var pro_tax = parseFloat($('#pro_tax').text().trim());
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            if (posdiscount) {
                ...
                if (customer_group_discount == '1') {
                    // Customer group discount overrides coupon (matches server Pos.php:
                    //   $item_order_discount = ($customer_group_discount==1) ? '0' : $item_order_discount)
                    var ds_grp = item_ds ? String(item_ds) : '0';
                    if (ds_grp.indexOf("%") !== -1) {
                        var pds = ds_grp.split("%");
                        if (!isNaN(pds[0])) {
                            item_discount = formatDecimal((parseFloat(((base_price) * parseFloat(pds[0])) / 100)), 6);
                        } else {
                            item_discount = formatDecimal(ds_grp, 6);
                        }
                    } else {
                        item_discount = formatDecimal(ds_grp, 6);
                    }
                    item_order_discount = 0;
                    item_discount_on_mrp = item_discount;
                    // item_ds stays as the customer group discount for server submission
                } else {
                    // Coupon only: compute order-level discount share on actual selling price.
                    // Do NOT touch item_discount (leave = 0); sub-total stays intact.
                    if (ods.indexOf("%") !== -1) {
                        var pds = ods.split("%");
                        if (!isNaN(pds[0])) {
